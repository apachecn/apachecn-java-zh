...
*   [**JIRA**](https://www.atlassian.com/software/jira) ：Atlasian 创建的专有缺陷跟踪系统。除了 bug 和问题跟踪，它还提供管理功能，如 SCRUM 和看板、查询问题的语言（JIRA 查询语言）、与外部系统的集成（例如 GitHub、Bitbucket）以及使用来自 Atlasian Marketplace 的插件扩展 JIRA 的附加机制（ [https://marketplace.atlassian.com/](https://marketplace.atlassian.com/) ）。
