...
我们将在第 3 章*红-绿重构中所做的基础上，从失败到成功再到完美*。我们将扩展 Tic Tac Toe 以使用 MongoDB 作为数据存储。我们的单元测试都不会真正使用 MongoDB，因为所有的通信都将被模拟。最后，我们将创建一个集成测试，以验证我们的代码和 MongoDB 确实是集成的。
...
我们将继续我们在第 3 章*红绿重构中结束的内容，从失败到成功，直到完美*。到目前为止开发的应用程序的完整源代码可在[中找到 https://bitbucket.org/vfarcic/tdd-java-ch06-tic-tac-toe-mongo.git](https://bitbucket.org/vfarcic/tdd-java-ch06-tic-tac-toe-mongo.git)。使用 IntelliJ IDEA 中的 VCS | Checkout from Version Control | Git 选项克隆代码。与任何其他项目一样，我们需要做的第一件事是将依赖项添加到`build.gradle`：
...
一旦一切就绪，我们使用 JUnit 期望，就像我们之前在第 3 章、*红绿重构中所做的一样——从失败到成功，直到完美*。
