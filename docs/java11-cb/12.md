...
1.  您应该将`%JAVA_HOME%/bin`（在 Windows 上）或`$JAVA_HOME/bin`（在 Linux 上）添加到您的`PATH`变量中。如果没有，请访问*在 Windows 上安装 JDK 18.9 并设置路径变量*和*在 Linux（Ubuntu，x64）上安装 JDK 18.9 并在第一章*中配置路径变量*配方、*安装和偷窥 Java 11*。*
