...
显然，了解 JDBCAPI 是不够的；您还必须阅读数据库的文档。以下是 PostgreSQL（[的文档内容 https://jdbc.postgresql.org/documentation/80/binary-data.html](https://jdbc.postgresql.org/documentation/80/binary-data.html) 必须说明 LOB 处理：
...
这里是 PostgreSQL 文档中的另一条建议（您可以在[上访问）https://jdbc.postgresql.org/documentation/80/binary-data.html](https://jdbc.postgresql.org/documentation/80/binary-data.html) ：
...
在撰写本书时，最新的 MyBatis 文档可在此处找到：[http://www.mybatis.org/mybatis-3/index.html](http://www.mybatis.org/mybatis-3/index.html)
...
MyBatis 还提供了构建动态 SQL、SqlBuilder 类的工具，以及许多其他方法来构造和执行任何复杂的 SQL 或存储过程。有关详细信息，请阅读[中的文档 http://www.mybatis.org/mybatis-3](http://www.mybatis.org/mybatis-3) 。
...
规范中描述的接口、枚举、注释和异常属于包`javax.persistence`（[https://javaee.github.io/javaee-spec/javadocs **Java 企业版**（**EE**中包含的](https://javaee.github.io/javaee-spec/javadocs)）。JPA 由几个框架实现，其中最流行的是：

*   冬眠 ORM（[http://hibernate.org/orm](http://hibernate.org/orm) ）
*   日食（[http://www.eclipse.org/eclipselink](http://www.eclipse.org/eclipselink) ）
*   甲骨文 TopLink（[http://www.oracle.com/technetwork/middleware/toplink/overview/index.html](http://www.oracle.com/technetwork/middleware/toplink/overview/index.html) ）
...
请参阅 Oracle 文档（[https://docs.oracle.com/cd/E16439_01/doc.1013/e13981/cfgdepds005.htm](https://docs.oracle.com/cd/E16439_01/doc.1013/e13981/cfgdepds005.htm) 关于`persistence.xml`文件的配置。对于这个配方，我们使用了 Hibernate ORM，因此指定了`org.hibernate.jpa.HibernatePersistenceProvider`作为提供者。
