...
如果你想阅读更多关于微服务的文章，第一个也是最真实的来源是 Martin Fowler 的文章（[http://www.martinfowler.com/articles/microservices.html](http://www.martinfowler.com/articles/microservices.html) ）。
...
其中之一是 MVC，它适用于编写 web 应用程序，但通常适用于任何具有用户界面的应用程序。在我们的例子中，我们没有一个经典的用户界面，因为客户端也是一个程序；尽管如此，MVC 仍然是一个很好的选择。
...
Spring 框架是一个包含多个模块的庞大框架。该框架的第一个版本于 2003 年发布，自那时以来，已有四个主要版本提供了新的和增强的功能。目前，Spring 是实际使用的企业框架，可能比标准**EJB3.0**更广泛
...
核心模块的核心元素是上下文。当 Spring 应用程序启动时，容器需要一个上下文，容器可以在其中创建不同的 bean。这对于任何依赖项注入容器来说都是非常通用的。如果我们以编程方式创建两个不同的上下文，它们可能独立地生活在同一个**JVM**中。如果有一个 bean 声明为 singleton，因此应该只有一个实例，那么当我们需要一个实例时，容器将为上下文创建一个实例。表示上下文的对象引用了我们已经创建的对象。但是，如果有多个上下文，他们将不知道 JVM 中已经有实例的另一个上下文，容器将为另一个上下文创建一个新的单例 bean 实例。

通常，我们不会在一个程序中使用多个上下文，但是有很多例子表明在一个 JVM 中存在多个上下文。当不同的 servlet 在同一个 servlet 容器中运行时，它们运行在同一个 JVM 中，由类加载器分隔，并且它们可以各自使用 Spring。在本例中，上下文将属于 servlet，并且每个上下文都有一个新的上下文。

在上一章中，我们使用了 Guice。Spring 上下文类似于 Guice 注入器。在上一章中，我有点作弊，因为我正在编程 Guice 为每个请求创建一个新的注入器。这远远不是最优的，Guice 提供了一个可以处理 servlet 环境的注入器实现。作弊的原因是我想更多地关注 DI 体系结构的基本要素，我不想通过引入复杂的注入器实现来使代码复杂化。

在 Spring 上下文行为中，它所做的是由接口`ApplicationContext`定义的。该接口有两个扩展和许多实现。`ConfigurableApplicationContext`扩展了`ApplicationContext`，定义了 setter，`ConfigurableWebApplicationContext`定义了 web 环境中所需的方法。当我们编写 web 应用程序时，通常不需要直接干扰上下文。该框架以编程方式配置 servlet 容器，并包含用于创建上下文和调用方法的 servlet。这是为我们创建的所有样板代码。

上下文跟踪创建的 bean，但不创建它们。要创建 bean，我们需要 bean 工厂（至少一个）。春季豆子工厂最顶级的界面是`BeanFactory`。对象和 bean 之间的区别在于 bean 工厂创建 bean，它在上下文中注册，并且它有一个`String`名称。这样，程序可以通过名称引用 bean。

在 Spring 中，可以用几种不同的方式配置不同的 bean。最古老的方法是创建一个描述不同 bean 的 XML 文件，指定名称、创建 bean 必须实例化的类以及字段，以防 bean 在创建时需要注入其他 bean。

这种方法背后的动机是，通过这种方式，bean 的连接和配置可以完全独立于应用程序代码。它成为一个可以单独维护的配置文件。如果我们有一个可以在多个不同环境中工作的大型应用程序，那么可以通过多种方式访问库存数据。在一种环境中，可以通过调用 SOAP 服务来获取资源清册。在另一个环境中，可以在 SQL 数据库中访问数据。在第三种环境中，它可以在一些 NoSQL 存储中使用。这些访问中的每一个都作为一个单独的类实现，实现一个公共的库存访问接口。应用程序代码只依赖于接口，容器必须提供一个或另一个实现。

当 bean 连接的配置是 XML 时，则只需编辑此 XML 文件，并且可以通过实现适合该环境的接口来启动代码。

下一种可能性是使用注释配置 bean。很多时候，我们使用 bean 和 Spring 并不是因为有很多 bean 功能的实现，而是因为我们希望将对象实例的创建与功能分开。这是一种很好的风格：分离关注点，即使实现是单一的，没有替代方案。但是，在这种情况下，创建 XML 配置是多余的。如果在我们的代码中有一个接口和它的单个实现，那么为什么我要在 XML 中指定，通过创建一个具有实现该接口的类的对象，我应该使用实现该接口的类？很明显，不是吗？我们不喜欢编程可以自动化的东西。

为了表示类可以用作 bean，并可能提供名称，我们可以使用`@Component`注释。我们不需要提供名称作为参数。在这种情况下，名称将是一个空字符串，但是如果我们不引用它，为什么要有一个名称呢？Spring 扫描类路径上的所有类并识别注释的类，它知道它们是用于 bean 创建的候选类。当组件需要注入另一个 bean 时，该字段可以用`@Autowired`或`@Inject`注释。`@Autowired`注释是 Spring 注释，在`@Inject`注释标准化之前就已经存在。如果要在 Spring 容器之外使用代码，建议使用标准注释。在功能上，它们是等价的。

在我们的代码中，当 Spring 创建`ProductInformationController`组件的实例时，它看到它需要`ProductLookup`的实例。这是一个接口，因此，Spring 开始寻找实现这个接口的类，创建它的实例，可能首先创建其他 bean，然后注入它，设置字段。您可以决定注释字段的 setter，而不是字段本身。在这种情况下，Spring 将调用 setter，即使 setter 是私有的。您可以通过构造函数参数注入依赖项。setter、字段注入和构造函数注入之间的主要区别在于，如果使用构造函数注入，则无法创建没有依赖关系的 bean。当 bean 被实例化时，它应该并且将注入所有其他 bean，因此它依赖于使用构造函数注入。同时，需要通过 setter 注入或直接注入到字段中的依赖项可以稍后由容器在实例化类和准备 bean 之间的某个时间进行实例化。

在构造器代码变得比简单的依赖项设置更复杂或者依赖项变得复杂之前，这种细微的差异可能看起来并不有趣或重要。对于复杂的构造函数，代码应该注意对象没有完全创建。这通常适用于任何构造函数代码，但对于依赖项注入容器创建的 bean 来说，这更为重要。因此，建议使用构造函数注入。在这种情况下，依赖关系存在；如果程序员犯了一个错误，忘记了对象没有完全初始化，并在构造函数或从构造函数调用的方法中使用它，那么依赖性就存在了。此外，使用构造函数初始化依赖项并将这些字段声明为`final`是干净且结构良好的。

另一方面，构造函数注入也有其缺点。

如果不同的对象相互依赖，并且依赖关系图中有一个环，那么如果使用构造函数依赖关系，Spring 将面临困难。当类*A*需要类*B*反过来，作为最简单的循环时，如果依赖项注入是构造函数依赖项，则无论是*A*还是*B*都不能在没有对方的情况下创建。在这种情况下，不能使用构造函数注入，循环应该至少作为一个依赖项被打破。在这种情况下，setter 注入是不可避免的。

当存在可选依赖项时，Setter 注入也可能更好。很多时候，某些类可能不需要同时使用其所有依赖项。某些类可以使用数据库连接或 NoSQL 数据库句柄，但不能同时使用两者。尽管这也可能是一种代码气味，可能是 OO 设计糟糕的标志，但它可能会发生。这样做可能是一个深思熟虑的决定，因为纯粹的 OO 设计会导致太深的对象层次结构和太多的类，超出可维护的限制。如果是这种情况，可以使用 setter 注入更好地处理可选依赖项。有些是配置和设置的；有些保留默认值，通常为`null`。

最后但并非最不重要的一点是，如果注释不够，我们可以使用 Java 类配置容器。例如，在我们的代码库中有多个`ProductLookup`接口的实现。（如果您没有认识到这一点，请不要担心；我还没有告诉您。）有一个`ResourceBasedProductLookup`类从包中读取属性文件，主要用于测试应用程序，还有`RestClientProductLookup`，这是一个类似于产品的接口实现。如果我除了在`lookup`字段中添加`@Autowired`之外没有其他配置，Spring 将不知道使用哪个实现，并将在启动过程中奖励使用，并显示以下错误消息：
...
bean 工厂只返回实现`ProductLookup`接口的`ResourceBasedProductLookup`类的一个新实例。当没有可依赖的外部服务时，此实现可用于运行应用程序进行本地测试。此实现从打包到 JAR 应用程序中的本地资源文件读取产品数据。
...
`fromProperties`方法创建`ProductInformation`的一个实例，并根据`Properties`对象中给定的参数进行填充。`Properties`类是一种古老且广泛使用的类型。虽然有更多的现代格式和类，但它仍然被广泛使用，并且您很可能会遇到这个类。这正是我们在这里使用它的原因。
...
同时，该类还在封送处理或某些其他功能的情况下提供配置参数，以便它自动需要这些参数。例如，您可以提供封送处理方法，但我建议您使用默认情况下可用的任何方法。在大多数情况下，当开发人员认为需要这些函数的特殊版本时，他们的代码的原始设计是有缺陷的。
...
当我们在项目中定义了 REST 服务时，我们可以在套件中创建一个新的测试套件和一个测试用例。然后，如果我们修改默认值（与参数名称相同，在本例中为`id`，我们可以向测试用例添加一个步骤，该步骤将使用参数`123`调用 REST 服务。我们可以使用窗口左上角的绿色三角形运行测试步骤，因为我们已经运行了测试应用程序和 soapUI 模拟服务，所以我们应该得到 JSON 的答案。我们必须在响应端选择 JSON；否则，soapUI 会尝试将响应解释为 XML，并且由于我们有一个 JSON 响应，因此不会产生太多的效果。我们看到的是以下窗口：
...
AspectJ 定义了其他指示符。SpringAOP 可以识别其中的一些，但它会抛出`IllegalArgumentException`，因为 Spring 只实现方法执行切入点。另一方面，AspectJ 还可以拦截 PCD 正在初始化的对象创建，例如。除了执行之外，一些其他 PCD 可以限制 PCD 的执行。例如，PCD`within`可用于将方面限制为属于某些包内类的连接点，或者`@target`PCD 可用于将匹配限制为对象中的方法，这些对象在切入点表达式中的关键字`@target`之后的注释在`(`和`)`之间。
...
如果我们再次使用前面的命令启动应用程序、启动项目的 soapUI、启动模拟服务并执行测试，我们将在控制台上看到 aspects 打印的以下日志行：
