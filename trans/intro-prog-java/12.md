...
2007 年，Sun Microsystems 发布了 JavaFX，它最终成为一个软件平台，用于在许多不同的设备上创建和交付桌面应用程序，以取代 Swing 作为 JavaSE 的标准 GUI 库。它位于以`javafx`开头的软件包中，支持所有主要的桌面操作系统和多个移动操作系统，包括 Symbian 操作系统、Windows mobile 和一些专有的实时操作系统。
