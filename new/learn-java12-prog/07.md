...
JCL 是实现该语言的包的集合。更简单地说，它是 JDK 中包含并准备好使用的`.class`文件的集合。一旦安装了 Java，就可以将它们作为安装的一部分，并可以开始使用 JCL 类作为构建块来构建应用程序代码，这些构建块负责许多底层管道。JCL 的丰富性和易用性极大地促进了 Java 的普及。
...
这就是为什么，如果你在 Oracle 官方网站上查看 javaapi，你会发现不仅有`java`和`javax`包被列为标准，还有`jdk`、`com.sun`、`org.xml`以及其他一些包。这些额外的包主要由工具或其他专用应用程序使用。在我们的书中，我们将主要集中在主流 Java 编程上，只讨论`java`和`javax`包。
...
*   所有原语类型的包装类：`Byte`、`Boolean`、`Short`、`Character`、`Integer`、`Long`、`Float`、`Double`
...
*   `Math`类：提供基本数值运算的方法
*   `Throwable`类：所有异常的基类
*   `Error`类：一个异常类，它的所有子类都用来传递应用程序不应该捕捉到的系统错误
...
*   `Objects`：提供了各种与对象相关的实用方法，其中一些我们已经在[第 6 章](06.html)、*数据结构、泛型和流行实用程序*中进行了概述
*   `Arrays`：包含 160 种静态数组操作方法，其中一些方法我们在[第 6 章](06.html)、*数据结构、泛型和流行实用程序*中进行了概述
...
*   `java.io`包类允许在没有缓存的情况下读取/写入数据（我们在[第 5 章](05.html)、*字符串、输入/输出和文件*中讨论过），而`java.nio`包的类创建了一个缓冲区，允许在填充的缓冲区中来回移动
*   `java.io`包类阻塞流直到所有数据被读写，而`java.nio`包的类以非阻塞方式实现（我们将在[第 15 章](15.html)、*反应式编程*中讨论非阻塞方式）
...
我们将讨论这些包，并在[第 10 章](10.html)*管理数据库中的数据*中看到代码示例。

# java.net

`java.net`包包含支持以下两个级别的应用程序联网的类：
...
2007 年，Sun 微系统公司宣布创建 JavaFX，JavaFX 最终成为一个软件平台，用于在许多不同的设备上创建和交付桌面应用程序。它旨在取代 Swing 作为 javase 的标准 GUI 库。JavaFX 框架位于以`javafx`开头的包中，支持所有主要的桌面操作系统（DOS）和多个移动操作系统，包括 Symbian 操作系统、Windows 移动操作系统和一些专有的实时操作系统。

JavaFX 基于**层叠样式表**（**CSS**），将平滑动画、web 视图、音频和视频播放以及样式的支持添加到 GUI 开发人员的库中。但是，Swing 有更多的组件和第三方库，因此使用 JavaFX 可能需要创建很久以前在 Swing 中实现的自定义组件和管道。这就是为什么，尽管 JavaFX 被推荐为桌面 GUI 实现的首选，但根据 Oracle 网站（[上的官方回应，Swing 在可预见的未来仍将是 Java*的一部分 http://www.oracle.com/technetwork/java/javafx/overview/faq-1446554.html#6*](http://www.oracle.com/technetwork/java/javafx/overview/faq-1446554.html#6)。所以，可以继续使用 Swing，但如果可能，最好切换到 JavaFX。

我们将讨论 JavaFX，并在[第 12 章](12.html)、*javagui 编程*中看到代码示例。
...
我们讨论了[第 5 章](05.html)中的`org.apache.commons.io`包、*字符串、输入/输出和文件*。
...
据文献记载，`org.apache.commons.lang3`包*提供了高度可重用的静态实用方法，主要是为*`java.lang`*类增加价值。*这里有几个值得注意的例子：

*   `ArrayUtils`类：允许搜索和操作数组；我们在[第 6 章](06.html)、*数据结构、泛型和流行实用程序*中讨论和演示了它
...
*   `StringUtils`课堂：我们在[第 5 章](05.html)中讨论了*字符串、输入/输出和文件*
...
*   `CollectionUtils`、`ListUtils`、`MapUtils`、`MultiMapUtils`、`MultiSetUtils`、`QueueUtils`、`SetUtils`以及许多其他特定于接口的实用程序类

阅读包装文件（[https://commons.apache.org/proper/commons-collections](https://commons.apache.org/proper/commons-collections)了解更多细节。
...
您可以在 apachecommons 项目站点（[上阅读关于这个包的更多信息 https://commons.apache.org/proper/commons-codec](https://commons.apache.org/proper/commons-codec)）。
