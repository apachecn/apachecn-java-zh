...
这就是为什么，如果你在 Oracle 官方网站上查看 Java API，你会发现不仅有`java`和`javax`包被列为标准，还有`jdk`、`com.sun`、`org.xml`以及其他一些包。这些额外的包主要由工具或其他专用应用程序使用。在我们的书中，我们将主要集中在主流 Java 编程上，只讨论`java`和`javax`包。
