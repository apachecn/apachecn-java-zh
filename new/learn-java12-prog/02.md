...
当创建一个对象时，JVM 使用一个**构造器**。构造器的目的是初始化对象状态，为所有声明的属性赋值。如果类中没有声明构造器，JVM 只会将缺省值赋给属性。我们已经讨论了原始类型的默认值：整数类型的默认值是`0`，浮点类型的默认值是`0.0`，布尔类型的默认值是`false`。对于其他 Java 引用类型（参见[第 3 章](03.html)、*Java Fundamentals），*默认值为`null`，表示引用类型的属性没有赋值。
