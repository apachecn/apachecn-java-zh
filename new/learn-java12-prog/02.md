...
请注意注释`@Override`的用法。它告诉编译器程序员认为带注释的方法覆盖了一个祖先接口的方法。通过这种方式，编译器可以确保覆盖确实发生，否则会生成错误。例如，程序员可能会将方法的名称拼错如下：
...
`final`类不能扩展。它不能有孩子，这使得所有的方法都有效的`final`。此功能用于安全性，或者当程序员希望确保类功能不能由于某些其他设计考虑而被覆盖、重载或隐藏时。
