...
我们在[第 1 章](01.html)*Java 入门 12*中简要介绍了异常。在本章中，我们将更系统地讨论这个问题。Java 中有两种异常：checked 异常和 unchecked 异常。两者都将被演示，并解释两者之间的区别。读者还将了解与异常处理相关的 Java 构造的语法以及处理异常的最佳实践。本章将以可用于调试生产代码的断言语句的相关主题结束。
...
正如我们在[第一章](01.html)*Java 入门 12*中所描述的，一个意外的情况可能会导致**Java 虚拟机**（**JVM**）创建并抛出一个异常对象，或者应用程序代码可以这样做。一旦发生异常，如果异常是在一个`try`块中抛出的，那么控制流就被转移到`catch`子句。让我们看一个例子。考虑以下方法：
...
在屏幕截图上看到的红色部分称为**堆栈跟踪**。名称来自方法调用在 JVM 内存中的存储方式（作为堆栈）：一个方法调用另一个方法，而另一个方法又反过来调用另一个方法，依此类推。在最内部的方法返回后，遍历堆栈，并从堆栈中移除返回的方法（**堆栈帧**）（我们将在[第 9 章](09.html)、*JVM 结构和垃圾收集*中详细讨论 JVM 内存结构）。当发生异常时，所有堆栈内容（堆栈帧）都作为堆栈跟踪返回。它允许我们追踪导致问题的代码行。

在前面的代码示例中，根据异常的类型执行不同的代码块。在我们的案例中，是`java.lang.NullPointerException`。如果应用程序代码没有捕获它，这个异常将通过被调用方法的堆栈一直传播到 JVM 中，JVM 随后停止执行应用程序。为了避免这种情况的发生，可以捕获异常并执行一些代码来从异常情况中恢复。
...
所有错误都扩展了`java.lang.Error`类，而`java.lang.Error`类又扩展了`java.lang.Throwable`类。一个错误通常是由 JVM 抛出的，根据官方文档，*表示一个合理的应用程序不应该试图捕捉*的严重问题。以下是几个例子：

*   `OutOfMemoryError`：当 JVM 耗尽内存并且无法使用垃圾收集清理内存时抛出
*   `StackOverflowError`：当分配给方法调用堆栈的内存不足以存储另一个堆栈帧时抛出
...
尽管不是被迫的，程序员也经常捕捉运行时（未检查的）异常，以便更好地控制程序流，使应用程序的行为更稳定和可预测。顺便说一下，所有的错误都是运行时（未检查的）异常，但是，正如我们已经说过的，通常不可能以编程方式处理它们，因此捕捉`java.lang.Error`类的后代是没有意义的。
...
添加一个`finally`块不会改变所描述的行为。如果存在，不管是否生成了异常，它总是被执行。`finally`块通常用于释放资源：关闭数据库连接、文件等。但是，如果资源实现了`Closeable`接口，那么最好使用 try with resources 语句，该语句允许自动释放资源。下面是如何使用 Java 7 实现的：
...
Java 9 增强了 try with resources 语句功能，允许创建表示`try`块外资源的对象，然后在 try with resources 语句中使用这些对象，如下所示：
...
`throws`条款允许但不要求我们列出未经检查的例外情况。添加未检查的异常不会强制方法的用户处理它们。

最后，如果方法抛出几个不同的异常，可以列出基本的`Exception`异常类，而不是列出所有异常。这将使编译器感到高兴，但这并不是一个好的实践，因为它隐藏了方法用户可能期望的特定异常的细节。

请注意，编译器不会检查方法体中的代码可以引发何种异常。因此，可以在`throws`子句中列出任何异常，这可能会导致不必要的开销。如果程序员错误地在`throws`子句中包含一个 checked 异常，而该异常从未被方法实际抛出，那么该方法的用户可能会为它编写一个从未执行过的`catch`块
...
另外，要创建未检查的异常，请扩展`java.lang.RunitmeException`类，如下所示：
...
在这两种情况下，未检查的`NullPointerException`的对象开始在系统中传播，直到它被应用程序或 JVM 捕获。

# 断言语句

有时，程序员需要知道代码中是否发生了特定的情况，即使应用程序已经部署到生产环境中。同时，没有必要一直运行检查。这就是分支`assert`语句派上用场的地方。举个例子：
...
只有当 JVM 使用`-ea`选项运行时，`assert()`方法才会执行。`-ea`标志不应该在生产中使用，除非可能暂时用于测试目的，因为它会产生影响应用程序性能的开销。
...
*   如果有必要，将第三方检查的异常转换为未检查的异常，方法是将它们作为`RuntimeException`重新抛出，并显示相应的消息
*   除非必须，否则不要创建自定义异常
...
本章向读者介绍了 Java 异常处理框架，了解了两种异常：checked 和 unchecked（运行时），以及如何使用`try-catch-finally`和`throws`语句处理它们。读者还学习了如何生成（抛出）异常以及如何创建自己的（自定义）异常。本章最后介绍了异常处理的最佳实践。
