...
当有人说“[0]Java[1]”时，他们的意思可能完全不同：

*   **Java 程序设计语言**：一种高级程序设计语言，允许意图（程序）以人类可读的格式表达，并可翻译成计算机可执行的二进制代码
*   **Java 编译器**：一种程序，可以读取用 Java 编程语言编写的文本，并将其翻译成可由**Java 虚拟机**（**JVM**）解释为计算机可执行的二进制代码
*   **Java 虚拟机**（**JVM**）：读取已编译 Java 程序并将其解释为可由计算机执行的二进制代码的程序
...
正如我们已经提到的，JDK 包括一个 Java 编译器和 JVM。编译器的任务是读取一个`.java`文件，该文件包含用 Java 编写的程序的文本（称为**源代码**），并将其转换（编译）为存储在`.class`文件中的字节码。JVM 然后可以读取`.class`文件，将字节码解释为二进制代码，并将其发送到操作系统执行。编译器和 JVM 都必须从命令行显式调用

为了支持`.java`文件编译及其字节码执行，JDK 安装还包括称为**Java 类库**（**JCL**的标准 Java 库）。如果程序使用第三方库，它必须在编译和执行期间出现。它必须从调用编译器的同一命令行引用，稍后在 JVM 执行字节码时引用。另一方面，JCL 不需要显式地引用。假设标准 Java 库位于 JDK 安装的默认位置，因此编译器和 JVM 知道在哪里可以找到它们

如果不需要编译 Java 程序，只想运行已经编译的`.class`文件，可以下载安装**Java Runtime Environment**（**JRE**）。例如，它由 JDK 的一个子集组成，不包括编译器。

有时，JDK 被称为**软件开发工具包**（**SDK**），它是一组软件工具和支持库的总称，这些工具和库允许创建使用某种编程语言编写的源代码的可执行版本。因此，JDK 是一个用于 Java 的 SDK。这意味着可以将 JDK 称为 SDK。

您还可能听到与 JDK 相关的术语**Java 平台**和**Java 版本**。典型的平台是允许开发和执行软件程序的操作系统。由于 JDK 提供了自己的操作环境，因此也被称为平台。**版本**是针对特定目的组装的 Java 平台（JDK）的变体。有五个 Java 平台版本，如下所示：

*   **Java 平台标准版**（**Java SE**）：包括 JVM、JCL 等工具和工具。
*   **Java 平台企业版**（**Java EE**）：这包括 Java SE、服务器（为应用程序提供服务的计算机程序）、JCL 和其他库、代码示例、教程以及用于开发和部署大规模、多层和安全的网络应用程序的其他文档。
*   **Java 平台微版**（**Java ME**）：这是 Java SE 的一个子集，有一些专门的库，用于开发和部署用于手机、个人数字助理、电视机顶盒、打印机、传感器等嵌入式和移动设备的 Java 应用程序。javame 的一个变体（有自己的 JVM 实现）称为[4]androidsdk[5]。它是由 Google 为 Android 编程开发的。
*   **Java 卡**：它是 Java 版本中最小的一个，用于在小型嵌入式设备（如智能卡）上开发和部署 Java 应用程序。它有两个版本：**基于 ISO7816 和 ISO14443 通信的智能卡 Java 卡经典版**和**支持 web 应用模型和 TCP/IP 作为基本协议的 Java 卡连接版**，运行在高端安全微控制器上。

所以，**安装 Java 就意味着要安装 JDK**，也就是说**要在其中一个版本上安装 Java 平台**，在这本书中，我们只讨论和使用 Java SE。
...
9.  当计算机上的[0]命令显示正确的 Java 版本时，JDK 安装成功，如下面的屏幕截图所示：

![](img/3a7e798e-2c94-4ef5-8ef2-695f2dd2df16.png)

# 命令、工具和实用程序

如果按照安装说明进行操作，您可能已经注意到目录下给出的链接（JDK 的已安装目录结构）。它将带您进入一个页面，该页面描述已安装的 JDK 在您的计算机上的位置以及 JDK 根目录的每个目录的内容。[0]目录包含构成 Java 命令、工具和实用程序的所有可执行文件。如果目录`bin`没有自动添加到环境变量`PATH`，请考虑手动添加，这样就可以从任何目录启动 Java 可执行文件。

在上一节中，我们已经演示了[0]命令，[1]。其他可用 Java 可执行文件的列表（命令、工具、，和实用程序）可以在 JavaSE 文档中找到（[https://www.oracle.com/technetwork/java/javase/documentation/index.html](https://www.oracle.com/technetwork/java/javase/documentation/index.html) ）单击链接 Java 平台标准版技术文档站点，然后单击下一页的链接工具参考。您可以通过单击每个可执行工具的链接来了解其更多信息。
...
这些命令使编程成为可能。每个 Java 程序员都必须很好地理解自己的结构和功能。但是，如果您是 Java 编程新手，并且使用 IDE（请参阅*如何安装和运行 IDE*一节），则不需要立即掌握这些命令。一个好的 IDE 会在您每次对[0]文件进行更改时自动编译一个文件，从而对您隐藏它们。它还提供了一个图形元素，可以在每次单击它时运行程序。

另一个非常有用的 Java 工具是`jcmd`。它有助于与当前运行的任何 Java 进程（JVM）进行通信和诊断，并且有许多选项。但是在最简单的形式中，没有任何选项，它列出了当前运行的所有 Java 进程及其**进程 id**（**PIDs**）。您可以使用它来查看是否已经运行了 Java 进程。如果有，那么可以使用提供的 PID 终止这样的进程。

# 如何安装和运行 IDE

曾经只是一个专门的编辑器，允许像 Word 编辑器检查英语句子语法一样检查书面程序的语法，逐渐演变成一个**集成开发环境**（**IDE**）。它的主要功能在名称上。它集成了在一个**图形用户界面**（**GUI**下编写、编译和执行程序所需的所有工具。利用 Java 编译器的强大功能，IDE 可以立即识别语法错误，然后通过提供上下文相关的帮助和建议来帮助提高代码质量

# 选择 IDE

Java 程序员可以使用多种 IDE，如**NetBeans**、**Eclipse**、**IntelliJ IDEA**、**BlueJ**、**DrJava**、**JDeveloper**、**JCreator**、**jEdit**、**JSource**、**jCRASP**，还有**杰迪特**，举几个例子。最流行的是 NetBeans、Eclipse 和 IntelliJ IDEA。

NetBeans 开发始于 1996 年，是布拉格查尔斯大学的一个 javaide 学生项目。1999 年，该项目和围绕该项目创建的公司被 Sun Microsystems 收购。在甲骨文收购 Sun Microsystems 之后，NetBeans 成为了开源软件，许多 Java 开发人员也为这个项目做出了贡献。它与 JDK 8 捆绑在一起，成为 Java 开发的官方 IDE。2016 年，Oracle 将其捐赠给了 Apache 软件基金会。

有一个用于 Windows、Linux、Mac 和 Oracle Solaris 的 NetBeans IDE。它支持多种编程语言，并可以扩展插件。NetBeans 只与 jdk8 捆绑在一起，但是 netbeans8.2 也可以与 jdk9 一起工作，并使用 jdk9 引入的特性，例如 Jigsaw。在[上 netbeans.apache.org](https://netbeans.apache.org/) ，您可以阅读更多关于 netbeanside 的信息，并下载最新版本，截至本文撰写时，该版本为 11.0。
...
EclipseIDE 插件的数量和种类之多对初学者来说是一个挑战，因为您必须找到解决相同或类似特性的不同实现的方法，这些实现有时可能是不兼容的，并且可能需要深入的调查以及对所有依赖项的清楚理解。尽管如此，eclipseide 还是非常流行，并且有可靠的社区支持。您可以阅读有关 Eclipse IDE 的信息，并从[下载最新版本 www.eclipse.org/ide](http://www.eclipse.org/ide/) .

IntelliJ 有两个版本：付费版和免费社区版。付费版一直被评为最佳 Java IDE，但社区版也被列为三大主要 Java IDE 之一。开发该 IDE 的 JetBrains 软件公司在布拉格、圣彼得堡、莫斯科、慕尼黑、波士顿和新西伯利亚设有办事处。IDE 以其深刻的智能而闻名，即“*在每一个上下文中都给出相关的建议：即时而巧妙的代码完成、动态的代码分析和可靠的重构工具*”，正如作者在其网站上描述产品时所说（[www.jetbrains.com/idea](https://www.jetbrains.com/idea/) ). 在*安装和配置 IntelliJ IDEA*部分，我们将引导您完成 IntelliJ IDEA 社区版的安装和配置。
...
2.  不要选中 Create project from template（如果选中，IDE 会生成一个固定程序[0]和类似的程序，我们不需要），然后单击 Next。

3.  在 projectlocation 字段中选择所需的项目位置（这是新代码将驻留的位置）。
4.  在“项目名称”字段中输入任何您喜欢的内容（例如，本书中代码的项目名为`learnjava`），然后单击`Finish`按钮。
5.  您将看到以下项目结构：
...
主要目标是使一个项目的身份在世界上所有项目中独一无二。为了避免[0]冲突，约定要求从组织域名反向开始构建。例如，如果一个公司的域名是`company.com`，那么它的项目的组 ID 应该以`com.company`开头。这就是为什么本书中的代码使用了`groupId`值`com.packt.learnjava`。
...
现在，如果有人想在他们的应用程序中使用您的项目代码，他们会通过显示的三个值引用它，Maven（如果他们使用它）会将它引入（当然，如果您将您的项目上传到公共共享的 Maven 存储库中）。更多关于 Maven 的信息，请访问[https://maven.apache.org/guides](https://maven.apache.org/guides/) .

[0]值的另一个功能是定义保存项目代码的文件夹树的根目录。让我们打开`src`文件夹，您将看到下面的目录结构：

![](img/c34792dd-dadb-4f53-b009-d99575abd341.png)

`main`下的`java`文件夹将保存应用程序代码，`test`下的`java`文件夹将保存测试代码。
...
3.  点击【确定】按钮，在左侧面板中会看到一组新文件夹，最后一个是`com.packt.learnjava.ch01_start`：
...
6.  单击 OK，您将看到在包`com.packt.learnjava.ch01_start`中创建的第一个 Java 类`PrimitiveTypes`：
...
*   返回`void`（无）。
*   接受一个[0]数组作为输入，或者像我们所做的那样接受[1]。我们将在[第二章](https://cdp.packtpub.com/learn_java_12_programming/wp-admin/post.php?post=24&action=edit#post_25)中讨论`varargs`，*Java 面向对象编程（OOP）*。现在，只需说`String[] args`和`String... args`定义了本质上相同的输入格式

我们在命令行[1]部分的[0]执行示例中解释了如何使用命令行运行 main 类。您可以在 Oracle 官方文档中阅读更多关于 Java 命令行参数的信息：[https://docs.oracle.com/javase/tutorial/essential/environment/cmdLineArgs.html](https://docs.oracle.com/javase/tutorial/essential/environment/cmdLineArgs.html) . 也可以运行 IntelliJ IDEA 中的示例。

注意下面截图中左边的两个绿色三角形。点击其中任何一个，都可以执行`main()`方法。例如，让我们显示`Hello, world!`。
...
可以在用双斜杠`//`分隔的代码行右侧添加注释（任何文本）。编译器不读取此文本，只是保持原样。注释的出现并不影响性能，而是用来向人类解释程序员的意图。

# 导入项目

在本节中，我们将演示使用本书的源代码将现有代码导入 IntelliJ IDEA 的过程。我们假设您已经安装了 Maven（[0]https://maven.apache.org/install.html ）并且你有 Git（[https://gist.github.com/derhuerst/1b15ff4652a867391f03](https://gist.github.com/derhuerst/1b15ff4652a867391f03) ）也安装了，可以使用。我们还假设您已经安装了 jdk12，正如 javase 的*安装*一节所述。

要使用本书的代码示例导入项目，请执行以下步骤：

1.  转到源库（[0]https://github.com/PacktPublishing/Learn-Java-12-Programming ）并单击克隆或下载链接，如下图所示：
...
5.  新的[0]文件夹包含 Maven 项目以及本书中的所有源代码。现在运行 IntelliJ IDEA 并单击最顶部菜单中的“文件”，然后单击“新建”和“从现有源项目…”：
...
8.  选择`12`，点击【完成】。您将看到项目导入到 IntelliJ IDEA 中：
...
13.  如果列出了主模块和测试模块，请通过高亮显示它们并单击减号（[0]）来删除它们，如下屏幕所示：
...
3.  如果要运行[0]包中的示例文件，请使用不同的包和类名运行相同的命令：
...
请注意，Windows 命令具有不同的斜杠和分号（[0]）作为类路径分隔符。

4.  结果如下：

![](img/c589778c-12d6-4904-95f4-4912bd3921c7.png)

5.  这样，您就可以运行任何包含[0]方法的类。将执行`main()`方法的内容。

# Java 基元类型和运算符

有了所有主要的编程工具，我们就可以开始把 Java 作为一种语言来讨论了。语言语法由 Java 语言规范定义，您可以在[上找到 https://docs.oracle.com/javase/specs](https://docs.oracle.com/javase/specs) . 每次你需要澄清的时候，请不要犹豫参考它。这并不像很多人想象的那么令人畏惧

Java 中的所有值都分为两类：`reference`类型和`primitive`类型。我们从原始类型和运算符开始，作为任何编程语言的自然入口点。在本章中，我们还将讨论一种称为`String`的引用类型（参见*字符串类型和文字*部分）

所有的原始类型都可以分为两类：`boolean`类型和`numeric`类型。

# 布尔型

Java 中只有两个`boolean`类型值：`true`和`false`。这样的值只能赋给`boolean`类型的变量，例如：

```
boolean b = true;
```

`boolean`变量通常用于控制流语句中，我们将在*Java 语句*部分讨论。下面是一个例子：
...
**Java****数字类型**分为两组：整数类型（`byte`、`char`、`short`、`int`、`long`）和浮点类型（`float`、`double`）。
...
[0]类型是一个无符号整数，可以保存 0 到 65535 之间的值（称为[1]码位[2]）。它表示一个 Unicode 字符，这意味着有 65536 个 Unicode 字符。以下是构成 Unicode 字符基本拉丁列表的三条记录：
...
代码示例的最后两行解释了为什么将[0]类型视为整数类型，因为在算术运算中可以使用[1]值。在这种情况下，每个`char`值由其代码点表示。
...
始终可以从相应的 Java 常量中检索每个基元类型的最大值和最小值，如下所示：
...
最后两行中的构造`(int)`是**cast 操作符**用法的一个示例。它强制将值从一种类型转换为另一种类型，但这种转换并不总是保证成功。从我们的示例中可以看到，某些类型允许比其他类型更大的值。但是程序员可能知道某个变量的值永远不会超过目标类型的最大值，而 cast 操作符是程序员将自己的观点强加给编译器的方式。否则，如果没有 cast 运算符，编译器将引发错误，并且不允许赋值。但是，程序员可能会弄错，值可能会变大。在这种情况下，将在执行期间引发运行时错误。

但有些类型原则上不能转换为其他类型，或者至少不能转换为所有类型。例如，`boolean`类型值不能转换为整数类型值。

# 浮点类型

这组基元类型中有两种类型，`float`和`double`：
...
最大和最小负值与刚才显示的值相同，只是前面有一个减号（[0]）。因此，实际上，值`Float.MIN_VALUE`和`Double.MIN_VALUE`不是最小值，而是对应类型的精度。对于每种浮点类型，零值可以是`0.0`或`-0.0`
...
若要表示希望将其视为`float`类型，需要添加`f`或`F`。例如，以下分配不会导致错误：
...
您可能已经从示例中注意到，`d`和`D`表示一个`double`类型。但是我们能够将它们转换为`float`类型，因为我们确信`42.3`完全在`float`类型的可能值范围内。

# 基本类型的默认值

在某些情况下，即使程序员不想这样做，也必须给变量赋值。我们将在[第二章](02.html)，*Java 面向对象编程（OOP）*中讨论这种情况。在这种情况下，默认的基元类型值如下所示：
...
值的表示称为**文本**。`boolean`类型有两个文本：`true`和`false`。`byte`、`short`、`int`、`long`整型的文字默认为`int`型：
...
另外，要表示一个`long`类型的文字，可以在后面加上字母`l`或`L`：
...
字母`l`很容易与数字 1 混淆，因此为此使用`L`（而不是`l`）是一个很好的做法。

到目前为止，我们已经用十进制表示整数文字。同时，`byte`、`short`、`int`和`long`类型的文字也可以用二进制（以 2 为基数，数字 0-1）、八进制（以 8 为基数，数字 0-7）和十六进制（以 16 为基数，数字 0-9 和 a-f）数制表示。二进制文字以`0b`（或`0B`开头，后跟二进制表示的值。例如，小数点`42`表示为`101010 = 2^0*0 + 2^1*1 + 2^2*0 + 2^3 *1  + 2^4 *0  + 2^5 *1`（我们从右边`0`开始）。八进制文字以`0`开头，后跟八进制表示的值，因此`42`表示为`52 = 8^0*2+ 8^1*5`。十六进制文字以`0x`（或`0X`开头），后跟十六进制表示的值。因此，将`42`表示为`2a = 16^0*a + 16^1*2`，因为在十六进制中，符号`a`到`f`（或`A`到`F`映射到十进制值`10`到`15`。下面是演示代码：
...
数字文字的一个特点是对人友好。如果数字很大，可以将其分成三个部分，用下划线（`_`号）分隔。例如，请注意以下事项：
...
类型有两种文字：**单个字符**或**转义序列**。在讨论数字类型时，我们已经看到了`char`类型文字的示例：
...
转义序列以反斜杠（[0]）开头，后跟字母或其他字符。以下是转义序列的完整列表：
...
如您所见，`\b`删除上一个符号，`\t`插入制表符，`\n`换行并开始新的一行，`\f`强制打印机弹出当前页并在另一页的顶部继续打印，`/r`重新开始当前行。

# 新的压缩数字格式

[0]类以各种格式显示数字。它还允许根据所提供的格式（包括区域设置）调整格式。称为**压缩**或**短****数字格式**。
...
我们将不描述不常用的赋值运算符`&=`、`|=`、`^=`、`<<=`、`>>=`、`>>>=`和位运算符。您可以在 Java 规范（[中了解它们 https://docs.oracle.com/javase/specs](https://docs.oracle.com/javase/specs) ）。箭头`->`和方法参考`::`运算符将在[第 14 章](13.html)、*函数编程*中描述。实例创建操作符`new`、字段访问/方法调用操作符`.`、类型比较操作符`instanceof`将在[第 2 章](02.html)、*Java 面向对象编程*中讨论。至于`cast`运算符，我们已经在*积分类型*一节中描述过了。

# 算术一元（+和-）和二元运算符（+、-、*、/、和%）

大多数算术运算符和正负号（**一元运算符**运算符）我们都很熟悉。模运算符`%`将左操作数除以右操作数，并返回余数，如下所示：
...
`++`运算符将整型的值增加`1`，而`--`运算符将整型的值减少`1`，如果放在变量前面（前缀），则在返回变量值之前将其值更改 1。但是当放在变量后面（后缀）时，它会在返回变量值后将其值更改`1`。以下是几个例子：
...
但是，在比较浮点类型的值时，尤其是在比较计算结果时，要小心。在这种情况下，使用关系运算符（`<`、`>`、`<=`和`>=`）更可靠，因为例如，除法`1/3`会产生一个永无止境的小数部分`0.33333333...`，最终取决于精确实现（这是一个复杂的主题，超出了本书的范围）。

# 关系运算符（、<=, and >=）

**关系运算符**比较值并返回一个`boolean`值。例如，请观察以下情况：
...
[0]和[1]运算符产生的结果与我们刚才演示的[2]和[3]逻辑运算符相同：
...
不同之处在于`&&`和`||`运算符并不总是求值第二个操作数。例如，在`&&`运算符的情况下，如果第一个操作数是`false`，则不会求值第二个操作数，因为整个表达式的结果无论如何都将是`false`。类似地，在使用`||`运算符的情况下，如果第一个操作数是`true`，则整个表达式将被清楚地求值为`true`，而不求值第二个操作数。我们可以用以下代码来演示：
...
`? :`运算符称为**三元运算符**。它计算一个条件（在符号`?`之前），如果结果是`true`，则将第一个表达式（在符号`?`和`:`之间）计算的值赋给变量；否则，将第二个表达式（在符号`:`之后）计算的值赋给变量：
...
我们刚刚描述了 Java 语言的基本值类型。Java 中的所有其他值类型都属于一类**引用类型**。每个引用类型都是一个比值更复杂的构造。它由一个**类**来描述，该类用作创建一个**对象**的模板，该对象是一个包含类中定义的值和方法（处理代码）的存储区域。一个对象是由[0]操作符创建的。我们将在[第 2 章](02.html)、*Java 面向对象编程（OOP）*中更详细地讨论类和对象

在本章中，我们将讨论一种称为`String`的引用类型。它由`java.lang.String`类表示，如您所见，它属于 JDK 最基本的包`java.lang`。我们之所以在早期引入`String`类，是因为它在某些方面的行为与原始类型非常相似，尽管它是一个引用类型。

之所以称为引用类型，是因为在代码中，我们不直接处理此类型的值。引用类型的值比基元类型的值更复杂。它称为对象，需要更复杂的内存分配，因此引用类型变量包含内存引用。它指向*（指的是*对象所在的内存区域，因此得名。

当引用类型变量作为参数传递到方法中时，需要特别注意引用类型的这种性质。我们将在[第 3 章](03.html)、*Java 基础*中详细讨论。现在，关于`String`，我们将看到`String`作为引用类型如何通过只存储一次每个`String`值来优化内存使用。

# 字符串常量

[0]类表示 Java 程序中的字符串。我们见过好几根这样的弦。例如，我们已经看到了`Hello, world!`。这是一个字面意思。
...
但是由双引号（`"abc"`、`"123"`、`"a42%$#"`中的字符组成的文本只能是`String`类型。在这方面，`String`类作为引用类型，与基元类型有一些共同点。所有的`String`字面值都存储在一个称为**字符串池**的专用内存段中，两个字面值的拼写相同，表示池中的相同值：
...
JVM 作者选择了这样的实现来避免重复和提高内存使用率。前面的代码示例看起来很像带有基元类型的操作，不是吗？但是当使用`new`操作符创建`String`对象时，新对象的内存分配在字符串池之外，因此两个`String`对象或任何其他对象的引用总是不同的：
...
好消息是，您可能不需要使用[1]操作符创建[0]对象，而且大多数 Java 程序员从不这样做。但是，当一个[2]对象作为输入传递到您的代码中，而您无法控制其来源时，仅通过引用进行比较可能会导致错误的结果（如果字符串的拼写相同，但是由[3]创建的）。这就是为什么，当需要通过拼写（和大小写）使两个字符串相等时，为了比较两个文字或`String`对象，`equals()`方法是更好的选择：
...
稍后我们将讨论`equals()`和`String`类的其他方法。

使[0]文本和对象看起来像原始值的另一个特性是，可以使用算术运算符[1]添加它们：
...
其他算术运算符不能应用于[0]文本或对象。

最后，Java12 引入了一个新的`String`文本，称为**原始字符串文本**。它允许保留缩进和多行，而无需在引号中添加空格。例如，程序员将如何在 Java 12 之前添加缩进，并使用`\n`断行：
...
如您所见，原始字符串文本由一个或多个字符组成，这些字符包含在反引号（```（`\u0060`）中，也称为**反引号**或**重音符号**。

# 字符串不变性

因为所有的[0]文本都可以共享，所以 JVM 作者要确保，一旦存储，[1]变量就不能更改。它不仅有助于避免从代码的不同位置同时修改相同值的问题，而且还可以防止未经授权修改通常表示用户名或密码的[2]值。

下面的代码看起来像是一个`String`值修改：
...
我们将在[第 5](05.html)章中详细讨论字符串、*字符串、输入/输出和文件*。

# 标识符和变量

在我们上学的时候，我们对什么是变量有了直观的理解。我们认为它是一个代表一个值的名称。我们使用`x`加仑水或`n`英里等变量来解决问题。在 Java 中，变量名被称为**标识符**，可以通过一定的规则来构造。使用标识符，可以声明（定义）和初始化变量。

# 标识符

根据 Java 语言规范（[https://docs.oracle.com/javase/specs](https://docs.oracle.com/javase/specs) ），标识符（变量名）可以是表示字母、数字 0-9、美元符号（[0]）或下划线（[1]）的 Unicode 字符序列。

其他限制如下：

*   标识符的第一个符号不能是数字。
*   标识符的拼写不能与关键字相同（参见[第 3 章](03.html)、*Java 基础*中的 Java 关键字）。
*   不能拼写为`boolean`文字`true`或`false`或文字`null`。
...
变量有名称（标识符）和类型。通常，它指的是存储值的内存，但可能不指任何内容（`null`）或根本不指任何内容（那么它就不会被初始化）。它可以表示类属性、数组元素、方法参数和局部变量。最后一种是最常用的变量

在使用变量之前，必须声明并初始化它。在其他一些编程语言中，变量也可以被*定义为*，因此 Java 程序员有时使用*定义*这个词作为*声明*的同义词，这并不完全正确。
...
Java10 中引入了一种类型保持器`var`。Java 语言规范对其定义如下：“`var`*不是关键字，而是具有特殊含义的标识符，作为局部变量声明的类型*
...
在前面的示例中，编译器可以合理地假设`x`具有基元类型`int`。
...
也就是说，如果没有初始化，编译器就无法在使用`var`时确定变量的类型。

# Java 语句

**Java 语句**是可以执行的最小构造。它描述一个操作，并以分号（`;`结束。我们已经看到许多声明。例如，这里有三种说法：
...
*   只有一个符号`;`（分号）的空语句
*   类或接口声明语句（我们将在[第 2 章](02.html)、*Java 面向对象编程（OOP）*中讨论）
*   局部变量声明语句：`int x;`
*   同步声明：这超出了本书的范围
*   表达式语句
*   控制流语句
...
我们将在*expression statements*部分中详细讨论 expression 语句。
...
前面的表达式将一个值赋给一个`x`变量，并具有将`1`加到`y`变量的值上的副作用。
...
*   一个 lambda 表达式：`x -> x + 1`（参见[第 14 章](13.html)、*函数式编程*）

如果表达式由其他表达式组成，则括号通常用于清楚地标识每个表达式。这样，更容易理解和设置表达式的优先级。

# 控制流语句

当一个 Java 程序被执行时，它是一个语句一个语句地执行的。有些语句必须根据表达式求值的结果有条件地执行。这样的语句被称为**控制流语句**，因为在计算机科学中，控制流（或控制流）是执行或计算单个语句的顺序。
...
*   `if`（表达式）{做点什么}
*   `if`（表达式）{做点什么}`else`{做点别的}
*   `if`（表达式）{do something}`else if`{do something other}`else`{do something other}
*   `switch`案例陈述
...
在 Java 12 中，在预览模式中引入了一个新特性—一个不太冗长的[0]语句：
...
如您所见，它使用箭头`->`，而不使用`break`语句。要利用此功能，您必须在[3]和[4]命令中添加一个[2]选项。如果从 IDE 运行示例，则需要将此选项添加到配置中。在 IntelliJ IDEA 中，该选项应添加到两个配置屏幕：编译器和运行时：
...
Java12[0]语句甚至可以返回一个值。例如，这里的情况是，必须根据`switch...case`语句结果分配另一个变量：
...
在一些示例中，我们使用了不给另一行馈电（不在其输出端添加换行控件）的方法来代替`println()`方法。`print()`方法在一行中显示输出。
...
它与[0]语句不同，它总是在计算表达式之前至少执行一次语句块：
...
*   `init`语句初始化某个变量。
*   使用当前变量值计算`boolean expression`：如果是`true`，则执行语句块，否则退出`for`语句。
...
或者，您可以使用更紧凑的形式的[0]语句来生成相同的结果，如下所示：
...
我们将在[第 6 章](06.html)中讨论集合、*数据结构、泛型和流行实用程序*。

# 异常处理语句

在 Java 中，有称为**异常**的类，它们表示中断正常执行流的事件。它们的名字通常以*例外*结尾：`NullPointerException`、`ClassCastException`、`ArrayIndexOutOfBoundsException`等等。

所有异常类都扩展了`java.lang.Exception`类，而`java.lang.Exception`类又扩展了`java.lang.Throwable`类（我们将在[第 2 章](02.html)、*Java 面向对象编程（OOP）】*中解释这意味着什么。这就是为什么所有异常对象都有一个共同的行为。它们包含有关异常情况的原因及其起源位置（源代码行号）的信息。

每个异常对象可以由 JVM 自动生成（抛出），也可以由应用程序代码使用关键字`throw`自动生成（抛出）。如果一个代码块抛出异常，您可以使用`try-catch`或`try-catch-finally`构造来捕获抛出的异常对象，并将执行流重定向到另一个代码分支。如果周围的代码没有捕获异常对象，它将从应用程序传播到 JVM 并强制它退出（并中止应用程序执行）。因此，在所有可能引发异常的地方使用`try-catch`或`try-catch-finally`是一种很好的做法，您不希望应用程序中止执行。
...
在前面的代码段中，`x > 10`的情况下不执行`normal processing flow`。相反，将执行[2]块。但在`x <= 10`的情况下，运行`normal processing flow`块，忽略`do what has to be done`块。
...
我们将在[第 4 章](04.html)、*异常处理*中详细介绍异常处理。
...
如果我们需要找到第一个包含`"3"`的列表元素，我们可以在`condition s.contains("3")`计算为`true`后立即停止执行。其余的列表元素将被忽略。

在更复杂的场景中，使用嵌套的`for`语句，可以设置一个标签（带有`:`列）来指示必须退出哪个`for`语句：
...
我们已选择标签名称`exit`，但也可以将其命名为任何其他名称。
...
它与[0]的不同之处在于告诉[1]语句中的哪一个继续，而不是仅仅退出。
`return`语句用于返回方法的结果：
...
在下一章中，我们将讨论 Java 的面向对象方面。我们将介绍主要概念，解释什么是类，什么是接口，以及它们之间的关系。术语*重载*、*重写*、*隐藏*也将在代码示例中定义和演示，以及`final`关键字的用法。
