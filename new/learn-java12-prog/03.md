...
将原始类型值转换为相应包装类的对象称为**装箱**。此外，从包装类的对象到相应的基元类型值的转换被称为**解装箱**。

# 拳击

原始类型的装箱可以自动完成（称为**自动装箱**），也可以显式使用每个包装器类型中可用的`valueOf()`方法完成：
...
从示例中的注释可以看出，从包装器类型到相应的原始类型的转换不是称为**自动拆箱**，而是称为**隐式拆箱**。与自动装箱不同的是，即使在包装和不匹配的基元类型之间也可以使用隐式取消装箱。
