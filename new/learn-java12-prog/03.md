...
*   `hashCode()`方法：这个方法返回的整数唯一地标识这个特定的对象（但是请不要期望它在应用的不同运行之间是相同的）。如果唯一需要的方法是`equals()`，则不需要实现此方法。尽管如此，我们还是建议在`Set`或基于哈希代码的另一个集合中收集此类的对象时使用它（我们将在第 6 章、“数据结构、泛型和流行实用程序”中讨论 Java 集合）
