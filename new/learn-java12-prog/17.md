...
在本章中，读者将介绍一个允许测量各种代码性能特征的**Java 微基准线束**（**JMH**）项目。如果性能是应用程序的一个重要问题，那么这个工具可以帮助您识别瓶颈，精确到方法级别。使用它，读者不仅能够测量代码的平均执行时间和其他性能值（例如吞吐量），而且能够以一种受控的方式进行测量，不管是否有 JVM 优化、预热运行等等。
...
根据字典，**基准**是*一个标准或参照点，可以用来比较或评估事物*。在编程中，它是比较应用程序性能的一种方法，或者只是比较方法。**微序言**关注的是后者，即较小的代码片段，而不是整个应用程序。JMH 是衡量单个方法性能的框架。

这似乎非常有用。我们能不能不只是在一个循环中运行一个方法一千次或十万次，测量它所用的时间，然后计算方法性能的平均值？我们可以。问题是 JVM 是一个比代码执行机器复杂得多的程序。它的优化算法专注于使应用程序代码尽可能快地运行。
...
我们在[0]方法中填充了一些代码，这些代码没有多大意义，但使该方法保持忙碌。要测试此方法的性能，很有可能将代码复制到某个测试方法中并在循环中运行：
...
这可能会说服 JVM 每次都执行代码，但不能保证。JVM 可能会注意到，输入到计算中的数据并没有改变，这个算法每次运行都会产生相同的结果。由于代码是基于常量输入的，所以这种优化称为**常量折叠**。此优化的结果是，此代码可能只执行一次，并且每次运行都假定相同的结果，而不实际执行代码。
...
JMH 的创建是为了帮助避免这种情况和类似的陷阱。在*JMH 用法示例*部分，我们将向您展示如何使用 JMH 解决死代码和常量折叠优化问题，使用`@State`注释和`Blackhole`对象。
...
请注意[0]注释。它告诉框架必须测量此方法的性能。如果您运行前面的`main()`方法，您将看到类似以下的输出：

![](img/3316accf-de05-4c3a-a117-a67dd492be62.png)

这只是包含不同条件下的多次迭代的广泛输出的一部分，目的是避免或抵消 JVM 优化。它还考虑了一次运行代码和多次运行代码之间的差异。在后一种情况下，JVM 开始使用 just-In-time 编译器，它将经常使用的字节码编译成本地二进制代码，甚至不读取字节码。预热周期就是为了达到这个目的而执行代码的，而不去衡量它的性能，就像一个*预热*JVM 的干运行。
...
您可能已经猜到，运行基准测试的一种方法就是执行[0]方法。可以直接使用`java`命令，也可以使用 IDE。我们在[第 1](01.html)章*Java 入门 12*中讨论过。不过，运行基准测试有一种更简单、更方便的方法：使用 IDE 插件。
...
1.  要开始安装插件，请同时按[0]命令[1]键和逗号（[2]、[3]），或者只需单击顶部水平菜单中的扳手符号（带有悬停文本首选项）：
...
3.  选择 Plugins（插件），如前面的屏幕截图所示，并观察具有以下顶部水平菜单的窗口：

![](img/c997082a-cb91-4937-9daf-e17839eb00be.png)

4.  选择 Marketplace，在 Marketplace 输入框的 Search plugins 中输入*JMH*，然后按*回车*。如果您有 internet 连接，它将显示一个 JMH 插件符号，类似于此屏幕截图中显示的符号：
...
6.  IDE 重新启动后，插件就可以使用了。现在，您不仅可以运行`main()`方法，而且还可以选择要执行的基准测试方法（如果您有多个带有`@Benchmark`注释的方法）。要执行此操作，请从“运行”下拉菜单中选择“运行…”：
...
显示结果所用的时间单位可以使用`@OutputTimeUnit`注释指定：
...
[0]值用于在测试之间共享数据。在我们的例子中，只有一个测试使用了`TestCase`类对象，我们不需要共享。否则，可以将该值设置为`Scope.Group`或`Scope.Benchmark`，这意味着我们可以在`TestState`类中添加 setter，并在其他测试中读取/修改它。
...
如您所见，我们刚刚添加了一个参数`Blackhole`对象，并对其调用了`consume()`方法，从而假装使用了测试方法的结果。

当我们运行此版本的测试时，得到以下结果：

![](img/c655a032-9ced-4a09-bce1-a2c5fcc28a5d.png)

这一次，结果看起来没什么不同。显然，即使在添加[0]用法之前，恒定折叠优化也被中和了
...
*   `Mode.EXCLUDE`：为避免此方法编译

# 使用@Param 注释

有时，有必要对不同的输入数据集运行相同的基准测试。在这种情况下，`@Param`注释非常有用。
...
探查器的描述及其用法可在`openjdk`项目（[中找到 http://hg.openjdk.java.net/code-tools/jmh/file/tip/jmh-samples/src/main/java/org/openjdk/jmh/samples](http://hg.openjdk.java.net/code-tools/jmh/file/tip/jmh-samples/src/main/java/org/openjdk/jmh/samples) ). 在相同的示例中，您将看到 JMH 基于注释生成的代码的描述。
...
在本章中，读者了解了 JMH 工具，并能够将其用于特定的实际案例，类似于他们在编程应用程序时遇到的那些案例。读者已经学习了如何创建和运行基准，如何设置基准参数，以及如何在需要时安装 IDE 插件。我们也提供了实用的建议和参考资料供进一步阅读。

在下一章中，读者将介绍设计和编写应用程序代码的有用实践。我们将讨论 Java 习惯用法、它们的实现和用法，并提供实现[0]、[1]、[2]和[3]方法的建议。我们还将讨论`StringBuffer`和`StringBuilder`类的用法之间的区别、如何捕获异常、最佳设计实践以及其他经验证的编程实践。
