...
`ServerSocket`和`SocketClass`类执行非常相似的任务。唯一的区别是，`ServerSocket`类有`accept()`方法，*接受来自客户机的*请求。这意味着服务器必须先启动并准备好接收请求。然后，连接由客户机启动，客户机创建自己的套接字来发送连接请求（来自`Socket`类的构造器）。然后服务器接受请求并创建一个连接到远程套接字的本地套接字（在客户端）。
