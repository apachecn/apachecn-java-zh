...
在本章中，我们将描述和讨论最流行的网络协议——**用户数据报协议**（**UDP**）、**传输控制协议**（**TCP**）、**超文本传输协议**（**HTTP**），**WebSocket**——以及**Java 类库**（**JCL**提供的支持。我们将演示如何使用这些协议以及如何用 Java 代码实现客户机-服务器通信。我们还将回顾基于**统一资源定位器**（**URL**）的通信和最新的**Java HTTP 客户端 API**。
...
*   **链路层**：客户端与主机物理连接时使用的一组协议，三个核心协议包括**地址解析协议**（**ARP**）、**反向地址解析协议****协议**（**RARP**），以及**邻居发现协议**（**NDP**）。
*   **网际网路层**：由 IP 位址所指定，用来将网路封包从始发主机传送至目的主机的一组网际网路方法、通讯协定及规格。这一层的核心协议是**互联网协议版本 4**（**IPv4**）和**互联网协议版本 6**（**IPv6**）；IPv6 指定了一种新的数据包格式，并为点式 IP 地址分配 128 位，而 IPv4 为 32 位。IPv4 地址的一个例子是`10011010.00010111.11111110.00010001`，其结果是 IP 地址为`154.23.254.17`。
*   **传输层**：主机对主机通信服务组。它包括 TCP，又称 TCP/IP 协议和 UDP（我们稍后将讨论）；这组协议中的其他协议有**数据报拥塞控制协议**（**DCCP**）和**流控制传输协议**（**SCTP**）。
*   **应用层**：通信网络中主机使用的一组协议和接口方法。包括**Telnet**、**文件传输协议**（**FTP**）、**域名系统**（**DNS**）、**简单邮件传输协议**（**SMTP**）、**轻量级目录访问协议**（**LDAP**）， **超文本传输协议**（**HTTP**）、**超文本传输协议安全**（**HTTPS**）、**安全壳**（**SSH**）。
...
Java 通过`java.net`包的类支持 TCP 和 UDP 协议，而 HTTP 协议可以在 Java 应用程序中使用`java.net.http`包的类（Java 11 引入）来实现。
...
数据报由`java.net.DatagramPacket`类表示。此类的对象可以使用六个构造函数中的一个来创建；以下两个构造函数是最常用的：

*   `DatagramPacket(byte[] buffer, int length)`：此构造函数创建一个数据报包，用于接收数据包；`buffer`保存传入的数据报，`length`是要读取的字节数。
...
构造后，`DatagramPacket`对象将公开以下方法，这些方法可用于从对象中提取数据或设置/获取其属性：
...
一旦创建了一个`DatagramPacket`对象，就可以使用`DatagramSocket`类来发送或接收它，该类表示用于发送和接收数据包的无连接套接字。这个类的对象可以使用六个构造函数中的一个来创建；以下三个构造函数是最常用的：

*   `DatagramSocket()`：这将创建一个数据报套接字，并将其绑定到本地主机上的任何可用端口。它通常用于创建发送套接字，因为目标地址（和端口）可以在包内设置（参见前面的`DatagramPacket`构造函数和方法）。
*   `DatagramSocket(int port)`：创建一个数据报套接字，绑定到本地主机的指定端口。当任何本地机器地址（称为[1]通配符地址[2]）足够好时，它用来创建一个接收套接字。
*   `DatagramSocket(int port, InetAddress address)`：创建一个数据报套接字，绑定到指定的端口和指定的本地地址，本地端口必须在`0`和`65535`之间。它用于在需要绑定特定的本地计算机地址时创建接收套接字。

`DatagramSocket`对象的以下两种方法是发送和接收消息（或数据包）最常用的方法：

*   `void send(DatagramPacket p)`：发送指定的数据包。
*   `void receive(DatagramPacket p)`：通过将接收到的数据填充指定的`DatagramPacket`对象的缓冲区来接收数据包。指定的`DatagramPacket`对象还包含发送方的 IP 地址和发送方机器上的端口号。
...
我们现在可以运行发送器，但是如果没有接收器运行，就没有人能收到消息。所以，我们先启动接收器。它在端口`3333`上侦听，但没有消息，因此等待。然后，我们运行发送方，接收方显示以下消息：
...
这就是 UDP 协议的基本思想。发送方将消息发送到某个地址和端口，即使该地址和端口上没有*侦听*的套接字。它不需要在发送消息之前建立任何类型的连接，这使得 UDP 协议比 TCP 协议更快、更轻量级（TCP 协议要求您首先建立连接）。通过这种方式，TCP 协议将消息发送到另一个可靠性级别—通过确保目标存在并且消息可以被传递。

# 基于 TCP 的通信

TCP 于 1970 年代由**国防高级研究计划局**（**DARPA**）设计，用于**高级研究计划局网络**（**ARPANET**）。它是对 IP 的补充，因此也被称为 TCP/IP。TCP 协议，甚至其名称，都表明它提供了可靠的（即，错误检查或控制的）数据传输。它允许在 IP 网络中按顺序传递字节，广泛用于 web、电子邮件、安全 shell 和文件传输
...
[0]和[1]类执行非常相似的任务。唯一的区别是`ServerSocket`类有`accept()`方法，即*接受*来自客户机的请求。这意味着服务器必须先启动并准备好接收请求。然后，连接由客户机启动，客户机创建自己的套接字来发送连接请求（来自`Socket`类的构造函数）。然后服务器接受请求并创建一个连接到远程套接字的本地套接字（在客户端）。

建立连接后，数据传输可以使用 I/O 流进行，如[第 5 章](05.html)、*字符串、输入/输出和文件*所述。`Socket`对象具有`getOutputStream()`和`getInputStream()`方法，提供对套接字数据流的访问。来自本地计算机上的`java.io.OutputStream`对象的数据似乎来自远程机器上的`java.io.InputStream`对象。

现在让我们仔细看看[0]和[1]类，然后运行它们的一些用法示例。
...
*   `ServerSocket(int port)`：创建绑定到所提供端口的服务器套接字对象。`port`值必须介于`0`和`65535`之间。如果端口号指定为值`0`，则表示需要自动绑定端口号。默认情况下，传入连接的最大队列长度为`50`。
*   `ServerSocket(int port, int backlog)`：提供与`ServerSocket(int port)`构造函数相同的功能，允许您通过`backlog`参数设置传入连接的最大队列长度。
*   `ServerSocket(int port, int backlog, InetAddress bindAddr)`：这将创建一个服务器套接字对象，该对象类似于前面的构造函数，但也绑定到提供的 IP 地址。当`bindAddr`值为`null`时，默认接受任何或所有本地地址的连接。

`ServerSocket`类的以下四种方法是最常用的，它们是建立套接字连接所必需的：

*   `void bind(SocketAddress endpoint)`：将`ServerSocket`对象绑定到特定的 IP 地址和端口。如果提供的地址是`null`，则系统会自动获取一个端口和一个有效的本地地址（以后可以使用`getLocalPort()`、`getLocalSocketAddress()`、`getInetAddress()`方法进行检索）。另外，如果`ServerSocket`对象是由构造函数在没有任何参数的情况下创建的，则需要调用此方法或下面的`bind()`方法才能建立连接。
*   `void bind(SocketAddress endpoint, int backlog)`：这与前面的方法类似，`backlog`参数是套接字上挂起的最大连接数（即队列的大小）。如果`backlog`值小于或等于`0`，则将使用特定于实现的默认值。
*   `void setSoTimeout(int timeout)`：设置调用`accept()`方法后套接字等待客户端的时间（毫秒）。如果客户端没有调用并且超时过期，则抛出一个`java.net.SocketTimeoutException`异常，但是`ServerSocket`对象仍然有效，可以重用。`0`的`timeout`值被解释为无限超时（在客户端调用之前，`accept()`方法阻塞）。
*   `Socket accept()`：在客户端调用或超时期限（如果设置）到期之前，此功能一直阻止。

该类的其他方法允许您设置或获取`Socket`对象的其他属性，它们可以用于更好地动态管理套接字连接。您可以参考该类的联机文档来更详细地了解可用选项。

下面的代码是使用`ServerSocket`类的服务器实现的示例：
...
让我们浏览前面的代码。在 try with resources 语句中，我们基于新创建的 socket 创建了`Socket`、`DataInputStream`和`DataOutputStream`对象，并创建了`BufferedReader`对象来从控制台读取用户输入（我们将使用它来输入数据）。在创建套接字时，`accept()`方法会阻塞，直到客户端尝试连接到本地服务器的端口`3333`

然后，代码进入无限循环。首先，使用`DataInputStream`的`readUTF()`方法，将客户端发送的字节读取为修改后的 UTF-8 格式编码的 Unicode 字符串。结果以`"Client said: "`前缀打印。如果收到的消息是一个[3]字符串，那么代码退出循环，服务器程序退出。如果消息不是`"end"`，则控制台上显示`"Say something: "`提示，`readLine()`方法阻塞，直到用户键入内容并点击*进入*

服务器从屏幕获取输入，并使用[0]方法将其作为 Unicode 字符串写入输出流。正如我们已经提到的，服务器的输出流连接到客户机的输入流。如果客户机从输入流中读取数据，它将接收服务器发送的消息。如果发送的消息为`"end"`，则服务器退出循环并退出程序。如果不是，则再次执行循环体。
...
[0]类现在应该是您熟悉的了，因为它是在前面的示例中使用的。我们使用它来访问连接的套接字的输入和输出流。现在我们将系统地回顾`Socket`类，并探讨如何使用它来创建 TCP 客户机。`Socket`类有四个构造函数：

*   `Socket()`：这将创建一个未连接的套接字。它使用`connect()`方法将此套接字与服务器上的套接字建立连接。
*   `Socket(String host, int port)`：这将创建一个套接字并将其连接到`host`服务器上提供的端口。如果抛出异常，则无法建立到服务器的连接；否则，可以开始向服务器发送数据。
*   `Socket(InetAddress address, int port)`：其作用方式与前面的构造函数类似，只是主机作为`InetAddress`对象提供。
*   `Socket(String host, int port, InetAddress localAddr, int localPort)`：它的工作方式与前面的构造函数类似，只是它还允许您将套接字绑定到提供的本地地址和端口（如果程序在具有多个 IP 地址的计算机上运行）。如果提供的`localAddr`值为`null`，则选择任何本地地址。或者，如果提供的`localPort`值是`null`，则系统在绑定操作中拾取一个空闲端口。
*   `Socket(InetAddress address, int port, InetAddress localAddr, int localPort)`：除了本地地址作为`InetAddress`对象提供外，其作用与前面的构造函数类似。

下面是我们已经使用过的`Socket`类的两种方法：

*   `InputStream getInputStream()`：返回一个表示源（远程套接字）的对象，并将数据（输入）带入程序（本地套接字）。

*   `OutputStream getOutputStream()`：返回一个表示源（本地套接字）的对象，并将数据发送（输出）到远程套接字。
...
前面的`TcpClient`代码看起来与我们回顾的`TcpServer`代码几乎完全相同。唯一主要的区别是`new Socket("localhost", 3333)`构造函数试图立即与`"localhost:3333"`服务器建立连接，因此它期望`localhost`服务器启动并监听端口`3333`，其余与服务器代码相同。
...
现在运行`TcpServer`和`TcpClient`程序。如果我们先启动`TcpClient`，我们会得到`java.net.ConnectException`的连接拒绝消息。所以，我们先启动`TcpServer`程序。当它启动时，不显示任何消息，而是等待客户端连接。因此，我们开始`TcpClient`并在屏幕上看到以下消息：
...
*   此外，协议要求标头与数据包一起发送。TCP 数据包的头大小是 20 字节，而数据报数据包是 8 字节。UDP 报头包含`Length`、`Source Port`、`Destination Port`、`Checksum`，TCP 报头除 UDP 报头外，还包含`Sequence Number`、`Ack Number`、`Data Offset`、`Reserved`、`Control Bit`、`Window`、`Urgent Pointer`、`Options`、`Padding`
*   有基于 TCP 或 UDP 协议的不同应用程序协议。基于**TCP**的协议有**HTTP**、**HTTPS**、**Telnet**、**FTP**和**SMTP**。基于**UDP**的协议有**动态主机配置协议**（**DHCP**）、**DNS**、**简单****网络管理协议**（**SNMP**）、**普通文件传输协议**（**TFTP**），**Bootstrap 协议**（**BOOTP**），以及早期版本的**网络文件系统**（**NFS**）。
...
一般来说，URL 语法遵循**统一资源标识符**（**URI**）的语法，其格式如下：

```
scheme:[//authority]path[?query][#fragment]
```

方括号表示组件是可选的。这意味着 URI 将至少由[0]组成。`scheme`分量可以是`http`、`https`、`ftp`、`mailto`、`file`、`data`或其他值。`path`组件由一系列由斜线（`/`分隔的路径段组成。以下是仅由`scheme`和`path`组成的 URL 示例：
...
[0]组件可以是空的，但这样 URL 看起来就没用了。然而，空路径通常与`authority`结合使用，其格式如下：
...
最后，可选的`fragment`组件是 HTML 文档的一个节的标识符，这样浏览器就可以将该节滚动到视图中。
...
*   `URL(String spec)`：从 URL 创建一个字符串形式的`URL`对象。
*   `URL(String protocol, String host, String file)`：根据`protocol`、`host`、`file`（`path`、`query`）提供的值，以及基于`protocol`提供的默认端口号，创建一个`URL`对象。
...
*   `URL(String protocol, String host, int port, String file, URLStreamHandler handler)`：这与前面的构造函数的作用方式相同，并且允许您传入特定协议处理程序的对象；所有前面的构造函数都自动加载默认处理程序。
...
*   `URL(URL context, String spec, URLStreamHandler handler)`：它的作用方式与前面的构造函数相同，另外还允许您传入特定协议处理程序的对象。

创建后，`URL`对象允许您获取基础 URL 的各个组件的值。方法提供对从 URL 接收的数据流的访问。实际上，实现为`openConnection.getInputStream()`。`URL`类的`URLConnection openConnection()`方法返回一个`URLConnection`对象，其中有许多方法提供与 URL 连接的详细信息，包括允许向 URL 发送数据的`getOutputStream()`方法。

让我们看一下代码示例；我们首先从一个`hello.txt`文件中读取数据，这个文件是我们在[第 5 章](05.html)中创建的本地文件，*字符串、输入/输出和文件*。文件只有一行：“*你好！* “下面是代码：
...
现在，让我们演示 Java 代码如何从指向 internet 上源的 URL 读取数据。让我们用一个`Java`关键字来调用 Google 搜索引擎：
...
在这里，我们提出了`https://www.google.com/search?q=Java&num=10`URL，并在经过一些研究和实验后请求了属性。我们无法保证它会一直工作，因此如果它不返回我们描述的相同数据，请不要感到惊讶。此外，这是一个实时搜索，因此结果可能会随时更改

前面的代码还演示了由`getPath()`和`getFile()`方法返回的值之间的差异。您可以在前面的代码示例中查看内联注释。

与使用文件 URL 的示例相比，Google 搜索示例使用了`URLConnection`对象，因为我们需要设置请求头字段：

*   `Accept`告知服务器调用者请求的内容类型（`understands`。
*   `Connection`通知服务器收到响应后，连接将关闭。
*   `Accept-Language`告知服务器调用者请求哪种语言（`understands`）。
*   `User-Agent`告诉服务器关于呼叫者的信息；否则，Google 搜索引擎（[www.google.com](https://www.google.com/) ）以 403（禁止的）HTTP 代码响应。

上一个示例中的其余代码只是读取来自 URL 的输入数据流（HTML 代码），然后逐行打印它。我们捕获了结果（从屏幕上复制），将其粘贴到在线 HTML 格式化程序中（[https://jsonformatter.org/html-pretty-print](https://jsonformatter.org/html-pretty-print) ），然后运行它。结果显示在以下屏幕截图中：

![](img/fe98844c-59e3-4467-a885-3ee785ee7e23.png)

如您所见，除了左上角没有返回 HTML 的*Google*图像外，它看起来像一个带有搜索结果的典型页面。
...
前面的代码要求在端口`3333`上的`localhost`服务器上运行一个服务器，该服务器可以处理具有`"/something"`路径的`POST`请求。如果服务器没有检查方法（是`POST`还是其他 HTTP 方法）并且没有检查`User-Agent`值，则不需要指定任何方法。因此，我们对设置进行注释，并将它们保留在那里，只是为了演示如何在需要时设置这些值和类似的值。

注意，我们使用了`setDoOutput()`方法来指示必须发送输出；默认情况下，它被设置为`false`。然后，让输出流将查询参数发送到服务器

前面代码的另一个重要方面是在打开输入流之前必须关闭输出流。否则，输出流的内容将不会发送到服务器。虽然我们显式地这样做了，但更好的方法是使用 try with resources 块来确保调用[0]方法，即使在块中的任何地方引发了异常。
...
为了演示此示例的工作原理，我们还创建了一个简单的服务器，它侦听`localhost`的端口`3333`，并分配了一个处理程序来处理`"/something"`路径中的所有请求：
...
请求处理完成后，服务器发回消息*“收到！谢谢。”*让我们看看它是怎么工作的，我们先运行服务器。它开始侦听端口`3333`并阻塞，直到请求带有`"/something"`路径。然后，我们执行客户端并在服务器端屏幕上观察以下输出：
...
`URL`和`URLConnection`类的其他方法允许您设置/获取其他属性，并可用于对客户机-服务器通信进行更动态的管理。在`java.net`包中还有`HttpUrlConnection`类（和其他类），它简化并增强了基于 URL 的通信。您可以阅读`java.net`软件包的在线文档，以便更好地了解可用的选项。

# 使用 HTTP 2 客户端 API

HTTP 客户端 API 是在 Java9 中引入的，作为`jdk.incubator.http`包中的孵化 API，在 Java11 中被标准化并移到`java.net.http`包中，它是`URLConnection`API 的一个更丰富、更易使用的替代品。除了所有与连接相关的基本功能外，它还使用`CompletableFuture`提供非阻塞（异步）请求和响应，并支持 HTTP1.1 和 HTTP2。
...
*   `HttpClient`：用于同步和异步发送请求和接收响应。可以使用默认设置的静态`newHttpClient()`方法创建实例，也可以使用允许自定义客户端配置的`HttpClient.Builder`类（由静态`newBuilder()`方法返回）。一旦创建，实例是不可变的，可以多次使用。
*   `HttpRequest`：创建并表示一个包含目标 URI、头和其他相关信息的 HTTP 请求。可以使用`HttpRequest.Builder`类（由静态`newBuilder()`方法返回）创建实例。一旦创建，实例是不可变的，可以多次发送。
*   `HttpRequest.BodyPublisher`：发布某个源的 body（对于`POST`、`PUT`、`DELETE`方法），比如字符串、文件、输入流、字节数组。
*   `HttpResponse`：客户端发送 HTTP 请求后收到的 HTTP 响应。它包含源 URI、头、消息体和其他相关信息。创建实例后，可以多次查询实例。
*   `HttpResponse.BodyHandler`：接受响应并返回`HttpResponse.BodySubscriber`实例的函数接口，可以处理响应体。
...
您可以阅读[0]包的联机文档，以了解有关这些以及其他相关类和接口的更多信息。接下来，我们将看一看并讨论一些使用 httpapi 的示例。

# 阻止 HTTP 请求

下面的代码是一个简单的 HTTP 客户端的示例，它向 HTTP 服务器发送一个`GET`请求：
...
为了演示客户端的功能，我们将使用我们已经使用过的`UrlServer`类。提醒一下，它是如何处理客户的请求并以`"Got it! Thanks."`响应的：
...
在服务器返回响应之前，`HttpClient`类的`send()`方法被阻止
...
在服务器返回相同响应之前，`HttpClient`类的`send()`方法被阻止：

![](img/4831b1f9-02fa-4ea7-bc2d-ca959ff00a27.png)  

到目前为止，演示的功能与我们在上一节中看到的基于 URL 的通信没有太大区别。现在我们将使用在 URL 流中不可用的`HttpClient`方法。
...
与使用`send()`方法（返回`HttpResponse`对象）的示例相比，`sendAsync()`方法返回`CompletableFuture<HttpResponse>`类的实例。如果您阅读`CompletableFuture<T>`类的文档，您将看到它实现了`java.util.concurrent.CompletionStage`接口，该接口提供了许多可以链接的方法，并允许您设置各种函数来处理响应。

以下是`CompletionStage`界面中声明的方法列表：`acceptEither`、`acceptEitherAsync`、`acceptEitherAsync`、`applyToEither`、`applyToEitherAsync`、`applyToEitherAsync`、`handle`、`handleAsync`、`handleAsync`、`runAfterBoth`、`runAfterBothAsync`、`runAfterBothAsync`、`runAfterEither`、`runAfterEitherAsync`、`runAfterEitherAsync`、`thenAccept`、`thenAcceptAsync`、`thenAcceptAsync`、`thenAcceptBoth`、`thenAcceptBothAsync`、`thenAcceptBothAsync`，`thenApply`、`thenApplyAsync`、`thenApplyAsync`、`thenCombine`、`thenCombineAsync`、`thenCombineAsync`、`thenCompose`、`thenComposeAsync`、`thenComposeAsync`、`thenRun`、`thenRunAsync`、`thenRunAsync`、`whenComplete`、`whenCompleteAsync`、`whenCompleteAsync`。

我们将在[第 13 章](13.html)、*函数编程*中讨论函数以及如何将它们作为参数传递。现在我们只提一下，`resp -> System.out.println("Response: " + resp.statusCode() + " : " + resp.body())`构造表示与下面方法相同的功能：
...
`thenAccept()`方法将传入的功能应用于链的上一个方法返回的结果。

返回`CompletableFuture<Void>`实例后，前面的代码打印异步发送的请求…消息并在`CompletableFuture<Void>`对象的`get()`方法上阻塞。此方法有一个重载版本`get(long timeout, TimeUnit unit)`，有两个参数—指定单元数的`TimeUnit unit`和`long timeout`，指示该方法应该等待`CompletableFuture<Void>`对象表示的任务完成多长时间。在我们的例子中，任务是向服务器发送消息并获取响应（并使用提供的函数进行处理）。如果任务没有在分配的时间内完成，`get()`方法被中断（堆栈跟踪被打印在 catch 块中）。

在我们的例子中，`Exit the client...`消息应该在 5 秒钟内或者在`get()`方法返回之后出现在屏幕上。
...
如您所见，请求是异步发送的…消息在服务器返回响应之前出现。这就是异步调用的要点；向服务器发送的请求已发送，客户端可以继续执行任何其他操作。传入的函数将应用于服务器响应。同时，您可以传递`CompletableFuture<Void>`对象，并随时调用它来获得结果。在我们的例子中，结果是`void`，所以`get()`方法只是指示任务已经完成

我们知道服务器返回消息，所以我们可以使用`CompletionStage`接口的另一种方法来利用它。我们选择了`thenApply()`方法，它接受一个返回值的函数：
...
请注意传入请求的任意序列；这是因为客户端使用一个由`Executors.newCachedThreadPool()`个线程组成的池来发送消息。每个消息都由不同的线程发送，池有自己的使用池成员（线程）的逻辑。如果消息的数量很大，或者每个消息都占用大量内存，那么限制并发运行的线程数量可能是有益的

[0]类允许您指定用于获取发送消息的线程的池：
...
与 executor 类似，可以在[0]对象上设置其他几个对象来配置连接以处理身份验证、请求重定向、cookies 管理等。
...
WebSocket 协议规范将 WebSocket（`ws`）和 WebSocket Secure（`wss`）定义为两种方案，分别用于未加密和加密连接。该协议不支持分段，但允许使用*URL 语法*部分中描述的所有其他 URI 组件。
...
服务器也可以用类似的方式实现，但是服务器实现超出了本书的范围，为了演示前面的客户端代码，我们将使用`echo.websocket.org`网站提供的 WebSocket 服务器。它允许 WebSocket 连接并将接收到的消息发送回；这样的服务器通常称为**回送服务器**。
...
前面的代码使用`WebSocket.Builder`类创建一个`WebSocket`对象。`buildAsync()`方法返回`CompletableFuture`对象。`CompletableFuture`类的`join()`方法在完成时返回结果值，或者抛出异常。如果没有生成异常，那么正如我们已经提到的，`WebSocket`通信将继续，直到任何一方发送**关闭**消息。这就是为什么我们的客户机等待 200 毫秒，然后发送**Close**消息并退出。如果运行此代码，将看到以下消息：
...
9.  如何识别 TCP 会话？
10.  说出`ServerSocket`和`Socket`功能的一个主要区别。
11.  TCP 和 UDP 哪个更快？
12.  TCP 和 UDP 哪个更可靠？
13.  说出三个基于 TCP 的协议。
