...
`URL`和`URLConnection`类的其他方法允许您设置/获取其他属性，并且可以用于客户端-服务器通信的更动态的管理。在`java.net`包中还有`HttpUrlConnection`类（以及其他类），它简化并增强了基于 URL 的通信。您可以阅读`java.net`软件包的在线文档，以便更好地了解可用的选项。
