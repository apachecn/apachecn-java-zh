...
`A`是一个函数接口，因为它只有一个抽象方法`method1()`。`B`也是一个函数接口，因为它只有一个抽象方法，即从`A`接口继承的相同`method1()`。`C`是一个函数接口，因为它只有一个抽象方法`method1()`，它覆盖父接口`A`的抽象`method1()`。接口`D`不能是函数式接口，因为它有两个抽象方法-`method1()`来自父接口`A`和`method5()`。
...
如您所见，创建函数接口很容易，尤其是使用 lambda 表达式。但在此之前，请考虑使用包`java.util.function`中提供的 43 个函数式接口之一。这不仅可以让您编写更少的代码，还可以帮助其他熟悉标准接口的程序员更好地理解您的代码。
...
`java.util.function`包中提供的大部分接口是以下四种接口的专门化：`Consumer<T>`、`Predicate<T>`、`Supplier<T>`和`Function<T,R>`。让我们回顾一下它们，然后简单地概述一下其他 39 个标准函数式接口。
...
如果您要使用函数式接口，我们鼓励您学习`java.util.functional`包（[的接口 APIhttps://docs.oracle.com/en/java/javase/12/docs/api/java.base/java/util/function/package-summary.html](https://docs.oracle.com/en/java/javase/12/docs/api/java.base/java/util/function/package-summary.html)）。
