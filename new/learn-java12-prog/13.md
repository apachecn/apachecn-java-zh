...
本章将读者带入函数式编程的世界。它解释了什么是函数接口，概述了 JDK 附带的函数接口，并定义和演示了 lambda 表达式以及如何将它们与函数接口一起使用，包括使用**方法引用**。
...
在前面的章节中，我们实际使用了函数式编程。在[第 6 章](06.html)、*数据结构、泛型和流行实用程序*中，我们讨论了`Iterable`接口及其`default void forEach (Consumer<T> function)`方法，并提供了以下示例：
...
*   `default boolean remove(Predicate<E> filter)`方法，尝试从集合中删除满足给定谓词的所有元素；`Predicate`函数接受集合中的一个元素并返回一个`boolean`值
...
我们描述了`Map`接口，它的方法`default V merge(K key, V value, BiFunction<V,V,V> remappingFunction)`，以及如何使用它来连接`String`值：`map.merge(key, value, String::concat)`。`BiFunction<V,V,V>`接受两个相同类型的参数，并返回相同类型的值。`String::concat`构造称为方法引用，将在*方法引用*一节中解释。
...
它获取两个`String`参数，然后将第一个参数与`null`进行比较。如果第一个参数是`null`，则返回`-1`，否则使用`compareTo()`方法比较第一个参数和第二个参数。

在[第 11 章](11.html)*网络编程*中，我们看了下面的代码：
...
`BodyHandler`对象（函数）由`BodyHandlers.ofString()`工厂方法生成，作为参数传入`send()`方法。在方法内部，代码调用其`apply()`方法：

```
BodySubscriber<T> apply​(ResponseInfo responseInfo)
```

最后，在[第 12 章](12.html)*Java GUI 编程*中，我们在下面的代码片段中使用了一个`EventHandler`函数作为参数：
...
第一个函数是`EventHanlder<ActionEvent>`。它打印一条消息并强制应用程序退出。第二个是`EventHandler<WindowEvent>`函数。它只是打印信息。

所有这些例子都很好地说明了如何构造函数并将其作为参数传递。这种能力构成了函数式编程。它存在于许多编程语言中。它不需要管理对象状态。函数是无状态的。它的结果只取决于输入数据，不管调用了多少次。这样的编码使得结果更加可预测，这是函数式编程最吸引人的方面。

从这种设计中受益最大的领域是并行数据处理。函数式编程允许将并行性的责任从客户机代码转移到库中。在此之前，为了处理 Java 集合的元素，客户机代码必须遍历集合并组织处理。在 Java8 中，添加了新的（默认）方法，这些方法接受一个函数作为参数，然后根据内部处理算法将其并行或不并行地应用于集合的每个元素。因此，组织并行处理是图书馆的责任。
...
这就是为什么只有一个抽象方法的接口被称为**功能接口**。请注意，**只有一个抽象方法**的要求包括从父接口继承的方法。例如，考虑以下接口：
...
`A`是一个函数接口，因为它只有一个抽象方法`method1()`。`B`也是一个函数接口，因为它只有一个抽象方法，即继承自`A`接口的相同`method1()`。`C`是一个函数接口，因为它只有一个抽象方法`method1()`，它覆盖了父接口`A`的抽象`method1()`。接口`D`不能是功能接口，因为它有两个抽象方法-`method1()`来自父接口`A`和`method5()`。

为了避免运行时错误，Java8 中引入了`@FunctionalInterface`注释。它将意图告诉编译器，以便编译器可以检查并查看在带注释的接口中是否确实只有一个抽象方法。此注释还警告读代码的程序员，此接口故意只有一个抽象方法。否则，程序员可能会浪费时间将另一个抽象方法添加到接口中，结果在运行时发现它无法完成。

出于同样的原因，`Runnable`和`Callable`接口从 Java 早期版本就已经存在，在 Java8 中被注释为`@FunctionalInterface`。这种区别是明确的，并提醒用户，这些接口可用于创建函数：
...
如果我们仔细看前面的代码，就会发现有不必要的开销。首先，不需要重复接口名称，因为我们已经将其声明为对象引用的类型。其次，对于只有一个抽象方法的函数接口，不需要指定必须实现的方法名。编译器和 Java 运行时可以解决这个问题。我们只需要提供新的功能。为此特别引入了 lambda 表达式。
...
lambda 表达式的语法包括参数列表、箭头标记（[0]）和正文。参数列表可以是空的，例如不带括号的`()`（如果只有一个参数），或者用逗号分隔的参数列表用括号括起来。正文可以是单个表达式，也可以是大括号内的语句块（`{}`。我们来看几个例子：

*   `() -> 42;`总是返回`42`。
*   `x -> x*42 + 42;`将`x`值乘以`42`，然后将`42`加到结果中并返回。
*   `(x, y) -> x * y;`将传入的参数相乘，返回结果。
*   `s -> "abc".equals(s);`比较变量`s`和文字`"abc"`的值，返回`boolean`结果值。
*   `s -> System.out.println("x=" + s);`打印前缀为`"x="`的`s`值。
*   `(i, s) -> { i++; System.out.println(s + "=" + i); };`递增输入整数并打印前缀为`s + "="``s`的新值，第二个参数的值。

如果没有函数式编程，在 Java 中，将某些功能作为参数传递的唯一方法是编写一个实现接口的类，创建其对象，然后将其作为参数传递。但即使是使用匿名类的最简单的样式也需要编写太多的样板代码。使用函数接口和 lambda 表达式可以使代码更短、更清晰、更具表现力。
...
如您所见，创建函数接口很容易，尤其是使用 lambda 表达式。但在此之前，请考虑使用包`java.util.function`中提供的 43 个功能接口之一。这不仅可以让您编写更少的代码，还可以帮助其他熟悉标准接口的程序员更好地理解您的代码。
...
在 Java 11 中，使用`var`类型 holder 引入了另一种参数类型声明方法，类似于 Java 10 中引入的局部变量类型 holder`var`（参见[第 1 章](01.html)，*Java 12 入门*。
...
新的局部变量样式语法允许我们添加注释，而无需显式定义参数类型。将以下依赖项添加到`pom.xml`文件：
...
当参数是具有很长名称的类的对象时，如果我们需要使用注释，那么在 lambda 参数的情况下局部变量语法的优势就变得很明显了。在 Java 11 之前，代码可能如下所示：
...
`java.util.function`包中提供的大部分接口是以下四个接口的专门化：`Consumer<T>`、`Predicate<T>`、`Supplier<T>`和`Function<T,R>`。让我们回顾一下它们，然后简单地概述一下其他 39 个标准功能接口。

# 消费者

通过查看`Consumer<T>`接口定义，可以<indexentry content=“standard functional i 接口：消费者“>已经猜到这个接口有一个抽象方法，它接受一个类型为`T`的参数，并且不返回任何内容。当只列出一个类型时，它可以定义返回值的类型，就像在`Supplier<T>`接口中一样。但接口名称作为线索，**消费者**名称表示该接口的方法只取值，不返回任何值，**供应商**返回值。这条线索并不精确，但有助于唤起记忆。

关于任何功能接口的最佳信息源是`java.util.function`包 API 文档（[https://docs.oracle.com/en/java/javase/12/docs/api/java.base/java/util/function/package-summary.html](https://docs.oracle.com/en/java/javase/12/docs/api/java.base/java/util/function/package-summary.html) ). 如果我们阅读它，我们会发现`Consumer<T>`接口有一个抽象和一个默认方法：

*   `void accept(T t)`：将运算应用于给定参数
...
`setRecord`函数获取`Person`对象属性的值和来自外部源的一些数据，并将结果值设置为`record`属性值。显然，它可以用其他几种方法来实现，但我们这样做只是为了演示。我们还要创建一个函数来打印[3]属性：
...
此函数接口`Predicate<T>`有一个抽象方法、五个默认值和一个允许谓词链接的静态方法：

*   `boolean test(T t)`：评估提供的参数是否符合条件
...
其他方法（也叫**操作**）也可以用来创建操作链（也叫**管道**），如下例：
...
这个函数接口`Supplier<T>`只有一个抽象方法`T get()`，返回一个值。基本用法如下：
...
这个和其他返回值的函数接口的表示法，包括作为泛型列表中最后一个的返回类型的列表（在本例中为`R`）和它前面的输入数据的类型（在本例中为`T`类型的输入参数）。因此，符号`Function<T, R>`表示此接口的唯一抽象方法接受`T`类型的参数并生成`R`类型的结果。让我们看看在线文档（[https://docs.oracle.com/en/java/javase/12/docs/api/java.base/java/util/function/function.html](https://docs.oracle.com/en/java/javase/12/docs/api/java.base/java/util/function/Function.html) ).

`Function<T, R>`接口有一个抽象方法`R apply(T)`和两个操作链接方法：

*   `default <V> Function<T,V> andThen(Function<R, V> after)`：返回一个组合函数，首先将当前函数应用于其输入，然后将`after`函数应用于结果。
*   `default <V> Function<V,R> compose(Function<V, T> before)`：返回一个组合函数，首先将`before`函数应用于输入，然后将当前函数应用于结果。
...
现在，复合函数的组合顺序与执行顺序不匹配。如果函数[0]还没有创建，而您想在线创建它，这可能非常方便。则以下构造将不编译：
...
它允许在构建功能管道时具有更大的灵活性，因此在创建下一个操作时，可以以流畅的方式构建它，而不会打断连续的行。
...
然后，我们决定在某些情况下，`multiplyByTwo()`函数不应执行任何操作。我们可以给它添加一个条件关闭来打开/关闭它。但是，如果我们想保持函数的完整性，或者如果这个函数是从第三方代码传递给我们的，我们可以只执行以下操作：
...
`java.util.function`包中的其他 39 个功能接口是我们刚才回顾的四个接口的变体。创建这些变体是为了实现以下一个或任意组合：

*   通过显式使用`int`、`double`或`long`原语避免自动装箱和拆箱，从而提高性能
*   允许两个输入参数和/或更短的符号

以下只是几个例子：

*   方法`R apply(int)`中的`IntFunction<R>`提供了一个较短的表示法（输入参数类型没有泛型），并通过要求原语`int`作为参数来避免自动装箱。
*   `BiFunction<T,U,R>`方法`R apply(T,U)`允许两个输入参数；`BinaryOperator<T>`方法`T apply(T,T)`允许两个类型为`T`的输入参数，并返回相同类型的值`T`。
*   方法为`int applAsInt(int,int)`的`IntBinaryOperator`接受两个类型为`int`的参数，并返回类型为`int`的值。

如果您要使用功能接口，我们鼓励您学习`java.util.functional`包（[的接口 APIhttps://docs.oracle.com/en/java/javase/12/docs/api/java.base/java/util/function/package-summary.html](https://docs.oracle.com/en/java/javase/12/docs/api/java.base/java/util/function/package-summary.html) ).
...
*   lambda 表达式中的[0]关键字是指封闭上下文，而不是 lambda 表达式本身。
...
不过，对于这个限制，有一个可能的解决方法。如果局部变量是引用类型（而不是`String`或原语包装类型），则可以更改其状态，即使在 lambda 表达式中使用了此局部变量：
...
匿名类中的`this`关键字引用匿名类的实例。相比之下，在 lambda 表达式中，`this`关键字是指围绕表达式的类的实例，也称为**封闭实例**、**封闭上下文**或**封闭范围**。
...
如您所见，匿名类中的关键字`this`表示匿名类实例，而 lambda 表达式中的`this`表示封闭类实例。lambda 表达式没有字段，也不能有字段。lambda 表达式不是类实例，不能被`this`引用。根据 Java 的规范，这种方法*通过将`this`与周围的上下文处理相同，为实现*提供了更大的灵活性。
...
方法引用的语法为`Location::methodName`，其中`Location`表示`methodName`方法属于哪个对象或类，两个冒号（`::`作为位置和方法名之间的分隔符。使用方法引用表示法，前面的示例可以重写如下：
...
本章通过解释和演示函数接口和 lambda 表达式的概念，向读者介绍函数式编程。JDK 附带的标准函数接口概述帮助读者避免编写自定义代码，而方法引用表示法允许读者编写易于理解和维护的结构良好的代码。
...
4.  在`Consumer<T>`接口的实现中，返回值的类型是什么？
5.  `Predicate<T>`接口的实现有多少个输入参数？
6.  在`Predicate<T>`接口的实现中，返回值的类型是什么？
7.  `Supplier<T>`接口的实现有多少个输入参数？
8.  在`Supplier<T>`接口的实现中，返回值的类型是什么？
9.  `Function<T,R>`接口的实现有多少个输入参数？
10.  在`Function<T,R>`接口的实现中，返回值的类型是什么？
