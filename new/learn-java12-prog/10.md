...
此外，创建另一个名为纲要的逻辑层是一个很好的实践，它可以拥有自己的一组用户和权限。这样，可以隔离同一数据库中的多个模式，并且每个用户（其中一个是您的应用程序）只能访问某些模式。在企业级，通常的做法是为数据库模式创建同义词，以便任何应用程序都不能直接访问原始结构。然而，为了简单起见，我们在本书中不这样做。
...
在下一章中，我们将描述和讨论最流行的网络协议，演示如何使用它们，以及如何使用最新的 Java HTTP 客户机 API 实现客户机-服务器通信。所回顾的协议包括基于 TCP、UDP 和 URL 的通信协议的 Java 实现
