...
在本章中，您将了解什么是微服务，它们与其他体系结构样式的区别，以及现有的微服务框架如何支持消息驱动体系结构。我们还将帮助您决定微服务的大小，并讨论服务大小是否对将其标识为微服务起到任何作用。在本章结束时，您将了解如何构建微服务，并将它们用作创建反应式系统的基础组件。我们将通过使用 Vert.x 工具箱构建的小型反应式系统的详细代码演示来支持讨论。
...
随着处理负载的不断增加，解决这个问题的传统方法是添加更多部署了相同[0]或[1]文件的服务器，然后将它们连接到一个集群中。这样，故障服务器可以自动替换为另一个服务器，系统性能不会下降。支持所有集群服务器的数据库通常也是集群的
...
*微服务必须有多小这个问题没有一个普遍的答案？* 一般共识与微服务的以下特征一致（没有特定顺序）：
...
*   它应该提供一种检查其*运行状况*的方法，以证明服务已启动并正在运行，具有所有必要的资源，并且已准备好执行该工作

在设计过程、开发和部署后需要考虑资源共享，并在从不同过程访问同一资源时对干扰程度（例如阻塞）假设的验证进行监控。在修改同一持久性数据的过程中也需要特别小心，无论是在数据库、模式之间共享，还是在同一模式中的表之间共享。如果*最终的一致性*是可以接受的（这通常是用于统计目的的较大数据集的情况），则需要采取特殊措施。但是，对事务完整性的需求常常带来一个难题。
...
目前有十多种框架用于构建微服务。最流行的两种是弹簧靴（[https://spring.io/projects/spring-boot](https://spring.io/projects/spring-boot) ）和微文件（[https://microfile.io](https://microprofile.io) ）的目标是为基于微服务的体系结构优化企业 Java。轻量级开源微服务框架，KumuluzEE（[https://ee.kumuluz.com](https://ee.kumuluz.com) ）与文件兼容。

以下是其他框架的列表（按字母顺序排列）：

*   **Akka**：这是一个用于为 Java 和 Scala（`akka.io`构建高度并发、分布式和弹性的消息驱动应用程序的工具箱。
*   **Bootique**：这是一个针对可运行 Java 应用程序的最低限度的固执己见的框架（`bootique.io`。
*   **Dropwizard**：这是一个开发操作友好、高性能、RESTful web 服务的 Java 框架（[www.dropwizard.io](https://www.dropwizard.io/1.3.9/docs/) ).
...
*   **忍者**：这是一个 Java 的全栈框架（[https://www.ninjaframework.org/](https://www.ninjaframework.org/) ).
...
Vert.xapi 有两个源树：第一个源树以`io.vertx.core`开头，第二个源树以`io.vertx.rxjava.core`开头。第二个源树是`io.vertx.core`类的反应版本。事实上，无功源树是基于非无功源的，所以这两个源树并不是不兼容的。相反，被动版本是在非被动实现的*增补*中提供的。因为我们的讨论集中在反应式编程上，所以我们将主要使用`io.vertx.rxjava`源代码树的类和接口，也称为**rxfied Vert.xapi**。

首先，我们将向[0]文件添加以下依赖项，如下所示：
...
实现`io.vertx.core.Verticle`接口的类是基于 Vert.x 的应用程序的构建块，`io.vertx.core.Verticle`接口有四个抽象方法：
...
为了在实践中简化编码，有一个抽象的`io.vertx.rxjava.core.AbstractVerticle`类实现了所有的方法，但是这些方法都是空的，什么都不做。它允许通过扩展`AbstractVerticle`类并只实现应用程序所需的`Verticle`接口的那些方法来创建垂直体。在大多数情况下，仅仅实现`start()`方法就足够了。

Vert.x 有自己的系统，通过事件总线交换消息（或事件）。通过使用`io.vertx.rxjava.core.eventBus.EventBus`类的`rxSend(String address, Object msg)`方法，任何 verticle 都可以向任何地址发送消息（只是一个字符串）：
...
`vertx`对象（它是`AbstractVerticle`的受保护属性，可用于每个垂直）允许访问事件总线和`rxSend()`调用方法。`Single<Message<String>>`返回值表示响应消息可以返回的回复；您可以订阅它，或者以任何其他方式处理它。
...
如果多个 verticle 注册为同一地址的使用者，`rxSend()`方法使用循环算法仅将消息传递给其中一个使用者。

或者，可以使用`publish()`方法将消息传递给使用相同地址注册的所有使用者：
...
为了演示如果使用 Vert.x 实现，微服务的反应式系统会是什么样子，我们将创建一个 HTTP 服务器，它可以接受系统中基于 REST 的请求，向另一个 verticle 发送一个基于`EventBus`的消息，接收一个回复，然后将响应发送回原始请求。
...
如您所见，创建的服务器侦听指定的端口，并用 Hello…响应每个传入的请求。默认情况下，主机名为`localhost`。如有必要，可以使用相同方法的重载版本为主机指定另一个地址：
...
请注意，…is waiting…消息会立即出现，甚至在任何请求传入之前也会出现–这是此服务器的异步特性。`name`前缀被构造为包含类名、线程名、主机名和端口。注意，线程名称告诉我们服务器监听事件循环线程`0`。

现在我们可以使用`curl`命令向部署的服务器发出请求，响应如下：

![](img/b6e4ed70-a4b5-4ba4-979d-0fd81e245186.png)

如您所见，我们已发出 HTTP`GET`（默认）请求，并返回了预期名称为的 Hello…消息

下面的代码是`start()`方法的一个更现实的版本：
...
现在我们使用`Router`类，并根据 HTTP 方法（`GET`或`POST`和路径向不同的处理程序发送请求。它要求您向`pom.xml`文件添加以下依赖项：
...
第一条路由为`/some/path/:name/:address/:anotherParam`路径，包含三个参数（`name`、`address`、`anotherParam`）。HTTP 请求在`RoutingContext`对象内部传递给以下处理程序：
...
第二条路由的`/some/path/send`路径有以下处理程序：
...
第三条路由的`/some/path/publish`路径有以下处理程序：
...
前面的屏幕截图说明了我们向第一个 HTTP`GET`请求发送了预期的消息，但在响应第二个 HTTP`GET`请求时未找到接收到的资源。这是因为我们的服务器中没有 HTTP`GET`请求的`/some/path/send`路由。然后我们切换到 HTTP`POST`请求，并接收到两个`POST`请求的预期消息。

从路径名可以猜到我们将使用`/some/path/send`路由发送`EventBus`消息，使用`/some/path/publish`路由发布`EventBus`消息，但是在实现相应的路由处理程序之前，我们先创建一个接收`EventBus`消息的 verticle。
...
正如我们所承诺的，我们现在将以更现实的方式重新实现[0]垂直体的处理程序。`GET`方法处理程序现在看起来像以下代码块：
...
如您所见，`RoutingContext`类提供了`pathParam``()`方法，该方法从路径中提取参数（如果它们被标记为`:`，如我们的示例所示）。然后，我们再次使用`EventBus`对象向作为参数提供的地址异步发送消息。`subscribe()`方法使用提供的函数处理来自消息接收者的回复，并将响应发送回 HTTP 服务器的原始请求。
...
对于 HTTP`POST`请求，我们希望发送 JSON 格式的有效负载，其值与作为 HTTP`GET`请求参数发送的值相同。方法的其余部分与`processGetSomePath()`实现非常相似。让我们重新部署`HttpServerVert`和`MessageRcvVert`verticles，然后用有效负载发出 HTTP`POST`请求，结果如下：

![](img/aade93a7-98e3-44a8-981a-564a72fbe360.png)

这看起来与设计的 HTTP`GET`请求的结果完全相同。在后端，将显示以下消息：
...
这一次，我们使用了`publish()`方法发送消息。请注意，此方法无法接收答复。这是因为，正如我们已经提到的，`publish()`方法将消息发送给所有注册到此地址的接收器。如果我们使用`/some/path/publish`路径发出 HTTP`POST`请求，结果会略有不同：
...
在下一节中，我们将部署发送方和接收方的几个实例，并通过`rxSend()`和`publish()`方法检查消息分布之间的差异。
...
如您所见，我们将部署两个使用相同的[0]地址接收消息的 verticle 和一个使用[1]地址的 verticle。如果我们运行上述程序，屏幕将显示以下消息：
...
如前所述，如果有多个注册在同一地址的 verticle，`rxSend()`方法使用循环算法来选择应该接收下一条消息的 verticle。第一个请求通过`ID="1"`发送给接收者，第二个请求通过`ID="2"`发送给接收者，第三个请求再次通过`ID="1"`发送给接收者。
...
如您所见，`publish()`方法将消息发送到所有注册到指定地址的 verticle。注意，带有`ID="3"`（用`Two`地址注册）的 verticle 从未收到消息。
...
本章向读者介绍了微服务的概念，以及如何使用微服务创建反应式系统。我们讨论了应用程序大小的重要性，以及它如何影响您将其转换为微服务的决策。您还了解了现有的微服务框架如何支持消息驱动体系结构，并有机会在实践中使用其中的一个工具 Vert.x 工具包。

在下一章中，我们将探讨**Java Microbenchmark Harness**（**JMH**项目，它允许您测量代码性能和其他参数。我们将定义什么是 JMH，如何创建和运行基准，基准参数是什么，以及支持的 IDE 插件。
...
7.  事件总线的`send`方法如何决定在 Vert.x 中发送消息的接收者？
