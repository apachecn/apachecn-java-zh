...
对于这个问题没有一个普遍的答案*微服务必须有多小？*一般共识与微服务的以下特征一致（没有特定顺序）：
...
目前有十多种框架用于构建微服务。最流行的两种是弹簧靴（[https://spring.io/projects/spring-boot](https://spring.io/projects/spring-boot)和微文件（[https://microfile.io](https://microprofile.io)），其目标是优化基于微服务架构的企业 Java。轻量级开源微服务框架，KumuluzEE（[https://ee.kumuluz.com](https://ee.kumuluz.com)）符合 MicroFile。
...
Vert.xapi 有两个源代码树：第一个源代码树以`io.vertx.core`开头，第二个源代码树以`io.vertx.rxjava.core`开头。第二个源树是`io.vertx.core`类的反应版本。事实上，无功源树是基于非无功源的，所以这两个源树并不是不兼容的。相反，在非反应性实现的*附加*中提供了反应性版本。因为我们的讨论集中在反应式编程上，所以我们将主要使用`io.vertx.rxjava`源代码树的类和接口，也称为**rxfied Vert.xapi**。
...
在下一节中，我们将部署几个发送者和接收者的实例，并通过`rxSend()`和`publish()`方法检查消息分布之间的差异。
...
如前所述，如果有多个注册在同一地址的垂直站点，`rxSend()`方法使用循环算法来选择应该接收下一条消息的垂直站点。第一个请求通过`ID="1"`发送给接收者，第二个请求通过`ID="2"`发送给接收者，第三个请求再次通过`ID="1"`发送给接收者。
