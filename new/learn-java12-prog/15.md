...
非阻塞 API 的客户机希望能够快速返回结果，也就是说，不会被阻塞很长时间。因此，非阻塞 API 的概念意味着一个高度响应的应用。它可以同步或异步地处理请求—这对客户端并不重要。但实际上，这通常意味着应用使用异步处理，这有助于提高吞吐量和性能。
...
您还将在“多线程（调度程序）”部分中看到这些运算符用法的其他示例。
