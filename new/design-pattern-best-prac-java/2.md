...
同步单例的代码简单而有效，但是有一种情况我们应该考虑。如果我们在多线程应用程序中使用我们的代码，可能是两个线程在实例为`null`时同时调用`getInstance`方法。当发生这种情况时，可能是第一个线程继续使用`new`操作符实例化单例，在完成之前，第二个线程检查单例是否为`null`。因为第一个线程没有完成实例化，所以第二个线程会发现实例为空，所以它也会开始实例化它。
