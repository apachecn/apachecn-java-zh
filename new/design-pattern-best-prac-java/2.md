...
同步单例的代码简单而有效，但是有一种情况我们应该考虑。如果我们在多线程应用程序中使用我们的代码，可能是两个线程在实例为 null 时同时调用`getInstance`方法。当发生这种情况时，可能是第一个线程继续使用 new 操作符实例化 singleton，在完成之前，第二个线程检查 singleton 是否为 null。因为第一个线程没有完成实例化，所以第二个线程会发现实例为空，所以它也会开始实例化它。
...
根据创建实例对象的时间，单例可以分为两类。如果单例是在应用程序启动时创建的，则认为它是一个**早期/急切实例化**。否则，如果第一次调用`getInstance`方法时调用了单例构造函数，则认为是**懒加载单例**。
...
如前一章所讨论的，继承是面向对象编程的基本概念之一。与亚型多态性一起，它给出了 is/a 关系。`Car`对象可以作为`Vehicle`对象处理。`Truck`对象也可以作为`Vehicle`对象处理。一方面，这种抽象使我们的代码更薄，因为同一段代码可以处理`Car`和`Truck`对象的操作。另一方面，它给我们提供了一个选项，通过简单地添加新的类，比如`Bike`和`Van`，而不修改它，就可以将代码扩展到新类型的`Vehicle`对象。
...
但这里有两个问题。首先，我们的类应该为扩展而开放，为修改而关闭（open/closed 原则）。第二，每个班级应该只有一个改变的理由（单一责任原则）。每次添加一个新类时更改主代码将打破开放/关闭原则，让主类除了功能外还负责实例化`vehicle`对象将打破单一责任原则。
...
如果我们采用抽象工厂模式，并将其应用于包含单个对象的*族，那么我们就有了工厂方法模式。工厂方法只是抽象工厂的一个特例。*
...
当我们有一个复杂的类需要实例化为具有不同结构或不同内部状态的不同对象时，我们可以使用不同的类来封装实例化逻辑。这些类被称为**构建器**。每次我们需要来自同一类的具有不同结构的对象时，我们都可以创建另一个生成器来创建这样的实例。

同样的概念不仅可以用于需要不同表示的类，也可以用于由其他对象组成的复杂对象。

创建生成器类来封装实例化复杂对象的逻辑符合单一责任原则和打开/关闭原则。实例化复杂对象的逻辑被移动到一个单独的**生成器**类。当我们需要不同结构的对象时，我们可以添加新的构建器类，这样代码就可以关闭进行修改，打开进行扩展，如图所示：
...
*   `ConcreteBuilder`：实现`Builder`接口中声明的方法的具体生成器。除了在`Builder`抽象类中声明的方法外，它还有一个`getResult`方法返回生成的产品。
*   `Director`：一个类，指导如何构建对象。在生成器模式的某些变体中，这个类被删除，它的角色由客户端或生成器承担。

# 汽车制造商示例

在本节中，我们将把 builder 模式应用到汽车软件中。我们有一个`Car`类，我们需要创建它的实例。根据我们在汽车上添加的部件，我们可以制造轿车和跑车。当我们开始设计软件时，我们意识到：

*   `Car`类相当复杂，创建类对象也是一项复杂的操作。在`Car`构造函数中添加所有实例化逻辑将使类变得相当大。
...
`CarBuilder`是生成器基类，它包含四个抽象方法。我们创建了两个混凝土建筑商：`ElectricCarBuilder`和`GasolineCarBuilder`。每个具体的建设者都必须实现所有的抽象方法。不需要的方法，例如`ElectricCarBuilder`的`addGasTank`被保留为空，或者它们可以抛出异常。电动汽车和汽油汽车有不同的内部结构。
...
在生成器模式的一些实现中，`Director`类可以被删除。在我们的类示例中，它封装的逻辑非常简单，因此在这种情况下，我们实际上不需要控制器。在本例中，简化的生成器模式如下所示：

![](img/f83fa1cf-2400-42ea-8f9e-beeea0254f39.png)

在`Director`类中实现的代码被简单地移动到`Client`。当`Builder`和`Product`类太复杂或使用生成器从数据流构建对象时，不建议进行此更改。

# 具有方法链接的匿名生成器

如前所述，处理来自同一类且应采用不同形式的对象的最直观方法是为每个场景创建几个构造函数来实例化它们。使用生成器模式来避免这种情况是一种很好的做法。在*Effective Java 中，*Joshua Bloch 建议使用内部构建器类和方法链接来替换多个构造函数。
...
在实践中，我们应该根据每个场景来决定是需要深度克隆、浅层克隆还是混合克隆。通常，浅克隆对应于[第一章](1.html)中描述的聚合关系，从面向对象到函数式编程中描述的*，而深克隆对应于组合关系。*
