...
singleton 模式可能是自 Java 诞生以来使用最广泛的设计模式。这是一个简单的模式，易于理解和使用。有时它被过度使用，在不需要它的情况下。在这种情况下，使用它的缺点大于它带来的好处。因此，单例有时被认为是反模式。然而，有许多场景需要单例。

顾名思义，singleton 模式用于确保只能创建对象的单个实例。除此之外，它还提供对该实例的全局访问。下面的类图描述了单例模式的实现：
...
在`getInstance`方法中，我们检查实例是否为 null。如果实例不为 null，则表示该对象是在之前创建的；否则，我们将使用 new 操作符创建它。之后，无论哪种情况，它都不再为 null，因此我们可以返回 instance 对象。

# 同步单态

同步单例的代码简单而有效，但是有一种情况我们应该考虑。如果我们在多线程应用程序中使用我们的代码，则可能是两个线程在实例为 null 时同时调用[0]方法。当发生这种情况时，可能是第一个线程继续使用 new 操作符实例化 singleton，在完成之前，第二个线程检查 singleton 是否为 null。因为第一个线程没有完成实例化，所以第二个线程会发现实例为空，所以它也会开始实例化它。
...
*   将`if (instance == null)`条件包装在`synchronized`块中。当我们在这个上下文中使用`synchronized`块时，我们需要指定一个提供锁的对象。我们使用`Singleton.class`对象，如下代码片段所示：
...
在`synchronized`块前增加一个附加条件，只有当单例还没有实例化时，才会移动线程安全锁：
...
根据创建实例对象的时间，单例可以分为两类。如果单例是在应用程序启动时创建的，则认为它是一个**早/急实例化**。否则，如果在第一次调用`getInstance`方法时调用了单例构造函数，则认为它是一个**懒加载单例**。
...
如前一章所讨论的，继承是面向对象编程的基本概念之一。与亚型多态性一起，它给出了 is/a 关系。一个`Car`对象可以作为一个`Vehicle`对象来处理。一个`Truck`对象也可以作为`Vehicle`对象来处理。一方面，这种抽象使我们的代码更薄，因为同一段代码可以处理`Car`和`Truck`对象的操作。另一方面，它给我们提供了一个选项，可以通过简单地添加新类（如`Bike`和`Van`，而不修改它，将代码扩展到新类型的`Vehicle`对象。
...
但这里有两个问题。首先，我们的类应该为扩展而开放，为修改而关闭（open/closed 原则）。第二，每个班级应该只有一个改变的理由（单一责任原则）。每次添加新类时更改主代码都将打破打开/关闭原则，让主类除了功能之外还负责实例化`vehicle`对象将打破单一责任原则。

在这种情况下，我们需要为代码提供更好的设计。我们可以添加一个新类来负责实例化`vehicle`对象。我们将基于这个`SimpleFactory`类来调用模式。
...
`SimpleFactory`类实现实例化`ConcreteProduct1`和`ConcreteProduct2`的代码。当客户端需要一个对象时，它调用`SimpleFactory`的`createProduct()`方法，参数指示它需要的对象的类型。`SimpleFactory` 实例化相应的具体产品并返回它。返回的产品被转换为基类类型，因此客户端将以相同的方式处理任何`Product`，而不管它是`ConcreteProduct1`还是`ConcreteProduct2`。
...
工厂看起来非常简单，负责实例化`vehicle`类，遵循单一责任原则。它帮助我们减少耦合，因为客户机只依赖于`Vehicle`接口，符合依赖倒置原则。如果我们需要添加一个新的`vehicle`类，我们需要更改`VehicleFactory`类，这样就打破了开/关原则。

我们可以改进这个简单的工厂模式，通过使用一种机制来注册新的类，在需要时实例化这些类，从而使它在扩展时打开，在修改时关闭。有两种方法可以实现这一点：

*   注册产品类对象并使用反射实例化它们
*   注册产品对象并向每个产品添加一个`newInstance`方法，该方法返回一个与其自身相同类的新实例
...
现在是编写示例代码的时候了。假设我们有一个汽车工厂。目前，我们生产两种车型：小型跑车和大型家用车。在我们的软件中，客户可以决定他们是想要一辆小型车还是一辆大型车。首先，我们正在创建一个包含两个子类的`Vehicle`类：`SportCar`和`SedanCar`。
...
在这一点上，我们意识到一个汽车厂能带来多少利润。是时候扩展我们的业务了，我们的市场调查告诉我们，卡车的需求量很大。那么我们来构建一个`TruckFactory`：
...
我们继续前面的代码，添加了一个`BikeFactory`，客户可以从中选择一辆小自行车或一辆大自行车。我们不需要创建单独的类文件就可以做到这一点；我们只需创建一个匿名类，直接在客户机代码中扩展[1]：
...
如果我们采用抽象工厂模式并将其应用于包含单个对象的*族，那么我们就有了工厂方法模式。工厂方法只是抽象工厂的一个特例。*
...
builder 模式的作用与其他创造性模式相同，但它以不同的方式和出于不同的原因。在开发复杂的应用程序时，代码往往变得更加复杂。类倾向于封装更多的功能，同时，类结构变得更加复杂。随着功能的增长，需要覆盖更多的场景，对于这些场景，需要不同的类表示。

当我们有一个复杂的类需要实例化为具有不同结构或不同内部状态的不同对象时，我们可以使用不同的类来封装实例化逻辑。这些类称为**构建器**。每次我们需要来自同一类的具有不同结构的对象时，我们都可以创建另一个生成器来创建这样的实例。

同样的概念不仅可以用于需要不同表示的类，也可以用于由其他对象组成的复杂对象。

创建生成器类来封装实例化复杂对象的逻辑符合单一责任原则和打开/关闭原则。实例化一个复杂对象的逻辑被移到一个单独的**生成器**类中。当我们需要不同结构的对象时，我们可以添加新的构建器类，这样代码就可以关闭进行修改，打开进行扩展，如图所示：
...
*   `ConcreteBuilder`：实现`Builder`接口中声明的方法的具体生成器。除了在`Builder`抽象类中声明的方法外，它还有一个返回生成产品的`getResult`方法。
*   `Director`：一个类，指导如何构建对象。在生成器模式的某些变体中，这个类被删除，它的角色由客户端或生成器承担。

# 汽车制造商示例

在本节中，我们将把 builder 模式应用到汽车软件中。我们有一个[0]类，需要创建它的实例。根据我们在汽车上添加的部件，我们可以制造轿车和跑车。当我们开始设计软件时，我们意识到：

*   [0]类非常复杂，创建类对象也是一个复杂的操作。在`Car`构造函数中添加所有实例化逻辑将使类变得相当大。
*   我们需要制造几种类型的汽车。通常，对于这个场景，我们会添加几个不同的构造函数，但是我们的直觉告诉我们这不是最好的解决方案。
...
`CarBuilder`是生成器基类，包含四个抽象方法。我们创造了两个混凝土建筑商：`ElectricCarBuilder`和`GasolineCarBuilder`。每个具体的建设者都必须实现所有的抽象方法。不需要的方法，例如`ElectricCarBuilder`的`addGasTank`被保留为空，或者它们可以抛出异常。电动汽车和汽油汽车有不同的内部结构。

[0]类使用生成器创建新的 car 对象。`buildElectricCar` 与`buildGasolineCar`可能相似，但略有不同：
...
在生成器模式的某些实现中，`Director`类可以被删除。在我们的类示例中，它封装的逻辑非常简单，因此在这种情况下，我们实际上不需要控制器。在本例中，简化的生成器模式如下所示：

![](img/f83fa1cf-2400-42ea-8f9e-beeea0254f39.png)

在`Director`类中实现的代码只需移动到`Client`。当`Builder`和`Product`类太复杂或使用生成器从数据流构建对象时，不建议进行此更改。

# 具有方法链接的匿名生成器

如前所述，处理来自同一类且应采用不同形式的对象的最直观方法是为每个场景创建几个构造函数来实例化它们。使用生成器模式来避免这种情况是一种很好的做法。在*有效的 Java 中，*Joshua Bloch 建议使用内部生成器类和方法链接来替换多个构造函数。

方法链接是从某些方法返回当前对象（`this`）的技术。这样，可以在链中调用这些方法。例如：
...
但是，在本例中，我们将使`builder`成为`Car`对象的内部类。因此，当我们需要新客户时，我们可以执行以下操作：
...
*   `Prototype`：这是基类，或声明派生对象必须实现的`clone()`方法的接口。在一个简单的场景中，我们可能没有基类，直接的具体类就足够了。
...
[0]方法可以在接口中声明，因此实现该方法的类必须实现该方法。这种强制是在编译时完成的。但是，对于继承自在具有多个级别的层次结构中实现`clone()`方法的类的类，它不会强制执行。

# 浅克隆与深克隆

在克隆物体时，我们应该意识到克隆的深度。当我们克隆一个包含简单数据类型（如[0]和[1]）的对象，或者不可变对象（如字符串）的对象时，我们应该简单地将这些字段复制到新对象，就这样。

当我们的对象包含对其他对象的引用时，问题就出现了。例如，如果我们必须为一个有引擎和四个轮子列表的`Car`类实现一个克隆方法，我们不仅要创建一个新的`Car`对象，还要创建一个新的`Engine`和四个新的`Wheel`对象。毕竟，两辆车不能共用同一台发动机和同一个车轮。这叫做**深克隆**。

浅层克隆是一种只克隆被克隆对象的方法。例如，如果我们必须为一个`Student`对象实现一个 clone 方法，我们就不会克隆它指向的`Course`对象。多个`Student`对象可以指向同一`Course`对象。

在实践中，我们应该根据每个场景来决定是需要深度克隆、浅层克隆还是混合克隆。通常，浅层克隆对应于[第一章](1.html)中描述的聚合关系，从面向对象到函数式编程的*，从深层克隆到组合关系。*
...
*   `ResourcePool`：封装逻辑的类，用于保存和管理资源列表。
*   `Resource`：封装有限资源的类。`Resource`类总是被`ResourcePool`引用，所以只要`ResourcePool`没有被反分配，它们就永远不会被垃圾回收。
*   `Client`：使用资源的类。

当一个`Client`需要一个新的`Resource`时，它会向`ResourcePool`请求。池检查并获取第一个可用资源并将其返回给客户端：
...
然后，当`Client`使用完`Resource`后，它将其释放。资源被添加回工具，以便可以重用。
...
在这一章中，我们讨论了创造性的设计模式。我们讨论了 singleton、factory、builder、prototype 和对象池模式的变体。所有这些模式都用于实例化新对象，并在创建对象时提供代码灵活性和可重用性。在下一章中，我们将介绍行为模式。虽然创建模式帮助我们管理对象的创建，但行为模式提供了管理对象行为的简单方法。