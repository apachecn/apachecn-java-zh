...
**控制器/ Web 服务层**是第三方请求的入口点。此请求可以来自表示层（主要）或其他服务；例如，移动或桌面应用程序。因为这是任何请求的入口点，所以这一层将应用于任何初始级别的检查、数据清理、验证、安全需求，例如认证和授权等。一旦这个层得到满足，请求就被接受和处理。
...
顾名思义，Web 服务是通过 Web 或互联网提供的服务。Web 服务有助于普及面向服务的架构，因为它们使人们很容易从互联网上公开的服务的角度来考虑应用程序。在因特网上公开服务的方式有很多种，**简单对象访问协议**（**SOAP**）和 REST 是最常见的两种实现方式。
