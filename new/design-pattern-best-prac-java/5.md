...
本章的目的是学习函数模式，以及通过引入函数式编程风格（现在在最重要的编程语言中是可能的）对传统模式所做的更改。Java8 引入了一些功能特性，增加了一个新的抽象级别，影响了我们编写一些面向对象设计模式的方式，甚至使其中一些模式变得无关紧要。在本章中，我们将看到设计模式是如何被新的语言特性所改变，甚至取代的。在他的论文《动态语言中的设计模式》中，Peter Norvig 注意到 23 种设计模式中有 16 种更简单，或者被动态语言中现有的语言特性所取代，比如 Dylan。全文见[http://norvig.com/design-patterns/](http://norvig.com/design-patterns/) . 在这一章中，我们将看到什么可以被取代，以及新出现的模式是怎样和怎样的。正如 peternorvig 在他的论文中所说的，*很久以前，子程序调用只是一种模式*，随着语言的发展，模式会发生变化或被替换。

为了运行本章中的代码，我们使用了 Java 中可用的 jshell REPL 实用程序，该实用程序可从 Windows 中的[0]访问。

# 函数式编程简介

在 20 世纪 30 年代，数学家阿隆佐教会发展了 lambda 微积分。这是函数式编程范式的起点，因为它提供了理论基础。下一步是 John McCarthy 于 1958 年设计的**LISP**（简称**List Programming**）。LISP 是第一种函数式编程语言，它的一些风格，如 commonlisp，至今仍在使用。

在函数式编程（通常缩写为 FP）中，函数是一级公民；这意味着软件是通过将函数而不是对象组合为 OOP 来构建的。这是以声明的方式完成的，*告诉不要问*，通过组合函数，促进不变性，避免副作用和共享数据。这就产生了一个更简洁的代码，它对变化具有弹性、可预测性，并且更易于维护和业务人员阅读。
...
这个名字来自 lambda 演算，希腊字母 lambda（*λ*）用于将一个术语绑定到一个函数。lambda 项可以是变量（*x*，例如，*λ.x.M，*等抽象，其中*M*是函数或应用，其中*M*和*N*两个项相互应用。通过构造（合成）术语，现在可以进行表达式缩减和/或转换。Lambda 表达式缩减可以通过使用解释器进行在线测试，例如 Berkeley 的解释器：[https://people.eecs.berkeley.edu/~gongliang13/lambda/](https://people.eecs.berkeley.edu/~gongliang13/lambda/) .

以下是当已知*x*、*y*坐标时用于计算圆半径平方的 lambda 演算 lambda 表达式的示例：
...
java8 引入了 lambda 表达式（以前通过使用匿名类提供），实现使用了 java7 中引入的 invoke 动态，而不是匿名类，以提高性能（需要加载太多生成的类）和定制（将来的更改）的原因。
...
由于纯函数的输出是可预测的，因此也可以用缓存的输出替换它；这就是为什么纯函数被称为提供引用透明性的原因。纯函数更容易阅读和理解，在他的书*干净的代码*中，Robert Martin 写道：
...
在数学中，函数是用一个函数的输出作为下一个函数的输入而组合起来的。同样的规则也适用于函数式编程，其中一级函数由高阶函数使用。前面的代码已经包含了这样一个示例，请参见 map 函数中的[0]pure 函数的用法。

为了使函数的组成更加直观，我们可以使用`andThen`方法重写转换公式：
...
**Curry**是一种将 n 元函数转化为级数或一元函数的过程，以美国数学家 Haskell Curry 的名字命名。形式`g:: x -> y -> z`是`f :: (x, y) -> z`的咖喱形式。对于前面给出的平方半径公式，`f(x,y) = x<sup class="calibre33">2</sup> + y<sup class="calibre33">2</sup>`，一个 curried 版本，不使用双函数，将使用 apply 多次。一个函数的单一应用程序只会用一个值替换参数，正如我们前面看到的。下面的代码展示了如何创建一个双参数函数，对于*n*个参数，`Function<X,Y>`类的 apply 函数将有 n 个调用：
...
闭包是实现词汇作用域的一种技术。词法范围允许我们访问内部范围内的外部上下文变量。假设在前面的示例中，*y*变量已经被赋值。lambda 表达式可以保持一元表达式，并且仍然使用*y*作为变量。这可能会导致一些很难找到的 bug，如在下面的代码中，我们希望函数的返回值保持不变。闭包捕获一个对象的当前值，正如我们在下面的代码中看到的，我们的期望是`add100`函数总是将 100 加到给定的输入中，但是它没有：
...
在这里，我们希望得到 109，但是它用 110 回答，这是正确的（101 加 9 等于 110）；我们的[0]a[1]变量从 100 变为 101。闭包需要谨慎使用，而且，根据经验，使用 final 关键字来限制更改。闭包并不总是有害的；在我们想要共享当前状态的情况下（并且在需要的时候能够修改它），闭包非常方便。例如，我们将在需要提供数据库连接（抽象连接）的回调的 API 中使用闭包；我们将使用不同的闭包，每个闭包提供基于特定数据库提供程序设置的连接，通常从外部上下文中已知的属性文件读取。它可以用函数的方式实现模板模式。

# 不变性

在*有效的 Java*中，Joshua Bloch 提出了如下建议：“*将对象视为不可变的*”。在 OOP 世界中需要考虑这个建议的原因在于可变代码有许多可移动的部分；它太复杂，不容易理解和修复。促进不变性简化了代码，并允许开发人员专注于流，而不是关注一段代码可能产生的副作用。最糟糕的副作用是，一个地方的微小变化可能会在另一个地方产生灾难性的结果（蝴蝶效应）。可变代码有时很难并行化，并且常常使用不同的锁。

# 函子

函子允许我们对给定的容器应用函数。他们知道如何从包装对象中展开值，应用给定的函数，并返回另一个包含结果/转换包装对象的函子。它们很有用，因为它们抽象了多种习惯用法，如集合、未来（承诺）和选择。下面的代码演示了如何使用 Java 中的[0]functor，其中[1]可以是将函数应用于现有包装值（即[3]中的[2]）的结果的给定值：
...
应用程序添加了一个新级别的包装，而不是将函数应用于包装对象，函数也被包装。在下面的代码中，函数被包装在一个可选的。为了证明应用程序的一个用法，我们还提供了一个可选的标识（所有内容都保持不变），以防所需的函数（在本例中为[0]）为空。由于没有语法糖来自动应用包装函数，因此我们需要手动执行，请参阅`get().apply()`代码。注意 Java 9 added 方法`Optional.or()`的用法，如果我们的输入 Optional 为空，它会延迟返回另一个`Optional`：
...
前面的代码是我们的应用程序，它将标识函数（输出与输入相同）应用于给定的字符串。
...
Haskell 使用以下 monad（在其他函数式编程语言中导入）。它们对于 Java 世界也很重要，因为它们具有强大的抽象概念（参见[https://wiki.haskell.org/All\u 关于\u Monads](https://wiki.haskell.org/All_About_Monads) ):

*   读卡器 monad 允许共享和读取环境状态。它在软件的可变部分和不可变部分之间提供了边缘功能。
*   writer monad 用于将状态附加到多个 writer，非常类似于记录到多个 writer（console/file/network）的日志过程。
*   国家单子既是读者又是作家。

为了掌握函子、应用程序和单子的概念，我们建议检查一下[http://adit.io/posts/2013-04-17-fuctors 中的、\u 应用程序、\u 和\u 单子 _ 图片.html](http://adit.io/posts/2013-04-17-functors,_applicatives,_and_monads_in_pictures.html) 和[https://bartoszmilewski.com/2011/01/09/monads-for-the-quirry-programmer-part-1/](https://bartoszmilewski.com/2011/01/09/monads-for-the-curious-programmer-part-1/) . 在[的剑水蚤反应图书馆里也有一些功能性的好东西 https://github.com/aol/cyclops-react](https://github.com/aol/cyclops-react) .
...
Lambda 表达式是用于[0]包接口的语法。最重要的是：

*   `BiConsumer<T,U>`：一种使用两个输入参数而不返回结果的操作，通常用在 maps`forEach`方法中。支持使用`andThen`方法链接`BiConsumers`。
*   `BiFunction<T,U,R>`：通过调用其`apply`方法，接受两个参数并产生结果的函数。
...
*   `Consumer<T>`：使用单个输入参数的操作。就像它的二进制对应项一样，它支持链接，并通过调用它的`apply`方法来应用，如下面的示例所示，其中 consumer 是`System.out.println`方法：
...
*   `UnaryOperator<T>`：作用于单个操作数的专用函数，其结果与其操作数的类型相同；可以用`Function<T, T>`替换。
...
流可以是连续的，也可以是并行的。它们可以从[0]接口、JarFile、ZipFile 或位集创建，也可以从 Java9 开始从[1]方法创建。`Collection`类支持`parallelStream()`方法，可以返回并行流或串行流。可以通过调用适当的`Arrays.stream(...)`来构造各种类型的流，例如装箱原语（Integer、Long、Double）或其他类。为一个基元类型调用它的结果是以下特定流：`IntStream`、`LongStream`或`DoubleStream`。这些专门的流类可以使用它们的静态方法之一来构造流，例如`generate(...)`、`of(...)`、`empty()`、`iterate(...)`、`concat(...)`、`range(...)`、`rangeClosed(...)`或`builder()`。通过调用`lines(...)`方法可以很容易地从`BufferedReader`对象获取数据流，该方法也以静态形式存在于`Files`类中，用于从路径给定的文件获取所有行。`Files`类提供了其他流创建者方法，如`list(...)`、`walk(...)`和`find(...)`。

Java9 除了前面提到的`Optional`之外，还添加了更多返回流的类，比如`Matcher`类（方法`results(...)`）或`Scanner`类（方法`findAll(...)`和`tokens()`）。
...
*   `unordered()`：无序处理输入。它使得序列流的输出顺序具有不确定性，并且通过允许更有效地实现一些聚合函数（如 distinct 或[1]），可以提高并行执行的性能。
...
下面的代码显示了`peek`、`limit`和`skip`方法的用法。它计算出商务旅行折合成欧元的费用。第一笔和最后一笔费用与业务无关，因此需要过滤掉（也可以使用`filter()`方法）。`peek`方法是打印费用合计中使用的费用：
...
除了前面介绍的`Stream<T>.ofNullable`方法外，Java9 还引入了`dropWhile`和`takeWhile`。它们的目的是让开发人员更好地处理无限流。在下面的代码中，我们将使用它们将打印的数字限制在 5 到 10 之间。移除上限（由`takeWhile`设置）将导致无限大的递增数字打印（在某个点上，它们将溢出，但仍会继续增加–例如，在迭代方法中使用*x->x+100000*）：
...
*   迭代器（如`iterator()`和`spliterator()`方法）
...
*   聚合（还原）可以是以下任何一种：
    ...
    *   **自定义**：使用`reduce(...)`或`collect(...)`方法。一些可用的收集器包括`maxBy()`、`minBy()`、`reducing()`、`joining()`和`counting()`。
...
使用闭包和`Supplier<T>`可以重新实现单例模式。Java 混合代码可以利用`Supplier<T>`接口，比如在下面的代码中，singleton 是一个 enum（根据函数式编程，singleton 类型是那些只有一个值的类型，就像 enum 一样）。下面的示例代码与[第 2](2.html)章*Creational Patterns*中的代码类似：
...
Lombock 库将生成器作为其功能的一部分引入。只需使用`@Builder`注释，任何类都可以自动访问`builder`方法，如 Lombock 示例代码在[中所示 https://projectlombok.org/features/Builder](https://projectlombok.org/features/Builder) :
...
最好的例子是使用 map 函数，它执行从旧接口到新接口的自适应。我们将重用[第 4 章](4.html)中的示例*结构模式*，稍加改动；地图模拟适配器代码：
...
decorator 可以通过利用函数组合来实现。例如，如前所示，可以使用 stream`peek`方法将日志添加到现有函数调用，并从提供的 peek`Consumer<T>`将日志记录到控制台。

我们的[第 4 章](4.html)，*结构模式*，decorator 示例可以用函数式重写；请注意，decorator 用于使用与初始 decorated consumer 相同的输入：
...
解释器的语法可以存储为关键字映射，相应的操作存储为值。在[第二章](2.html)*创造模式*中，我们使用了一个数学表达式求值器，将结果累加到一个堆栈中。这可以通过将表达式存储在映射中来实现，并使用 reduce 来累加结果：
...
在 Java8 中，观察者模式被 lambda 表达式取代。最明显的例子是[0]替换。使用匿名类侦听器的旧代码被替换为一个简单的函数调用：
...
我们将通过基于给定的 Sleuth 跨度解析和聚合来自多个 web 服务的日志并计算每个命中端点的总持续时间来演示 MapReduce 模式的用法。日志取自[https://cloud.spring.io/spring-cloud-sleuth/spring-cloud-sleuth.html](https://cloud.spring.io/spring-cloud-sleuth/spring-cloud-sleuth.html) 并拆分为相应的服务日志文件。下面的代码并行读取所有日志、映射、排序和过滤相关日志条目，收集并减少（聚合）结果。如果有结果，它将被打印到控制台。导入的日期/时间类用于排序比较。`flatMap`代码需要处理`Exception`，如下代码所示：
...
Loan 模式确保资源一旦超出范围就被决定性地处置。资源可以是数据库连接、文件、套接字或任何处理本机资源的对象（内存、系统句柄、任何类型的连接）之一。这与 MSDN 上描述的 Dispose 模式的意图类似。
...
**尾部调用优化**（**TCO**）是一些编译器在不使用堆栈空间的情况下调用函数的技术。Scala 通过用[0]注释递归代码来利用它。这基本上告诉编译器使用一个特殊的循环，称为 trampoline，它反复运行函数。函数调用可以处于一种或多种要调用的状态。在完成时，它返回结果（head），在更多的情况下，它返回当前循环而不返回 head（tail）。这个模式已经被独眼巨人反应库提供给我们了。

# 意图

其目的是在不破坏堆栈的情况下启用递归调用。它只用于大量的递归调用，对于少数调用，它可能会降低性能。

# 示例

独眼巨人 react 的维护者 John McClean 演示了 TCO 的用法，TCO 计算 Fibonacci 序列中[0]处的数字 https://gist.github.com/johnmcclean/fb1735b49e6206396bd5792ca11ba7b2 . 代码简洁易懂，基本上是从初始状态 a 和 b 开始累加斐波那契数，*f（0）=0*，*f（1）=1-*，应用*f（n）=f（n-1）+f（n-2）*函数：
...
在度量每个版本的代码的性能时，前面的代码似乎都在重复。这可以通过 executearound 方法模式解决，方法是将执行的业务代码包装到 lambda 表达式中。这种模式的一个很好的例子是单元测试前后的设置/拆卸函数。这类似于前面描述的模板方法和贷款模式。
...
在下一章中，我们将深入到反应式世界，学习如何使用 RxJava 创建响应式应用程序。