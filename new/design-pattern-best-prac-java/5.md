...
使用闭包和`Supplier<T>`可以重新实现单例模式。Java 混合代码可以利用`Supplier<T>`接口，比如在下面的代码中，singleton 是一个 enum（根据函数编程，singleton 类型是那些只有一个值的类型，就像 enum 一样）。以下示例代码与[第 2 章](2.html)“创建模式”中的代码类似：
...
解释器的语法可以存储为关键字映射，相应的操作存储为值。在[第二章](2.html)“创建模式”中，我们使用了一个数学表达式求值器，将结果累加成一个堆栈。这可以通过将表达式存储在映射中来实现，并使用 reduce 来累加结果：
