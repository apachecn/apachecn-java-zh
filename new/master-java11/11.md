...
本章介绍 Java 11，Java 平台的**标准版**（**SE**）可从 Oracle 官方下载网站[下载 http://www.oracle.com/technetwork/java/javase/downloads/index.html](http://www.oracle.com/technetwork/java/javase/downloads/index.html) .
...
在本节中，我们将回顾 Java 的**超文本传输协议**（**HTTP**）客户机，首先看一看旧的 Java 9 之前的客户机，然后深入到作为当前 Java 平台一部分的新 HTTP 客户机。最后，我们将看看当前 HTTP 客户机的局限性。需要这种方法来支持对变化的理解。
...
从前面的字段列表中可以看到，已经有了对 HTTP 的强大支持。除了构造函数之外，还有大量可用的方法，包括以下方法：
...
前面列出的类方法是继承自`java.net.URLConnection`类和`java.lang.Object`类的方法之外的方法。

原来的 HTTP 客户机存在一些问题，这些问题使得它适合用新的 Java 平台进行更新。这些问题如下：

*   基本的`URLConnection`API 已经失效的协议，比如 Gopher 和 FTP，这些年来成为了一个越来越多的问题
*   `HttpURLConnection`API 早于 HTTP1.1，过于抽象，不易使用
...
HTTP 客户机 API 有一些故意的缺点。虽然这听起来可能有悖常理，但新的 API 并不是要完全取代当前的`HttpURLConnection`API。相反，新的 API 最终将取代当前的 API。

下面的代码片段提供了一个示例，说明如何实现`HttpURLConnect`类来打开和读取 Java 应用程序中的 URL：
...
Java 9 之前的 Doclet API 或`com.sun.javadoc`包使我们能够查看源代码中的 Javadoc 注释。调用 Doclet 是使用`start`方法完成的。此方法的签名为`public static boolean start(RootDoc root)`。我们将使用`RootDoc`实例作为程序结构信息的容器。
...
当前面列出的项目用于调用`javadoc`时，提供一个文档集作为过滤列表。如果我们的目标是获得一份全面的、未经筛选的清单，我们可以使用`allClasses(false)`。
...
正如您通过完整注释的代码所看到的，访问[0]内容相对容易。在前面的示例中，我们将通过在命令行中使用以下代码来调用`AllThrows`类：
...
[0]类提供了如何创建类来启动 Doclet 的示例。它包含一个空的`Doclet()`构造函数和以下方法：
...
*   更新`com.sun.javadoc`Doclet API 以利用多个 Java SE 和 JDK API
...
编译器树 API 在`com.sun.source.doctree`包中。它提供了几个接口来记录源代码级别的注释。这些 API 表示为**抽象语法树**（**AST**）。

有两个枚举，如下所示：

*   `AttributeTree.ValueKind`，常数如下：
*   `DOUBLE`
*   `EMPTY`
*   `SINGLE`
*   `UNQUOTED`
...
`AnnotatedConstruction`接口为语言模型 API 提供了一个可注释的构造，该 API 自 1.8 版以来一直是 Java 平台的一部分。它适用于元素（接口`Element`）或类型（接口`TypeMirror`）的构造，每个构造的注释不同，如下表所示：
...
*   `getAnnotationsByType(Class<A> annotationType)`：此方法返回构造的关联注释
...
随着 Java 平台的正式发布，`SourceVersion`enum 将被更新为包含`RELEASE_10`和`RELEASE_11`。
...
如果`CharSequence`是有效名称，则此方法返回`true`。
...
[0]类扩展了[1]，是未知异常的超类。类构造函数如下所示：

```
protected UnknownEntityException(String message)
```

构造函数使用作为字符串参数提供的消息创建一个新的`UnknownEntityException`实例。该方法不接受其他参数。

此类没有自己的方法，但继承了来自`java.lang.Throwable`和`class.java.lang.Object`类的方法，如下所示：
...
下面的简短 Java 应用程序只生成一个宽为[0]、高为[1]的帧。这里显示的是没有任何 Javadoc 标记的，我们将在本节后面讨论：
...
完成包或类后，您可以使用 Javadoc 工具生成 Javadoc。您可以从命令行或 IDE 中运行位于 JDK`/bin`目录中的 Javadoc 工具。每个 IDE 处理 Javadoc 生成的方式都不同。例如，在 Eclipse 中，您可以从下拉菜单中选择 Project，然后选择 generatejavadoc。在 IntelliJ IDEA IDEA 中，选择 Tools 下拉菜单，然后选择 Generate Javadoc。

下面的屏幕截图显示了 Generate Javadoc 功能的 IntelliJ IDEA 接口。如您所见，`-html5`命令行参数已经包括在内：
...
有关如何为 Javadoc 工具编写文档注释的更多信息，请访问 Oracle 的官方说明，网址为[http://www.oracle.com/technetwork/articles/java/index-137868.html](http://www.oracle.com/technetwork/articles/java/index-137868.html) .
...
**mJRE**（简称**Multiple JRE**）特性以前用于指定一个特定的 JRE 版本或一系列版本来启动我们的应用程序。我们可以通过命令行选项`-version`或者通过 JAR 文件清单中的一个条目来实现这一点。以下流程图说明了根据我们的选择所发生的情况：
...
*   已删除 mJRE 功能。
*   现在只要使用[0]命令行选项，启动程序就会产生错误。这是一个终端错误，处理将无法继续。
...
在这里，您可以看到一个打开的终端窗口，它首先检查 Java 的版本，然后使用[0]命令启动 Nashorn shell。在本例中，Java 和 Nashorn 的版本都是 1.8.0.121。或者，我们可以简单地用`jjs`命令启动 Nashorn，shell 将在没有版本标识的情况下打开：
...
为了让 Java 运行这个 JavaScript 应用程序，我们将使用[0]命令。下面是输出：
...
如您所见，使用[0]选项可以通过使用 Nashorn 作为文本编辑器来创建脚本。使用 Nashorn 时，有几个内置函数非常有用：

*   `echo()`：类似`System.out.print()`Java 方法
*   `exit()`：这是纳索恩的出口
*   `load()`：从给定路径或 URL 加载脚本
*   `print()`：类似`System.out.print()`Java 方法
*   `readFull()`：读取文件内容
...
**ECMA**（简称**欧洲计算机制造商协会**）成立于 1961 年，是信息系统和通信系统的标准组织。今天，ECMA 继续制定标准并发布技术报告，以帮助标准化消费电子、信息系统和通信技术的使用方式。ECMA 有 400 多项标准，其中大部分已被采用。
...
创建新的 API`jdk.nashorn.api.tree`是为了允许将来对 Nashorn 类进行更改。在新的解析器 API 之前，IDEs 使用 Nashorn 的内部 AST 表示进行代码分析。根据 Oracle 的说法，`jdk.nashorn.internal.ir`包的使用阻止了 Nashorn 内部类的现代化。
...
下图显示了新的多版本 JAR 文件结构，其中包含 Java 8 和 Java 9 的特定于 Java 版本的类文件：
...
*   `URLClassLoader`是基于 JAR 的，经过修改，可以从指定版本读取类文件。
*   新的基于模块的类加载器是 java9 的新成员，它可以从指定的版本读取类文件。
*   修改了[0]类，以便从多版本 JAR 文件中选择适当的类版本。
*   jarurl 方案的协议处理程序被修改，以便它从多版本 JAR 文件中选择适当的类版本。
*   Java 编译器`javac`是用来读取类文件的已标识版本的，这些版本标识是通过使用`JavacFileManager`API 和`ZipFileSystem`API 的`-target`和`-release`命令行选项进行的。
*   修改了以下工具以利用对`JavacFileManager`API 和`ZipFileSystem`API 的更改：
*   `javah`：生成 C 头文件和源文件
...
[0]Javadoc 标记已被更合适的注释替换。此外，这些新注释现在在运行时被处理，以便动态生成`BeanInfo`类。Java 的模块化导致了这种变化。自定义`BeanInfo`类的创建已经简化，客户机库已经模块化。

为了完全掌握这一变化，我们将在进一步讨论本 JEP 之前回顾`JavaBean`、`BeanProperty`和`SwingContainer`。

# 爪哇豆

`JavaBean`是 Java 类。与其他 Java 类一样，`JavaBean`是可重用的代码。它们在设计上是独特的，因为它们将多个对象封装成一个对象。`JavaBean`类必须遵循三个约定：
...
`BeanProperty`是批注类型。我们使用这个注解来指定一个属性，这样我们就可以自动生成`BeanInfo`类。这是一个相对较新的 Java 注释，从 Java9 开始
...
`SwingContainer`是批注类型。我们使用这个注释来指定与 swing 相关的属性，这样我们就可以自动生成`BeanInfo`类。
...
现在我们已经复习了`JavaBean`、`BeanProperty`和`SwingContainer`，让我们看看`BeanInfo`类。

# BeanInfo 类

在大多数情况下，`BeanInfo`类是在运行时自动生成的。例外是`Swing`类。这些类基于`@beaninfo`Javadoc 标记生成`BeanInfo`类。这是在编译时完成的，而不是在运行时。从 Java9 开始，`@beaninfo`标记被替换为`@interface JavaBean`、`@interface BeanProperty`和`@interface SwingContainer`注释。

这些新注释用于根据前面部分中提到的可选元素设置相应的属性。例如，下面的代码段设置了`SwingContainer`的属性：
...
*   在`Bean`类中指定属性比创建单独的`BeanInfo`类要容易得多
*   我们将能够删除自动生成的类
*   使用这种方法，客户机库更容易模块化

# TIFF 支持

图像输入/输出插件已经为现代 Java 平台进行了扩展，包括对 TIFF 图像格式的支持。[0]类扩展了[1]类，是 JavaSE 的一部分。这个类包含几种编码和解码图像的方法。以下是静态方法列表：
...
| `read(File input)` | 带`ImageReader`的`BufferedImage` |
...
| `read(URL input)` | 带`ImageReader`的`BufferedImage` |
...
从提供的方法中可以看出，图像输入/输出框架为我们提供了使用图像编解码器的方便方法。从 Java 7 开始，`javax.imageio`实现了以下图像格式插件：
...
如您所见，TIFF 不在图像文件格式列表中。TIFF 是一种常见的文件格式，2001 年，macOS 随着 macOS X 的发布，广泛使用了这种格式

当前的 Java 平台包括用于 TIFF 的`ImageReader`和`ImageWriter`插件。这些插件是用 Java 编写的，并被捆绑在新的`javax.imageio.plugins.tiff`包中。

# 平台日志记录

现代 Java 平台包括一个日志 API，它使平台类能够记录消息，并提供相应的服务来操作日志。在深入了解日志 API 和服务的新特性之前，让我们先回顾一下 Java7 中引入的`java.util.logging.api`。
...
| `Handler` | `public abstract class Handler extends Object` | 导出`Logger`条消息 |
...
`java.base`模块已更新以处理日志功能，不依赖`java.util.logging`API。它有一个独立的 facade 和实现组件。这意味着，当使用第三方框架时，JDK 只需要提供实现组件并返回与请求日志框架一起工作的平台日志记录器。
...
*   将使用`CatalogManager`创建`CatalogResolvers`。
*   将遵循 OASIS 打开目录文件语义：
*   将外部标识符映射到 URI 引用
*   将 URI 引用映射到另一个 URI 引用
...
Oracle 对集合文字的定义是*一种语法表达式形式，其计算结果为聚合类型，例如数组、列表或映射*（[http://openjdk.java.net/jeps/186](http://openjdk.java.net/jeps/186) ).
...
现代 Java 平台包括一个公共 API，它使我们能够编写能够访问特定于平台的桌面功能的应用程序。这些功能包括与任务栏/工作台交互以及侦听应用程序和系统事件。

macOS X`com.apple.eawt`包是一个内部 API，从 java9 开始，就不能再访问了。为了支持 Java 的嵌入式平台特定的桌面特性，`apple.applescript`类被删除而不进行替换。它们在 Java9、10 或 11 中不可用。
...
前面的类都是`java.lang.invoke`包的一部分，该包已针对现代 Java 平台进行了更新。这些改进是通过使用`MethodHandle`个组合、`for`个循环和`try...finally`个块的查找细化实现的。
...
这一增强源于开发人员的反馈，以及希望使`MethodHandle`、`MethodHandles`和`MethodHandles.Lookup`类更易于使用的愿望，还呼吁添加其他用例。
...
最近进行了三次更新以改进[0]参数处理。这些变化如下：

*   使用[0]的参数折叠以前没有位置参数：
    *   使用[0]方法的参数集合以前不支持将参数集合到数组中，但尾部元素除外。这一点已经改变，现在有一个额外的`asCollector`方法来支持该功能。

*   在参数集合的反向方法中，使用[0]方法的参数扩展将尾部数组的内容扩展到多个参数。已修改参数扩展，以支持在方法签名的任何位置扩展数组。

下一节将提供更新的`asCollector`和`asSpreader`方法的新方法定义。

# 附加组合

添加了以下附加组合以支持对`java.lang.invoke`包的`MethodHandle`、`MethodHandles`和`MethodHandles.Lookup`类的易用性和优化：
...
这些工具分别在 JavaSE5 和 JDK1.1 中引入。`@Deprecated`注释的目的是注释不应该使用的程序组件，因为它们被认为是危险的和/或有更好的选择。这就是预期用途，实际用途各不相同，而且由于警告只在编译时提供，因此几乎没有理由忽略带注释的代码。

增强的弃用工作是为了向开发人员提供关于规范文档中 api 的预期配置的更清晰的信息。这方面的工作还产生了一个分析程序使用不推荐的 api 的工具。

为支持信息的保真度，已将以下组件添加到[0]批注类型中：

*   `forRemoval()` :
*   如果 API 元素已被安排在将来删除，则返回布尔值`true`
*   如果 API 元素尚未计划在将来删除，但已弃用，则返回布尔值`false`
*   默认为``false``
...
在下一章中，我们将介绍并发增强，我们的主要关注点是对`Flow`类 API 提供的反应式编程的支持。我们还将探讨额外的并发增强。
...
4.  命名一个或多个属于 Doclet 类的方法（构造函数除外）。
5.  编译器树 API 中的枚举是什么？
...
*   *Docker Fundamentals【综合课程】**[提供 https://www.packtpub.com/virtualization-and-cloud/docker-fundaments-integrated-course](https://www.packtpub.com/virtualization-and-cloud/docker-fundamentals-integrated-course) .*
**   *Java 9:构建健壮的模块化应用程序*，在[提供 https://www.packtpub.com/application-development/java-9-building-robust-modular-applications](https://www.packtpub.com/application-development/java-9-building-robust-modular-applications) .*