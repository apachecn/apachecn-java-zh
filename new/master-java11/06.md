...
在本章中，我们将首先介绍新的命令行，Java 中的**Read Eval Print Loop**（也称为**REPL**）工具，以及**Java Shell**（**JShell**）。我们将首先回顾一些关于这个工具的介绍性信息，REPL 概念，然后讨论可以与 JShell 一起使用的命令和命令行选项。我们将采用实践者的方法来回顾 JShell，并包括您可以自己尝试的示例。
...
您已经看到了我们用`jshell -h`执行的`-h`选项。这提供了 JShell 选项的列表。

要登录 JShell，只需使用[0]命令即可。您将看到命令窗口中的提示会相应更改：
...
| `/exit` | 此命令用于退出 JShell。语法只是[0]，没有任何可用的选项或参数。 |
| `/history` | history 命令提供您所键入内容的历史记录。语法只是[0]，没有任何可用的选项或参数。 |
| `/<id>` | 此命令用于通过引用[0]重新运行以前的代码段。语法如下：`/<id>`您也可以使用`/-<n>`参照*n<sup>th</sup>*前面的代码段来运行特定的代码段。 |
| `/imports` | 可以使用此命令列出导入的项目。语法为`/imports`，不接受任何选项和参数。 |
...
| `/reload` | [0]命令使您能够重置和重放相关历史记录。语法如下：`/reload [-restore] [-quiet] [-class-path <path>] [-module-path <path>]` |
...
在 JShell 中输入[0]或[1]命令可以提供一个完整的命令列表和可以在 shell 中使用的语法。该清单如下：

![](img/00a3354b-8757-4fd3-abed-bc4733f6d427.png)

我们鼓励您尝试使用 JShell 命令。您可以使用前面的屏幕截图来提醒自己正确的语法。

如果您还不熟悉 JShell，`/help`命令会特别有用。您可以在下面的屏幕截图中看到，只需输入`/help intro`命令，我们就可以获得 JShell 的简介：

![](img/f0ad218d-880a-4b74-ae2b-960bb9d32bb0.png)

如果您发现自己经常使用 JShell，那么您可能会受益于下面列出的一个或多个快捷方式。可以随时使用[0]命令从 JShell 中列出这些内容：

![](img/7aa11d03-2235-4955-871f-337e9840a532.png)

您可以在 JShell 中使用[0]命令，然后使用您想要获得其他帮助的命令，来获得其他帮助。例如，输入`/help reload`提供有关`/reload`命令的详细信息。该信息提供如下：
...
您会注意到我们使用`-q`作为`concise`模式，而不是`-c`。`-c`选项具有`-c<flag>`语法，用于将`<flag>`传递给编译器。有关这些标志的更多信息，请参阅本章*进一步阅读*部分中列出的参考资料。
...
为了开始我们的第一个测试，我们将在`jshell>`提示符处执行`/set feedback normal`命令，将 JShell 反馈模式设置为`normal`。进入`normal`反馈模式后，我们将输入必要的命令来运行演示：

![](img/8831a562-feae-4a1e-9cd7-edddaf9d4e44.png)

进入`normal`反馈模式后，我们进入`int myVar = 3`，收到`myVar ==> 3`作为反馈。在下一个命令中，我们更改了相同变量的值，并用新值接收相同的输出。我们的下一个语句`void quickMath() {System.out.println("Your result is " + (x*30 + 19));}`使用了一个未声明的变量，您将看到由两部分组成的反馈，一部分指示方法已创建，另一部分通知您在声明未声明的变量之前无法调用该方法。接下来，我们修改了我们的方法以包含`myVar`变量，并且反馈报告该方法被修改。我们的最后一步是使用`quickMath();`运行这个方法，结果和我们预期的一样

让我们在`concise`模式下尝试同样的反馈演示：

![](img/1029d7ca-3c65-43aa-9d58-bd4a6589fcdf.png)

从前面的截图可以看出，`concise`反馈模式给我们的反馈要少得多。我们创建并修改了变量，但没有收到任何反馈。当我们使用未声明的变量创建方法时，我们收到的反馈与我们在`normal`模式中收到的反馈相同。我们在没有确认或其他反馈的情况下更新了方法。

我们下次使用反馈演示将在`silent`模式下进行：

![](img/b86dc24b-efda-488f-9fce-771fb84a33bc.png)

当我们进入`silent`反馈模式时，您可以在前面的截图中看到，JShell 提示从`jshell>`变为`->`。当我们创建`myVar`变量、修改`myVar`变量或创建`quickMath()`方法时，没有提供反馈。我们故意创建了`quickMath()`方法来使用未声明的变量。因为我们处于`silent`反馈模式，所以没有通知我们该方法有未声明的变量。基于这种缺乏反馈的情况，我们运行了这个方法，没有得到任何输出或反馈。接下来，我们更新了方法以包含[8]声明的变量，然后运行该方法。

由于没有提供反馈，`silent`反馈模式似乎毫无意义，但这种模式有很大的实用价值。使用`silent`模式可能适合于流水线，或者仅当您希望最小化终端输出量时。例如，您可以使用隐式[2]命令包含特定的条件输出。
...
作为一个例子，让我们浏览一下 truncation 设置，该设置要求在每个输出行上显示多少个字符。使用`/set truncation`命令，如下图所示，显示当前的截断设置：
...
正如您在上一个屏幕截图中看到的，基于我们使用的[4]JShell 命令，我们的[0]反馈模式的截断成功地从[2]模式继承的[1]更改为[3]。
...
我们还可以使用[0]命令和选项组合来查看 JShell 导入了哪些包。正如您在下面的屏幕截图中看到的，JShell 导入了几个包，使我们在 shell 中的工作更加方便，从而节省了我们在方法中导入这些标准包的时间：

![](img/6b657374-17e1-47d9-9b0c-83d79d6dfa27.png)

如果只想列出启动导入，可以使用`/list -start`命令和选项组合。正如您在下面的屏幕截图中看到的，每个启动导入都有一个`s`前缀，并按数字顺序排列：
...
| 当使用*Ctrl*+*Y*（或 Macintosh 上的 c*md+Y*时，您可以使用*Alt+Y*键盘组合循环浏览先前删除的文本 | *Alt*+*Y* | *alt*+*Y* |
...
JShell 会记住您输入的代码段和命令。它维护此历史记录，以便您可以重用已输入的代码段和命令。要循环浏览代码段和命令，您可以按住[0]Ctrl[1]键（Macintosh 上的 c[2]md[3]），然后使用上下箭头键，直到看到所需的代码段或命令。

# 高级编辑命令

还有几个编辑选项可用，以便您可以包括搜索功能、宏创建和使用等。JShell 的编辑器基于 JLine2，这是一个用于解析控制台输入和编辑的 Java 库。您可以在这里了解更多关于 JLine2 的信息：[https://github.com/jline/jline2/wiki/jline-2.x-wiki](https://github.com/jline/jline2/wiki/JLine-2.x-Wiki.) .
...
默认情况下，`DEFAULT`启动脚本由 JShell 使用。如果要使用不同的启动脚本，只需使用[1]命令即可。举个例子：
...
*   加载脚本的第二个选项是在命令提示符处使用[0]命令。这将启动 JShell 并加载[1]JShell 脚本。

# 保存脚本

除了在外部编辑器中创建 JShell 脚本之外，我们还可以在 JShell 环境中创建它们。采用这种方法时，需要使用[0]命令保存脚本。在下面的屏幕截图中可以看到，`/save`命令至少需要一个文件名参数：

![](img/4cb493b3-45ee-432c-8f8e-4849de3e3017.png)

使用`/save`命令有三个可用选项：

*   [0]选项可用于将所有代码段的源代码保存到指定的文件中。
*   [0]选项保存自启动 JShell 以来输入的所有命令和代码段的连续历史记录。JShell 执行此操作的能力告诉您，它维护您输入的所有内容的历史记录。
*   `-start`选项将当前的启动定义保存到指定的文件中。
...
在本章中，我们研究了 JShell，Java 的 REPL 命令行工具，我们从介绍该工具的信息开始，仔细研究了 REPL 的概念。我们花了大量时间来检查 JShell 命令和命令行选项。我们的报道包括反馈模式的实用指南、资产列表和 shell 中的编辑。我们还获得了使用脚本的经验
...
1.  什么是 REPL？
2.  什么是地狱？
3.  你能说出 JShell 最近的四个特性吗？它们使 JShell 的使用更加高效？
