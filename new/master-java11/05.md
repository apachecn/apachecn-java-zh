...
在上一章中，我们仔细研究了由 projectjigsaw 指定的 Java 模块的结构，并研究了如何实现 projectjigsaw 来改进 Java 平台。我们还回顾了 Java 平台的主要内部变化，特别关注新的模块化系统。我们从模块化入门开始，从好处和需求方面了解了 Java 的模块化系统。接下来，我们探讨了 JDK 的模块化，包括源代码是如何重组的。我们还研究了 JDK 的七个主要工具类别，并了解到 Java 模块化扩展到了运行时映像，从而提高了可维护性、性能和安全性。引入了**链接时间**的概念，作为编译时和运行时之间的可选阶段。在本章的结尾，我们介绍了 Java 链接器以及 Java 如何封装内部 api。

在本章中，我们将探讨如何将现有的应用程序迁移到当前的 Java 平台。我们将研究手动和半自动迁移过程。本章旨在为您提供一些见解和过程，使您的非模块化 Java 代码能够在当前的 Java 平台上工作。
...
一个**集成开发环境**（**IDE**软件包就足够了。来自 JetBrains 的 IntelliJ IDEA 用于与本章和后续章节相关的所有编码。IntelliJ IDEA 社区版可从网站下载（[https://www.jetbrains.com/idea/features/](https://www.jetbrains.com/idea/features/) ).
...
自 1995 年以来，Java 以一种令人印象深刻的方式不断发展，随着每一步的发展，JDK 都变得越来越大。与 Java8 一样，JDK 已经变得非常庞大。在 Java 9 之前，由于 JDK 的整体性，存在一些问题，包括以下问题：
...
在编写 Java 应用程序时，您的模块声明将放在一个`module-info.java`文件中。一旦这个文件完成，您只需运行 Java 编译器 Javac 来生成`module-info.class`Java 类文件。完成此任务的方式与当前将`.java`文件编译为`.class`文件的方式相同

您还可以创建模块化 JAR 文件，将您的[0]文件放在其根目录下。这代表了极大的灵活性。
...
Java 模块概念的核心是理解基本模块。在编程 Java 应用程序或移植使用旧版本 Java 编程的现有应用程序时，必须使用基本模块（`java.base`）。每个模块都需要`java.base`模块，因为它定义了关键的或基础的 Java 平台 API
...
如您所见，`java.base`模块不需要任何模块，它导出了许多包。将这些导出的列表放在手边是很有用的，这样当您开始使用 Java 平台创建应用程序时，就可以知道哪些是可用的。

您会注意到，在上一节中，我们在[1]模块的声明中没有包含必需的[0]：代码行。更新后的代码如下所示，现在包括所需的`java.base`行代码：
...
正如本章前面提到的，模块为我们的 Java 应用程序提供了可靠的配置，解决了 Java 平台早期版本中的类路径问题。
...
下面是一个运行时错误的示例，如果找不到模块`com.three19.isrisCore`，但`com.three19.irisScan`应用程序需要该模块，则会发生该错误：
...
在前面的示例中，`com.three19.irisScan`模块有一个供内部使用的`irisScanner`包和一个`irisScanResult`类。如果`com.three19.access`应用程序试图导入并使用`irisScanResult`类，Java 编译器将产生以下错误消息：
...
如果您要维护使用 Java8 或更早版本构建的 Java 应用程序，则应考虑将应用程序更新到现代 Java 平台。因为后 java8 平台与早期版本有很大的不同，所以在迁移应用程序时有必要采用一种有目的的方法。谨慎的做法是提前计划并考虑你最可能遇到的问题。在我们研究这些问题之前，让我们在下一节中测试一个简单的 Java 应用程序。

# 测试一个简单的 Java 应用程序

以下代码由单个 Java 类组成，`GeneratePassword`。此类提示用户输入所需的密码长度，然后根据用户请求的长度生成密码。如果用户要求长度小于 8，则将使用默认长度 8。这段代码是用 Java SE 1.7 JRE 系统库编写的：
...
在下面的屏幕截图中，我们在运行 Java8 的 Mac 上测试`GeneratePassword`应用程序。如您所见，我们首先查询 Java 以验证当前版本。在这个测试中，使用了 Java`1.8.0_121`。接下来，我们使用`javac`实用程序编译`GeneratePassword`Java 文件。最后，我们运行应用程序：

![](img/449e6f07-52e1-4e66-88a4-16153d77be70.png)

从前面的测试中可以看到，`GeneratePassword.java`已成功编译，生成了`GeneratePassword.class`文件。应用程序是使用[2]命令运行的。提示用户输入所需的密码长度，并输入了[3]。然后，应用程序成功地生成了一个 32 个字符的随机密码，并提供了相应的输出

这个测试证明了这个示例应用程序使用 jdk1.8 可以成功地工作。接下来，让我们使用 JDK 10 测试相同的应用程序：

![](img/101f88cf-d405-49ce-b2e7-02c368a01bf4.png)

如您所见，我们清楚地演示了 Java9 之前的应用程序有可能在 Java10 上成功运行，而无需进行任何修改。这是一个简单的案例研究，具有一个非常基本的 Java 程序。当然，这是最好的情况，不能假设。您需要测试应用程序，以确保它们在当前 Java 平台上按预期运行。

在下一节中，我们将回顾在使用新的 Java 平台测试 Java9 之前的应用程序时可能遇到的一些潜在问题。

# 潜在的迁移问题

本节介绍的潜在迁移问题包括直接访问 JRE、访问内部 api、访问内部 JAR、jarurl 折旧、扩展机制和 JDK 的模块化。让我们看看每一个潜在的迁移问题。

# JRE

创建 Java 的模块化系统使得开发工具和实用工具的数量和位置得到了一些简化。一个这样的例子是 JDK 对 JRE 的使用。在所有 Java9 之前的版本中，Java 平台都将 JDK 和 JRE 作为两个独立的组件包含在内。从 Java9 开始，这些组件已经组合在一起。这是一个重要的变化，开发人员应该非常清楚。如果您有一个专门指向 JRE 目录的应用程序，则需要进行更改以避免出现问题。JRE 内容如下：

![](img/3ac2b50a-213b-487d-9196-e9926eda2abd.png)

# 访问内部 API

当前的 Java 平台封装了内部 api，以提高用 Java 编写的平台和应用程序的安全性。与以前版本的 Java 平台不同，用 java9、10 或 11 编写的应用程序将不具有对 JDK 内部 api 的默认访问权限。Oracle 已经确定一些内部 API 是关键的，这些 API 仍然可以通过`jdk.unsupported`JDK 模块访问。
...
如果您有实现任何[0]或[1]包的 Java 9 之前的应用程序，那么将应用程序迁移到当前 Java 平台时可能会遇到问题。为了解决这个问题，您应该检查您的类文件以使用`sun.*`和`com.sun.*`包。或者，您可以使用 Java 依赖性分析工具`jdeps`来帮助确定您的 Java 程序是否对 JDK 内部 API 有任何依赖性。

[0]工具是 Java 依赖性分析工具；它可以用来帮助确定 Java 程序是否对 JDK 内部 API 有任何依赖性。

# 访问内部 jar

从版本 9 开始，Java 不允许访问`lib/ant-javax.jar`、`lib/dt.jar`和`lib`目录中列出的其他内部 JAR，如下所示：

![](img/cb3914b7-bbf3-4e0d-b0fb-8332a456c621.png)

这里需要注意的关键是，如果您的 Java 应用程序依赖于驻留在[0]文件夹中的这些工具之一，则需要相应地修改代码。

建议您在开始使用 Java10 和 Java11 之后测试 IDE，以确保 IDE 得到更新并正式支持最新版本的 Java。如果您使用多个 IDE 进行 Java 开发，请测试每一个 IDE 以避免意外。

# JAR URL 折旧

JAR 文件 URL 在 Java9 之前被一些 API 用来标识运行时映像中的特定文件。这些 URL 包含一个带有两条路径的`jar:file:`前缀，一条路径指向`jar`，另一条路径指向`jar`中的特定资源文件。以下是 Java 9 JAR 之前的 URL 的语法：
...
新的 URL 架构`jrt`现在已就位，用于命名运行时映像中的资源。这些资源包括类和模块。新的模式允许在不给运行时映像带来安全风险的情况下识别资源。这种增强的安全性确保运行时映像的形式和结构保持隐藏。新架构如下：
...
*   `jrt:/$MODULE`：此结构提供对`$MODULE`参数指定的模块内所有资源文件的引用
...
Java 平台以前有一个扩展机制，使开发人员能够为所有应用程序提供定制 api。如下图所示，扩展是 Java 平台的插件或附加组件。默认情况下，每个扩展中的 API 和类都自动可用：

![](img/58c14de5-53a4-475d-86a4-6c490cc40f7d.png)

如图所示，Java 应用程序可以访问 Java 平台和扩展，而不需要类路径。此功能在 Java8 中已被弃用，并且在当前版本的 Java 中不再存在。
...
从 8 或更早版本迁移应用程序时，应用程序及其库将成为未命名的模块。因此，您需要确保所有模块都在模块路径中

另外需要注意的是，运行时映像不会包含整个 JDK，而是只包含应用程序所需的模块。值得回顾一下 JDK 是如何在 Java 中模块化的。下表包含当前 JDK 的 API 规范：
...
请记住，默认情况下，所有应用程序都可以访问模块路径中的`java.base`。
...
*   `java.smartcardio`定义 Java 智能卡输入输出的 API。

有关这些模块的详细信息，请访问 Oracle 的*Java®平台，Standard Edition Java Development Kit Version 10 API Specification*网站：[http://docs.oracle.com/javase/10/docs/api/overview-summary.html](http://docs.oracle.com/javase/10/docs/api/overview-summary.html) .
...
Oracle 提供了一个五步流程，帮助开发人员将 Java 应用程序从 Java 9 以前的版本迁移到现代版本 9、10 或 11。以下列出了这些步骤，随后的部分将介绍这些步骤：
...
如果您是在 Java11（18.9）正式发布之前阅读这本书，那么您可以从以下链接获得 JDK11 早期访问构建：[http://jdk.java.net/11/](http://jdk.java.net/11/) .

早期版本可用于 Windows（32 和 64）、macOS（64）、Linux（32 和 64）以及各种 Linux ARM、Solaris 和 Alpine Linux 版本。

在正式发布 Java11 之前花点时间测试 Java9 应用程序并迁移它们，这将有助于确保依赖于 Java 应用程序的服务不会出现任何停机。
...
如本章前面所述，您现有的 Java 应用程序有可能在 Java11 平台上运行而不进行修改。因此，在进行任何更改之前，请尝试在 Java9 平台上运行当前的应用程序。如果您的应用程序在 Java11 上运行得很好，那就太好了，但是您的工作还没有完成。回顾下面三个部分：更新第三方库和工具、编译应用程序以及在代码上运行[0]。

# 更新第三方库和工具

第三方库和工具可以帮助扩展我们的应用程序并缩短开发时间。对于 Java 兼容性，确保您使用的每个第三方库和工具都与 JDK 的最新版本兼容并支持它是很重要的。在 Java11 上运行应用程序并不能为您提供所需的洞察力级别，以确保您不会遇到兼容性问题。建议您查看每个库和工具的官方网站，以验证与 JDK18.9 的兼容性和支持。

如果您使用的库或工具的版本确实支持 JDK18.9，请下载并安装它。如果您发现一个还不支持 JDK18.9，请考虑找一个替代品。

在我们的上下文中，工具包括 IDE。NetBeans、Eclipse 和 IntelliJ 都有支持 JDK 11 的 IDE 版本。这些网站的链接如下：

*   **上网本**：[http://bits.netbeans.org/download/trunk/nightly/latest/](http://bits.netbeans.org/download/trunk/nightly/latest/)
*   **日食**：[http://www.eclipse.org/downloads/packages/release/oxygen/m2](http://www.eclipse.org/downloads/packages/release/oxygen/m2)
*   **智能**：[https://www.jetbrains.com/idea/download/#部分=窗口](https://www.jetbrains.com/idea/download/#section=windows)

# 编译应用程序

下一步是使用 JDK 的`javac`编译应用程序。这一点很重要，即使您的应用程序可以很好地与最新的 JDK 配合使用。您可能不会收到编译器错误，但也要注意警告。以下是您的应用程序可能无法使用新 JDK 编译的最常见原因，假设它们在 Java9 之前编译良好。

首先，如本章前面所述，大多数 JDK 的内部 api 在默认情况下是不可访问的。在运行时或编译时，您的指示将是一个[0]错误。您需要更新代码，以便使用可访问的 api

Java9 之前的应用程序可能无法使用 JDK18.9 编译的第二个原因是，如果将下划线字符用作单个字符标识符。根据 Oracle 的说法，这种做法在 java8 中生成警告，在 java9、10 和 11 中生成错误。让我们看一个例子。下面的 Java 类实例化了一个名为`_`的`Object`，并向控制台输出一条单数消息：
...
当我们用 Java 8 编译这个程序时，我们收到一个警告，在 Java SE 8 之后的版本中可能不支持使用[0]作为标识符：

![](img/5061c9ec-024b-4633-82ab-79506b9257e8.png)

正如您在下面的屏幕截图中看到的，这只是一个警告，应用程序运行正常：

![](img/341ac55f-8605-4e2e-a83b-c4231bf73ffc.png)

现在让我们尝试使用 JDK 9 编译同一个类：

![](img/7a3e7d9b-9d68-4c10-b74c-d54b7693bb63.png)

如您所见，使用下划线作为单个字符标识符仍然只会导致警告而不是错误。应用程序已成功运行。这个测试是在 jdk9 还处于早期版本时运行的。

在 Java10 和 Java11 中，使用`_`作为标识符是非法的。下面的屏幕截图显示了编译`Underscore.java`应用程序的尝试：

![](img/cafb30ff-4504-4dcd-9788-03c54a6c6dcc.png)

您的预 Java9 编程应用程序不使用 JDK9、10 或 11 编译的第三个潜在原因是您使用的是[0]和[1]编译器选项。让我们看看 Java9 之前和 Java10 之后的`-source`和`-target`编译器选项。

# Java9 之前的源和目标选项

[0]选项指定 Java SE 版本，并具有以下可接受的值：

| **值** | **说明** |
| 1.3 | `javac`不支持 Java SE 1.3 之后引入的功能。 |
| 1.4 | `javac`接受 Java SE 1.4 中引入语言特性的代码。 |
| 1.5 或 5 | `javac`接受 Java SE 1.5 中引入语言特性的代码。 |
| 1.6 或 6 | `javac`将编码错误报告为错误，而不是警告。值得注意的是，JavaSE1.6 没有引入新的语言特性。 |
| 1.7 或 7 | `javac`接受 Java SE 1.7 中引入语言特性的代码。如果不使用`-source`选项，这是默认值。 |

[0]选项告诉[1]目标 JVM 的版本。`-target`选项的可接受值为：`1.1`、`1.2`、`1.3`、`1.4`、`1.5`或`5`、`1.6`或`6`和`1.7`或`7`。如果不使用`-target`选项，则默认的 JVM 目标取决于与`-source`选项一起使用的值。以下是`-source`值及其相关`-target`的表格：
...
| 8 | 将“支持”设置为 1.8。 |
| 7 | 将支持设置为 1.7。 |
| 6 | 将 support 设置为 1.6，并生成一个警告（不是错误）来指示 JDK 6 已折旧。 |

# 在代码上运行 jdeps

`jdeps`类依赖性分析工具对 Java 来说并不新鲜，但对于开发人员来说，它可能从未像 Java 模块化系统的出现那样重要。将应用程序迁移到 Java9、10 或 11 的一个重要步骤是运行`jdeps`工具来确定应用程序及其库的依赖关系。如果您的代码依赖于任何内部 api，那么[2]工具可以很好地建议替换。

下面的屏幕截图显示了使用`jdeps`分析器时可用的选项：
...
现在让我们使用`javac`来编译这个类，使用 Java 8：

![](img/995554eb-8ba6-47c5-9272-0d38a8a4ea60.png)

如您所见，Java8 成功地编译了类并运行了应用程序。编译器确实给了我们一个`DependencyTest.java:6: warning: BASE64Encoder is internal proprietary API and may be removed in a future release`警告。现在让我们看看当我们尝试使用 Java 9 编译这个类时会发生什么：

![](img/81066cdf-2703-412c-b6f6-c9e21202eb3e.png)

在本例中，对于 Java9，编译器给了我们两个警告，而不是一个。第一个警告针对的是[0]；语句，第二个警告针对的是[1]；方法调用。如您所见，这些只是警告而不是错误，因此成功地编译了`DependencyTest.java`类文件。

接下来，让我们运行应用程序：

![](img/b4dc2327-b9f3-46dd-95d1-60d43e2de2be.png)

现在我们可以清楚地看到，Java9 将不允许我们运行应用程序。接下来，让我们使用`jdeps`analyzer 工具运行依赖性测试。我们将使用以下命令行语法-`jdeps DependencyTest.class`：

![](img/0149ecb1-e039-4ade-8d5e-ec3c0d12b93b.png)

如您所见，我们有三个依赖项：`java.io`、`java.lang`和`sun.misc`。在这里，我们被建议用`rt.jar`替换我们的`sun.misc`依赖关系。

作为最后的测试，我们将尝试使用 Java 10 编译`DependencyTest`：

![](img/3c201148-e7cc-4aa4-a14d-ca23ec4813fc.png)

在这里，我们看到我们根本无法编译应用程序。JDK10 和 11 都提供了相同的错误。
...
您可以使用`--add-opens`运行时选项来允许您的代码访问非公共成员。这可以称为**深刻反思**。进行这种深度反思的图书馆能够访问所有成员，包括私人和公共。要授予此类型的代码访问权限，请使用`--add-opens`选项。语法如下：
...
只有在认为绝对必要时才应使用[0]命令行选项。除短期解决方案外，不建议使用此选项。常规使用它的危险在于，对引用的内部 api 的任何更新都可能导致代码无法正常工作。

# --permit 非法访问选项

打破封装的第三个选择是使用`--permit-illegal-access`选项。当然，谨慎的做法是与第三方库创建者核实是否有更新的版本。如果这不是一个选项，则使用`--permit-illegal-access`非法访问要在类路径上实现的操作。由于这里的操作非常非法，每次发生这些操作时，您都会收到警告。
...
在 Java9 中，Java 平台版本的显示方式发生了变化。以下是 Java 8 版本格式的示例：

![](img/b20c7e9a-8ef1-474c-8998-2ab168283906.png)

现在让我们看看 Java 9 是如何报告其版本的：

![](img/f1eacb95-a8b3-4297-81ae-146768f2a64a.png)

如您所见，对于 Java9，版本模式是`$MAJOR.$MINOR.$SECURITY.$PATCH`。这与以前的 Java 版本有明显的不同。只有当您有解析由[1]命令和选项返回的字符串的代码时，这才会影响您的应用程序。

最后，让我们看看 Java 10（18.3）如何报告其版本：

![](img/70f59c64-ace3-41e1-9abd-f0098fda0299.png)

对于 Java10、11，在可预见的将来，版本模式是`$YY.$MM`。这是从 Java10 开始的变化。如果您有任何代码对[1]命令和选项返回的内容进行求值，则可能需要更新代码。

# JDK 和 JRE 的布局

文件在 JDK 和 JRE 中的组织方式在 Java 的新版本中发生了变化。花时间熟悉新的文件系统布局是值得的。下面的截图显示了 JDK 的`/bin`文件夹的文件结构：
...
已删除认可的标准覆盖机制。在 Java 中，`javac`和`java`如果检测到这个机制都会退出。该机制用于应用服务器覆盖一些 JDK 组件。在 Java 中，可以使用可升级的模块来实现相同的结果。

如本章前面所述，*伸缩机构*也已拆除。

以下列出的 API 以前已被折旧，已被删除，在当前 Java 平台中不可访问。删除这些 API 是 Java 平台模块化的结果：
...
*   Windows 32 位客户端虚拟机。jdk9 确实支持 32 位服务器 JVM，但不支持 32 位客户机 VM。这一变化的重点是提高 64 位系统的性能。

# 更新的垃圾回收

垃圾收集一直是 Java 声名鹊起的原因之一。在 Java9 中，**垃圾优先**（**G1**）垃圾收集器现在是 32 位和 64 位服务器上的默认垃圾收集器。在 Java8 中，默认的垃圾收集器是并行垃圾收集器。Oracle 报告说，有三种垃圾收集组合将禁止您的应用程序在 Java9 中启动。这些组合如下：

*   新建+CMS
*   增量 CMS
*   新+旧

我们将在[第 7 章](07.html)*中深入了解 Java 9 垃圾回收，利用默认的 G1 垃圾回收器*。

# 部署应用程序

在部署应用程序时，从 Java8 或更早版本迁移到当前 Java 平台时，有三个问题需要注意。这些问题包括 JRE 版本选择、序列化小程序和 JNLP 更新。

**JNLP** is the acronym for **Java Network Launch Protocol** and is covered in a later section of this chapter.

# 选择 JRE 版本

在 Java9、10 和 11 之前，开发人员可以在启动应用程序时请求 JRE 版本，而不是正在启动的版本。这可以通过命令行选项或正确的 JAR 文件清单配置来实现。由于我们通常部署应用程序的方式，jdk9 中已经删除了这个特性。以下是三种主要方法：
...
JNLP 用于使用 web 服务器上的资源在桌面客户机上启动应用程序。JNLP 客户机包括 javawebstart 和 Java 插件软件，因为它们能够启动远程托管的 applet。该协议有助于启动 RIA。

**RIAs**（简称**富互联网应用程序**），当使用 JNLP 启动时，可以访问各种 JNLP API，在用户许可的情况下，可以访问用户的桌面。
...
对[0]元素进行了两个更改。首先，可选的`type`属性被添加到可以注释的应用程序类型中。默认类型是`Java`，因此如果您的程序是 Java 应用程序，则不需要包含`type`属性
...
如果您指出 JNLP 客户机不支持的应用程序类型，那么您的应用程序启动将失败。有关 JNLP 的更多信息，请参阅官方文档：[https://docs.oracle.com/javase/7/docs/technotes/guides/javaws/developersguide/faq.html](https://docs.oracle.com/javase/7/docs/technotes/guides/javaws/developersguide/faq.html) .

对`<application-desc>`元素的第二个更改是对`param`子元素的添加。这允许我们使用`value`属性提供参数的名称及其值。下面是 JNLP 文件的`<application-desc`元素在包含`param`子元素和`value`属性的情况下的外观示例。
...
本节重点介绍三种工具，它们可以帮助您将应用程序迁移到当前的 Java 平台。

# Java 环境-jEnv

如果您在使用 Linux 或 macOS 的计算机上开发，您可以考虑使用**jEnv**，一种开源 Java 环境管理工具。这是一个命令行工具，所以不要期望 GUI。您可以在以下网址下载该工具：[https://github.com/gcuisinier/jenv](https://github.com/gcuisinier/jenv) .
...
安装**jEnv**后，需要在系统上进行配置，如下图所示，需要修改脚本以反映实际路径：

```
$ jenv add /Library/Java/JavaVirtualMachines/jdk17011.jdk/Contents/Home
```

您需要为系统上的每个 JDK 版本重复[0]命令。通过每个`jenv add`命令，您将收到特定 JDK 版本已添加到 jEnv 的确认，如下所示：
...
您可以通过在命令提示符下使用[0]来检查添加到 jEnv 中的 JDK 版本。这将产生一个输出列表。
...
你可以在这个网站上阅读关于 Maven 目标的更多细节：[https://maven.apache.org/what-is-maven.html](https://maven.apache.org/what-is-maven.html) . 要下载 Maven，请访问以下网站：[https://maven.apache.org/download.cgi](https://maven.apache.org/download.cgi) . 此处提供了 Windows、macOS、Linux 和 Solaris 的安装说明：[https://maven.apache.org/install.html](https://maven.apache.org/install.html) .
...
*   您可以使用软件向导来创建新的 Maven 项目、创建[0]文件，并为普通 Java 项目启用 Maven 支持
...
1.  首先打开当前的 eclipseide。接下来，选择 Preferences | Install/Update | Available Software Sites，如以下屏幕截图所示：

![](img/75648bd7-cdc5-444c-97e3-7b4c981aaf2a.png)

2.  下一个任务是将 M2Eclipse 存储库站点添加到可用软件站点列表中。要完成此操作，请单击 Add 按钮并在 Name 和 Location 文本输入框中输入值。在“名称”中输入一些内容，以帮助您记住此站点上提供了 M2Eclipse。对于位置，请输入 URL:[http://download.eclipse.org/technology/m2e/releases](http://download.eclipse.org/technology/m2e/releases) . 然后，单击“确定”按钮：

![](img/b9a68d06-3541-49b4-a1f6-37fdaa548e8d.png)

3.  现在您应该看到 M2Eclipse 站点列在可用软件站点列表中，如下面的屏幕截图所示。最后一步是单击“确定”按钮：

![](img/85ad2f79-ca47-4f6f-b625-7a1b453e11ac.png)

4.  现在，当您启动一个新项目时，您将看到 Maven 项目作为一个选项：

![](img/20fe004b-c900-4c0b-ae83-9ce76902c000.png)

# 摘要

在本章中，我们探讨了将现有应用程序迁移到当前 Java 平台时可能涉及的问题。我们研究了手动和半自动迁移过程，本章为您提供了一些见解和过程，使您的 Java8 代码能够在新的 Java 平台上工作。具体来说，我们对项目 Jigsaw 进行了快速回顾，研究了模块如何适应 Java 环境，提供了迁移规划的技巧，共享了 Oracle 关于迁移的建议，以及可以在开始时使用的共享工具。
...
*   *Maven 速成班*，在[提供 https://www.packtpub.com/application-development/maven-crash-course-video](https://www.packtpub.com/application-development/maven-crash-course-video) .