...
这个测试证明了这个示例应用程序使用 JDK1.8 可以成功地工作。接下来，让我们使用 JDK10 测试相同的应用程序：
...
Oracle 提供了一个五步流程，帮助开发人员将 Java 应用程序从 Java9 以前的版本迁移到现代版本 9、10 或 11。以下列出了这些步骤，随后的部分将介绍这些步骤：
...
| 8 | 将“支持”设置为 1.8。 |
| 7 | 将支持设置为 1.7。 |
| 6 | 将 support 设置为 1.6，并生成一个警告（不是错误）来指示 JDK6 已折旧。 |
...
如您所见，Java8 成功地编译了类并运行了应用程序。编译器确实给了我们一个警告。现在让我们看看当我们尝试使用 Java9 编译这个类时会发生什么：
