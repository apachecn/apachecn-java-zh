...
Nashorn 是一个轻量级、高性能的 JavaScript 运行时，它允许在 Java 应用中嵌入 JavaScript。它是为 Java8 创建的，并取代了以前基于 MozillaRhino 的 JavaScript 脚本引擎。纳索恩已经有了这个功能。它提供对任何未知类型的对象（如`obj.something`）的高级操作之间的链接，其中它产生以下结果：
