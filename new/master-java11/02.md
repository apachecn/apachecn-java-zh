...
在本章中，我们将回顾 29 个最重要的变化。每个变更都与一个**JDK 增强方案**（**JEP**相关。JEP 索引并存放在[openjdk.java.net/jeps/0](http://openjdk.java.net/jeps/0)。您可以访问此链接以获取有关每个 JEP 的更多信息。
...
本章及后续章节以 Java 11 为特色，Java 平台的**标准版**（**SE**）可从 Oracle 官方下载网站的链接（[下载 http://www.oracle.com/technetwork/java/javase/downloads/index.html](http://www.oracle.com/technetwork/java/javase/downloads/index.html)）。
...
lint 和 doclint 警告的综合列表可以在**JDK 错误系统**（**JBS**中查看，可在[中获得 https://bugs.openjdk.java.net](https://bugs.openjdk.java.net)。
...
*   **DPI unknowledge application**：不尝试检测和缩放显示器特定像素密度的图像的应用程序。
*   **HiDPI 图形**：每英寸高点图形。
...
当 JVM 被要求在堆栈空间不足且没有分配额外空间权限的线程中执行数据计算时，JVM 抛出`StackOverflowError`。这是一个异步异常。JVM 还可以在调用方法时同步抛出`StackOverflowError`异常。

调用方法时，将使用内部进程报告堆栈溢出。虽然当前模式足以报告错误，但调用应用程序没有空间轻松地从错误中恢复。这不仅会给开发者和用户带来麻烦。如果`StackOverflowError`是在关键的计算操作期间抛出的，则数据可能已损坏，从而导致其他问题。
...
有关`INVOKEDYNAMIC`的更多信息，请参见 JEP 276。
...
您可以通过以下链接[了解更多关于 Oracle 基础架构框架的信息 http://www.oracle.com/technetwork/oem/frmwrk-infra-496656.html](http://www.oracle.com/technetwork/oem/frmwrk-infra-496656.html)。
...
*   *JavaFX Essentials*[提供 https://www.packtpub.com/web-development/javafx-essentials](https://www.packtpub.com/web-development/javafx-essentials)。