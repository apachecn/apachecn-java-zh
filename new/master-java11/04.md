...
在最后一章中，我们讨论了 Java 平台在变量处理程序方面的最新变化，以及它们与原子工具包的关系。我们还讨论了贬值警告，以及在特定情况下抑制贬值警告的原因。研究了与项目 Coin 相关的变化，以及导入语句处理、推断局部变量和线程局部握手。我们通过查看堆分配、根证书、动态类文件常量以及 javaee 和 CORBA 模块的删除，进一步探讨了 Java 语言的变化。

在本章中，我们将研究由 projectjigsaw 指定的 Java 模块的结构，深入探讨如何将 projectjigsaw 作为 Java 平台的一部分来实现。我们还将回顾 Java 平台与模块化系统相关的关键内部更改。
...
这个概念可以应用于计算机体系结构和软件系统。我们的系统可以由几个组件组成，而不是一个庞然大物系统。正如您可能想象的那样，这为我们提供了一些特定的好处：

*   我们应该能够扩展 Java 应用程序以在小型设备上运行
*   我们的 Java 应用程序将更小
*   我们的模块化代码可以更有针对性
*   更多地使用面向对象编程模型
*   还有其他封装的机会
...
Java 向模块化系统的转变是必要的，原因有几个。以下是 Java 9 之前的 Java 平台导致在当前 Java 平台中创建模块化系统的主要条件：
...
*   **Java 存档**（**JAR**文件）也太大。这使得支持小型设备成了问题
...
*   所有受 JCP 管理的模块都必须以字符串`java`开头。因此，如果正在开发一个关于空间实用程序的模块，它的名称应该是`java.spatial.util`。
...
*   第四个原则处理标准模块中使用的标准和非标准 API 包。以下流程图说明了本原则契约的实施情况：
...
开发工具位于`\bin`目录下。这些工具包括七个大的分类，每一个都将在后面的章节中详细介绍。

# 部署

这是一组用于帮助部署 Java 应用程序的工具：

*   `appletviewer`：此工具使您能够运行和调试 Java 小程序，而无需使用 web 浏览器。
...
*   `unpack200`：此工具解压`pack200`个文件，生成 JAR 文件。
...
*   `serialver`：返回类`serialVersionUID`值

# 安全

这组安全工具使开发人员能够创建可在开发人员的计算机系统以及远程系统上实施的安全策略：
...
JDK 的源代码是 Java 9 之前的版本，具有以下基本组织架构：
...
接下来，我们有 javaapi 包的名称，后跟文件扩展名。文件扩展名取决于`.java`、`.c`等内容。
...
在上一节中，您了解到 Java 9 之前的源代码组织模式如下：
...
**JAR**文件是**Java 存档**文件。这是一种基于传统 ZIP 格式的压缩文件格式。
...
Java 中有两种类型的运行时映像：JDK 和 JRE。从 Java9 开始，这两种图像类型都被重新构造，以区分用户可以使用和修改的文件和开发人员及其应用程序可以使用但不能修改的内部文件。
...
Java9 运行时映像如下图所示。如图所示，完整的 JDK 映像包含与模块化运行时映像相同的目录以及`demo`、`sample`、`man`，并且包含目录：
...
JEP-220 的最终目标是确保现有的课程不会受到负面影响。这是指不依赖于内部 JDK 或 JRE 运行时映像的应用程序。
...
让我们简要回顾一下这些路径。编译模块路径仅在编译时适用，并且包含模块定义。升级模块路径具有已编译的模块定义。系统模块是内置的，包括 javase 和 JDK 模块。最后一个路径，即应用程序模块路径，包含来自应用程序模块和库模块的已编译模块定义。
...
在 Java9 中引入 Java 链接器工具`jlink`之前，运行时映像创建包括复制整个 JRE。然后，移除未使用的组件。简单地说，`jlink`有助于只使用所需模块创建运行时映像。`jlink` 由 Java 打包程序用来生成嵌入式运行时映像。
...
在下一章中，我们将探讨如何将现有的应用程序迁移到当前的 Java 平台。我们将研究手动和半自动迁移过程。
