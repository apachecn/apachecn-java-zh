...
本章和随后的几章主要介绍 Java11。Java 平台的**标准版**（**SE**）可从 Oracle 官网（[下载 http://www.oracle.com/technetwork/java/javase/downloads/index.html](http://www.oracle.com/technetwork/java/javase/downloads/index.html) ).
...
*   **伪造**：用伪造的发送者传送数据包。
*   **网络包**：用于传输的格式化数据单元。
*   **篡改**：在发送方发送数据包后，在目标接收方接收到数据包之前，对数据包的篡改。
...
DTLS 协议版本 1.0 于 2006 年建立，为数据报协议提供通信安全。以下是其基本特征：
...
本节中的代码来自 DTLS 协议文档，并根据 IETF 的*法律规定重新发布。*

记录层包含我们打算发送到记录中的信息。信息开始于一个`DTLSPlaintext`结构中，然后在握手发生后，记录被加密，并且可以通过通信流发送。记录层格式遵循 1.2 版本中的新字段，并在代码注释中用`// New field`注释，如下所示：
...
*   使用**椭圆曲线 Diffie-Hellman**（**ECDH**）匿名密钥协商协议时使用 192 位以上。192 位的值基于**美国国家标准与技术研究所**（**NIST**）的建议。
*   强烈建议使用带有相关数据（**AEAD**）的**认证加密，这是一种加密形式。AEAD 为加密和解密的数据提供真实性、机密性和完整性保证。**
*   在实现握手重新协商时，始终实现[0]扩展。
*   在使用通信协议的所有 Java 应用程序中建立**前向保密**（**FS**功能。实现 FS 可以确保过去的会话加密密钥不会在长期加密密钥受损时受损。理想情况下，一个**完美的前向保密**（**PFS**），其中每个密钥仅对单个会话有效，将用于要求传输数据最大安全性的 Java 应用程序。

# 创建 PKCS12 密钥库

Java 平台为密钥库提供了更高的安全性。在默认情况下创建 PKCS12 密钥库之前，我们将首先回顾密钥库的概念，查看[0]类，然后查看 Java 平台的最新更新。

# 密钥库底漆

`KeyStore`的概念比较简单。它本质上是一个存储公钥证书和私钥的数据库文件或数据存储库文件。`KeyStore`将存储在`/jre/lib/security/cacerts`文件夹中。正如您将在下一节中看到的，这个数据库是由 Java 的`java.security.KeyStore`类方法管理的。
...
[0]类是加密密钥和证书的存储设施。这个类扩展了`java.lang.Object`，如下所示：

```
public class KeyStore extends Object
```

有三种类型的条目由`KeyStore`管理，每种类型都实现`KeyStore.Entry`接口，这是`KeyStore`类提供的三个接口之一。下表定义了条目实现：
...
这个类从 1.2 版开始就是 Java 平台的一部分。它有一个构造函数、三个接口、六个子类和几个方法。构造函数定义如下：
...
*   `public static interface KeyStore.Entry`：此接口作为`KeyStore`条目类型的标记，不包含任何方法。
...
[0]类还包含六个嵌套类，每个类都将在后面的部分中进行检查。

[0]类定义了密钥库的[1]服务提供者接口[2]（[3]SPI[4]）。
...
此调用提供了一个基于密码的`ProtectionParameter`实现。此类具有以下方法：

*   `public void destroy() throws DestroyFailedException`：清除密码
...
*   `public Certificate[] getCertificateChain()`：将`Certificate`链作为`Certificates`的数组返回
*   `public PrivateKey getPrivateKey()`：返回当前分录的`PrivateKey`
*   `public String toString()`：将`PrivateKeyEntry`返回为`String`
...
此类持有`SecretKey`，有以下方法：

*   `public SecretKey getSecretKey()`：返回分录的`SecretKey`
*   `public String toString()`：将`SecretKeyEntry`返回为`String`
...
此类拥有一个受信任的`Certificate`，并具有以下方法：
...
在 Java9 之前，默认的`KeyStore`类型是**Java 密钥库**（**JKS**）。当前的 Java 平台使用 PKCS 作为默认的`KeyStore`类型，更确切地说，是 PKCS12。
...
另外还有三个与强制安全策略相关的类-`ProtectionDomain`、`PermissionCollection`和`SecureClassLoader`：

*   `ProtectionDomain`类用于封装一组类，以便向域授予权限。
*   [0]类表示权限对象的集合。
*   扩展了`ClassLoader`类的`SecureClassLoader`类提供了额外的功能，用于定义具有系统策略检索权限的类。在 Java 中，这个类使用`ConcurrentHashMap`来提高安全性。

# 权限评估

在权限评估类别下，进行了三项优化：

*   [0]列表以前有用于同步的策略提供程序代码。此代码已被删除，在 Java9、10 或 11 中不可用。
*   `PermissionCollection`个条目现在存储在`ConcurrentHashMap`中。它们以前作为`HashMap`存储在`Permission`类中。
...
Java 平台包括一个改进的`java.security.CodeSource`的`hashCode`方法来优化 DNS 查找。这些可能是处理器密集型的，因此使用代码源 URL 的字符串版本来计算哈希代码。
...
*   `public boolean equals(Object obj)`：如果对象相等，则返回 true。这将重写[1]类中的 equals 方法。
...
当运行安装了安全管理器的应用程序时，Java 最近的性能改进是以`java.lang.SecurityManager`包增强的形式出现的。更具体地说，修改了`checkPackageAccess`方法的包检查算法。

[0]类允许应用程序在特定操作上实现安全策略。此类的`public void checkPackageAccess(String pkg)`方法从`getProperty()`方法接收逗号分隔的受限包列表。如这里所示，`checkPackageAccess`方法可以抛出两个异常中的一个，具体取决于评估：
...
最近对`javax.net.ssl`包进行了增强，支持**传输层安全**（**TLS****ALPN**（简称**应用层协议协商**）扩展。此扩展允许 TLS 连接的应用程序协议协商。
...
[0]包包含与安全套接字包相关的类。这允许我们以 SSL 为例，可靠地检测引入网络字节流的错误。它还提供了加密数据以及提供客户端和服务器身份验证的能力。
...
下面的说明是 TLS 握手的五个基本步骤。针对 Java 9 进行了更新并在此处显示，下图显示了在客户端和服务器之间共享协议名称的位置：

![](img/87ae496e-9220-4439-9c50-7c3d55027d91.png)

TLS 握手：共享协议名称

一旦接收到客户端的应用层协议列表，服务器就可以选择服务器的首选交集值，从外部扫描初始明文`ClientHellos`并选择一个 ALPN 协议。应用程序服务器将执行以下操作之一：
...
如您所见，**散列算法**被输入明文和**Salt**，结果是一个新的散列密码和**Salt**被存储。以下是带有示例输入/输出的相同图形，以演示功能：
...
**在线证书状态协议**（**OCSP**）装订是检查数字证书吊销状态的方法。确定 SSL 证书有效性的 OCSP 装订方法被评估为既安全又快速。通过允许 web 服务器提供其组织证书的有效性信息，而不是从证书的颁发供应商处请求验证信息的较长过程，可以实现确定速度。
...
*   当浏览器没有连接到 OCSP 响应程序时，False**会导致**失败
*   OCSP 响应程序易受拒绝服务攻击
...
*   `jdk.tls.stapling.responderURI` :
    *   默认设置：无
    *   可以为没有**权限信息访问**（**AIA**扩展名）的证书设置默认 URI
    *   除非设置了[0]属性，否则不重写 AIA 扩展
...
客户机和服务器端 Java 实现现在都支持[0]和[1]TLS hello 扩展。
...
您可以在[了解每种 DRBG 机制的具体情况 http://nvlpubs.nist.gov/nistpubs/SpecialPublications/nist.SP.800-90Ar1.pdf](http://nvlpubs.nist.gov/nistpubs/SpecialPublications/NIST.SP.800-90Ar1.pdf) .

以下是三个新的 API：

*   `SecureRandom`：新方法，允许配置`SecureRandom`具有以下可配置属性的对象：
    *   播种
    *   重新播种
    *   随机位生成
*   `SecureRandomSpi`：实现`SecureRandom`方法的新方法。
*   `SecureRandomParameter`：新接口，可以将输入传递给新的`SecureRandom`方法。
...
*   *即时 Java 密码和身份验证安全*，在[提供 https://www.packtpub.com/application-development/instant-java-password-and-authentication-security-instant](https://www.packtpub.com/application-development/instant-java-password-and-authentication-security-instant) .