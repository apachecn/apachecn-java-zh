...
`java.net.ssl`包包含与安全套接字包相关的类。这允许我们以 SSL 为例，可靠地检测引入网络字节流的错误。它还提供了加密数据以及提供客户端和服务器身份验证的能力。
...
您可以在[了解每个 DRBG 机制的细节 http://nvlpubs.nist.gov/nistpubs/SpecialPublications/nist.SP.800-90Ar1.pdf](http://nvlpubs.nist.gov/nistpubs/SpecialPublications/NIST.SP.800-90Ar1.pdf)。
