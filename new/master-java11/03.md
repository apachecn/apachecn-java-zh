...
从 Java10 的发布开始，有一组默认的**证书颁发机构**（**CA**）证书作为 JDK 的一部分。Java10 之前的 JDK 的`cacerts`和`keystore`不包含一组证书。在此 Java 版本之前，开发人员需要为`cacerts`和`keystore`创建和配置一组根证书
...
在 Java11 中，Java 类文件的文件格式被扩展为支持`CONSTANT_Dynamic`，它将创建委托给引导方法。Java 平台中增加了一个新的常量形式`CONSTANT_Dynamic`，包含两个组件：
