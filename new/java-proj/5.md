...
创建`ParallelGamePlayer`是为了用-1、1、4 和 8`IntervalGuesser`线程玩游戏，在每种情况下，都有一个测试运行，队列长度分别为 1、10、100 和 100 万。这是 16 个测试执行。当线程数为负数时，构造函数使用`LinkedBlockingDeque`。还有另一个单独的测量方法来测量非平行玩家。测试是用独特的猜测和秘密（没有颜色使用超过一次）、十种颜色和六列来执行的。
