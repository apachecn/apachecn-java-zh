...
用`@Repeatable`注释注释的声明，表示注释可以在一个地方多次应用。此注解的参数是注解类型，该类型应具有类型为的参数，该参数是此注解的数组。不要试图理解！我来举个例子。我已经有了，事实上我们有`@PoweredDevices`。它有一个参数是一个数组`@ParameteredPoweredDevice`。我们现在把这个`@interface`注释如下：
