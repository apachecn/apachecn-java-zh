...
在编写异常抛出测试时，我们已经在第 3 章中使用了 lambda 表达式，*优化了排序代码 Professional*。在该代码中，我们将比较器设置为一个特殊值，该值在每次调用时抛出`RuntimeException`：
