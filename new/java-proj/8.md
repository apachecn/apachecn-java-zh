...
在上一章中，我们开始开发一个电子商务应用，并创建了基于产品 ID 和几个参数来查找产品的功能。在本章中，我们将扩展此功能，以便我们也可以订购所选的产品。在此过程中，我们将学习新技术，重点关注 Java 中的函数式编程和其他一些语言特性，如运行时的反射和注释处理，以及脚本接口。
...
注释，就像方法或字段一样，可以在类层次结构之间继承。如果一个注释声明被标记为`@Inherited`，那么用这个注释扩展另一个类的类可以继承它。如果子类具有注释，则可以重写注释。因为 Java 中没有多重继承，所以不能继承接口上的注释。即使继承了注释，检索特定元素注释的应用代码也可以区分继承的注释和在实体本身上声明的注释。有两种方法可以获取注释，另外两种方法可以获取在实际元素上声明的、未继承的已声明注释。
...
在本章中，我们开发了我们企业应用的订购系统。随着代码的开发，我们遇到了很多新的东西。您了解了注释以及如何通过反射处理它们。虽然没有很强的相关性，但是您学习了如何使用 Lambda 表达式和流来表示比常规循环更简单的几个编程构造。在本章的最后一部分，我们通过从 Java 调用 JavaScript 函数和从 JavaScript 调用 Java 方法，使用脚本扩展了应用。
