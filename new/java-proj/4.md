...
`Map`接口的实现是 Java 程序中使用最频繁的一种，实现经过微调，使用可以配置。最简单的方法是创建一个调用默认构造器的`HashMap`。如果我们碰巧知道地图上有多少元素，那么我们可以指定它，并将[T2”传递给构造器。在这种情况下，构造器将分配一个数组，数组的大小不小于我们所需的容量，大小是两个幂。
