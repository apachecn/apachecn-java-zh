...
当一个元素要存储在这个映射中时，对 key 对象调用`hashCode()`方法，返回值用来决定底层哈希表的哪个 bucket 应该存储这个新元素。bucket 包含一个简单的二叉树结构。如果桶是空的，在这个结构中存储元素就像元素直接存储在桶中一样简单。另一方面，当两个或多个元素具有相同的`hashCode()`值时，它们也可以存储在同一个存储桶中的映射中，尽管效率有点下降。
