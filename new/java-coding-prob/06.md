...
本章包括 20 个涉及文件 javai/O 的问题。从操作、行走和观察流文件的路径，以及读/写文本和二进制文件的有效方法，我们将介绍 Java 开发人员可能面临的日常问题。

通过本章所学到的技能，您将能够解决大多数涉及 javai/O 文件的常见问题。本章中的广泛主题将提供大量有关 Java 如何处理 I/O 任务的信息

# 问题

为了测试您的 javai/O 编程能力，请看下面的问题。我强烈建议您在使用解决方案和下载示例程序之前，先尝试一下每个问题：

129.  **创建文件路径**：编写创建几种文件路径的示例（如绝对路径、相对路径等）。
130.  **转换文件路径**：写几个转换文件路径的例子（例如，将文件路径转换成字符串、URI、文件等）。
131.  **连接文件路径**：写几个连接（组合）文件路径的例子。定义一个固定路径并向其附加其他不同的路径（或用其他路径替换其中的一部分）。
132.  **构建两个位置之间的路径**：编写几个示例，在两个给定路径（从一条路径到另一条路径）之间构建相对路径。
133.  **比较文件路径**：写几个比较给定文件路径的例子。
134.  **行走路径**：编写一个程序，访问一个目录下的所有文件，包括子目录。此外，编写一个程序，按名称搜索文件、删除目录、移动目录和复制目录。
135.  **监视路径**：编写多个程序，监视某条路径发生的变化（如创建、删除、修改）。
136.  **流式传输文件内容**：编写流式传输给定文件内容的程序。
137.  **搜索文件树中的文件/文件夹**：编写一个程序，在给定的文件树中搜索给定的文件/文件夹。
...
[0]类是文件系统中路径的编程表示。路径字符串包含以下信息：

*   文件名
*   目录列表
*   依赖于操作系统的文件分隔符（例如，Solaris 和 Linux 上的正斜杠`/`和 Microsoft Windows 上的反斜杠【1）】
*   其他允许的字符，例如，`.`（当前目录）和`..`（父目录）符号

[0]类处理不同文件系统（[1]）中的文件，这些文件可以使用不同的存储位置（[2]是底层存储）。

定义[0]的常见解决方案是调用[2]helper 类的[1]方法之一。另一种解决方案依赖于`FileSystems.getDefault().getPath()`方法。
...
另一种方法是从**统一资源标识符**（**URI**）创建`Path`。Java 通过`URI`类包装`URI`，然后我们可以通过`URI.create(String uri)`方法从`String`中获得`URI`。此外，`Paths`类提供了一个`get()`方法，该方法将`URI`对象作为参数并返回相应的`Path`。
...
相对于当前文件存储根目录的路径（例如，[0]）必须以文件分隔符开头。在下面的例子中，如果当前文件存储根为`C`，则绝对路径为`C:\learning\packt\JavaModernChallenge.pdf`：
...
当我们创建一个相对于当前工作文件夹的路径时，路径应该*而不是*以文件分隔符开头。如果当前文件夹名为`books`且在`C`根目录下，则以下代码段返回的绝对路径将为`C:\books\learning\packt\JavaModernChallenge.pdf`：
...
我们理解快捷方式是`.`（当前目录）和`..`（父目录）符号。这种路径可以通过`normalize()`方法进行规范化。此方法消除了冗余，如`.`和`directory/..`：
...
为了创建与当前操作系统 100%兼容的路径，我们可以依赖于`FileSystems.getDefault().getPath()`，或者是`File.separator`（依赖于系统的默认名称分隔符）和`File.listRoots()`（可用的文件系统根目录）的组合。对于相对路径，我们可以依赖以下示例：
...
将文件路径转换为`String`、`URI`、`File`等是一项常见的任务，可以在广泛的应用程序中进行。让我们考虑以下文件路径：

```
Path path = Paths.get("/learning/packt", "JavaModernChallenge.pdf");
```

现在，基于 JDK 7 和 NIO.2 API，我们来看看如何将一个`Path`转换成一个`String`、一个`URI`、一个绝对路径、一个*实*路径和一个文件：

*   将`Path`转换为`String`非常简单，只需（显式或自动）调用`Path.toString()`方法。注意，如果路径是通过`FileSystem.getPath()`方法获得的，那么`toString()`返回的路径字符串可能与用于创建路径的初始`String`不同：
...
*   可以通过`Path.toURI()`方法将`Path`转换为`URI`（浏览器格式）。返回的`URI`包装了一个可以在 web 浏览器地址栏中使用的路径字符串：
...
假设我们要将`URI`/`URL`中的文件名提取为`Path`（这是常见的情况）。在这种情况下，我们可以依赖以下代码片段：
...
*   通过`Path.toRealPath()`方法可以将`Path`转换成*实*`Path`，其结果取决于实现。如果所指向的文件不存在，则此方法将抛出一个[3]。但是，根据经验，调用此方法的结果是没有冗余元素的绝对路径（标准化）。此方法获取一个参数，该参数指示应如何处理*符号链接*。默认情况下，如果文件系统支持*符号链接*，则此方法将尝试解析它们。如果您想忽略*符号链接*，只需将`LinkOption.NOFOLLOW_LINKS`常量传递给方法即可。此外，path name 元素将表示目录和文件的实际名称。

例如，让我们考虑以下[0]和调用此方法的结果（注意，我们故意添加了几个冗余元素并将[1]文件夹大写）：
...
*   可以通过`Path.toFile()`方法将`Path`转换成文件。将一个文件转换成一个`Path`，我们可以依赖`File.toPath()`方法：
...
这可以通过 NIO.2 以及`Path.resolve()`和`Path.resolveSibling()`方法来实现。
...
我们可以使用这个特性来循环一组文件；例如，让我们循环一个`String[]`本书：
...
这次，我们可以通过`resolveSibling()`方法将文件名（`JavaModernChallenge.pdf`替换为另一个名称。此方法根据此路径的父路径解析给定路径，如下例所示：
...
基本上，得到的相对路径（由`Path.relativize()`返回）从一条路径开始，在另一条路径结束。这是一个强大的功能，它允许我们使用相对路径在不同的位置之间导航，相对路径是根据前面的路径解析的。
...
请注意，`JBossTools3.pdf`和`JavaModernChallenge.pdf`是兄弟。这意味着我们可以通过上一级然后下一级从一个导航到另一个。以下示例也揭示了这种导航情况：
...
所以，`path3`和`path4`共享相同的根元素`/learning`。从`path3`到`path4`需要上两层下一层。另外，从`path4`到`path3`导航，需要上一层，下两层。查看以下代码：
...
根据我们如何看待两个文件路径之间的相等性，有几种解决方案。主要地，`Path`平等性可以通过不同的方式为不同的目标进行验证。

假设我们有以下三种路径（考虑在您的计算机上复制[0]：
...
`path1`等于`path2`吗？或者，`path2`等于`path3`吗？好吧，如果我们通过`Path.equals()`进行这些测试，那么可能的结果会显示`path1`等于`path2`，但`path2`不等于`path3`：
...
然而，这可能不是我们想要的那种比较。如果两条路径是相同的文件或文件夹，那么说它们相等就更有意义了。这可以通过`Files.isSameFile()`方法实现。此方法分为两个步骤：
...
请注意，您可能会获得与上一示例不同的值。此外，在您的业务逻辑中，重要的是依赖于它们的含义，而不是依赖于它们的值（例如，说[0]，避免使用[1]）。

# 部分比较

可通过`Path.startsWith()`和`Path.endsWith()`方法进行部分比较。使用这些方法，我们可以测试当前路径是否以给定路径开始/结束：
...
*   `CONTINUE`：遍历过程应继续（访问下一个文件、文件夹、跳过失败等）
*   `SKIP_SIBLINGS`：遍历过程应在不访问当前文件/文件夹的同级的情况下继续
*   `SKIP_SUBTREE`：遍历过程应在不访问当前文件夹中的条目的情况下继续
*   `TERMINATE`：遍历应该野蛮终止
...
*   `FileVisitResult postVisitDirectory​(T dir, IOException exc) throws IOException`：在目录（包括子目录）中的内容被访问后，或在文件夹的迭代过程中，发生 I/O 错误或以编程方式中止访问后自动调用
...
例如，假设我们将电子课程存储在`D:/learning`文件夹的子文件夹中，并希望通过`FileVisitor`API 访问每个子文件夹。如果在子文件夹的迭代过程中出现问题，我们只会抛出报告的异常。

为了塑造这种行为，我们需要重写[0]方法，如下所示：
...
在计算机上搜索某个文件是一项常见的任务。通常，我们依赖于操作系统提供的工具或其他工具，但如果我们希望通过编程方式实现这一点（例如，我们可能希望编写一个具有特殊功能的文件搜索工具），那么`FileVisitor`可以帮助我们以非常简单的方式实现这一点。本申请存根如下：
...
[0]方法不能用于查找文件夹。改用`preVisitDirectory()`或`postVisitDirectory()`方法。

*   `visitFileFailed()`是第二个重要关卡。调用此方法时，我们知道在访问当前文件时出现了问题。我们宁愿忽略任何这样的问题和`CONTINUE`搜索。停止搜索过程毫无意义：
...
`preVisitDirectory()`和`postVisitDirectory()`方法没有任何重要的任务，为了简洁起见，我们可以跳过它们。

为了开始搜索，我们依赖于`Files.walkFileTree()`方法的另一种风格。这一次，我们指定搜索的起点（例如，所有根）、搜索过程中使用的选项（例如，跟随*符号链接*）、要访问的最大目录级别数（例如，`Integer.MAX_VALUE`）和`FileVisitor`（例如，`SearchFileVisitor`）：
...
*   `visitFile()`是从给定文件夹或子文件夹中删除每个文件的最佳位置（如果无法删除某个文件，则我们只需将其传递到下一个文件，但可以随意调整代码以满足您的需要）：
...
*   只有当文件夹为空时，才可以删除它，因此[0]是执行此操作的最佳位置（我们忽略任何潜在的[1]，但可以随意调整代码以满足您的需要（例如，记录无法删除的文件夹的名称或引发异常以停止进程））：
...
在`visitFileFailed()`和`preVisitDirectory()`中，我们只需返回`CONTINUE`。
...
将`SearchFileVisitor`和`DeleteFileVisitor`结合，可以得到一个搜索删除应用。

# 复制文件夹

为了复制文件，我们可以使用`Path copy​(Path source, Path target, CopyOption options) throws IOException`方法。此方法使用指定如何执行复制的参数`options`将文件复制到目标文件。

通过将`copy()`方法与自定义`FileVisitor`相结合，我们可以复制整个文件夹（包括其所有内容）。本海关`FileVisitor`存根代码如下：
...
*   如果文件无法访问，则调用`visitFileFailed()`。现在是检测*循环链接*并报告它们的好时机。通过以下链接（`FOLLOW_LINKS`，我们可以遇到文件树与父文件夹有*循环链接*的情况。这些病例通过`visitFileFailed()`中的`FileSystemLoopException`例外情况报告：
...
将`D:/learning/packt`文件夹复制到`D:/e-courses`：
...
通过将`CopyFileVisitor`和`DeleteFileVisitor`结合起来，我们可以很容易地形成一个移动文件夹的应用程序。在本书附带的代码中，还有一个移动文件夹的完整示例。基于我们迄今为止积累的专业知识，代码应该是非常容易访问的，没有更多的细节。

在记录有关文件的信息（例如，处理异常的情况）时要注意，因为文件（例如，它们的名称、路径和属性）可能包含可被恶意利用的敏感信息。

# JDK 8 号，文件.walk()

从 JDK8 开始，`Files`类用两个`walk()`方法进行了丰富。这些方法返回一个懒洋洋地填充有`Path`的`Stream`。它通过使用给定的最大深度和选项遍历以给定起始文件为根的文件树来实现这一点：
...
此方法是*弱一致的*。它不会在迭代过程中冻结文件树。对文件树的潜在更新可能会反映出来，也可能不会反映出来。

# 135 监视路径

监视路径的变化只是通过 jdk7nio.2（低级的`WatchService`API）可以实现的线程安全目标之一。
...
*   `Watchable`：实现此接口的任何对象都是*可观察对象*，因此可以观察其变化（例如【1）】

*   `StandardWatchEventKinds`：此类定义了标准的*事件类型*（这些是我们可以注册通知的事件类型：
    *   `ENTRY_CREATE`：已创建目录项
    *   `ENTRY_DELETE`：删除目录条目
    *   `ENTRY_MODIFY`：目录条目已修改；被视为修改的内容在某种程度上是特定于平台的，但实际上修改文件的内容应该始终触发此事件类型
    *   `OVERFLOW`：表示事件可能丢失或丢弃的特殊事件

`WatchService`被称为*观察者*，我们说*观察者*观察*可观察的*。在下面的示例中，将通过`FileSystem`类创建`WatchService`，并监视已注册的`Path`。

# 监视文件夹的更改

让我们从一个 stub 方法开始，该方法获取应监视其更改的文件夹的[0]作为参数：
...
当给定文件夹上发生任何`ENTRY_CREATE`、`ENTRY_DELETE`和`ENTRY_MODIFY`事件类型时，`WatchService`将通知我们。为此，我们需要遵循以下几个步骤：
...
2.  注册需要通知的事件类型，通过`Watchable.register()`完成：
...
对于每个*可观察对象*，我们接收一个注册令牌作为`WatchKey`实例（*观察键*）。我们在注册时收到此*监视键*，但每次触发事件时`WatchService`都会返回相关的`WatchKey`。

3.  现在，我们需要等待传入事件。这是在无限循环中完成的（当事件发生时，*观察者*负责将相应的*观察键*排队等待以后检索，并将其状态更改为*信号*）：
...
4.  现在，我们需要检索一个*监视键*——至少有三种方法专门用于检索一个*监视键*：
    *   `poll()`：返回队列中的下一个密钥并将其删除（或者，如果没有密钥，则返回`null`）。
    *   `poll​(long timeout, TimeUnit unit)`：返回队列中的下一个密钥并将其删除；如果没有密钥，则等待指定的超时并重试。如果密钥仍然不可用，则返回`null`。
...
5.  接下来，我们需要检索*监视键*的挂起事件。一个*有信号*状态的*监视键*至少有一个挂起事件，我们可以通过`WatchKey.pollEvents()`方法检索并删除某个*监视键*的所有事件（每个事件用一个`WatchEvent`实例表示）：
...
7.  接下来，我们重置*手表键*。*监视键*的状态可以是*就绪*（创建时的初始状态）、*发信号*或*无效。* 当*发出*信号后，一个*监视键*保持这样，直到我们调用`reset()`方法，尝试将其恢复到*就绪*状态，接受事件的状态。如果*signaled*向*ready*（resume waiting events）转换成功，则`reset()`方法返回`true`；否则返回`false`，表示*watch key*可能是*invalid*。*监视键*如果不再处于活动状态，则可能处于*无效*状态（不活动可能是显式调用*监视键*的`close()`方法、关闭监视者、删除目录等原因造成的）：
...
当一个*表键*处于*无效*状态时，就没有理由停留在无限循环中。只需调用`break`即可跳出循环。

8.  最后，我们关闭*观察者*。这可以通过显式调用`WatchService`的`close()`方法或依赖*使用资源*来实现，如下所示：
...
从这个应用程序开始，本书附带的源代码附带了另外两个应用程序。其中一个是视频捕获系统的模拟，而另一个是打印机托盘观察器的模拟。依靠我们在本节中积累的知识，理解这两个应用程序应该非常简单，无需进一步的细节。

# 136 流式传输文件内容

流式传输文件内容是一个可以通过 JDK 8 使用`Files.lines()`和`BufferedReader.lines()`方法解决的问题。

`Stream<String> Files.lines​(Path path, Charset cs)`将文件中的所有行作为`Stream`读取。当流被消耗时，这种情况会缓慢发生。在终端流操作的执行过程中，不应该修改文件的内容；否则，结果是未定义的。

让我们看一个读取[0]文件内容并将其显示在屏幕上的示例（注意，我们使用资源[2]在[1]中运行代码，因此通过关闭流来关闭文件）：
...
*   找到所有以`.properties`扩展名结尾的文件，并按照*符号链接*：
...
如果我们希望将约束表示为表达式（例如正则表达式），那么我们可以使用`PathMatcher`接口。这个接口附带了一个名为`matches(Path path)`的方法，它可以判断给定的路径是否匹配这个匹配器的模式。

一个`FileSystem`实现通过`FileSystem.getPathMatcher(String syntaxPattern)`支持*glob*和*regex*语法（也可能支持其他语法）。约束采用`syntax:pattern`的形式。

基于`PathMatcher`，我们可以编写能够覆盖广泛约束的 helper 方法。例如，下面的 helper 方法仅获取符合给定约束的文件作为`syntax:pattern`：
...
在 Java 中，高效地读取文件需要选择正确的方法。为了更好地理解下面的示例，我们假设平台的默认字符集是 UTF-8。通过编程方式，可以通过`Charset.defaultCharset()`获取平台的默认字符集。

首先，我们需要从 Java 的角度区分原始二进制数据和文本文件。处理原始二进制数据是两个`abstract`类的工作，即`InputStream`和`OutputStream`。对于原始二进制数据的流文件，我们重点讨论了`FileInputStream`和`FileOutputStream`类，它们一次读/写一个字节（8 位）。对于著名的二进制数据类型，我们也有专门的类（例如，音频文件应该通过`AudioInputStream`而不是`FileInputStream`进行处理）。
...
事情已经回到正轨，但仍然很慢！现在，应用程序可以一次读取多个单字节（取决于字符集），并使用指定的字符集将它们解码为字符。但再多几个字节仍然很慢。

`InputStreamReader`是射线二进制数据流和字符流之间的桥梁。但是 Java 也提供了`FileReader`类。它的目标是消除由字符文件表示的字符流的桥接。

对于文本文件，我们有一个专门的类，称为`FileReader`类（或分别称为`FileWriter`）。这个类一次读取 2 或 4 个字节（取决于使用的字符集）。实际上，在 JDK11 之前，`FileReader`不支持显式字符集。它只是使用了平台的默认字符集。这对我们不利，因为以下代码不会产生预期的输出：
...
但从 JDK 11 开始，`FileReader`类又增加了两个支持显式字符集的构造函数：
...
一次读取 2 或 4 个字节仍然比读取 1 个字节好，但仍然很慢。此外，请注意，前面的解决方案使用一个`int`来存储检索到的`char`，我们需要显式地将其强制转换为`char`，以便显示它。基本上，从输入文件中检索到的`char`被转换成`int`，然后我们将其转换回`char`。
...
同样的原理也用于类，如用于原始二进制流的`BufferedInputStream`、`BufferedOutputStream`和用于字符流的`BufferedReader`、`BufferedWriter`。其主要思想是在处理之前对数据进行缓冲。这一次，`FileReader`将数据返回到`BufferedReader`直到到达行尾（例如，`\n`或`\n\r`）。`BufferedReader`使用 RAM 存储缓冲数据：
...
要将文本文件的内容提取为`Stream<T>`，请查看*流式处理文件内容*部分中的问题。
...
[0]类提供了两个方法，可以读取内存中的整个文本文件。其中之一是`List<String> readAllLines​(Path path, Charset cs)`：
...
虽然这些方法对于相对较小的文件非常方便，但对于较大的文件来说并不是一个好的选择。尝试在内存中获取大文件很容易出现[0]，而且显然会消耗大量内存。或者，对于大文件（例如 200GB），我们可以关注内存映射文件（`MappedByteBuffer`。`MappedByteBuffer` 允许我们创建和修改巨大的文件，并将它们视为非常大的数组。它们看起来像是在记忆中，即使它们不是。一切都发生在本机级别：
...
对于每个专用于读取文本文件的类/方法（例如，`BufferedReader`和`readString()`），Java 提供了用于写入文本文件的对应类/方法（例如，`BufferedWriter`和`writeString()`）。下面是通过`BufferedWriter`写入文本文件的示例：
...
将[0]写入文本文件的一种非常方便的方法是[1]。例如，让我们将列表的内容写入文本文件（列表中的每个元素都写在文件中的一行上）：
...
通过`StandardOpenOption`可以控制文件的打开方式。在前面的示例中，如果文件不存在（[1]），则创建这些文件，并打开这些文件进行写访问（[2]）。还有许多其他选项可用（例如，`APPEND`、`DELETE_ON_CLOSE`等等）。

最后，通过`MappedByteBuffer`写一个文本文件可以如下完成（这对于写大的文本文件很有用）：
...
在上一个问题*高效读写文本文件*中，我们讨论了*缓冲流*（为了清晰的图像，请考虑在本问题之前读取该问题）。对于二进制文件也一样，因此我们可以直接跳到一些示例中。
...
然而，前面的例子不是很有效。从这个输入流中读取`buffer.length`字节到字节数组时，可以通过`BufferedInputStream`实现高效率，如下所示：
...
也可以通过`Files.newInputStream()`方法获得`FileInputStream`。这种方法的优点在于它直接支持`Path`：
...
如果文件太大，无法放入文件大小的缓冲区，则最好通过具有固定大小（例如 512 字节）的较小缓冲区和[0]样式读取文件，如下所示：
...
没有参数的[0]方法将逐字节读取输入流。这是最低效的方法，尤其是在不使用缓冲的情况下。
...
这两个类只是 Java 提供的*数据过滤器*中的两个。有关所有受支持的*数据筛选器*的概述，请查看`FilterInputStream`的子类。此外，`Scanner`类是读取某些类型数据的好选择。有关更多信息，请查看*使用扫描仪*部分中的问题。
...
写入二进制文件的一种有效方法是使用[0]。例如，向文件写入`byte[]`可以如下完成：
...
如果要逐字节写入数据，请使用`write(int b)`方法；如果要写入数据块，请使用`write​(byte[] b, int off, int len)`方法。

将[0]写入文件的一种非常方便的方法是[1]。例如，让我们编写前面缓冲区的内容：
...
最后，如果我们正在写一段数据（不是原始的二进制数据），那么我们可以依赖于`DataOutputStream`。这个类为不同类型的数据提供了`write*Foo*()`个方法。例如，让我们将几个浮点值写入一个文件：
...
注意，以下实现假设字符串*11*在*111*中只出现一次，而不是两次。此外，前三个实现依赖于[第 1 章](01.html)*字符串、数字和数学**在另一个字符串*节中计算字符串的帮助方法：
...
从前面的问题我们已经知道，`BufferedReader`对于读取文本文件是非常有效的。因此，我们也可以用它来读取一个大文件。在读取时，对于通过`BufferedReader.readLine()`获得的每一行，我们需要通过`countStringInString()`计算搜索到的字符串出现的次数：
...
如果内存（RAM）对我们来说不是问题，那么我们可以尝试将整个文件读入内存（通过[0]）并从那里进行处理。将整个文件放在内存中支持并行处理。因此，如果我们的硬件可以通过并行处理突出显示，那么我们可以尝试依赖`parallelStream()`，如下所示：
...
如果`parallelStream()`没有任何好处，我们可以直接切换到`stream()`。这只是一个基准问题。

# 解决方案基于文件.lines()

我们也可以尝试通过`Files.lines()`利用流。这一次，我们将文件作为懒惰的`Stream<String>`获取。如果我们可以利用并行处理（基准测试显示出更好的性能），那么通过调用`parallel()`方法来并行化`Stream<String>`是非常简单的：
...
从 JDK 9 开始，`Scanner`类附带了一个方法，该方法返回分隔符分隔的标记流`Stream<String> tokens()`。如果我们将要搜索的文本作为`Scanner`的分隔符，并对`tokens()`返回的`Stream`的条目进行计数，则得到正确的结果：
...
JDK10 中添加了支持显式字符集的 scanner 构造函数。

# 基于 MappedByteBuffer 的解决方案

我们将在这里讨论的最后一个解决方案是基于 JavaNIO.2、[0]和[1]。此解决方案从给定文件的[3]打开内存映射字节缓冲区（[2]）。我们遍历提取的字节缓冲区并查找与搜索字符串的匹配项（该字符串被转换为一个`byte[]`并逐字节进行搜索）。
...
如今，JSON 和 CSV 文件无处不在。读取（反序列化）JSON/CSV 文件可能是一项日常任务，通常位于业务逻辑之前。编写（序列化）JSON/CSV 文件也是一项常见的任务，通常发生在业务逻辑的末尾。在读写这些文件之间，应用程序将数据用作对象。
...
在`melons_raw.json`中，每行有一个 JSON 条目。每一行都是一段独立于前一行的 JSON，但具有相同的模式。在`melons_array.json`中，我们有一个 JSON 数组，在`melons_map.json`中，我们有一个非常适合 Java`Map`的 JSON。

对于每个文件，我们都有一个`Path`，如下所示：
...
现在，我们来看三个专用库，它们将这些文件的内容作为`Melon`实例读取：
...
javaee8 附带了一个类似 JAXB 的声明性 JSON 绑定，称为 JSON-B（JSR-367）。JSON-B 与 JAXB 和其他 javaee/seapi 保持一致。jakartaee 将 javaee8json（P 和 B）提升到了一个新的层次。其 API 通过`javax.json.bind.Jsonb`和`javax.json.bind.JsonbBuilder`类公开：
...
*   我们把`melons_array.json`读作`Melon`的`List`：
...
*   我们把`melons_map.json`读作`Melon`的`Map`：
...
Gson 是另一个专门用于处理（序列化/反序列化）JSON 数据的 fast 库。在 Maven 项目中，可以作为依赖项添加到`pom.xml`。它的 API 依赖于类名`com.google.gson.Gson`。本书附带的代码提供了一组示例。
...
如果 CSV 数据有 pojo 对应项（例如，我们的 CSV 是序列化一堆`Melon`实例的结果），则可以反序列化，如下例所示：
...
通常，在 Windows 中，默认的临时文件夹是`C:\Temp, %Windows%\Temp`，或者是`Local Settings\Temp`中每个用户的一个临时目录（这个位置通常通过`TEMP`环境变量控制）。在 Linux/Unix 中，全局临时目录是`/tmp`和`/var/tmp`。前一行代码将返回默认位置，具体取决于操作系统。
...
这个问题的解决依赖于*关闭挂钩*机制，可以通过`Runtime.getRuntime().addShutdownHook()`方法来实现。当我们需要在 JVM 关闭之前完成某些任务（例如，清理任务）时，这种机制非常有用。它被实现为一个 Java 线程，当 JVM 在 shutdown 时执行*shutdown hook*时，调用该线程的`run()`方法。如下代码所示：
...
在异常/强制终止的情况下（例如 JVM 崩溃、触发终端操作等），将不执行*关闭挂钩*。当所有线程完成或调用[0]时运行。建议快速运行，因为如果出现问题（例如，操作系统关闭），可以在完成之前强制停止它们。通过编程，*关闭挂钩*只能由`Runtime.halt()`停止。
...
当应用程序管理少量临时文件夹/文件时，建议仅使用此方法（`deleteOnExit()`）。此方法可能会消耗大量内存（它会为每个注册删除的临时资源消耗内存），并且在 JVM 终止之前，此内存可能不会被释放。请注意，因为需要调用此方法才能注册每个临时资源，而删除的顺序与注册的顺序相反（例如，我们必须先注册临时文件夹，然后再注册其内容）。

# 通过 DELETE\u ON\u CLOSE 删除临时文件

删除临时文件的另一个解决方案依赖于`StandardOpenOption.DELETE_ON_CLOSE`（当流关闭时删除文件）。例如，下面的代码通过`createTempFile()`方法创建一个临时文件，并为其打开一个缓冲的 writer 流，其中明确指定了`DELETE_ON_CLOSE`：
...
从[0]中筛选文件是一项非常常见的任务。例如，我们可能只需要特定类型的文件、具有特定名称模式的文件、今天修改的文件等等。

# 过滤通过文件.newDirectoryStream()

无需任何过滤器，我们可以通过`Files.newDirectoryStream(Path dir)`方法轻松循环文件夹的内容（一级深度）。这个方法返回一个`DirectoryStream<Path>`，它是一个对象，我们可以用来迭代目录中的条目：
...
如果我们想用过滤器丰富这段代码，那么我们至少有两种解决方案。一种解决方案依赖于另一种口味的`newDirectoryStream()`方法`newDirectoryStream​(Path dir, String glob)`。除了`Path`之外，该方法还使用*glob*语法接收一个过滤器。例如，我们可以在`D:/learning/books/spring`文件夹中筛选 PNG、JPG 和 BMP 类型的文件：
...
一种解决方法是使用`MappedByteBuffer`。这个解决方案是超级快速和易于实施。我们只需打开两个`FileChannels`（每个文件一个），然后逐字节进行比较，直到找到第一个不匹配或 EOF。如果文件的字节长度不同，则我们假设文件不相同，并立即返回：
...
JDK-13 已制备出非挥发性的释放剂`MappedByteBuffers`。敬请期待！

从 JDK12 开始，`Files`类通过一个新方法进行了丰富，该方法专门用于指出两个文件之间的不匹配。此方法具有以下签名：
...
为了使用这个过滤器，我们需要将它传递给重载的`File.list​(FilenameFilter filter)`或`File.listFiles​(FilenameFilter filter)`方法：
...
取结果为`File[]`时，应该调用`listFiles()`而不是`list()`。

# 通过 FileFilter 过滤

`FileFilter`是另一个可以过滤文件和文件夹的功能接口。例如，让我们只筛选文件夹：
...
循环缓冲区依赖于预先分配的数组（预先分配的容量），但某些实现可能也需要调整大小的功能。元素写入/添加到后面（*尾*），从前面删除/读取（*头*），如下图所示：

![](img/0014d546-a369-4e06-ba75-2a32525b8fd3.png)

对于主操作，即读（get）和写（put），循环缓冲区维护一个指针（读指针和写指针）。两个指针都围绕着缓冲区容量。我们可以找出有多少元素可以读取，有多少空闲的插槽可以随时写入。此操作发生在`O(1)`中。
...
2.  我们把**hello**对应的 5 个字节放在缓冲区中，`readPointer`保持在原来的位置，`writePointer`指向插槽 5。
3.  我们得到与**h**对应的字节，所以`readPointer`移动到插槽 1。
4.  最后，我们尝试将**世界**的字节放入缓冲区。这个字由 5 个字节组成，但在达到缓冲区容量之前，我们只有 4 个空闲插槽。这意味着我们只能写与**世界**对应的字节。
...
1.  前两个步骤与前一个场景中的步骤相同。
2.  我们得到了**地狱**的字节。这将把[0]移动到位置 4。
3.  最后，我们将**世界**的字节放入缓冲区。这一次，字放入缓冲区，`writePointer`移动到槽 2。
...
如果我们检查 JavaNIO.2`ByteBuffer`API，我们会注意到它公开了`get()`和`put()`方法的几种风格。例如，我们应该能够将一个`byte[]`传递给`get()`方法，这个方法应该将一系列元素从缓冲区复制到这个`byte[]`中。从当前的`readPointer`开始从缓冲区读取元素，从指定的`offset`开始写入给定的`byte[]`。

下图显示了`writePointer`大于`readPointer`的情况：

![](img/345337ae-3f8b-47ae-adac-d3206a178c61.png)

在左边，我们正在读 3 个字节。这将[0]从其初始插槽 1 移动到插槽 4。在右边，我们正在读取 4 个（或超过 4 个）字节。由于只有 4 个字节可用，`readPointer`从其初始插槽移动到与`writePointer`相同的插槽（插槽 5）。

现在，我们来分析一个例子，`writePointer`小于`readPointer`：

![](img/e0970f73-bc5c-4415-af45-59c4b8c92334.png)

在左边，我们正在读 3 个字节。这会将[0]从其初始插槽 6 移动到插槽 1。在右边，我们正在读取 4 个（或超过 4 个）字节。这会将[1]从其初始插槽 6 移动到插槽 2（与[2]相同的插槽）。

既然我们已经考虑到了这两个用例，那么我们可以编写一个`get()`方法来将一个字节范围从缓冲区复制到给定的`byte[]`，如下所示（这个方法尝试从缓冲区读取`len`个字节，然后将它们写入给定的`byte[]`，从给定的`offset`开始）：
...
现在，让我们集中精力将给定的[0]放入缓冲区。元素从指定的`offset`开始从给定的`byte[]`读取，从当前的`writePointer`开始写入缓冲区。下图显示了`writePointer`大于`readPointer`的情况：
...
既然我们已经考虑到了这两个用例，那么我们可以编写一个`put()`方法来将给定`byte[]`中的一个字节范围复制到缓冲区中，如下所示（该方法尝试从给定`offset`开始从给定`byte[]`中读取`len`个字节，并尝试从当前`writePointer`开始将它们写入缓冲区）：
...
如前所述，有时需要调整缓冲区的大小。例如，我们可能希望通过简单地调用[0]方法将其大小增加一倍。基本上，这意味着将所有可用字节（元素）复制到一个容量加倍的新缓冲区中：
...
此文件包含一些服装物品及其详细信息，以[0]字符分隔。单一条款如下：

```
article name | color \ no. available items / size
```

在这里，我们有几个分隔符（[0]、[1]、[2]、[3]）和一个非常具体的格式。

现在，让我们来看看几个解决方案，它们可以从这个文件中提取信息并将其标记为一个`List`。我们将在实用程序类`FileTokenizer`中收集这些信息。

获取`List`中物品的一个解决方案依赖于`String.split()`方法。基本上，我们必须逐行读取文件并对每行应用`String.split()`。每行标记化的结果通过`List.addAll()`方法收集到`List`中：
...
另一个解决方案可以依赖 JDK8 的`Pattern.splitAsStream()`方法。此方法从给定的输入序列创建流。为了便于修改，这次我们通过`Collectors.joining(";")`收集结果列表：
...
为了获得更好的性能和正则表达式支持（即高灵活性），建议使用`String.split()`而不是`StringTokenizer`。从同一类别中，也考虑使用扫描器的[2]部分。
...
*   `[flags]`是可选的，由修改输出的标准方法组成。通常，它们用于格式化整数和浮点数。
*   `[width]`是可选的，用于设置输出的字段宽度（写入输出的最小字符数）。
*   `[.precision]`可选，指定浮点值的精度位数（或从`String`中提取的子串长度）。
*   [0]是必需的，它告诉我们参数的格式。最常用的转换字符如下：
    *   `s`：用于格式化字符串
    *   `d`：用于格式化十进制整数
    *   `f`：用于格式化浮点数
    *   `t`：用于格式化日期/时间值
...
通过`Formatter`类可以提供另一种解决方案。此类专用于格式化字符串，并使用与`String.format()`相同的格式化规则。它有一个`format()`方法，我们可以用它重写前面的代码片段：
...
`Scanner`公开了一个 API，用于解析字符串、文件、控制台等中的文本。解析是将给定的输入标记化并根据需要返回它的过程（例如，整数、浮点、双精度等）。默认情况下，`Scanner`使用空格（默认分隔符）解析给定的输入，并通过一组`next*Foo*()`方法（例如，`next()`、`nextLine()`、`nextInt()`、`nextDouble()`等）公开令牌。

对于同一类问题，请考虑[0]标记化文件[1]部分。

例如，假设我们有一个文件（`doubles.txt`），其中包含由空格分隔的双数，如下图所示：

![](img/e89c5754-4095-4295-82e3-9bb178aac4d1.png)

如果我们想把这个文本作为 double 来获取，那么我们可以阅读它并依赖于一段*意大利面条*代码来标记它并将其转换为 double。或者，我们可以依赖于`Scanner`及其`nextDouble()`方法，如下所示：
...
但是，文件可能包含不同类型的混合信息。例如，下图中的文件（[0]）包含由不同分隔符（逗号和分号）分隔的字符串和整数：
...
从 JDK9 开始，`Scanner`公开了一个名为`tokens()`的新方法。此方法返回来自`Scanner`的分隔符分隔标记流。例如，我们可以用它来解析`people.txt`文件并在控制台上打印出来，如下所示：
...
在*大文件搜索*部分中，有一个如何使用此方法在文件中搜索某段文本的示例。
...
在`tokens()`方法方面，JDK9 还附带了一个名为`findAll()`的方法。这是一种非常方便的方法，用于查找所有与某个正则表达式相关的标记（以[2]或[3]形式提供）。此方法返回一个`Stream<MatchResult>`，可以这样使用：
...
前面的代码选择所有表示年龄在 40 到 49 岁之间的标记，即 40、43 和 43。
...
注意，对于数字输入（通过`nextInt()`、`nextFloat()`等读取），我们也需要使用换行符（当我们点击*回车*时会出现这种情况）。基本上，`Scanner`在解析一个数字时不会获取这个字符，因此它将进入下一个标记。如果我们不通过添加`nextLine()`代码行来消耗它，那么从这一点开始，输入将变得不对齐，并导致`InputMismatchException`类型的异常或过早结束。
jdk10 中引入了支持字符集的`Scanner`构造函数。

我们来看看`Scanner`和`BufferedReader`的区别。

# 扫描仪与缓冲阅读器

那么，我们应该使用`Scanner`还是`BufferedReader`？好吧，如果我们需要解析这个文件，那么`Scanner`就是最好的方法；否则`BufferedReader`更合适。对它们进行一个正面比较，可以发现以下几点：

*   由于不执行任何解析操作，`BufferedReader`比`Scanner`快。
*   `BufferedReader`在阅读方面优于`Scanner`，在解析方面优于`Scanner`。
...
*   一个`Scanner`可以使用一个`BufferedReader`，反之则不行。如下代码所示：
