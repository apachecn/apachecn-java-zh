...
本章包括涉及 Java 并发的 13 个问题，涉及 fork/join 框架、[0]、[1]、[2]、[3]、原子变量、任务取消、可中断方法、线程局部、死锁等领域。对于任何开发人员来说，并发性都是必需的主题之一，在工作面试中不能被忽视。这就是为什么这一章和最后一章如此重要。读完本章，您将对并发性有相当的了解，这是每个 Java 开发人员都需要的。
...
***   **Fork/join 框架**：编写一个依赖 Fork/join 框架对列表元素求和的程序。编写一个依赖 fork/join 框架的程序来计算给定位置的 Fibonacci 数（例如 F<sub>12</sub>=144）。另外，编写一个程序来举例说明[0]的用法。*   **Fork/join 框架和`compareAndSetForkJoinTaskTag()`**：编写一个程序，将 Fork/join 框架应用于一组相互依赖的任务，这些任务只能执行一次（例如，*任务 D*依赖于*任务 C*和*任务 B*，而*任务 C*依赖于*任务 B*同样；因此，*任务 B*必须只执行一次，而不是两次）。**

 **216.  **`CompletableFuture`**：写几段代码，通过`CompletableFuture`举例说明异步代码。
217.  **组合多个`CompletableFuture`****对象**：写几段代码举例说明组合多个`CompletableFuture`对象的不同解决方案。
218.  **优化忙等待**：写一个概念证明来举例说明通过`onSpinWait()`优化一个*忙等待*技术。
...
223.  **`ReentrantReadWriteLock`**：编写一个程序，通过`ReentrantReadWriteLock`模拟读写过程的编排。
224.  **`StampedLock`**：编写一个程序，通过`StampedLock`模拟读写过程的编排。
225.  **死锁（餐饮哲学家）**：编写一个程序，揭示和解决著名餐饮哲学家问题中可能出现的死锁（*循环等待*或*致命拥抱*）。**  **# 解决
...
所谓可中断方法，是指可以抛出`InterruptedException`的阻塞方法，例如`Thread.sleep()`、`BlockingQueue.take()`、`BlockingQueue.poll(long timeout, TimeUnit unit)`等。阻塞线程通常处于**阻塞**、**等待**或**定时等待**状态，如果中断，则该方法尝试尽快抛出`InterruptedException`。

因为`InterruptedException`是一个选中的异常，所以我们必须捕获它和/或抛出它。换句话说，如果我们的方法调用抛出[1]的方法，那么我们必须准备好处理这个异常。如果我们可以抛出它（将异常传播给调用方），那么它就不再是我们的工作了。打电话的人必须进一步处理。所以，当我们必须抓住它的时候，让我们把注意力集中在这个案子上。当我们的代码在`Runnable`内运行时，就会出现这种情况，因为`Runnable`不能引发异常。

让我们从一个简单的例子开始。尝试通过`poll(long timeout, TimeUnit unit)`从`BlockingQueue`获取元素可以写为：
...
尝试轮询队列中的元素可能导致`InterruptedException`。有一个 3000 毫秒的窗口可以中断线程。在中断的情况下（例如，`Thread.interrupt()`），我们可能会认为调用`catch`块中的`Thread.currentThread().isInterrupted()`将返回`true`。毕竟，我们身处一个`InterruptedException catch`街区，所以相信这一点是有道理的。实际上会返回`false`，答案在`poll(long timeout, TimeUnit unit)`方法的源代码中，如下所示：
...
更准确地说，答案在第 3 行。如果线程被中断，那么`Thread.interrupted()`将返回`true`，并指向第 5 行（`throw new InterruptedException()`。但是除了测试之外，如果当前线程被中断，`Thread.interrupted()`清除线程的中断状态。请查看以下连续调用中断线程：
...
现在，让我们回到我们的案子。所以，我们知道线程在捕获到`InterruptedException`后被中断，但中断状态被`Thread.interrupted()`清除。这也意味着我们代码的调用者不会意识到中断。

我们有责任做一个好公民，通过调用`interrupt()`方法恢复中断。这样，我们代码的调用者就可以看到发出了中断，并相应地采取行动。正确的代码如下：
...
在每次迭代中，如果当前线程中断状态为`false`，则尝试从`BlockingQueue`中获取一个元素。
...
作为一个调用者（另一个线程），我们启动上面的线程，睡眠 1.5 秒，只是给这个线程时间进入`poll()`方法，然后我们中断它。如下代码所示：
...
下一步，`while`对`Thread.currentThread().isInterrupted()`求值到`false`并退出。
...
根据经验，当我们可以接受中断（而不是恢复中断）时，唯一可以接受的情况是我们可以控制整个调用堆栈（例如，[0]）。

否则，抓捕的`InterruptedException`也应该包含`Thread.currentThread().interrupt()`。
...
在 JDK 8 之前，推荐的方法依赖于一个`public static`变量，如下所示：
...
对于自定义`ForkJoinPool`，依赖于此类的构造函数。jdk9 添加了迄今为止最全面的一个（详细信息见文档）。

[0]对象操作任务。在`ForkJoinPool`中执行的任务的基本类型是`ForkJoinTask<V>`。更准确地说，执行以下任务：

*   `void`任务的`RecursiveAction`
*   `RecursiveTask<V>`对于返回值的任务
*   `CountedCompleter<T>`对于需要记住挂起任务计数的任务

这三种类型的任务都有一个名为`compute()`的`abstract`方法，在这个方法中，任务的逻辑是成形的。
...
*   `fork()`用于安排在池中异步执行此任务，`join()`用于计算完成后返回计算结果

让我们从一个通过`RecursiveTask`解决的问题开始。

# 通过 RecursiveTask 计算和

为了演示框架的分叉行为，我们假设我们有一个数字列表，并且我们要计算这些数字的总和。为此，我们使用`createSubtasks()`方法递归地拆分（fork）这个列表，只要它大于指定的`THRESHOLD`。每个任务都被添加到`List<SumRecursiveTask>`中。最后通过`invokeAll​(Collection<T> tasks)`方法将此列表提交给`ForkJoinPool`。这是使用以下代码完成的：
...
通过`RecursiveAction`实现斐波那契数可以实现如下：
...
`CountedCompleter`是 JDK 8 中添加的`ForkJoinTask`类型。

`CountedCompleter`的任务是记住挂起的任务计数（不能少，不能多）。我们可以通过`setPendingCount()`设置挂起计数，也可以通过`addToPendingCount​(int delta)`显式增加`delta`。通常，我们在分叉之前调用这些方法（例如，如果分叉两次，则根据具体情况调用`addToPendingCount(2)`或`setPendingCount(2)`）。

在`compute()`方法中，我们通过`tryComplete()`或`propagateCompletion()`来减少挂起计数。当调用挂起计数为零的`tryComplete()`方法或调用无条件的`complete()`方法时，调用`onCompletion()`方法。`propagateCompletion()`方法与`tryComplete()`类似，但不调用`onCompletion()`。

`CountedCompleter`可以选择返回计算值。为此，我们必须重写[1]方法以返回一个值。
...
现在，我们已经熟悉了 fork/join 框架，让我们看看另一个问题。这次假设我们有一组相互依赖的`ForkJoinTask`对象。下图可以被视为一个用例：

![](img/9940750d-4790-4d22-afe8-d0e7c5ddd2fa.png)

以下是前面图表的说明：

*   **TaskD**有三个依赖项：**TaskA**、**TaskB**和**TaskC**。
...
`Adder`是一个简单的`Callable`，每个任务只能执行一次（因此，对于**TaskD**、**TaskC**、**TaskB**、**TaskA**执行一次）。在以下代码中启动`Adder`：
...
我们已经知道如何将 fork/join 框架用于具有非循环和/或不可重复（或者我们不关心它们是否重复）完成依赖关系的任务。但是如果我们这样实现，那么每个任务都会多次调用[0]。例如，**TaskA**作为其他三个任务的依赖项出现，因此`Callable`将被调用三次。我们只想要一次。

JDK 8 中添加的一个非常方便的特性`ForkJoinPool`是用一个`short`值进行原子标记：

*   `short getForkJoinTaskTag()`：返回该任务的标签。
*   `short setForkJoinTaskTag​(short newValue)`：自动设置此任务的标记值并返回旧值。
*   `boolean compareAndSetForkJoinTaskTag​(short expect, short update)`：若当前值等于`expect`且变更为`update`，则返回`true`。
...
jdk8 通过用`CompletableFuture`增强`Future`，在异步编程领域迈出了重要的一步。`Future`的主要局限性是：
...
这一次，异步任务必须返回一个结果，因此[0]没有用处。这是`supplyAsync()`的工作。取`Supplier<T>`返回`CompletableFuture<T>`。`T` 是通过`get()`方法从该供应商处获得的结果类型。在代码行中，我们可以如下解决此问题：
...
默认情况下，与前面的示例一样，异步任务在从全局[0]获取的线程中执行。通过简单地记录`Thread.currentThread().getName()`，我们可以看到`ForkJoinPool.commonPool-worker-3`这样的东西。
...
用户问题：*取某客户的订单发票，然后计算合计并签字。*

依赖阻塞[0]对于此类问题不是很有用。我们需要的是一个回调方法，当`CompletableFuture`的结果可用时会自动调用。

所以，我们不想等待结果。当发票准备就绪时（这是`CompletableFuture`的结果），回调方法应该计算总值，然后，另一个回调应该对其签名。这可以通过`thenApply()`方法实现。

当`CompletableFuture`的结果到达时，`thenApply()`方法用于处理和转换结果。它以`Function<T, R>`为论据。让我们在工作中看看：
...
此行为可通过`thenAccept()`方法获得。取`Consumer<T>`返回`CompletableFuture<Void>`。此方法可以对`CompletableFuture`的结果进行处理和转换，但不返回结果。因此，它可以接受一个订单，这是`CompletableFuture`的结果，并按下面的代码片段打印出来：
...
通知客户应在交付订单后完成。这只是*亲爱的客户的短信息，您的订单已经在*类型的今天送达，所以通知任务不需要知道关于订单的任何信息。这类任务可以通过`thenRun()`来完成。此方法取`Runnable`，返回`CompletableFuture<Void>`。让我们在工作中看看：
...
为了进一步的并行化，`thenApply()`、`thenAccept()`和`thenRun()`伴随着`thenApplyAsync()`、`thenAcceptAsync()`和`thenRunAsync()`。其中每一个都可以依赖于全局线程池`ForkJoinPool.commonPool()`或自定义线程池（`Executor`。当`thenApply`/`Accept`/`Run()`与之前执行的`CompletableFuture`任务在同一个线程中执行时（或在主线程中），可以在不同的线程中执行`thenApplyAsync`/`AcceptAsync`/`RunAsync()`（来自`ForkJoinPool.commonPool()`或自定义线程池（【16）】）。

# 通过 exception（）处理异步任务的异常
...
因此，在`supplyAsync()`中，如果发生异常，则不会调用以下回调。除此之外，未来的问题也会得到解决。相同的规则适用于每个回调。如果异常发生在第一个`thenApply()`中，则不调用下面的`thenApply()`和`thenAccept()`。
...
用户问题：*取发票，计算总数，签字。如果出现问题，则抛出*`IllegalStateException`*并停止进程。*

如果我们使用`supplyAsync()`取发票，使用`thenApply()`计算合计，使用另一个`thenApply()`签字，那么我们可能认为正确的实现如下：
...
这一次，任何隐含的[0]中发生的异常都将停止进程。以下是可能的输出：
...
从 JDK12 开始，异常情况可以通过`exceptionallyAsync()`进一步并行化，该`exceptionallyAsync()`可以使用与引起异常的代码相同的线程或给定线程池（`Executor`中的线程）。举个例子：
...
现在，如果没有打印服务，那么我们应该依靠备份打印机。这可以通过 JDK 12[0]实现，如下所示：
...
为了进一步的并行化，我们可以依赖于`exceptionallyComposeAsync()`。

# 通过 handle（）处理异步任务的异常

用户问题：*计算订单总数。如果出现问题，则抛出一个*`IllegalStateException`*。*

有时我们希望执行一个异常代码块，即使没有发生异常。就像`try`-`catch`块的`finally`子句。这可以使用`handle()`回调。不管是否发生异常，都会调用这个方法，就像一个`catch`+`finally`。它使用一个函数来计算返回的值`CompletionStage, BiFunction<? super T,​Throwable,​? extends U>`，并返回`CompletionStage<U>`（`U`是函数的返回类型）。
...
如果我们需要异常完成，那么我们可以通过`completeExceptionally()`继续，如下例所示：
...
通过`cancel()`方法可以取消执行并抛出`CancellationException`：
...
`CompletableFuture`可以使用`complete​(T value)`、`completeAsync​(Supplier<? extends T> supplier)`和`completeAsync​(Supplier<? extends T> supplier, Executor executor)`显式完成。`T` 是`get()`返回的值。这里是一个创建`CompletableFuture`并立即返回的方法。另一个线程负责执行一些税务计算并用相应的结果完成`CompletableFuture`：
...
同时检查`whenComplete()`和`whenCompleteAsync()`的文档。
...
通过组合`CompletableFuture`个实例，我们可以形成复杂的异步解决方案。通过这种方式，多个`CompletableFuture`实例可以结合它们的能力来达到一个共同的目标。
...
现在，我们要获取某个客户的订单，一旦订单可用，我们就要计算这个订单的总数。这意味着我们需要先调用`fetchOrder()`，然后再调用`computeTotal()`。我们可以通过`thenApply()`实现：
...
显然，这不是一个方便的解决方案，因为结果是[0]类型的。为了避免嵌套`CompletableFuture`实例，我们可以依赖`thenCompose()`如下：
...
当我们需要从`CompletableFuture`个实例链中获得一个展平结果时，我们可以使用`thenCompose()`。这样我们就避免了`CompletableFuture`实例的嵌套示例。

使用`thenComposeAsync()`可以获得进一步的并行化。

# 通过合并（）合并

当`thenCompose()`用于链接两个依赖的`CompletableFuture`实例时，`thenCombine()`用于链接两个独立的`CompletableFuture`实例。当两个`CompletableFuture`实例都完成时，我们可以继续。
...
为了交付客户订单，我们需要计算总额（用于发出发票）并包装订购的产品。这两个动作可以并行完成。最后，我们交付了包含订购产品和发票的包裹。通过`thenCombine()`可以实现如下操作：
...
如果我们只需要在两个`CompletableFuture`实例正常完成时（这个实例和另一个实例）做一些事情，那么我们可以依赖`thenAcceptBoth()`。此方法返回一个新的`CompletableFuture`，将这两个结果作为所提供操作的参数执行。这两个结果是此阶段和另一个给定阶段（它们必须正常完成）。下面是一个示例：
...
如果不需要这两个`CompletableFuture`实例的结果，则更倾向于使用`runAfterBoth()`。
...
此时，我们有一个`CompletableFuture`个实例的数组，因此有一个异步计算的数组。此外，我们希望并行运行它们。这可以使用`allOf​(CompletableFuture<?>... cfs)`方法来完成。结果由以下[2]组成：
...
`join()`方法与`get()`类似，但如果基础`CompletableFuture`异常完成，则抛出未检查的异常。

因为我们是在所有相关的`CompletableFuture`完成后调用`join()`，所以没有阻塞点。
...
要找到抽奖的获胜者，我们要并行运行`cfCustomers`，第一个完成的`CompletableFuture`就是获胜者。因为`raffle()`方法阻塞随机数秒，所以将随机选择获胜者。我们对其余的`CompletableFuture`个例子不感兴趣，所以应该在获胜者被选中后立即完成。

这是`anyOf​(CompletableFuture<?>... cfs)`的作业。它返回一个新的`CompletableFuture`，当所涉及的`CompletableFuture`个实例中的任何一个完成时，这个新的`CompletableFuture`就完成了。让我们在工作中看看：
...
注意依赖于`CompletableFuture`返回不同类型结果的场景。因为`anyOf()`返回`CompletableFuture<Object>`，所以很难知道先完成的`CompletableFuture`类型。

# 218 优化忙等待

*忙等待*技术（也称为*忙循环*或*旋转*）由检查条件（通常是标志条件）的循环组成。例如，以下循环等待服务启动：
...
Java9 引入了`Thread.onSpinWait()`方法。这是一个热点，它向 JVM 提示以下代码处于自旋循环中：
...
这里的重点是[0]变量。注意，这个变量被声明为`volatile`（也称为轻量级同步机制）。作为一个`volatile`变量，它不会被线程缓存，对它的操作也不会在内存中重新排序；因此，线程看不到旧值。任何读取[3]字段的线程都将看到最近写入的值。这正是我们所需要的，以便将取消操作传递给对该操作感兴趣的所有正在运行的线程。下图描述了`volatile`和非`volatile`的工作原理：

![](img/d2cedc7f-38c9-46e4-9d8b-701345f4643e.png)

注意，`volatile`变量不适合读-修改-写场景。对于这样的场景，我们将依赖原子变量（例如，`AtomicBoolean`、`AtomicInteger`、`AtomicReference`等等）。
...
Java 线程共享相同的内存，但有时我们需要为每个线程提供专用内存。Java 提供`ThreadLocal`作为一种方法，分别存储和检索每个线程的值。`ThreadLocal`的一个实例可以存储和检索多个线程的值。如果线程`A`在`ThreadLocal`的同一实例中存储了`x`值，线程`B`在`ThreadLocal`的同一实例中存储了`y`值，那么稍后，线程`A`检索`x`值，线程`B`检索`y`值。
...
假设我们有一个使用`StringBuilder`类型的全局变量的单线程应用程序。为了在多线程应用程序中转换应用程序，我们必须处理[1]，它不是线程安全的。基本上，我们有几种方法，比如同步和`StringBuffer`或其他方法。不过，我们也可以使用`ThreadLocal`。这里的主要思想是为每个线程提供一个单独的`StringBuilder`。使用`ThreadLocal`，我们可以做到如下：
...
此线程局部变量的当前线程的*初始值*是通过`initialValue()`方法设置的。在 Java 8 中，可以通过`withInitial()`重写如下：
...
使用`get()`和`set()`完成对`ThreadLocal`的操作。`set()`的每次调用都将给定的值存储在只有当前线程有权访问的内存区域中。稍后，调用`get()`将从该区域检索值。另外，一旦工作完成，建议通过调用`ThreadLocal`实例上的`remove()`或`set(null)`方法来避免内存泄漏。

让我们看一个使用`Runnable`的`ThreadLocal`在工作：
...
因此，每个线程都会修改`CustomerOrder`的某个实例（每个实例都有一个特定的线程）。
...
在前一种情况下，避免使用`ExecutorService`。无法保证（给定的`customerId`的）每个`Runnable`在每次执行时都会由同一个线程处理。这可能会导致奇怪的结果。
...
在一个线程中，读取`count`值并将其递增一，另一个线程读取旧的值，导致错误的结果。在多线程应用程序中，调度程序可以在每个字节码指令之间停止当前线程的执行，并启动一个新线程，该线程在同一个变量上工作。我们可以通过同步，或者更好的是，通过原子变量来解决问题。

原子变量类在`java.util.concurrent.atomic`中可用。它们是包装类，将争用范围限制在单个变量上；它们比 Java 同步轻得多，并且基于**CAS**（缩写**Compare and Swap**：现代 CPU 支持这种技术，它将给定内存位置的内容与给定值进行比较，如果当前值等于预期值，则更新为新值）。主要是原子复合动作，它们以类似于`volatile`的无锁方式影响单个值。最常用的原子变量是标量：
...
注意，我们写的不是`count++`，而是`count.incrementAndGet()`。这只是`AtomicInteger`提供的方法之一。此方法以原子方式递增变量并返回新值。这次的`count`是 1000000。
...
从 jdk9 开始，原子变量类已经被多种方法所丰富，比如`get`/`setPlain()`、`get`/`setOpaque()`、`getAcquire()`以及它们的同伴。要了解这些方法，请看一下 Doug Lea 的*使用 JDK 9 内存顺序模式*，见[http://gee.cs.oswego.edu/dl/html/j9mm.html，](http://gee.cs.oswego.edu/dl/html/j9mm.html)在撰写本文时。

# 加法器和累加器

遵循 Java API 文档，如果多线程应用程序更新频繁，但读取频率较低，建议使用`LongAdder`、`DoubleAdder`、`LongAccumulator`和`DoubleAccumulator`，而不是`Atomic*Foo*`类。对于这样的场景，这些类的目的是优化线程的使用。

这就是说，我们不需要使用`AtomicInteger`来计算从 1 到 1000000 的整数，而是可以使用`LongAdder`如下：
...
[0]接口包含一组锁定操作，可以显式地用于微调锁定过程（它提供了比内在锁定更多的控制）。其中，我们有轮询、无条件、定时和可中断的锁获取。基本上，`Lock`以附加功能公开了`synchronized`关键字的未来。`Lock`界面如下图所示：
...
`Lock`的一个实现是`ReentrantLock`。一个*可重入的*锁的作用如下：当线程第一次进入锁时，保持计数设置为 1。在解锁之前，线程可以重新进入锁，从而使每个条目的保持计数增加一。每个解锁请求将保留计数减一，当保留计数为零时，将打开锁定的资源。

与`synchronized`关键字坐标相同，`ReentrantLock`遵循以下实现习惯用法：
...
对于非公平锁，未指定线程被授予访问权限的顺序。如果锁应该是公平的（优先于等待时间最长的线程），那么使用[0]构造函数。
...
如果持有此锁的线程被中断，则抛出[0]。用`lock()`代替`lockInterruptibly()`不接受中断。
...
使用`ReentrantReadWriteLock`的习惯用法如下：
...
在决定依赖`ReentrantReadWriteLock`之前，请考虑它可能会挨饿（例如，当作家优先时，读者可能会挨饿）。此外，我们无法将读锁升级为写锁（可以从 writer 降级为 reader），并且不支持乐观读。如果其中任何一个问题对您来说很重要，那么请考虑`StampedLock`，我们将在下一个问题中讨论。

# 224 冲压锁

简而言之，`StampedLock`的性能比`ReentrantReadWriteLock`更好，并且支持乐观读取。它不像*可重入的；*因此容易死锁。锁获取主要返回一个戳记（一个[2]值），它在[3]块中用于解锁。每次尝试获取一个锁都会产生一个新的戳，如果没有可用的锁，那么它可能会阻塞，直到可用为止。换句话说，如果当前线程持有锁，并且再次尝试获取锁，则可能导致死锁。

`StampedLock`读写编排过程通过以下几种方法实现：
...
在这个习惯用法中，注意初始值（[0]）是在获得乐观读锁之后分配给[1]变量的。然后利用`validate()`flag 方法验证自给定戳的发射度以来，戳锁没有被独占获取。如果`validate()`返回`false`（相当于在获得乐观锁之后，写锁被线程获取），则通过阻塞`readLock()`获取读锁，并再次赋值（`x`。请记住，如果有任何写锁，读锁可能会阻塞。获取乐观锁允许我们读取值，然后验证这些值是否有任何更改。只有在存在的情况下，我们才能通过阻塞读锁。

下面的代码表示一个[0]用例，它读取和写入一个整数量变量。基本上，我们通过乐观的方式重申了前一个问题的解决方案：
...
在本书附带的代码中，还有一个应用程序用于举例说明[0]方法。此外，您可能对开发使用`tryConvertToReadLock​()`和`tryConvertToOptimisticRead()`的应用程序感兴趣。
...
简单的死锁可以解释为一个持有`L`锁并试图获取`P `锁的`A`线程，同时还有一个持有`P`锁并试图获取`L`锁的`B`线程。这种死锁称为**c****循环等待**。Java 没有死锁检测和解决机制（就像数据库一样），因此死锁对于应用程序来说非常尴尬。死锁可能会完全或部分阻塞应用程序，会导致严重的性能损失、奇怪的行为等等。通常情况下，死锁很难调试，解决死锁的唯一方法是重新启动应用程序，并希望取得最好的结果。
...
主要任务是找到解决这个问题的办法，让哲学家们以这样的方式思考和吃饭，以避免饿死。

在代码中，我们可以将每个哲学家看作一个`Runnable`实例。作为`Runnable`个实例，我们可以在不同的线程中执行它们。每个哲学家都能拿起放在他左右两侧的两个叉子。如果我们将 fork 表示为`String`，那么我们可以使用以下代码：
...
因此，哲学家可以选择`leftFork`和`rightFork`。但既然哲学家们共用这两个叉子，哲学家就必须在这两个叉子上获得独占锁。在`leftFork`和`rightFork`上有一个专用锁相当于他手里有两个叉子。拥有`leftFork`和`rightFork`的独占锁相当于哲学家吃东西。释放两个独占锁相当于哲学家不吃不思。
...
`doIt()`方法通过一个随机的睡眠来模拟所涉及的动作（思考、进食、挑选和放回叉子）。这可以从以下代码中看出：
...
这是僵局！每个哲学家手里都有他的左叉子（上面有独占锁），等待右叉子放在桌子上（锁被释放）。显然，这种期望是不能满足的，因为只有五把叉子，每个哲学家手里都有一把。
...
好吧，就这些！本章讨论了 fork/join 框架、[0]、[1]、[2]、[3]、原子变量、任务取消、可中断方法、线程局部和死锁等问题
