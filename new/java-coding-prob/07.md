...
151.  **通过反射构造函数来实例化**：编写通过反射创建实例的程序。

152.  **获取接收器类型的注解**：编写获取接收者类型注解的程序。
...
通过使用*合成*构造，我们几乎可以理解编译器添加的任何构造。更确切地说，符合 Java 语言规范：*Java 编译器引入的任何构造，如果在源代码中没有对应的构造，则必须标记为合成，除了默认构造函数、类初始化方法以及`Enum`类的`valueOf()`方法和`values`*。
