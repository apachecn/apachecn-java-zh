...
150.  **检查类和超类：**写几个检查类和超类的例子（例如，通过类名、修饰符、实现的接口、构造函数、方法和字段获取`Class`）。
151.  **通过反射构造函数实例化：**编写通过反射创建实例的程序。

152.  **获取接收者类型的注解：**编写获取接收者类型注解的程序。
153.  **获得合成和桥结构：**编写一个程序，通过反射获得*合成*和*桥*结构。
154.  **检查变量个数：**编写一个程序，检查一个方法是否获得变量个数。
155.  **检查默认方法：**编写程序检查方法是否为`default`。
156.  **基于嵌套的反射访问控制：**编写一个程序，通过反射提供对基于嵌套的结构的访问。
157.  **getter 和 setter 的反射：**写几个例子，通过反射调用 getter 和 setter。另外，编写一个程序，通过反射生成 getter 和 setter。
...
通过使用 JavaReflectionAPI，我们可以检查类的详细信息，对象的类名、修饰符、构造函数、方法、字段、实现接口等。
...
类的`public`构造函数可以通过`Class.getConstructors()`类获得。返回结果为`Constructor<?>[]`：
...
获取字段的值可以通过一个名为`Object get(Object obj)`的通用方法和一组`get*Foo*()`方法来完成（有关详细信息，请参阅文档）。`obj`表示`static`或实例字段。例如，假设`ProcedureOutputs`类有一个名为`callableStatement`的`private`字段，其类型为`CallableStatement`。让我们用`Field.get()`方法访问此字段，检查`CallableStatement`是否关闭：
...
一个`Car`实例可以通过这四个构造函数中的一个来创建。`Constructor`类公开了一个方法，该方法接受构造函数的参数类型，并返回反映匹配构造函数的`Constructor`对象。这种方法称为`getConstructor​(Class<?>... parameterTypes)`。
...
此外，`Constructor.newInstance​(Object... initargs)`可以返回`Car`的实例，该实例对应于被调用的构造函数：
...
Java 反射 API 也可以通过其`private`构造函数来实例化类。例如，假设我们有一个名为`Cars`的实用程序类。按照最佳实践，我们将此类定义为`final`，并使用`private`构造函数来禁止实例：
...
取这个构造函数可以通过`Class.getDeclaredConstructor()`完成，如下：
...
在这个实例中调用`newInstance()`会抛出`IllegalAccessException`，因为被调用的构造函数有`private`访问权限。但是，Java 反射允许我们通过 flag 方法`Constructor.setAccessible()`修改访问级别。这一次，实例化按预期工作：
...
为了获取构造函数的每个参数的详细信息，我们可以调用`Constructor.getParameters()`。该方法返回`Parameter`数组（JDK 8 中添加了该类，提供了*解剖*参数的综合方法列表）：
...
通过 jdk8，采用`java.lang.reflect.Executable.getAnnotatedReceiverType()`方法，可以在显式*接收机*参数上进行反射注释。该方法在`Constructor`和`Method`类中也有，因此可以这样使用：
...
通过使用*合成*构造，我们几乎可以理解编译器添加的任何构造。更确切地说，符合 Java 语言规范：*Java 编译器引入的任何构造，如果在源代码中没有对应的构造，则必须标记为合成，除了默认构造函数、类初始化方法以及*的值和`*valueOf()*`*方法`*Enum*`*级*。*

有不同种类的*合成*构造（例如，字段、方法和构造函数），但是让我们看一个*合成*字段的示例。假设我们有以下类：
...
与本例类似，我们可以通过`Method.isSynthetic()`和`Constructor.isSynthetic()`方法检查方法或构造函数是否是*合成的*。
...
在 JDK11 的特性中，我们有几个*热点*（字节码级别的变化）。其中一个*热点*被称为 jep181，或者**基于嵌套的访问控制**（**嵌套**）。基本上，*nest*术语定义了一个新的访问控制上下文，*允许逻辑上属于同一代码实体的类，但是用不同的类文件编译的类，访问彼此的私有成员，而不需要编译器插入可访问性方法。【第 11 页】*
...
我们可以看到，为了从`Engine.addEngine()`方法访问封闭类字段`Car.type`，Java 修改了代码并添加了一个*桥*`package`-`private`方法，称为`access$000()`。主要是综合生成的，可以通过`Method.isSynthetic()`和`Method.isBridge()`方法反射看到。
...
简单提醒一下，getter 和 setter 是用于访问类的字段（例如，`private`字段）的方法（也称为访问器）。
...
让我们从一个通过反射（例如，通过`Class.getDeclaredMethods()`）获取类的所有声明方法的解决方案开始。现在，循环`Method[]`并通过特定于 getter 和 setter 的约束对其进行过滤（例如，从`get`/`set`前缀开始，返回`void`或某个类型，等等）。

另一种解决方案是通过反射（例如，通过`Class.getDeclaredFields()`）获取类的所有声明字段。现在，循环`Field[]`并尝试通过`Class.getDeclaredMethod()`将字段的名称（前缀为`get`/`set`/`is`和第一个大写字母）和字段的类型（对于 setters）传递给它来获得 getter 和 setters。

最后，一个更优雅的解决方案将依赖于`PropertyDescriptor`和`Introspector`api。这些 api 在`java.beans.*`包中提供，专门用于处理 JavaBeans。

这两个类暴露的许多特征依赖于场景背后的反射。

`PropertyDescriptor`类可以通过`getReadMethod()`返回用于读取 JavaBean 属性的方法。此外，它还可以通过`getWriteMethod()`返回用于编写 JavaBean 属性的方法。依靠这两种方法，我们可以获取`Melon`类的 getter 和 setter，如下所示：
...
假设`Melon`有三个字段（`type`、`weight`和`ripe`），只定义`type`的 getter 和`ripe`的 setter：
...
为了生成丢失的 getter 和 setter，我们首先识别它们。下面的解决方案循环给定类的声明字段，并假设`foo`字段没有 getter，如果以下情况适用：
...
此外，解决方案循环给定类的声明字段，并假设`*foo*`字段没有 setter，如果以下情况适用：
...
然后，获取一个`Melon`类的实例。我们想要调用一个实例方法；因此，我们需要一个实例。依靠`Melon`的空构造函数和 Java 反射 API，我们可以做到：
...
对于 start，我们可以得到一个`Module`实例，如下两个例子所示：
...
*动态代理*可用于支持不同功能的实现，这些功能属于 C**类**（**CCC**类）。CCC 是那些表示核心功能的辅助功能的关注点，例如数据库连接管理、事务管理（例如 Spring`@Transactional`）、安全性和日志记录。
...
因为我们要计算`List`的方法的调用次数，所以我们应该存储所有的方法签名以及每个方法的调用次数。这可以通过在`CountingInvocationHandler`的构造函数中初始化`Map`来实现（这是我们的`InvocationHandler`实现，`invocationHandler`是它的一个实例）：
