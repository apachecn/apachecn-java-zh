...
本章包括涉及 Java 反射 API 的 17 个问题。从经典主题，如检查和实例化 Java 工件（例如，模块、包、类、接口、超类、构造函数、方法、注释和数组），到*合成*和*桥接*构造或基于嵌套的访问控制（JDK 11），本章详细介绍了 Java 反射 API。在本章结束时，javareflectionapi 将不会有任何秘密未被发现，您将准备好向您的同事展示 Reflection 可以做什么。
...
150.  **检查类和超类：**编写几个检查类和超类的示例（例如，通过类名、修饰符、实现的接口、构造函数、方法和字段获取`Class`）。
151.  **通过反射构造函数实例化：**编写通过反射创建实例的程序。

152.  **获取接收者类型的注解：**编写一个获取接收者类型注解的程序。
153.  **获取合成和桥结构：**编写一个程序，通过反射获取*合成*和*桥结构*。
154.  **检查变量个数：**编写一个程序，检查一个方法是否获得变量个数。
155.  **检查默认方法：**编写程序检查方法是否为`default`。
156.  **通过反射进行基于嵌套的访问控制：**编写一个程序，通过反射提供对基于嵌套的结构的访问。
157.  **getter 和 setter 的反射：**编写几个通过反射调用 getter 和 setter 的示例。另外，编写一个程序，通过反射生成 getter 和 setter。
158.  **反射注解：**写几个通过反射获取不同种类注解的例子。
159.  **调用实例方法：**编写一个程序，通过反射调用实例方法。
160.  **获取`static`个方法：**编写一个程序，对给定类的`static`个方法进行分组，并通过反射调用其中一个方法。
...
165.  **动态代理：**编写依赖*动态代理*统计给定接口方法调用次数的程序。
...
当我们需要获取特定包的信息时，`java.lang.Package`类是我们的主要关注点。使用这个类，我们可以找到包的名称、实现这个包的供应商、它的标题、包的版本等等。

此类通常用于查找包含特定类的包的名称。例如，`Integer`类的包名可以很容易地得到，如下所示：
...
基于`getPackages()`方法，我们可以列出调用者的类加载器定义的所有包，以及以给定前缀开头的祖先包，如下所示：
...
类被包装在可以在 jar 中存档的包中，尽管它们不必这样。为了涵盖这两种情况，我们需要发现给定的包是否存在于 JAR 中。我们可以通过`ClassLoader.getSystemClassLoader().getResource(package_path)`加载资源，并检查返回的资源 URL。如果包不在 JAR 中，那么资源将是以`file:`方案开始的 URL，如下面的示例（我们使用的是`modern.challenge`）：

```
file:/D:/Java%20Modern%20Challenge/Code/Chapter%207/Inspect%20packages/build/classes/modern/challenge
```

但是如果包在 JAR 中（例如，[0]），那么 URL 将以[1]方案开始，如下例所示：

```
jar:file:/D:/.../commons-lang3-3.9.jar!/org/apache/commons/lang3/builder
```

如果我们考虑到来自 JAR 的包的资源以[0]前缀开头，那么我们可以编写一个方法来区分它们，如下所示：
...
因此，如果给定的包在 JAR 中，那么我们调用另一个 helper 方法`fetchClassesFromJar()`；否则，我们调用这个 helper 方法`fetchClassesFromDirectory()`。顾名思义，这些助手知道如何从 JAR 或目录中提取给定包的类。

主要来说，这两种方法只是一些用于识别扩展名为[0]的文件的代码片段。每个类都通过`Class.forName()`，以确保返回的是`Class`，而不是`String`。这两种方法在本书附带的代码中都可用。

如何列出不在系统类加载器中的包中的类，例如，外部 JAR 中的包？实现这一点的便捷方法依赖于`URLClassLoader`。此类用于从引用 JAR 文件和目录的 url 搜索路径加载类和资源。我们将只处理 jar，但对目录也这样做非常简单。

因此，根据给定的路径，我们需要获取所有 jar 并将它们返回为`URL[]`（需要这个数组来定义`URLClassLoader`。例如，我们可以依赖`Files.find()`方法遍历给定的路径并提取所有 jar，如下所示：
...
注意，我们正在扫描所有子目录，从给定的路径开始。当然，这是一个设计决策，很容易参数化搜索深度。现在，让我们从[0]文件夹中获取 jar（不需要为此安装 Tomcat，只需使用 jar 的任何其他本地目录并进行适当的修改）：
...
这将为给定的 URL 构造一个新的`URLClassLoader`对象，并使用当前类加载器进行委托（第二个参数也可以是`null`）。我们的`URL[]`只指向 JAR，但根据经验，假设任何`jar:`方案 URL 引用 JAR 文件，并且假设任何以`/`结尾的`file:`方案 URL 引用目录。

存在于`tomcat8/lib`文件夹中的一个 jar 称为`tomcat-jdbc.jar`。在这个罐子里，有一个叫做`org.apache.tomcat.jdbc.pool`的包。让我们列出这个包的类：
...
通过使用 javareflectionapi，我们可以检查类的细节、对象的类名、修饰符、构造函数、方法、字段、实现的接口等等。
...
通过拥有`Pair`的实例（对象），我们可以通过调用`getClass()`方法找到它的类名，也可以调用`Class.getName()`、`getSimpleName()`、`getCanonicalName()`，如下例所示：
...
为了得到类的修饰符（`public`、`protected`、`private`、`final`、`static`、`abstract`、`interface`，我们可以调用`Class.getModifiers()`方法。此方法返回一个`int`值，表示每个修饰符为标志位。为了解码结果，我们依赖于`Modifier`类，如下所示：
...
通过`Class.getConstructors()`类可以获得类的`public`构造函数。返回结果为`Constructor<?>[]`：
...
要获取所有声明的构造函数（例如，`private`和`protected`构造函数），请调用`getDeclaredConstructors()`。搜索某个构造函数时，调用`getConstructor​(Class<?>... parameterTypes)`或`getDeclaredConstructor​(Class<?>... parameterTypes)`。
...
为了获取字段的实际名称，我们可以很容易地提供一个 helper 方法：
...
可以通过一个名为`Object get(Object obj)`的通用方法和一组`get*Foo*()`方法获取字段的值（有关详细信息，请参阅文档）。`obj`表示`static`或实例字段。例如，假设`ProcedureOutputs`类有一个名为`callableStatement`的`private`字段，其类型为`CallableStatement`。让我们用`Field.get()`方法访问此字段，检查`CallableStatement`是否关闭：
...
通过`Class.getMethods()`方法可以访问类的`public`方法。此方法返回一个数组`Method`：
...
为了获取方法的实际名称，我们可以快速提供一个 helper 方法：
...
取所有声明的方法（如`private`、`protected`），调用`getDeclaredMethods()`。要搜索某个方法，请调用`getMethod​(String name, Class<?>... parameterTypes)`或`getDeclaredMethod​(String name, Class<?>... parameterTypes)`。

# 获取 Pair 类模块

如果我们使用 jdk9 模块化，那么我们的类将生活在模块中。`Pair`类不在模块中，但是我们可以通过 JDK 9 的`Class.getModule()`方法很容易得到类的模块（如果类不在模块中，那么这个方法返回`null`）：
...
*   对于`Pair`，返回与`getName()`和`getCanonicalName()`相同的东西：
...
*   对于内部类（如`Entry`代表`Pair`，返回与`getName()`相同的内容：
...
*   对于数组，返回与`getCanonicalName()`相同的内容：
...
可以通过这四个构造函数中的一个来创建一个`Car`实例。`Constructor`类公开了一个方法，该方法接受构造函数的参数类型，并返回一个反映匹配构造函数的`Constructor`对象。此方法称为`getConstructor​(Class<?>... parameterTypes)`。
...
此外，`Constructor.newInstance​(Object... initargs)`还可以返回`Car`的实例，该实例对应于调用的构造函数：
...
现在，我们来看看如何通过反射实例化一个`private`构造函数。

# 通过私有构造函数实例化类

Java 反射 API 也可以通过类的`private`构造函数来实例化类。例如，假设我们有一个名为`Cars`的实用程序类。按照最佳实践，我们将此类定义为`final`，并使用`private`构造函数来禁止实例：
...
在此实例中调用`newInstance()`将引发`IllegalAccessException`，因为被调用的构造函数具有`private`访问权限。但是，Java 反射允许我们通过 flag 方法`Constructor.setAccessible()`修改访问级别。这一次，实例化按预期工作：
...
为了阻止这种方法，建议抛出一个来自`private`构造函数的错误，如下所示：
...
这一次，实例化尝试将失败，返回`AssertionError`。

# 从 JAR 实例化类

假设在[0]文件夹中有 Guava JAR，我们想创建一个[1]的实例并从一个文件中读取一个字节。

首先，我们为番石榴罐定义一个`URL[]`数组，如下所示：
...
接下来，我们将加载目标类（`CountingInputStream`是一个统计从`InputStream`读取的字节数的类）：
...
一旦目标类被加载，我们就可以获取它的构造函数（`CountingInputStream`有一个单独的构造函数包装给定的`InputStream`：
...
此外，我们还可以通过这个构造函数创建一个`CountingInputStream`的实例：
...
作为奖励，让我们看看在使用反射和构造函数时通常需要的几个代码片段。
...
为了获取构造函数的每个参数的详细信息，我们可以调用`Constructor.getParameters()`。这个方法返回一个数组`Parameter`（这个类是在 JDK 8 中添加的，它提供了一个全面的方法列表，用于*剖析*一个参数）：
...
从 JDK8 开始，我们可以使用显式的*接收器*参数。这主要意味着我们可以声明一个实例方法，该方法使用一个包含类型为[0]Java 关键字的参数。

通过显式的*receiver*参数，我们可以将类型注释附加到`this`。例如，假设我们有以下注释：
...
利用反射得到显式*接收机*参数的注释可以通过 jdk8 的`java.lang.reflect.Executable.getAnnotatedReceiverType()`方法完成。此方法在`Constructor`和`Method`类中也可用，因此我们可以这样使用它：
...
通过使用*合成*构造，我们几乎可以理解编译器添加的任何构造。更确切地说，符合 Java 语言规范：*Java 编译器引入的任何构造，如果在源代码中没有对应的构造，则必须标记为合成，除了默认的构造函数、类初始化方法以及*的值和`*valueOf()*`*方法`*Enum*` *班*。*

有不同类型的*合成*构造（例如，字段、方法和构造函数），但让我们看一个*合成*字段的示例。假设我们有以下类：
...
注意，我们有一个名为`Slice`的内部类。在编译代码时，编译器将通过添加一个用来引用顶级类的*合成*字段来更改此类。这个*合成的*字段提供了从嵌套类访问封闭类成员的便利。

为了检查这个*合成*字段的存在性，让我们获取所有声明的字段并进行计数：
...
即使我们没有显式声明任何字段，也要注意报告了一个字段。我们来看看它是不是*合成的*，看看它的名字：
...
与本例类似，我们可以通过`Method.isSynthetic()`和`Constructor.isSynthetic()`方法来检查一个方法还是一个构造函数是*合成的*。

现在，我们来谈谈*桥接*的方法。这些方法也是*合成的*，它们的目标是处理泛型的*类型擦除*。
...
在这里，我们实现了`Comparator`接口并重写了`compare()`方法。此外，我们还明确规定了`compare()`方法需要两个`Melon`实例。编译器将继续执行*类型擦除*，并创建一个包含两个对象的新方法，如下所示：
...
这种方法被称为*合成桥*方法。我们看不到，但是 Java 反射 API 可以：
...
例如，`plantation()`方法的参数个数可变，例如，`Seed... seeds`：
...
Java8 用`default`方法丰富了接口的概念。这些方法编写在接口内部，并有一个默认实现。例如，`Slicer`接口有一个名为`slice()`的默认方法：
...
现在，`Slicer`的任何实现都必须实现`type()`方法，并且可以选择性地重写`slice()`方法或依赖默认实现。
...
在 JDK11 的特性中，我们有几个热点（字节码级别的变化）。其中一个*热点*被称为 jep181，或者**基于嵌套的访问控制**（**嵌套**）。基本上，*nest*这个术语定义了一个新的访问控制上下文，*允许逻辑上属于同一代码实体的类，但是用不同的类文件编译的类，访问彼此的私有成员，而不需要编译器插入可访问性方法。*

因此，换句话说，*嵌套*允许将嵌套类编译到属于同一封闭类的不同类文件中。然后允许它们访问彼此的私有类，而无需使用*合成*/*桥接*方法。
...
让我们在 JDK10 中为`Car.class`运行`javap`（Java 类文件反汇编工具，允许我们分析字节码）。以下屏幕截图突出显示了此代码的重要部分：

![](img/e2d1c933-70ae-43e5-87b4-7e44b9ab0893.png)

我们可以看到，为了从`Engine.addEngine()`方法访问封闭类字段`Car.type`，Java 修改了代码并添加了一个*桥*`package`-`private`方法，称为`access$000()`。主要是综合生成的，通过`Method.isSynthetic()`和`Method.isBridge()`方法反射即可看到。

即使我们看到（或感觉到）`Car`（外部）和`Engine`（嵌套）类在同一个类中，它们也被编译到不同的文件（`Car.class`和`Car$Engine.class`）。与此一致，我们的期望意味着外部类和嵌套类可以访问彼此的`private`成员。

但是在不同的文件中，这是不可能的。为了维持我们的期望，Java 增加了*合成**桥*`package`—`private`方法`access$000()`。

但是，Java11 引入了*嵌套*访问控制上下文，它支持外部类和嵌套类中的`private`访问。这一次，outer 和 nested 类被链接到两个属性上，它们形成了一个*嵌套*（我们说它们是*嵌套伙伴*）。嵌套类主要链接到`NestMembers`属性，而外部类链接到`NestHost`属性。不会产生额外的*合成*方法。
...
如果没有基于嵌套的访问控制，反射功能也会受到限制。例如，在 JDK 11 之前，以下代码片段将抛出[0]：
...
从 JDK 11 开始，不需要调用`setAccessible()`。

此外，JDK11 还提供了三种方法，它们通过支持*嵌套*，丰富了 Java 反射 API。这些方法是`Class.getNestHost()`、`Class.getNestMembers()`和`Class.isNestmateOf()`。

让我们考虑下面的`Melon`类，其中包含多个嵌套类（`Slice`、`Peeler`和`Juicer`）：
...
这里应该强调两点。首先，注意`Melon`的`NestHost`是`Melon`本身。第二，注意`Peeler`的`NestHost`是`Melon`，而不是`Slice`。既然`Peeler`是`Slice`的一个内部类，我们可以认为它的`NestHost`是`Slice`，但这个假设是不成立的。
...
简单提醒一下，getter 和 setter 是用于访问类的字段（例如，[0]字段）的方法（也称为访问器）。

首先，让我们看看如何获取现有的 getter 和 setter。稍后，我们将尝试通过反射生成缺少的 getter 和 setter。

# 获取 getter 和 setter

主要有几种通过反射获得类的 getter 和 setter 的解决方案。假设我们要获取以下`Melon`类的 getter 和 setter：
...
让我们从一个通过反射（例如，通过[0]）获取类的所有声明方法的解决方案开始。现在，循环`Method[]`并通过特定于 getter 和 setter 的约束对其进行过滤（例如，从`get`/`set`前缀开始，返回`void`或某个类型，等等）。

另一种解决方案是通过反射（例如，通过[0]）获取类的所有声明字段。现在，循环`Field[]`并尝试通过`Class.getDeclaredMethod()`获得 getter 和 setters，方法是将字段的名称（前缀为`get`/`set`/`is`和第一个大写字母）和字段的类型（如果是 setters）传递给 getter 和 setters。

最后，一个更优雅的解决方案将依赖于`PropertyDescriptor`和`Introspector`API。这些 API 在`java.beans.*`包中提供，专门用于处理 JavaBeans。

这两个类暴露的许多特征依赖于场景背后的反射。

`PropertyDescriptor`类可以通过`getReadMethod()`返回用于读取 JavaBean 属性的方法。此外，它还可以通过`getWriteMethod()`返回用于编写 JavaBean 属性的方法。依靠这两个方法，我们可以获取`Melon`类的 getter 和 setter，如下所示：
...
现在，假设我们有下面的`Melon`实例：

```
Melon melon = new Melon("Gac", 1000);
```

在这里，我们称之为`getType()`getter：
...
假设`Melon`有三个字段（`type`、`weight`、`ripe`），只定义`type`的 getter 和`ripe`的 setter：
...
为了生成丢失的 getter 和 setter，我们首先识别它们。以下解决方案循环给定类的声明字段，并假设[0]字段没有 getter（如果以下情况适用）：

*   没有`get`/`isFoo()`方法
*   返回类型与字段类型不同
*   参数的数目不是 0

对于每个缺少的 getter，此解决方案在映射中添加一个包含字段名和类型的条目：
...
此外，解决方案循环给定类的声明字段，并假设[0]字段没有 setter，如果以下情况适用：
...
对于每个缺少的 setter，此解决方案在映射中添加一个包含字段名和类型的条目：
...
到目前为止，我们知道哪些领域没有 getter 和 setter。它们的名称和类型存储在地图中。让我们循环映射并生成 getter：
...
Java 注释从 javareflectionapi 得到了很多关注。让我们看看几种用于检查几种注释（例如，包、类和方法）的解决方案。

主要地，所有主要的反射 API 类（例如，[0]、[1]、[2]、[3]和[4]）表示支持注释的工件，它们揭示了一组处理注释的常用方法。常用方法包括：

*   `getAnnotations()`：返回特定工件的所有注解
*   `getDeclaredAnnotations()`：返回直接声明给某个工件的所有注释
*   `getAnnotation()`：按类型返回批注
*   `getDeclaredAnnotation()`：按类型返回直接声明给某个工件的注解（JDK1.8）
...
让我们把`methodAnnotations[0]`投给`Ripe`：
...
为了检查抛出异常的注解，需要调用`getAnnotatedExceptionTypes()`方法：
...
有了方法，我们可以通过调用`getParameterAnnotations()`来检查其参数的注解：
...
可以通过`getParameterTypes()`获取每个参数类型及其注释（在本例中，我们有一个`int`参数及其两个注释）。由于此方法也维护了声明顺序，因此我们可以提取一些信息，如下所示：
...
或者，直接使用`getDeclaredAnnotation()`方法获取正确的类型：
...
然后，获取`Melon`类的一个实例。我们想要调用一个实例方法；因此，我们需要一个实例。依靠`Melon`的空构造函数和 Java 反射 API，我们可以做到：
...
最后，重点介绍了`Method.invoke()`方法。主要是给这个方法传递调用`cultivate()`方法的实例和一些参数值：
...
另外，如果通过`System.out.println()`打印调用返回，则得到如下结果：
...
此类有两个`static`方法-`cultivate()`和`peel()`。让我们在`List<Method>`中获取这两种方法。
...
例如，我们调用`peel()`方法（注意传递的是`null`而不是`Melon`的实例，因为`static`方法不需要实例）：
...
假设我们有以下`Melon`类（列出的只是与此问题相关的部分）：
...
[0]类包含几个与不同工件相关联的泛型类型。超类、接口、类、方法、字段的泛型类型主要是`ParameterizedType`实例。对于每个`ParameterizedType`，我们需要通过`ParameterizedType.getActualTypeArguments()`获取实际的参数类型。此方法返回的`Type[]`可以迭代提取每个参数的信息，如下所示：
...
例如，让我们获取`slice()`和`asMap()`方法的泛型返回类型。可通过`Method.getGenericReturnType()`方法实现，如下所示：
...
方法的泛型参数可以通过`Method.getGenericParameterTypes()`获取，如下所示：
...
获取已实现接口的泛型可以通过调用当前类的`getGenericInterfaces()`方法来完成：

```
Type[] interfacesTypes = clazz.getGenericInterfaces();
```

此外，我们为每个`Type`调用`printGenerics()`。以下是输出（只有一个接口，`Comparable<Integer>`）：
...
异常的泛型类型在`TypeVariable`或`ParameterizedType`的实例中具体化。这一次，基于`TypeVariable`的泛型信息提取和打印的助手方法可以写为：
...
有了这个助手，我们就可以通过`getGenericExceptionTypes()`将方法抛出的异常传递给它。如果异常类型是类型变量（[1]）或参数化类型（[2]），则会创建它。否则，将解决：
...
`publicFields`列表只包含`public`个字段，`privateFields`列表只包含`private`个字段。如果我们通过`System.out.println()`快速打印这两个列表，那么输出如下：
...
Java 反射 API 附带了一个专用于处理数组的类。此类名为`java.lang.reflect.Array`。
...
从数组中取值可以通过`get()`和`getFoo()`方法完成（这些方法将数组和索引作为参数，并从指定的索引中返回值）：
...
对于 start，我们可以得到一个`Module`实例，如下两个例子：
...
有一个`Module`实例，我们可以调用几个方法来获取不同的信息。例如，我们可以确定某个模块是否已命名，或者是否已导出或打开某个包：
...
但我们可以通过反射来改变这一点。我们可以通过`Module.addExports()`方式进行导出（同一类别中我们有`addOpens()`、`addReads()`、`addUses()`：
...
*动态代理*可用于支持不同功能的实现，这些功能是 C**ross Cutting Concerns**（**CCC**类别的一部分。CCC 是那些表示核心功能的辅助功能的关注点，例如数据库连接管理、事务管理（例如 Spring`@Transactional`）、安全性和日志记录。

更确切地说，Java Reflection 附带了一个名为`java.lang.reflect.Proxy`的类，其主要目的是为在运行时创建接口的动态实现提供支持。`Proxy`反映了具体接口在运行时的实现。

我们可以将[0]看作一个将调用传递给正确方法的[2]前包装器[3]。可选地，`Proxy`可以在委托调用之前干预该过程。

动态代理依赖于单个类（[0]）和单个方法（[1]），如下图所示：
...
2.  动态代理将调用分派到`InvocationHandler`实现的实例（每个代理实例都有一个关联的调用处理程序）

3.  已调度的调用将作为包含代理对象、要调用的方法（作为实例）和此方法的参数数组的三元组命中[0]方法

4.  [0]将运行其他可选功能（例如，CCC）并调用相应的方法

5.  `InvocationHandler`将调用结果作为对象返回

如果我们尝试恢复此流，那么可以说动态代理通过单个类（[0]）和单个方法（[1]）支持对任意类的多个方法的调用。
...
这段代码返回了`List`接口的动态实现。此外，通过该代理的所有调用都将被调度到`invocationHandler`实例。

`InvocationHandler`实现的主要框架如下：
...
因为我们要计算`List`的方法的调用次数，所以我们应该存储所有的方法签名以及每个方法的调用次数。这可以通过`CountingInvocationHandler`的构造函数中初始化的`Map`来实现（这是我们的`InvocationHandler`实现，`invocationHandler`是它的一个实例）：
...
因为`add()`方法是通过它的两个签名调用的，所以结果`Map`包含两个条目。

# 摘要

这是本章的最后一个问题。希望我们已经完成了对 Java 反射 API 的全面遍历。我们已经详细讨论了有关类、接口、构造函数、方法、字段、注释等的问题
