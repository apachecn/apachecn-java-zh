...
在 Java 中创建不可修改/不可变的集合可以很容易地通过`Collections.unmodifiableFoo()`方法（例如，`unmodifiableList()`）完成，并且从 JDK 9 开始，通过来自`List`、`Set`、`Map`和其他接口的一组`of()`方法完成。

此外，我们将在一组示例中使用这些方法来获得不可修改/不可变的集合。主要目标是确定每个定义的集合是不可修改的还是不可变的。

在阅读本节之前，建议先阅读[第 2 章](02.html)、“对象、不变性和`switch`表达式”中有关不变性的问题。
...
根据经验，如果集合是通过`unmodifiableFoo()`或`of()`方法定义的，并且包含可变数据，则集合是不可修改的；如果集合是不可修改的，并且包含可变数据（包括原语），则集合是不可修改的。
...
然而，这三种解决方案只提供了地图的浅显副本。[获取深度拷贝的解决方案可以依赖于克隆库](https://github.com/kostaskougios/cloning)在[第 2 章](02.html)中介绍，“对象、不变性和`switch`表达式”。将使用克隆的助手方法可以编写如下：
...
另一种解决方案依赖于 Java8 功能风格，或者更准确地说，依赖于`UnaryOperator`函数式接口。
...
根据经验，一个较大的过滤器比一个较小的过滤器具有更少的*假正例*。此外，通过增加散列函数的数量，我们可以获得较少的*假正例*，但我们会减慢过滤器的速度，并将其快速填充。Bloom 过滤器的性能为`O(h)`，其中``h``是散列函数的个数。
