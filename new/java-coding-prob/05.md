...
100.  **寻找数组中的元素**：编写几个程序，举例说明如何在给定的数组中找到给定的元素（原语和对象）。查找索引和/或简单地检查值是否在数组中。
101.  **检查两个数组是否相等或不匹配**：编写一个程序，检查给定的两个数组是否相等或不匹配。
102.  **按字典法比较两个数组**：编写一个程序，按字典法比较给定的数组。
...
110.  **映射默认值**：编写一个程序，从`Map`获取一个值或一个默认值。
111.  **计算**`Map`中是否缺失/存在：编写一个程序，计算缺失键的值或当前键的新值。
112.  **从**`Map`删除：编写一个程序，用给定的键从`Map`删除。
113.  **替换**`Map`中的条目：编写一个程序来替换`Map`中给定的条目。
114.  **比较两幅地图**：编写一个比较两幅地图的程序。
115.  **合并两个地图**：编写一个程序，合并两个给定的地图。
...
120.  **`List`过滤集合：写几个`List`过滤集合的方案。揭示最好的方法。**
***   **替换**`List`的元素：编写一个程序，将`List`的每个元素替换为对其应用给定运算符的结果。*   **线程安全集合、堆栈和队列**：编写几个程序来举例说明 Java 线程安全集合的用法。*   **广度优先搜索**（**BFS**）：编写实现 BFS 算法的程序。*   **Trie**：编写一个实现 Trie 数据结构的程序。*   **元组**：编写实现元组数据结构的程序。*   **联合查找**：编写实现联合查找算法的程序。*   **Fenwick 树或二叉索引树**：编写一个实现 Fenwick 树算法的程序。*   **Bloom filter**：编写实现 Bloom filter 算法的程序。**
...
对于装箱基元类型的数组，解决方案可以依赖于`Collections.reverse()`方法，如下例所示：
...
为简洁起见，跳过了带有[T0]的冒泡排序优化版本，但它在绑定到本书的代码中可用。
...
在这里，我们有一个`Comparator`，它使用`thenComparing()`方法，按照 Java 8 函数式编写的类型和重量对西瓜进行排序：
...
在第一步，该算法使用提供的数组来构建这个堆，并将其转换为一个*Max heap*（该堆由另一个数组表示）。因为这是一个*Max 堆*，所以最大的元素是堆的根。在下一步中，根与堆中的最后一个元素交换，堆大小减少 1（从堆中删除最后一个节点）。堆顶部的元素按顺序排列。最后一步由*heapify*（以自顶向下的方式构建堆的递归过程）和堆的根（重构*Max heap*）组成。重复这三个步骤，直到堆大小大于 1：
...
8.  这是一个*Max Heap*，因此将根（**2**）与最后一个元素（**1**）交换并移除**2**：**1**、**2**、**4**、**5**、**7**。
9.  完成！堆中只剩下一个元素（**1**）。
...
如果数组已经排序，那么可以通过删除排序步骤来优化前面的方法。此外，如果数组被排序，前面的方法可以返回数组中元素出现的索引，而不是一个`boolean`。但是，如果数组没有排序，请记住返回的索引对应于排序的数组，而不是未排序的（初始）数组。如果不想对初始数组进行排序，则建议将数组的克隆传递给此方法。另一种方法是在这个 helper 方法中克隆数组。
...
第三，我们可以依赖 Java 8 功能风格和一个`Comparator`：
...
通过`Arrays.equals()`方法可以很容易地检查两个阵列是否相等。对于基本类型、`Object`和泛型，这个标志方法有很多种风格。它还支持比较器。
...
前面的例子检查两个数组是否相等，但是我们也可以通过 boolean`equals(int[] a, int aFromIndex, int aToIndex, int[] b, int bFromIndex, int bToIndex)`方法检查数组的两个段（或范围）是否相等。因此，我们通过范围[[T1]、[T2]来划分第一个数组的段，通过范围[[T3]、[T4]来划分第二个数组的段：
...
对于`Object`的数组，`Arrays`类还提供了一组专用的`compare()`方法。还记得`Melon`课吗？好吧，为了比较两个没有显式 T5 的 T4 数组，我们需要实现 T6 接口和 T7 方法。假设我们依赖于瓜的重量，如下所示：
...
我们可以通过`<T extends Comparable<? super T>> int compare(T[] a, int aFromIndex, int aToIndex, T[] b, int bFromIndex, int bToIndex)`方法在阵列的不同范围内进行比较。例如，在公共范围[1，2]中，`melons1`在字典上大于`melons2`，因为 Gac 的重量在`melons1`中为 1000g，在`melons3`中为 800g：
...
在这种情况下，`Arrays.stream()`方法可以再次提供帮助，唯一的区别是返回的结果是`IntStream`类型（这是`Stream`的`int`原语专门化）：
...
在 Java 中创建不可修改/不可变的集合可以很容易地通过`Collections.unmodifiable*Foo*()`方法（例如，`unmodifiableList()`）完成，并且从 JDK 9 开始，通过来自`List`、`Set`、`Map`和其他接口的一组`of()`方法完成。

此外，我们将在一组示例中使用这些方法来获得不可修改/不可变的集合。主要目标是确定每个定义的集合是不可修改的还是不可变的。

在阅读本节之前，建议先阅读[第 2 章](02.html)、*对象、不变性和开关表达式*中有关不变性的问题。
...
那么，`list`是不可修改的还是不变的？答案是不可更改的。虽然 mutator 方法会抛出`UnsupportedOperationException`，但底层的`melon1`和`melon2`是可变的。例如，我们把西瓜的重量设为`0`：
...
那么，这个列表是不可修改的还是不变的？答案是不可更改的。当 mutator 方法抛出`UnsupportedOperationException`时，硬编码实例可以通过`List.get()`方法访问。一旦可以访问它们，它们就可以变异：
...
那么，这个列表是不可修改的还是不变的？答案是不变的。Mutator 方法会抛出`UnsupportedOperationException`，我们不能对`ImmutableMelon`的实例进行变异。
...
另外，可以通过 JDK 10 从可修改/可变的`Map`中获得不可修改/不可变的`Map`，`Map.copyOf​(Map<? extends K,​? extends V> map)`方法：
...
因此，Java 中所有有用的数组都是可变的。但是我们可以在`Arrays.copyOf()`的基础上创建一个 helper 类来创建不可变数组，它复制元素并创建一个新数组（在幕后，这个方法依赖于`System.arraycopy()`。
...
在 jdk8 之前，这个问题的解决方案依赖于 helper 方法，它基本上检查`Map`中给定键的存在，并返回相应的值或默认值。这种方法可以在实用程序类中编写，也可以通过扩展`Map`接口来编写。通过返回默认值，我们可以避免在`Map`中找不到给定键时返回`null`。此外，这是依赖默认设置或配置的方便方法。
...
由于地图中没有 Derby DB，因此结果将是`null`。这不是我们想要的。实际上，当搜索到的数据库不在地图上时，我们可以在`69:89.31.226:27017`上使用 MongoDB，它总是可用的。现在，我们可以很容易地将此行为塑造为：
...
这种方法可以方便地建立 fluent 表达式，避免中断代码进行`null`检查。请注意，返回默认值并不意味着该值将被添加到`Map`。`Map`保持不变。
...
假设我们要为 MongoDB 构建 JDBC URL。这一次，如果映射中存在`mongodb`键，则应返回相应的值，而无需进一步计算。但是如果这个密钥不存在（或者与一个`null`值相关联），那么它应该基于这个密钥和当前 IP 进行计算并添加到映射中。如果计算值为`null`，则返回结果为`null`，地图保持不变。
...
如果我们的`Function`被求值为`null`，那么 map 保持不变，返回值为`null`。

再次调用`computeIfAbsent()`不会重新计算值。这次，由于`mongodb`在 map 中（在上一次调用中添加），所以返回的值将是`mongodb://192.168.100.10/customers_db`。这与尝试获取[T3]的 JDBC URL 是一样的，它将返回[T4]，而无需进一步计算。
...
现在，让我们计算 MySQL 的 JDBC URL。因为`mysql`键存在于映射中，所以计算将依赖于相应的值`192.168.0.50`。结果将更新地图中`mysql`键的值：
...
如果计算的结果是`null`（例如 JDBC URL 无法计算），并且 map 中存在 key（例如`mysql`），那么这个条目将从 map 中删除，返回的结果是`null`。
...
如果给定的键（`K`与某个值没有关联或与`null`关联，那么新的值将是`V`。如果给定键（`K`与非`null`值相关联，则基于给定的`BiFunction`计算新值。如果此`BiFunction`的结果是`null`，并且该键存在于地图中，则此条目将从地图中删除。
...
或者，我们添加 Derby DB 的版本`10.11.1.1`。这将导致地图中出现一个新条目，因为不存在`derby`键：
...
*   如果给定的键不在 map 中（或者与一个`null`值相关联），则将给定的值放入 map 并返回`null`。
...
迭代和从`Map`中移除至少可以通过两种方式来完成：第一，通过`Iterator`（捆绑代码中存在的解决方案），第二，从 JDK 8 开始，我们可以通过`removeIf​(Predicate<? super E> filter)`来完成：

```
map.entrySet().removeIf(e -> e.getValue().equals("mysql"));
```

有关从集合中删除的更多详细信息，请参见*删除集合中与谓词*匹配的所有元素。

# 113 替换映射中的条目

从`Map`替换条目是一个在很多情况下都会遇到的问题。要实现这一点并避免在 helper 方法中编写一段*意大利面条*代码，方便的解决方案依赖于 jdk8`replace()`方法。
...
只要我们依赖于`Map.equals()`方法，比较两张地图是很简单的。在比较两个映射时，该方法使用`Object.equals()`方法比较它们的键和值。

例如，让我们考虑两个具有相同条目的瓜地图（在`Melon`类中必须存在`equals()`和`hashCode()`：
...
这个问题源于这样一个事实：数组的`equals()`方法比较的是标识，而不是数组的内容。为了解决这个问题，我们可以编写一个 helper 方法如下（这次依赖于`Arrays.equals()`，它比较数组的内容）：
...
对`Map`进行排序的快速解决方案依赖于`TreeMap`。根据定义，`TreeMap`中的键按其自然顺序排序。此外，`TreeMap`还有一个`TreeMap​(Map<? extends K,​? extends V> m)`类型的构造函数：
...
一旦我们为地图创建了一个`Stream`，我们就可以很容易地用`Stream.sorted()`方法对它进行排序，不管有没有`Comparator`。这一次，让我们使用一个`Comparator`：
...
执行`HashMap`的浅拷贝的简便解决方案依赖于`HashMap`构造函数`HashMap​(Map<? extends K,​? extends V> m)`。以下代码是不言自明的：
...
然而，这三种解决方案只提供了地图的浅显副本。获取深度拷贝的解决方案可以依赖于克隆库（[https://github.com/kostaskougios/cloning](https://github.com/kostaskougios/cloning)在[第 2 章](02.html)中介绍，*对象、不变性和开关表达式*。将使用克隆的助手方法可以编写如下：
...
如果给定的键（`K`与值没有关联，或者与`null`关联，那么新的值将是`V`。如果给定键（`K`与非`null`值相关联，则基于给定的`BiFunction`计算新值。如果此`BiFunction`的结果是`null`，并且该键存在于地图中，则此条目将从地图中删除。
...
我们在应用程序中遇到的一个常见问题是用一个 T1 来过滤一个 T0。主要是从一个巨大的`Collection`开始，我们想从中提取与`List`元素匹配的元素。
...
当我们调用`HashSet.contains(element)`时，实际上我们调用`Map.containsKey(element)`。该方法基于给定元素的`hashCode()`，将给定元素与地图中的适当键进行匹配，比`equals()`快得多。
...
我们在应用程序中遇到的另一个常见问题是替换符合特定条件的`List`元素。
...
每当集合/堆栈/队列容易被多个线程访问时，它也容易出现特定于并发的异常（例如，`java.util.ConcurrentModificationException`。现在，让我们简要地概述一下 Java 内置的并发集合，并对其进行介绍。
...
| **单螺纹** | **多线程** |
| `HashSet``TreeSet`（排序集）`LinkedHashSet`（维护插入顺序）`BitSet``EnumSet` | `ConcurrentSkipListSet`（排序集）`CopyOnWriteArraySet`（经常阅读，很少更新） |
...
Java 提供了一个线程安全队列（**先进先出**（**FIFO**），由一个数组通过`ArrayBlockingQueue`支持。下表列出了由数组支持的单线程和多线程 Java 内置队列：
...
Java 提供了一个阻塞队列，其中每个 insert 操作必须等待另一个线程执行相应的 remove 操作，反之亦然，通过`SynchronousQueue`：
...
`Stack`类表示对象的**后进先出**（**后进先出**）堆栈。它通过几个操作扩展了`Vector`类，这些操作允许将向量视为堆栈。`Stack`的每一种方法都是同步的。创建一个`Stack`如下所示：
...
显而易见的问题是*并发集合和同步集合的区别是什么？*好吧，主要区别在于它们实现线程安全的方式。并发集合通过将数据划分为段来实现线程安全。线程可以并发地访问这些段，并且只能在所使用的段上获得锁。另一方面，synchronized collection 通过*内在锁定*锁定整个集合（调用 synchronized 方法的线程将自动获取该方法对象的内在锁，并在方法返回时释放它）。
...
3.  *而**Q 为非空*
4.  *取下 Q*的头部 h
5.  *标记 h*的所有（未访问的）邻居并排队

假设下图中的图，**步骤 0**：

![](img/da9ace7a-fd80-4dc8-905a-278b00375fa2.png)

在第一步（**步骤 1**），我们访问顶点**0**。我们把它放在`visited`列表中，它的所有相邻顶点放在`queue`（3，1）中。此外，在**步骤 2**中，我们访问`queue`、**3**前面的元素。顶点**3**在**2 中**有一个未访问的相邻顶点，所以我们将其添加到`queue`的后面。接下来，在**步骤 3**中，我们访问`queue`**1**前面的元素。该顶点有一个相邻的顶点（**0**），但该顶点已被访问。最后，我们访问顶点**2**，最后一个来自`queue`。这个有一个已经访问过的相邻顶点（**3**）。
...
下图表示构建包含单词**cat**、**插入符号**和**bye**的 Trie 的步骤顺序：
...
3.  如果当前节点（T0）为当前字符映射一个值（a`Node`，那么只需前进到该节点。否则，新建一个`Node`，将其字符设置为当前字符，并前进到此节点。
...
*   如果给定的单词不在 Trie 中，那么什么也不会发生（return`false`）
*   如果给定的单词是唯一的（不是另一个单词的一部分），则删除所有相应的节点（返回`true`）
*   如果给定的单词是 Trie 中另一个长单词的前缀，则将叶节点标志设置为`false`（返回`false`）
*   如果给定的单词至少有另一个单词作为前缀，则从给定单词的末尾删除相应的节点，直到最长前缀单词的第一个叶节点（返回【T0）】
...
此外，我们需要通过*联合*操作来定义我们的子集。我们可以通过（*父*、*子*对）序列来定义子集。例如，让我们定义以下三对-`union(0,1);`、`union(4, 9);`和`union(6, 5);`。每次一个元素（子集）成为另一个元素（子集）的子元素时，它都会修改其值以反映其父元素的值，如下图所示：
...
*联合*操作首先获取给定子集的根元素。此外，如果这两个根是不同的，它们需要依赖于它们的秩来决定哪一个将成为另一个的父（较大的秩将成为父）。如果它们的等级相同，则选择其中一个并将其等级增加 1：
...
**芬威克树**（**FT**）或**二叉索引树**（**位**）是为存储对应于另一给定数组的和而构建的数组。构建数组的大小与给定数组的大小相同，并且构建数组的每个位置（或节点）都存储给定数组中某些元素的总和。由于 BIT 存储给定数组的部分和，因此通过避免索引之间的循环和计算和，它是计算给定数组中两个给定索引（范围和/查询）之间的元素和的非常有效的解决方案。

位可以在线性时间或`O(n log n)`中构造。显然，我们更喜欢线性时间，所以让我们看看如何做到这一点。我们从给定的（原始）数组开始，该数组可以是（下标表示数组中的索引）：

3<sub xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:epub="http://www.idpf.org/2007/ops">3（1）号【T03】3（1）、1</sub>1<sub xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:epub="http://www.idpf.org/2007/ops">1<sub xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:epub="http://www.idpf.org/2007/ops">2</sub>2）、5<sub xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:epub="http://www.idpf.org/2007/ops">5</sub>5、8<sub xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:epub="http://www.idpf.org/2007/ops">4</sub>1、8<sub xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:epub="http://www.idpf.org/2007/ops">4</sub>1、12<sub xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:epub="http://www.idpf.org/2007/ops">5</sub>1）、1</sub>11<sub xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:epub="http://www.idpf.org/2007/ops">1【2】1（2）2【2】2【2】2【2】2】2【7</sub>5】5、55、0<sub xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:epub="http://www.idpf.org/2007/ops">6】（9）】</sub>1、8<sub xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:epub="http://www.idpf.org/2007/ops">10【T19）】、1【T19）】、1<sub xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:epub="http://www.idpf.org/2007/ops">1<sub xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:epub="http://www.idpf.org/2007/ops">1（11）】<sub xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:epub="http://www.idpf.org/2007/ops">1<sub xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:epub="http://www.idpf.org/2007/ops">1【11）】</sub>1</sub>1）】、4【T21、4</sub>1、4【T22<sub xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:epub="http://www.idpf.org/2007/ops">4<sub xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:epub="http://www.idpf.org/2007/ops">4<sub xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:epub="http://www.idpf.org/2007/ops">4<sub xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:epub="http://www.idpf.org/2007/ops">（13）</sub>、0<sub xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:epub="http://www.idpf.org/2007/ops">（14）】</sub>、11<sub xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:epub="http://www.idpf.org/2007/ops">（15）】</sub>、5<sub xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:epub="http://www.idpf.org/2007/ops">（16）】</sub></sub></sub></sub></sub></sub>

构建位的想法依赖于**最低有效位**（**LSB**概念。更准确地说，假设我们正在处理索引中的元素，*a*。那么，紧靠我们上方的值必须位于索引*b*，其中*b=a+LSB（a）*。为了应用该算法，索引 0 的值必须是 0；因此，我们操作的数组如下：

****0<sub>0<sub><sub>0<sub>0<sub><sub>0</sub>0<sub xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:epub="http://www.idpf.org/2007/ops">1<sub xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:epub="http://www.idpf.org/2007/ops">2</sub>2</sub>2、5<sub xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:epub="http://www.idpf.org/2007/ops">5<sub xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:epub="http://www.idpf.org/2007/ops">3</sub>5</sub>0、8<sub xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:epub="http://www.idpf.org/2007/ops">4<sub xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:epub="http://www.idpf.org/2007/ops">8【3】0、8<sub xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:epub="http://www.idpf.org/2007/ops">0<sub xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:epub="http://www.idpf.org/2007/ops">0</sub>0、8<sub xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:epub="http://www.idpf.org/2007/ops">5<sub xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:epub="http://www.idpf.org/2007/ops">0</sub>0</sub>3</sub>5【T9）】、8【T20【T20<sub xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:epub="http://www.idpf.org/2007/ops">0<sub xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:epub="http://www.idpf.org/2007/ops">20<sub xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:epub="http://www.idpf.org/2007/ops">20【9）】</sub></sub>1</sub>1】1【T21）】、3、3、3【T22【T22<sub xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:epub="http://www.idpf.org/2007/ops">2】2】2【2】2【2】2】2】2】2【10【10【10【10】10】10（2）</sub>，4<sub xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:epub="http://www.idpf.org/2007/ops">（12】</sub>、9<sub xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:epub="http://www.idpf.org/2007/ops">（13】</sub>、0<sub xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:epub="http://www.idpf.org/2007/ops">（14】</sub>、11<sub xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:epub="http://www.idpf.org/2007/ops">（15】</sub>、5<sub xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:epub="http://www.idpf.org/2007/ops">（16】</sub></sub></sub></sub></sub></sub></sub></sub>****

现在，让我们应用算法的几个步骤，用和填充位。在位的索引 0 处，我们有 0。此外，我们使用*b=a+LSB（a）*公式计算剩余和，如下所示：

1.  a=1：如果 a=1=00001<sub>2</sub>，则 b=00001<sub>2</sub>+00001<sub>2</sub>=1+1=2=00010<sub>2</sub>。我们说 2 负责 a（也就是 1）。因此，在位中，在索引 1 处，我们存储值 3，在索引 2 处，我们存储值和，3+1=4。
2.  a=2：如果 a=2=00010<sub>2</sub>，则 b=00010<sub>2</sub>+00010<sub>2</sub>2+2=4=00100<sub>2</sub>。我们说 4 负责 a（即 2）。因此，在索引 4 处，我们以位的形式存储值 sum，8+4=12。
3.  a=3：如果 a=3=00011<sub>2</sub>，则 b=00011<sub>2</sub>+00001<sub>2</sub>=3+1=4=00100<sub>2</sub>。我们说 4 负责 a（也就是 3）。因此，在位中，在索引 4 处，我们存储值和，12+5=17。
...
bloomfilter 是一种快速高效的数据结构，能够提供问题的概率答案*值 X 在给定的集合中吗？*

通常情况下，当集合很大且大多数搜索算法都面临内存和速度问题时，此算法非常有用。

Bloom 过滤器的速度和内存效率来自这样一个事实，即该数据结构依赖于位数组（例如，`java.util.BitSet`）。最初，该数组的位被设置为`0`或`false`。
...
显然，我们希望尽可能减少*假阳性*的数量。虽然我们不能完全消除它们，但我们仍然可以通过调整位数组的大小、哈希函数的数量和集合中元素的数量来影响它们的速率。
...
*   *假阳性*的概率，介于 0 和 1 之间的分数，或表示 1 英寸-`p`的数字：
...
根据经验，一个较大的过滤器比一个较小的过滤器具有更少的*假阳性*。此外，通过增加散列函数的数量，我们可以获得较少的*误报*，但我们会减慢过滤器的速度，并将其快速填充。Bloom 过滤器的性能为`O(h)`，其中`*h*`是散列函数的个数。

在本书附带的代码中，有一个 Bloom 过滤器的实现，它使用基于 SHA-256 和 murrur 的散列函数。由于这段代码太大，无法在本书中列出，因此请考虑将`Main`类中的示例作为起点。
