...
本章包括 20 个涉及日期和时间的问题。通过`Date`、`Calendar`、`LocalDate`、`LocalTime`、`LocalDateTime`、`ZoneDateTime`、`OffsetDateTime`、`OffsetTime`、`Instant`等，这些问题涵盖了广泛的主题（转换、格式化、加法、减法、定义期间/持续时间、计算等）。到本章结束时，您将在确定日期和时间方面没有问题，同时符合您的应用程序的需要。本章介绍的基本问题将非常有助于了解日期-时间 API 的整体情况，并将像拼图中需要拼凑起来的部分一样解决涉及日期和时间的复杂挑战。
...
***   **格式化日期和时间**：**解释日期和时间的格式模式。*****   **获取当前日期/时间（不含时间/日期）**：**编写程序，提取当前日期（不含时间或日期）。*****   `LocalDate`**和**`LocalTime`中的`LocalDateTime`**：**编写一个从`LocalDate`对象和`LocalTime`构建`LocalDateTime`的程序。它将日期和时间组合在一个`LocalDateTime`对象中。*******   **通过**`Instant`**类**：**解释并给出`Instant`API 示例的机器时间。*****   **定义使用基于日期的值（`Period`的时间段和使用基于时间的值（`Duration`）**的时间段：**解释并举例说明`Period`和`Duration`API 的用法。************

 ********64.  **获取日期和时间单位**：编写一个程序，从表示日期时间的对象中提取日期和时间单位（例如，从日期中提取年、月、分钟等）。
65.  **对日期时间**进行加减：编写一个程序，对日期时间对象进行加减（如年、日、分等）（如对日期加 1 小时，对`LocalDateTime`减 2 天等）。
66.  **获取 UTC 和 GMT 的所有时区**：**编写一个程序，显示 UTC 和 GMT 的所有可用时区。**
***   **获取所有可用时区的本地日期时间**：编写一个程序，显示所有可用时区的本地时间。*   **显示航班日期时间信息**：编写程序，显示 15 小时 30 分钟的航班时刻信息。更确切地说，是从澳大利亚珀斯飞往欧洲布加勒斯特的航班。*   **将 Unix 时间戳转换为日期时间**：编写将 Unix 时间戳转换为`java.util.Date`和`java.time.LocalDateTime`的程序。*   **查找月份的第一天/最后一天**：编写一个程序，通过 JDK 8，`TemporalAdjusters`查找月份的第一天/最后一天。*   **定义/提取区域偏移**：编写一个程序，展示定义和提取区域偏移的不同技术。*   `Date`**与**`Temporal`之间的转换：编写`Date`与`Instant`、`LocalDate`、`LocalDateTime`等之间的转换程序。*   **迭代一系列日期**：**编写一个程序，逐日（以一天的步长）迭代一系列给定日期。*****   **计算年龄**：**编写一个计算一个人年龄的程序。*****   **一天的开始和结束**：编写一个程序，返回一天的开始和结束时间。*   **两个日期之间的差异：**编写一个程序，计算两个日期之间的时间量（以天为单位）。*   **实现象棋时钟：**编写实现象棋时钟的程序。**************  ******# 解决
...
可以通过一组`parse()`方法将`String`转换或解析为日期和时间。从日期和时间到`String`的转换可以通过`toString()`或`format()`方法完成。

# JDK 8 之前

在 JDK 8 之前，这个问题的典型解决方案依赖于抽象类`DateFormat`的主扩展，名为`SimpleDateFormat`（这不是线程安全类）。在本书附带的代码中，有几个示例说明了如何使用此类。

# 从 JDK 8 开始

从 JDK 8 开始，`SimpleDateFormat`可以替换为一个新类-`DateTimeFormatter`。这是一个不可变（因此是线程安全的）类，用于打印和解析日期时间对象。这个类支持从预定义的格式化程序（表示为常量，如 ISO local date，2011-12-03，is`ISO_LOCAL_DATE`）到用户定义的格式化程序（依赖于一组用于编写自定义格式模式的符号）。

此外，除了[0]类之外，jdk8 还提供了几个新类，它们专门用于处理日期和时间。以下列表中显示了其中一些类（这些类也被称为临时类，因为它们实现了`Temporal`接口）：

*   `LocalDate`（ISO-8601 日历系统中没有时区的日期）
*   `LocalTime`（ISO-8601 日历系统中没有时区的时间）
*   `LocalDateTime`（ISO-8601 日历系统中没有时区的日期时间）
*   `ZonedDateTime`（ISO-8601 日历系统中带时区的日期时间），依此类推
*   `OffsetDateTime`（在 ISO-8601 日历系统中，与 UTC/GMT 有偏移的日期时间）
*   `OffsetTime`（在 ISO-8601 日历系统中，与 UTC/GMT 有偏移的时间）

为了通过预定义的格式化程序将`String`转换为`LocalDate`，它应该遵循`DateTimeFormatter.ISO_LOCAL_DATE`模式，例如 2020-06-01。`LocalDate` 提供一种`parse()`方法，可以使用如下：
...
类似地，在`LocalTime`的情况下，字符串应该遵循`DateTimeFormatter.ISO_LOCAL_TIME`模式，例如 10:15:30，如下代码片段所示：

```
LocalTime localTime = LocalTime.parse("12:23:44");
```

在`LocalDateTime`的情况下，字符串应该遵循`DateTimeFormatter.ISO_LOCAL_DATE_TIME`模式，例如`2020-06-01T11:20:15`，如下代码段所示：
...
在`ZonedDateTime`的情况下，字符串必须遵循`DateTimeFormatter.ISO_ZONED_DATE_TIME`模式，例如`2020-06-01T10:15:30+09:00[Asia/Tokyo]`，如下代码段所示：
...
在`OffsetDateTime`的情况下，字符串必须遵循`DateTimeFormatter.ISO_OFFSET_DATE_TIME`模式，例如`2007-12-03T10:15:30+01:00`，如下代码段所示：
...
如果字符串不符合任何预定义的格式设置程序，则是时候通过自定义格式模式使用用户定义的格式设置程序了；例如，字符串`01.06.2020`表示需要用户定义格式设置程序的日期，如下所示：
...
但是，像[0]这样的字符串需要用户定义的格式化程序，如下所示：
...
最后，像[0]这样的字符串需要一个用户定义的格式化程序，如下所示：
...
前面示例中的每个`ofPattern()`方法都支持`Locale`。
...
*   依赖`DateTimeFormatter.format()`方法。请注意，依赖于`DateTimeFormatter.format()`将始终使用指定的格式化程序打印日期/时间（默认情况下，时区将为`null`），如下所示：
...
上一个问题包含一些通过`SimpleDateFormat.format()`和`DateTimeFormatter.format()`格式化日期和时间的样式。为了定义*格式模式，*开发人员必须了解格式模式语法。换句话说，开发人员必须知道 Java 日期时间 API 使用的一组符号，以便识别有效的格式模式。

大多数符号对`SimpleDateFormat`（JDK 8 之前）和`DateTimeFormatter`（从 JDK 8 开始）都是通用的。下表列出了 JDK 文档中提供的最常见符号的完整列表：
...
从 JDK 8 开始，日期和时间可以通过专用类`LocalDate`和`LocalTime`从`java.time`包中获取：
...
[0]类公开了一系列的[1]方法，这些方法可用于获取不同类型的[2]实例。例如，从年、月、日、时、分、秒或纳秒获得的`LocalDateTime`类如下所示：
...
这将导致`LocalDate`和`LocalTime`，如下所示：
...
JDK 8 附带了一个新类，名为`java.time.Instant`。主要地，`Instant`类表示时间线上的一个瞬时点，从 1970 年 1 月 1 日（纪元）的第一秒开始，在 UTC 时区，分辨率为纳秒。

Java8`Instant`类在概念上与`java.util.Date`类似。两者都代表 UTC 时间线上的一个时刻。当`Instant`的分辨率高达纳秒时，`java.util.Date`的分辨率为毫秒。
...
调用`Instant.toString()`会产生一个符合 ISO-8601 标准的输出，用于表示日期和时间。
...
两个`Instant`对象的比较可以通过`Instant.isAfter()`和`Instant.isBefore()`方法完成。例如，让我们看看以下两个`Instant`对象：
...
*   在`Instant`和`LocalDateTime`之间转换-由于`LocalDateTime`不知道时区，请使用零偏移 UTC+0：
...
JDK8 提供了两个新类，分别命名为`java.time.Period`和`java.time.Duration`。让我们在下一节中详细了解它们。

# 使用基于日期的值的时间段

[0]类表示使用基于日期的值（年、月、周和天）表示的时间量。这段时间可以用不同的方法获得。例如，120 天的周期可以如下获得：
...
最后，可以从遵循 ISO-8601 周期格式`PnYnMnD`和`PnW`的`String`对象中获得`Period`。例如，P2019Y2M25D 字符串表示 2019 年、2 个月和 25 天：
...
但是，当用`Period`来表示两个日期之间的一段时间（例如`LocalDate`）时，它的真正威力就显现出来了。2018 年 3 月 12 日至 2019 年 7 月 20 日期间可表示为：
...
年、月、日的时间量可以通过`Period.getYears()`、`Period.getMonths()`、`Period.getDays()`获取。例如，以下 helper 方法使用这些方法将时间量输出为字符串：
...
我们将此方法称为`periodBetween`（差异为 1 年 4 个月 8 天）：

```
periodToYMD(periodBetween); // 1y:4m:8d
```

当确定某个日期是否早于另一个日期时，`Period`类也很有用。有一个 flag 方法，名为`isNegative()`。有一个`A`周期和一个`B`周期，如果`B`在`A`之前，应用`Period.between(A, B)`的结果可以是负的，如果`A`在`B`之前，应用`Period.between(A, B)`的结果可以是正的，进一步说，如果`B`在`A`之前，`isNegative()`返回`true`，如果`A`在`B`之前，`false`返回，我们的例子是这样的（基本上，如果年、月或日是负数，这个方法返回`false`）：
...
最后，`Period`可以通过加减一段时间来修改。方法有`plusYears()`、`plusMonths()`、`plusDays()`、`minusYears()`、`minusMonths()`、`minusDays()`等。例如，在`periodBetween`上加 1 年可以如下操作：

```
Period periodBetweenPlus1Year = periodBetween.plusYears(1L);
```

通过`Period.plus()`方法可以增加两个`Period`类，如下所示：
...
[0]类表示使用基于时间的值（小时、分钟、秒或纳秒）表示的时间量。这种持续时间可以通过不同的方式获得。例如，可以如下获得 10 小时的持续时间：
...
`Duration`也可以从`LocalDateTime`获取：
...
最后，可以从遵循 ISO-8601 持续时间格式`PnDTnHnMn.nS`的`String`对象中获取`Duration`，其中天被认为正好是 24 小时。例如，`P2DT3H4M`字符串有 2 天 3 小时 4 分钟：
...
但是，与`Period`的情况一样，当`Duration`用于表示两次之间的一段时间（例如`Instant`时，`Duration`的实权就显露出来了。从 2015 年 11 月 3 日 12:11:30 到 2016 年 12 月 6 日 15:17:10 之间的持续时间可以表示为两个`Instant`类之间的差异，如下所示：
...
以秒为单位，这个差值可以通过`Duration.getSeconds()`方法得到：
...
在前面的例子中，`Duration`是通过`Duration.getSeconds()`方法用秒来表示的，这是`Duration`类中的秒数。然而，`Duration`类包含一组方法，这些方法通过`toDays()`以天为单位、通过`toHours()`以小时为单位、通过`toMinutes()`以分钟为单位、通过`toMillis()`以毫秒为单位、通过`toNanos()`以纳秒为单位来表达`Duration`。

从一个时间单位转换到另一个时间单位可能会产生残余。例如，从秒转换为分钟可能导致秒的剩余（例如，65 秒是 1 分钟，5 秒是剩余）。残差可以通过以下方法获得：天残差通过`toDaysPart()`，小时残差通过`toHoursPart()`，分钟残差通过`toMinutesPart()`等等。

假设差异应该显示为天：小时:分：秒：nano（例如，9d:2h:15m:20s:230n）。将[0]和[1]方法的力合并到一个 helper 方法中会产生以下代码：
...
与`Period`类相同，`Duration`类有一个名为`isNegative()`的标志方法。当确定某个特定时间是否早于另一个时间时，此方法很有用。有持续时间`A`和持续时间`B`，如果`B`在`A`之前，应用`Duration.between(A, B)`的结果可以是负的，如果`A`在`B`之前，应用`Duration.between(A, B)`的结果可以是正的，进一步逻辑，如果`B`在`A`之前，`isNegative()`返回`true`，如果`A`在`B`之前，`false`返回，如以下情况：

```
durationBetweenLT.isNegative(); // false
```

最后，`Duration`可以通过增加或减少持续时间来修改。有`plusDays()`、`plusHours()`、`plusMinutes()`、`plusMillis()`、`plusNanos()`、`minusDays()`、`minusHours()`、`minusMinutes()`、`minusMillis()`、`minusNanos()`等方法进行。例如，在`durationBetweenLT`上加上 5 个小时，可以按如下方式进行：

```
Duration durationBetweenPlus5Hours = durationBetweenLT.plusHours(5);
```

通过`Duration.plus()`方法可以增加两个`Duration`类，如下所示：
...
对于`Date`对象，解决方案可能依赖`Calendar`实例。绑定到本书的代码包含此解决方案。
...
通过一些直觉和文档，很容易将此示例应用于`LocalDate`、`LocalTime`、`ZonedDateTime`等。
...
跳转到 jdk8，关注点是`LocalDate`、`LocalTime`、`LocalDateTime`、`Instant`等等。新的 Java date-time API 提供了专门用于添加或减去时间量的方法。`LocalDate`、`LocalTime`、`LocalDateTime`、`ZonedDateTime`、`OffsetDateTime`、`Instant`、`Period`、`Duration`以及许多其他方法，如`plus*Foo*()`和`minus*Foo*()`，其中*、`Foo`*可以替换为时间单位（例如：例如，`plusYears()`、`plusMinutes()`、`minusHours()`、`minusSeconds()`等）。
...
加 10 分钟和打电话`LocalDateTime.plusMinutes(long minutes)`一样简单，减 10 分钟和打电话`LocalDateTime.minusMinutes(long minutes)`一样简单：
...
除了按时间单位指定的方法外，这些类还支持`plus/minus(TemporalAmount amountToAdd)`和`plus/minus(long amountToAdd, TemporalUnit unit)`。
...
第一步，通过`ZoneId`类可以获得可用的时区 ID，如下所示：

```
Set<String> zoneIds = ZoneId.getAvailableZoneIds();
```

在第二步中，每个时区 ID 都应该用来创建一个`ZoneId`实例。这可以通过`ZoneId.of(String zoneId)`方法实现：
...
`getId()`方法返回规范化区域偏移 ID，+00:00 偏移作为`Z`字符返回；因此代码需要快速将`Z`替换为+00:00，以便与其余偏移对齐，这些偏移遵循+hh:mm 或+hh:mm:ss 格式。
...
在 JDK8 之前，获取当前本地日期时间的快速解决方案是调用[0]空构造函数。此外，还可以使用`Date`在所有可用的时区中显示，这些时区可以通过`TimeZone`类获得。绑定到本书的代码包含此解决方案。

# 从 JDK 8 开始

从 JDK 8 开始，获取默认时区的当前本地日期时间的一个方便的解决方案是调用`ZonedDateTime.now()`方法：
...
首先，让我们将这个日期时间与澳大利亚/珀斯（+08:00）的时区结合起来。这将导致一个特定于澳大利亚/珀斯的`ZonedDateTime`对象（这是出发时珀斯的时钟日期和时间）：
...
此外，让我们在[0]上加上 15 小时 30 分钟。结果`ZonedDateTime`表示珀斯的日期时间（这是到达布加勒斯特时珀斯的时钟日期和时间）：
...
为了将 Unix 时间戳转换为日期时间，必须知道 Unix 时间戳的分辨率以秒为单位，而[0]需要毫秒。因此，从 Unix 时间戳获取`Date`对象的解决方案需要将 Unix 时间戳乘以 1000，从秒转换为毫秒，如下两个示例所示：
...
从 JDK 8 开始，`Date`类使用`from(Instant instant)`方法。此外，`Instant`类附带了`ofEpochSecond(long epochSecond)`方法，该方法使用 1970-01-01T00:00:00Z 的 epoch 中给定的秒数返回`Instant`的实例：
...
上例中获得的瞬间可以用来创建`LocalDateTime`或`ZonedDateTime`，如下所示：
...
这个问题的正确解决将依赖于 jdk8、[0]和[1]接口。

[0]接口位于日期和时间的表示后面。换句话说，表示日期和/或时间的类实现了这个接口。例如，以下类只是实现此接口的几个类：

*   `LocalDate`（ISO-8601 日历系统中没有时区的日期）
*   `LocalTime`（ISO-8601 日历系统中没有时区的时间）
*   `LocalDateTime`（ISO-8601 日历系统中没有时区的日期时间）
*   `ZonedDateTime`（ISO-8601 日历系统中带时区的日期时间），依此类推
*   `OffsetDateTime`（在 ISO-8601 日历系统中，与 UTC/格林威治时间有偏移的日期时间）
*   `HijrahDate`（Hijrah 日历系统中的日期）
...
通过*区域偏移量*，我们了解需要从 GMT/UTC 时间中添加/减去的时间量，以便获得全球特定区域（例如，澳大利亚珀斯）的日期时间。通常，区域偏移以固定的小时和分钟数打印：+02:00、-08:30、+0400、UTC+01:00，依此类推。

因此，简而言之，时区偏移量是指时区与 GMT/UTC 之间的时间差。

# JDK 8 之前

在 JDK 8 之前，可以通过`java.util.TimeZone`定义一个时区，有了这个时区，代码可以通过`TimeZone.getRawOffset()`方法得到时区偏移量（原始的*部分是因为这个方法没有考虑夏令时）。绑定到本书的代码包含此解决方案。*

# 从 JDK 8 开始

从 JDK8 开始，有两个类负责处理时区表示。第一个是`java.time.ZoneId`，表示欧洲雅典等时区；第二个是`java.time.ZoneOffset`（扩展为`ZoneId`），表示指定时区的固定时间（偏移量），以 GMT/UTC 表示。

新的 Java 日期时间 API 默认处理夏令时；因此，使用夏令时的夏-冬周期区域将有两个`ZoneOffset`类。

UTC 区域偏移量可以很容易地获得，如下所示（这是+00:00，在 Java 中用[0]字符表示）：
...
这是一种非常方便的方法，可以将区域偏移快速添加到支持区域偏移的[0]对象。例如，使用它将区域偏移添加到`OffsetTime`和`OffsetDateTime`（用于在数据库中存储日期或通过导线发送的方便方法）：
...
我们的问题的另一个解决方案是依赖于从小时、分钟和秒定义`ZoneOffset`。`ZoneOffset`中的一个助手方法专门用于：
...
这里提供的解决方案将涵盖以下`Temporal`类—`Instant`、`LocalDate`、`LocalDateTime`、`ZonedDateTime`、`OffsetDateTime`、`LocalTime`和`OffsetTime`。

# 日期-即时

为了从`Date`转换到`Instant`，可以依赖`Date.toInstant()`方法求解。反向可通过`Date.from(Instant instant)`方法完成：
...
请记住，`Date`不支持时区，但它显示在系统默认时区中（例如，通过`toString()`）。`Instant` 是 UTC 时区。

让我们快速地将这些代码片段包装到两个实用程序方法中，它们在一个实用程序类-`DateConverters`中定义：
...
从`LocalDate`到`Date`的转换应该考虑到`LocalDate`不包含`Date`的时间成分，所以解决方案必须提供一个时间成分作为一天的开始（更多的细节可以在 S*开始和结束一天的*问题中找到）：
...
从`Date`到`DateLocalTime`的转换与从`Date`到`LocalDate`的转换相同，只是解决方案调用`toLocalDateTime()`方法如下：
...
`Date`到`ZonedDateTime`的转换可以通过从给定`Date`对象中获取`Instant`对象和系统默认时区来完成：
...
从`OffsetDateTime`到`Date`的转换方法需要两个步骤。首先将`OffsetDateTime`转换为`LocalDateTime`，然后将`LocalDateTime`转换为`Instant`，并对应偏移量：
...
在 JDK8 之前，解决方案可以依赖于`Calendar`实用类。绑定到本书的代码包含此解决方案。
...
由于`Period`类的原因，将月和日添加到结果中也很容易完成：

```
Period periodBetween = Period.between(startLocalDate, endLocalDate);
```

现在，可以通过`periodBetween.getYears()`、`periodBetween.getMonths()`、`periodBetween.getDays()`获得年、月、日的年龄。
...
找到 2019 年 2 月 28 日当天开始时间的解决方案依赖于一个名为`atStartOfDay()`的方法。此方法从该日期午夜 00:00 返回`LocalDateTime`：
...
或者，该溶液可以使用`of(LocalDate date, LocalTime time)`方法。此方法将给定的日期和时间组合成`LocalDateTime`。因此，如果经过的时间是`LocalTime.MIN`（一天开始时的午夜时间），则结果如下：
...
一个`LocalDate`对象的一天结束时间至少可以用两种方法得到。一种解决方案是依靠`LocalDate.atTime(LocalTime time)`。结果`LocalDateTime`可以表示这个日期和一天结束时的组合，如果解决方案作为参数通过，`LocalTime.MAX`（一天结束时午夜前的时间）：
...
因为`LocalDate`没有时区的概念，所以前面的例子容易出现由不同的角落情况引起的问题，例如夏令时。有些夏令时会在午夜（00:00 变为 01:00 AM）更改时间，这意味着一天的开始时间是 01:00:00，而不是 00:00:00。为了缓解这些问题，请考虑以下示例，这些示例将前面的示例扩展为使用夏令时感知的`ZonedDateTime`：
...
显而易见的解决方案是从`LocalDateTime`中提取`LocalDate`，并应用前面的方法。另一个解决方案依赖于`Temporal`接口（包括`LocalDate`接口）的每个实现都可以利用`with(TemporalField field, long newValue)`方法。主要是，`with()`方法返回这个日期的一个副本，其中指定的字段`ChronoField`设置为`newValue`。因此，如果解决方案将`ChronoField.NANO_OF_DAY`（一天的纳秒）设置为`LocalTime.MIN`，那么结果将是一天的开始。这里的技巧是通过`toNanoOfDay()`将`LocalTime.MIN`转换成纳秒，如下所示：
...
由于[0]和[1]存在许多问题，因此建议避免尝试用它们实现此问题的解决方案。
...
从 JDK 8 开始，建议通过`Temporal`来表示日期时间信息（例如，`DateTime`、`DateLocalTime`、`ZonedDateTime`等）。
...
计算这两个`Temporal`对象之间差异的最简单方法是通过`ChronoUnit`类。除了表示一组标准的日期周期单位外，`ChronoUnit`还提供了几种简便的方法，包括`between(Temporal t1Inclusive, Temporal t2Exclusive)`。顾名思义，`between()`方法计算两个`Temporal`对象之间的时间量。让我们来看看计算`ld1`和`ld2`之间的差，以天、月、年为单位：
...
在`LocalDateTime`的情况下，`ChronoUnit.convert()`方法也很有用。让我们考虑以下两个`LocalDateTime`对象：2018 年 1 月 1 日 22:15:15 和 2019 年 3 月 1 日 23:15:15：
...
现在，让我们看看`ldt1`和`ldt2`的区别，用分钟表示：
...
但是，`ChronoUnit.between()`和`until()`有一点非常棒，那就是它们与`ZonedDateTime`一起工作。例如，让我们考虑`ldt1`在欧洲/布加勒斯特时区和在澳大利亚/珀斯时区，加上一小时：
...
现在，我们用`ChronoUnit.between()`来表示`zdt1`和`zdt2`之间的分差，`ZonedDateTime.until()`来表示`zdt1`和`zdt2`之间的分差：
...
从 JDK 8 开始，`java.time`包有一个名为`Clock`的抽象类。这个类的主要目的是允许我们在需要时插入不同的时钟（例如，出于测试目的）。默认情况下，Java 有四个实现：`SystemClock`、`OffsetClock`、`TickClock`和`FixedClock`。对于每个实现，`Clock`类中都有`static`个方法。例如，下面的代码创建了`FixedClock`（一个总是返回相同`Instant`的时钟）：

```
Clock fixedClock = Clock.fixed(Instant.now(), ZoneOffset.UTC);
```

还有一个`TickClock`，它返回给定时区的当前`Instant`滴答声（以整秒为单位）：

```
Clock tickClock = Clock.tickSeconds(ZoneId.of("Europe/Bucharest"));
```

还有一种方法可以在整分钟内打勾`tickMinutes()`，还有一种通用方法`tick()`，可以指定`Duration`。

一个`Clock`类也可以支持时区和偏移量，但是一个`Clock`类最重要的方法是`instant()`。此方法返回`Clock`的瞬间：
...
我们的`ChessClock`将只与 UTC 一起工作；不支持其他时区。这意味着`getZone()`和`withZone()`方法可以实现如下（当然，以后可以修改）：
...
我们实现的高潮是`instant()`方法。难度在于管理两个`Instant`，一个是左边玩家的`instantLeft`，一个是右边玩家的`instantRight`。我们可以将`instant()`方法的每一次调用都与当前玩家已经执行了一个移动相关联，现在轮到另一个玩家了。所以，基本上，这个逻辑是说同一个玩家不能调用`instant()`两次。实现这个逻辑，`instant()`方法如下：
...
因此，根据哪个玩家调用`instant()`方法，代码计算该玩家在执行移动之前思考所需的秒数。而且，代码会切换播放器，因此下一次调用`instant()`将处理另一个播放器。
