...
238.  **不要在设置器参数中使用`Optional`**：举例说明在 setter 参数中使用`Optional`的不良做法。
...
本场景的候选对象是 Java 反射 API`Method.invoke()`（见[第 7 章](07.html)、“Java 反射类、接口、构造器、方法、字段”。
...
*不要使用*类别继续使用一个非常诱人的场景，包括在 setter 参数中使用`Optional`。应避免使用以下代码，因为它添加了额外的样板代码，并且违反了“请勿将`Optional`用于字段”部分（请检查`setIsbn()`方法）：
