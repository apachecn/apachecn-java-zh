...
您还可以检查 CDI 插件（[https://github.com/Pscheidl/FortEEPavel Pscheidl 开发的 Java EE（雅加达 EE）的](https://github.com/Pscheidl/FortEE)。这是一个利用`Optional`模式的 Jakarta EE/javaee 容错保护。它的力量在于它的简单。
...
假设我们有一个方法，它基于[T0]类返回结果。如果该[T1]类为空，则该方法返回计算值。`computeStatus()`方法计算此值：
...
`isbn`的吸气剂返回`Optional`。但是不要将此示例视为以这种方式转换所有 getter 的规则。有些 getter 返回集合或数组，在这种情况下，他们更喜欢返回空的集合/数组，而不是返回[T2]。使用此技术并记住 BrianGoetz（Java 语言架构师）的声明：
...
在方法参数中使用`Optional`只是另一个用例，可能会导致代码变得不必要的复杂。主要是建议承担`null`检查参数的责任，而不是相信调用方会创建`Optional`类，尤其是空`Optional`类。这种糟糕的做法会使代码变得混乱，而且仍然容易出现`NullPointerException`。来电者仍可通过`null`。所以你刚才又开始检查[T6]参数了。
...
检查创建所需[T0]类的此方法的以下调用。但是，很明显，通过`null`也是可能的，会导致`NullPointerException`，但这意味着你故意挫败了`Optional`——不要想通过`null`参数的检查来污染前面的代码，这真是个坏主意：
...
在`identity-sensitive`操作的上下文中，千万不要这样做（认为`Optional`是一个基于值的类，这样的类不应该用于锁定更多细节，请参见[https://rules.sonarsource.com/java/tag/java8/RSPEC-3436](https://rules.sonarsource.com/java/tag/java8/RSPEC-3436)：
