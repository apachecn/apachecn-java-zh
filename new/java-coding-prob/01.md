...
2.  **查找第一个非重复字符**：编写一个程序，返回给定字符串中的第一个非重复字符。
3.  **反转字母和单词**：编写一个反转每个单词字母的程序和一个反转每个单词字母和单词本身的程序。
4.  **检查字符串是否只包含数字**：编写程序检查给定字符串是否只包含数字。

5.  **计算元音和辅音**：编写一个程序，计算给定字符串中元音和辅音的数量。对于英语，有五个元音（a、e、i、o 和 u）。
6.  **统计某个字符的出现次数**：编写一个程序，对给定字符串中某个字符的出现次数进行统计。
7.  **将`String`转换为`int`、`long`、`float`或`double`**：编写一个程序，将给定的`String`对象（代表数字）转换为`int`、`long`、`float`或`double`。
8.  **删除字符串中的空格**：编写一个程序，删除给定字符串中的所有空格。
9.  **用分隔符连接多个字符串**：编写一个程序，用给定的分隔符连接给定的字符串。
10.  **生成所有排列**：编写一个程序，生成给定字符串的所有排列。
11.  **检查字符串是否为回文**：编写一个程序，确定给定的字符串是否为回文。
12.  **删除重复字符**：编写一个程序，从给定字符串中删除重复字符。
13.  **删除给定字符**：编写从给定字符串中删除给定字符的程序。
14.  **查找出现次数最多的字符**：编写程序查找给定字符串中出现次数最多的字符。
15.  **按长度排序字符串数组**：编写按给定字符串数组长度排序的程序。
16.  **检查字符串是否包含子字符串**：编写程序检查给定字符串是否包含给定子字符串。
17.  **计算子字符串出现次数字符串**：编写一个程序，计算给定字符串在另一个给定字符串中出现的次数。
18.  **检查两个字符串是否为 anagram**：编写一个检查两个字符串是否为 anagram 的程序。假设一个字符串的一个 anagram 是这个字符串的一个排列，忽略了大小写和空格。
19.  **声明多行字符串（文本块）**：编写声明多行字符串或文本块的程序。
20.  **串接同一字符串*n*次**：编写一个程序，将同一字符串串接给定次数。
21.  **删除前导和尾随空格**：编写一个程序，删除给定字符串的前导和尾随空格。

22.  **查找最长公共前缀**：编写一个程序，查找给定字符串的最长公共前缀。
23.  **应用缩进**：编写几段代码，对给定的文本应用缩进。
24.  **转换字符串**：编写几段代码，将一个字符串转换成另一个字符串。
25.  **计算两个数字的最小值和最大值**：编写一个程序，返回两个数字的最小值和最大值。
26.  **两个大的`int`/`long`数求和，运算溢出**：编写一个程序，对两个大的`int`/`long`数求和，运算溢出时抛出算术异常。
27.  **字符串作为基数**中的无符号数：编写一个程序，将给定字符串解析为给定基数中的无符号数（`int`或`long`）。
28.  **无符号转换成数字**：编写一个程序，将给定的`int`数字无符号转换成`long`。
29.  **比较两个无符号数**：编写一个程序，将给定的两个数作为无符号数进行比较。
30.  **无符号值的除法和模**：编写一个程序，计算给定无符号值的除法和模。
31.  **`double`/`float`为有限浮点值**：编写程序确定给定的`double`/`float`值是否为有限浮点值。
32.  **将逻辑 AND/OR/XOR 应用于两个布尔表达式**：编写一个程序，将逻辑 AND/OR/XOR 应用于两个布尔表达式。
33.  **将**`BigInteger`**转换成原语类型**：编写程序，从给定的`BigInteger`中提取原语类型值。
34.  **将`long`转换为`int`**：编写一个将`long`转换为`int`的程序。
35.  **除法下限和模的计算**：编写程序，计算给定除法（*x*）和除数（*y*）的下限除法和下限模。
36.  **下一个浮点值**：编写一个程序，返回给定的`float`/`double`值附近的正负无穷方向的下一个浮点值。
37.  **两个大的`int`/`long`值相乘，运算溢出**：写一个程序，两个大的`int`/`long`值相乘，运算溢出时抛出算术异常。

38.  **融合乘加**（**FMA**）：编写一个程序，取三个`float`/`double`值（*a*、*b*、*c*），高效计算*a*b+c*。
39.  **压缩数字格式化**：编写一个程序，将数字 1000000 格式化为 1M（美国地区）和 1ML（意大利地区）。另外，将一个字符串中的 1M 和 1MLN 解析为一个数字。

# 解决

以下各节介绍上述问题的解决方案。记住，通常没有一个正确的方法来解决一个特定的问题。另外，请记住，这里显示的解释只包括解决问题所需的最有趣和最重要的细节。您可以从[下载示例解决方案以查看更多详细信息并尝试使用程序 https://github.com/PacktPublishing/Java-Coding-Problems](https://github.com/PacktPublishing/Java-Coding-Problems) .
...
3.  最后，将字符（`Collectors.groupingBy()`）分组并计数（`Collectors.counting()`）。
...
现在，一个高代词后跟一个低代词定义了所谓的**代词对**。代理项对用于表示 65536（0x10000）和 1114111（0x10FFFF）之间的值。因此，某些字符（称为 Unicode 补充字符）被表示为 Unicode 代理项对（一个字符（符号）适合一对字符的空间），这些代理项对被合并到一个代码点中。Java 利用了这种表示，并通过一组方法（如`codePointAt()`、`codePoints()`、`codePointCount()`）将其公开，和`offsetByCodePoints()`（请查看 Java 文档了解详细信息）。调用`codePointAt()`而不是`charAt()`，`codePoints()`而不是`chars()`等等，可以帮助我们编写覆盖 ASCII 和 Unicode 字符的解决方案。

例如，众所周知的 two hearts 符号是一个 Unicode 代理项对，它可以表示为一个包含两个值的`char[]`：`\uD83D`和`\uDC95`。此符号的码位为`128149`。要从此代码点获取一个`String`对象，请调用`String str = String.valueOf(Character.toChars(128149))`。通过调用`str.codePointCount(0, str.length())`可以对`str`中的码位进行计数，即使`str`长度为 2，也返回 1；调用`str.codePointAt(0)`返回`128149`，调用`str.codePointAt(1)`返回`56469`。调用`Character.toChars(128149)`返回 2，因为需要两个字符来表示此代码点是 Unicode 代理项对。对于 ASCII 和 Unicode 16 位字符，它将返回 1。

因此，如果我们尝试重写第一个解决方案（迭代字符串字符并使用`Map`将字符存储为键，将出现次数存储为值）以支持 ASCII 和 Unicode（包括代理项对），我们将获得以下代码：
...
最后，尝试重写 Java 8 函数式解决方案以覆盖 Unicode 代理项对，可以执行以下操作：
...
此解决方案假定字符串中的每个字符都是扩展 ASCII 表（256 个代码）的一部分。如果代码大于 256，则需要相应地增加数组的大小（[http://www.alansoficespace.com/unicode/unicd99.htm](http://www.alansofficespace.com/unicode/unicd99.htm) ). 只要数组大小不超过`char`类型的最大值，即`Character.MAX_VALUE`，即 65535，该解决方案就可以工作。另一方面，`Character.MAX_CODE_POINT`返回 Unicode 码位的最大值 1114111。为了覆盖这个范围，我们需要另一个基于`codePointAt()`和`codePoints()`的实现
...
这里介绍的另一个解决方案依赖于`LinkedHashMap`。这个 Java 映射是一个*插入顺序*映射（它保持了键插入到映射中的顺序），对于这个解决方案非常方便。`LinkedHashMap`填充字符作为键，出现次数作为值。一旦`LinkedHashMap`完成，它将返回第一个值等于 1 的键。由于*插入顺序*功能，这是字符串中第一个不可重复的字符：
...
要更详细地理解此代码，请考虑[0]Unicode 字符是什么？ *计数重复字符*小节

# 三。反转字母和单词

首先，让我们只反转每个单词的字母。这个问题的解决方案可以利用`StringBuilder`类。第一步是使用空格作为分隔符（`Spring.split(" ")`将字符串拆分为一个单词数组。此外，我们使用相应的 ASCII 码反转每个单词，并将结果附加到`StringBuilder`。首先，我们将给定的字符串按空格拆分。然后，我们循环得到的单词数组，并通过`charAt()`按相反的顺序获取每个字符来反转每个单词：
...
对于第三方库支持，请考虑 Apache Commons Lang，`StringUtils.reverse()`。

# 4 检查字符串是否只包含数字

这个问题的解决依赖于`Character.isDigit()`或`String.matches()`方法

依赖于`Character.isDigit()`的解决方案非常简单，如果该方法返回`false`，则快速循环字符串并中断循环：
...
在 Java 8 函数式中，前面的代码可以使用`anyMatch()`重写：
...
另一个解决方案依赖于`String.matches()`。此方法返回一个[1]值，指示此字符串是否与给定的正则表达式匹配：
...
请注意，Java8 函数式和基于正则表达式的解决方案通常比较慢，因此如果速度是一个要求，那么最好依赖于使用[0]的第一个解决方案。

避免通过`parseInt()`或`parseLong()`解决此问题。首先，抓住`NumberFormatException`并在`catch`块中做出业务逻辑决策是不好的做法。其次，这些方法验证字符串是否为有效数字，而不是仅包含数字（例如，-4 有效）。
对于第三方库支持，请考虑 apachecommons Lang，`StringUtils.isNumeric()`。
...
注意，最初，给定的[0]对象被转换为小写。这有助于避免与大写字符进行比较。例如，只对`'a'`进行比较，而不是对`'A'`和`'a'`进行比较。
...
在 Java 8 函数式中，可以使用`chars()`和`filter()`重写此代码：
...
对于第三方库支持，请考虑 Apache Commons Lang、[0]、Spring Framework、[1]和 Guava、[2]。
...
将`String`转换成`Integer`、`Long`、`Float`或`Double`对象，可以通过以下 Java 方法完成—`Integer.valueOf()`、`Long.valueOf()`、`Float.valueOf()`、`Double.valueOf()`：
...
当[0]无法成功转换时，Java 抛出一个[1]异常。以下代码不言自明：
...
对于第三方库支持，请考虑 Apache Commons BeanUtils:`IntegerConverter`、`LongConverter`、`FloatConverter`和`DoubleConverter`。

# 8 从字符串中删除空格

这个问题的解决方案是使用带有[1]正则表达式的[0]方法。主要是`\s`删除所有的空白，包括不可见的空白，如`\t`、`\n`、`\r`：
...
从 JDK 11 开始，`String.isBlank()`检查字符串是空的还是只包含空格代码点。对于第三方库支持，请考虑 Apache Commons Lang，`StringUtils.deleteWhitespace()`和 Spring 框架，`StringUtils.trimAllWhitespace()`。
...
另一种解决方案依赖于`String.join()`方法。此方法是在 Java 8 中引入的，有两种风格：
...
注意通过`+=`运算符以及`concat()`和`String.format()`方法连接字符串。这些字符串可以用于连接多个字符串，但它们容易导致性能下降。例如，下面的代码依赖于`+=`，比依赖于`StringBuilder`慢得多：
...
`+=`被追加到一个字符串中，并重建一个新的字符串，这需要时间。

对于第三方库支持，请考虑 Apache Commons Lang，`StringUtils.join()`和 Guava，`Joiner`。

# 10 生成所有置换

涉及置换的问题通常也涉及*递归性*。基本上，递归性被定义为一个过程，其中给定了一些初始状态，每个*连续状态*根据*前一状态*定义。
...
将此解决方案表示为字符串[0]，可以这样做（注意排列是如何完成的）：
...
最初，前缀应该是空字符串`""`。在每次迭代中，前缀将连接（固定）字符串中的下一个字母。剩下的字母将再次通过该方法传递。

假设此方法存在于名为`Strings`的实用程序类中。你可以这样称呼它：
...
例如，如果传递的字符串是`TEST`，则`Set`将导致以下输出（这些都是唯一的排列）：
...
有 24 个排列。通过计算*n*阶乘（*n！* ). 对于*n=4*（字符串长度），*4！=1 x 2 x 3 x 4=24*。当用递归方式表示时，这是*n！=nx（n-1）！* .

自*n！* 以极快的速度生成大量数据（例如，*10！=3628800*），建议避免存储结果。对于 10 个字符的字符串（如直升机），有 3628800 个排列！

尝试用 Java 8 函数式实现此解决方案将导致如下结果：
...
作为一个快速提醒，*回文*（无论是字符串还是数字）在反转时看起来没有变化。这意味着可以从两个方向处理（读取）回文，并得到相同的结果（例如，单词*madam*是回文，而单词*madam*不是）。

一个易于实现的解决方案是用*中间相遇*的方法比较给定字符串的字母。基本上，此解决方案将第一个字符与最后一个字符进行比较，第二个字符与最后一个字符逐个进行比较，依此类推，直到到达字符串的中间。实现依赖于`while`语句：
...
用更简洁的方法重写前面的解决方案将包括依赖于一个`for`语句而不是一个`while`语句，如下所示：
...
在 Java8 函数风格中，也有一行代码用于此。只需定义从 0 到给定字符串的一半的[0]，并使用[1][2]短路[3]终端操作和谓词，按照[4]中间相遇[5]方法比较字母：
...
`StringBuilder`类公开了一个名为`indexOf()`的方法，该方法返回指定子字符串（在本例中是指定字符）第一次出现的给定字符串中的索引。因此，这个问题的一个潜在解决方案是循环给定字符串的字符，并在应用于当前字符的`indexOf()`方法每次返回-1 时将它们逐个添加到`StringBuilder`（这个负数意味着`StringBuilder`不包含当前字符）：
...
下一个解决方案依赖于`HashSet`和`StringBuilder`之间的协作。主要是，`HashSet`确保消除重复项，而`StringBuilder`存储结果字符串。如果`HashSet.add()`返回`true`，那么我们也在`StringBuilder`中添加字符：
...
首先，解决方案将给定的字符串转换为`Stream<String>`，其中每个条目实际上是一个字符。此外，该解决方案还应用了有状态的中间操作`distinct()`。此操作将从流中消除重复项，因此它返回一个没有重复项的流。最后，该解决方案调用`collect()`终端操作并依赖于`Collectors.joining()`，该操作简单地将字符按相遇顺序连接成一个字符串。

# 13 删除给定字符

依赖 JDK 支持的解决方案可以利用[0]方法。此方法将给定字符串中与给定正则表达式（在本例中，正则表达式是字符本身）匹配的每个子字符串（在本例中，每个字符）替换为给定的替换（在本例中，替换为空字符串，`""`）：
...
或者，如果我们想要移除一个 Unicode 代理项对，那么我们可以依赖于`codePointAt()`和`codePoints()`，如下面的实现所示：
...
对于第三方库支持，请考虑 Apache Commons Lang，`StringUtils.remove()`。
...
1.  首先，循环给定字符串的字符，并将键值对放在`HashMap`中，其中键是当前字符，值是当前出现的次数
2.  其次，计算`HashMap`中的最大值（例如，使用`Collections.max()`），表示出现的最大次数
3.  最后，通过循环[0]条目集，获得出现次数最多的字符

utility 方法返回[0]，其中包含出现次数最多的字符和出现次数（请注意，忽略了空格）。如果你不喜欢这个额外的课程，也就是说，`Pair`，那就依靠`Map.Entry<K, V>`：
...
如果使用[0]看起来很麻烦，那么另一个解决方案（稍微快一点）就是依赖 ASCII 码。此解决方案从 256 个索引的空数组开始（256 是扩展 ASCII 表代码的最大数目；更多信息可在*查找第一个非重复字符*部分中找到）。此外，此解决方案循环给定字符串的字符，并通过增加此数组中的相应索引来跟踪每个字符的出现次数：
...
我们将在这里讨论的最后一个解决方案依赖于 Java 8 功能风格：
...
首先，此解决方案将不同的字符收集为`Map`中的键，并将它们的出现次数作为值。此外，它使用 java8`Map.Entry.comparingByValue()`和`max()`终端操作来确定地图中具有最高值（最高出现次数）的条目。因为`max()`是一个终端操作，所以解决方案可以返回`Optional<Entry<Character, Long>>`，但是这个解决方案增加了一个额外的步骤，并将这个条目映射到`Pair<Character, Long>`。

# 15 按长度对字符串数组排序

排序时首先想到的是比较器的使用。

在这种情况下，解决方案应该比较字符串的长度，因此通过为给定数组中的每个字符串调用`String.length()`来返回整数。因此，如果整数被排序（升序或降序），那么字符串将被排序。

Java`Arrays`类已经提供了一个`sort()`方法，该方法使用数组进行排序，并提供了一个比较器。在这种情况下，`Comparator<String>`应该做这项工作。
...
原语数字类型的每个包装器都有一个`compare()`方法。

从 Java8 开始，`Comparator`接口丰富了大量有用的方法。其中一个方法是`comparingInt()`，它接受一个函数，该函数从泛型类型中提取`int`排序键，并返回一个`Comparator<T>`值，将其与该排序键进行比较。另一个有用的方法是`reversed()`，它反转当前的`Comparator`值。
...
我们在这里给出的解决方案返回`void`，这意味着它们对给定数组进行排序。为了返回一个新的排序数组而不改变给定的数组，我们可以使用 Java 8 函数样式，如下代码片段所示：
...
因此，代码从给定数组创建一个流，通过有状态的中间操作对其进行排序，并将结果收集到另一个数组中。
...
注意，正则表达式被包装在`Pattern.quote()`方法中。这是转义特殊字符（如“[1]”（[{\^-=$”）所必需的！|]})?*+. > 在给定的子串中。

对于第三方库支持，请考虑 Apache Commons Lang，`StringUtils.containsIgnoreCase()`。
...
在第一种情况下（111 中的 11 出现 1 次），可依赖于`String.indexOf()`方法求解。此方法的一种风格允许我们从指定的索引（如果没有这样的索引，则为-1）开始获取指定子字符串第一次出现的字符串中的索引。基于此方法，该解决方案可以简单地遍历给定的字符串并计算给定子字符串的出现次数。遍历从位置 0 开始，直到找不到子字符串为止：
...
# 18 检查两个字符串是否为 anagram

两个具有相同字符但顺序不同的字符串是 anagram。一些定义强制要求字谜不区分大小写和/或应忽略空格（空格）。
...
在遍历结束时，如果给定的字符串是 anagram，那么这个数组只包含 0。

# 19 声明多行字符串（文本块）

在撰写本书时，JDK12 提出了一个添加多行字符串的建议，称为[0]JEP326：原始字符串文本[1]。但这是在最后一刻掉的

从 JDK13 开始，重新考虑了这个想法，与被拒绝的原始字符串文字不同，文本块被三个双引号包围，`"""`，如下所示：
...
文本块对于编写多行 SQL 语句、使用 polyglot 语言等非常有用。详情请参见[https://openjdk.java.net/jeps/355](https://openjdk.java.net/jeps/355) .
...
从 JDK 8 开始，解决方案可能依赖于`String.join()`，如下所示：
...
在 JDK 8 之前，优雅的解决方案可能依赖于`StringBuilder`。本书附带的代码中提供了此解决方案。
...
如何处理多行字符串的每一行？好吧，一个快速的方法需要 JDK 11，它与`String.lines()`方法一起提供。该方法通过一个行分隔符（支持`\n`、`\r`、`\r\n`）对给定的字符串进行拆分，并将其转换为`Stream<String>`。或者，也可以使用`String.split()`方法（从 JDK1.4 开始提供），如果字符串的数量变得重要，建议将其放入一个文件中，并逐个读取/处理（例如，通过`getResourceAsStream()`方法），其他方法依赖于`StringWriter`或`BufferedWriter.newLine()`。

对于第三方库支持，请考虑 Apache Commons Lang、`StringUtils.join()`、Guava、`Joiner`和自定义注释`@Multiline`。

# 20 连接相同字符串 n 次

在 JDK 11 之前，可以通过`StringBuilder`快速提供解决方案，如下：
...
从 JDK11 开始，解决方案依赖于`String.repeat(int count)`方法。此方法返回一个字符串，该字符串是将此字符串连接[1]次而得到的。在幕后，这个方法使用了`System.arraycopy()`，这使得这个速度非常快：
...
对于第三方库支持，请考虑 Apache Commons Lang，`StringUtils.repeat()`和 Guava，`Strings.repeat()`。
...
该解决方案循环给定字符串的一半，并通过在`StringBuilder`中逐个字符地追加原始字符串，逐步将其替换为子字符串生成`""`。如果这些替换结果是空字符串，则表示给定的字符串是相同子字符串的序列。

# 21 删除前导空格和尾随空格

这个问题最快的解决方法可能依赖于`String.trim()`方法。此方法能够删除所有前导和尾随空格，即代码点小于或等于 U+0020 或 32 的任何字符（空格字符）：
...
前面的代码片段将按预期工作。修剪后的字符串将为`hello`。这只适用于所有正在使用的空格小于 U+0020 或 32（空格字符）。有 25 个字符（[https://en.wikipedia.org/wiki/Whitespace#Unicode 字符](https://en.wikipedia.org/wiki/Whitespace_character#Unicode) ）定义为空格，`trim()`只覆盖其中的一部分（简而言之，`trim()`不知道 Unicode）。让我们考虑以下字符串：
...
`\u2002`是`trim()`无法识别的另一种类型的空白（`\u2002`在`\u0020`之上）。这意味着，在这种情况下，`trim()`将无法按预期工作。从 JDK11 开始，这个问题有一个名为`strip()`的解决方案。此方法将`trim()`的威力扩展到 Unicode 领域：
...
此外，JDK11 还提供了两种类型的`strip()`，用于仅删除前导（`stripLeading()`）或尾随（`stripTrailing()`）空格。`trim()`方法没有这些味道。
...
这个问题的其他解决方案使用众所周知的算法，如**二进制搜索**或**Trie**。在本书附带的源代码中，还有一个基于二进制搜索的解决方案。
...
假设我们有一个字符串，我们想把它转换成另一个字符串（例如，把它转换成大写）。我们可以通过应用像`Function<? super String,​ ? extends R>`这样的函数来实现。

在 JDK 8 中，我们可以通过`map()`来实现，如下两个简单示例所示：
...
虽然`map()`更一般，但`transform()`专用于对字符串应用函数并返回结果字符串。

# 25 计算两个数的最小值和最大值

在 JDK 8 之前，一个可能的解决方案是依赖于`Math.min()`和`Math.max()`方法，如下所示：
...
[0]类为每个基元数字类型（[3]、[4]、[5]和[6]）提供了一个[1]和一个[2]方法。

从 JDK 8 开始，每个原始数字类型的包装器类（`Integer`、`Long`、`Float`和`Double`都有专用的`min()`和`max()`方法，在这些方法后面，还有来自`Math`类的对应调用。请参见下面的示例（这是一个更具表现力的示例）：
...
在函数式上下文中，潜在的解决方案将依赖于`BinaryOperator`函数接口。此接口有两种方式，`minBy()`和`maxBy()`：
...
这是一种非常简单的方法，适用于大多数涉及[0]、[1]、[2]和[3]的计算。
...
此时，`z`将等于-2，这不是预期的结果，即 4294967294。仅将`z`类型从`int`更改为`long`将无济于事。但是，将`x`和`y`的类型从`int`改为`long`以及*会有*的帮助：
...
从 JDK8 开始，`+`操作符被一个原语数字类型的包装器以一种更具表现力的方式包装。因此，`Integer`、`Long`、`Float`和`Double`类有一个`sum()`方法：

```
long z = Long.sum(); // -2
```

在幕后，`sum()`方法也使用了`+`操作符，因此它们只产生相同的结果。

但同样从 JDK8 开始，`Math`类用两个`addExact()`方法进行了丰富。一个`addExact()`用于两个`int`变量的求和，一个用于两个`long`变量的求和。如果结果容易溢出`int`或`long`，这些方法非常有用，如前面的例子所示。在这种情况下，这些方法抛出`ArithmeticException`，而不是返回误导性的结果，如下例所示：
...
在函数式上下文中，可能的解决方案将依赖于`BinaryOperator`函数接口，如下所示（只需定义相同类型的两个操作数的操作）：
...
除`addExact()`外，`Math`还有`multiplyExact()`、`substractExact()`、`negateExact()`。此外，众所周知的增量和减量表达式`i++`和`i--`可以通过`incrementExact()`和`decrementExact()`方法（例如`Math.incrementExact(i)`）控制其域溢出。请注意，这些方法仅适用于`int`和`long`。

在处理大数时，还要关注`BigInteger`（不可变的任意精度整数）和`BigDecimal`（不可变的任意精度有符号十进制数）类。

# 27 字符串作为基数中的无符号数

对无符号算术的支持从版本 8 开始添加到 Java 中。受此影响最大的是`Byte`、`Short`、`Integer`和`Long`类。

在 Java 中，表示正数的字符串可以通过[2]和[3]JDK 8 方法解析为无符号的[0]和[1]类型。例如，我们将以下整数视为字符串：
...
第一个参数是数字，第二个参数是基数。基数应在[2，36]或[0]范围内。
...
从 JDK9 开始，`parseUnsignedInt()`有了新的味道。除了字符串和基数之外，这个方法还接受一个[1]类型的范围。这一次，解析是在这个范围内完成的。例如，可以按如下方式指定范围[1，3]：
...
对于`Long`类中的长数字存在相同的方法集（例如，`parseUnsignedLong()`。

# 28 通过无符号转换转换成数字

这个问题需要我们通过无符号转换将给定的有符号的`int`转换为`long`。那么，让我们考虑签名的`Integer.MIN_VALUE`，即-2147483648。

在 JDK 8 中，使用`Integer.toUnsignedLong()`方法，转换如下（结果为 2147483648）：

```
long result = Integer.toUnsignedLong(Integer.MIN_VALUE);
```

下面是另一个将有符号的[0]和[1]转换为无符号整数的示例：
...
同一类别的其他方法有`Integer.toUnsignedString()`、`Long.toUnsignedString()`、`Byte.toUnsignedInt()`、`Byte.toUnsignedLong()`、`Short.toUnsignedInt()`、`Short.toUnsignedLong()`。
...
在 jdk8 中，这两个整数可以通过`Integer.compareUnsigned()`方法作为无符号值进行比较（这相当于无符号值的`Integer.compare()`）。该方法主要忽略了符号位的概念，最左边的位被认为是最高有效位。在无符号值保护伞下，如果比较的数字相等，则此方法返回 0；如果第一个无符号值小于第二个无符号值，则此方法返回小于 0 的值；如果第一个无符号值大于第二个无符号值，则此方法返回大于 0 的值。
...
在以 JDK 8 开头的`Integer`和`Long`类中，以及以 JDK 9 开头的`Byte`和`Short`类中，都可以使用`compareUnsigned()`方法。

# 30 无符号值的除法和模

JDK 8 无符号算术 API 通过`divideUnsigned()`和`remainderUnsigned()`方法支持计算两个无符号值的除法所得的无符号商和余数。

让我们考虑[0]和[1]有符号数，然后应用除法和模。这里没有什么新鲜事：
...
# 32 对两个布尔表达式应用逻辑 AND/OR/XOR

初等逻辑运算的真值表（**与**、**或**、**异或**如下：

![](img/482c6d19-72f9-41a3-9590-52cde694d0a0.png)

在 Java 中，逻辑**和**运算符表示为`&&`，逻辑**或**运算符表示为`||`，逻辑**异或**运算符表示为`^`。从 JDK 8 开始，这些运算符应用于两个布尔值，并被包装在三种方法中-`Boolean.logicalAnd()`、`Boolean.logicalOr()`和`Boolean.logicalXor()`：
...
# 33 将 BigInteger 转换为基元类型

[0]类是表示不可变的任意精度整数的非常方便的工具。

此类还包含用于将`BigInteger`转换为`byte`、`long`或`double`等基元类型的方法（源于`java.lang.Number`）。然而，这些方法会产生意想不到的结果和混乱。例如，假设我们有`BigInteger`来包装`Long.MAX_VALUE`：

```
BigInteger nr = BigInteger.valueOf(Long.MAX_VALUE);
```

让我们通过`BigInteger.longValue()`方法将这个`BigInteger`转换成一个原语 long：

```
long nrLong = nr.longValue();
```

到目前为止，由于`Long.MAX_VALUE`是 9223372036854775807，`nrLong`原语变量正好有这个值，所以一切都按预期进行。

现在，我们尝试通过`BigInteger.intValue()`方法将这个`BigInteger`类转换成一个原始的`int`值：

```
int nrInt = nr.intValue();
```

这一次，`nrInt`原语变量的值将为-1（相同的结果将产生`shortValue()`和`byteValue()`。根据文档，如果`BigInteger`的值太大，无法容纳指定的基元类型，则只返回低位*n*位（*n*取决于指定的基元类型）。但是如果代码没有意识到这个语句，那么它将在进一步的计算中把值推为-1，这将导致混淆。

但是，从 jdk8 开始，添加了一组新的方法。这些方法专门用于识别从`BigInteger`转换为指定的基元类型时丢失的信息。如果检测到丢失信息，则抛出`ArithmeticException`。这样，代码表示转换遇到了一些问题，并防止了这种不愉快的情况。
...
注意，`intValueExact()`没有将-1 作为`intValue()`返回。这一次，由于试图将最大的`long`值转换为`int`而导致的信息丢失通过`ArithmeticException`类型的异常发出信号。

# 34 将 long 转换为 int

将[0]值转换为[1]值似乎很容易。例如，潜在的解决方案可以依赖于以下条件：
...
两种方法都很有效。现在，假设我们有以下[0]值：

```
long nrMaxLong = Long.MAX_VALUE;
```

这一次，两种方法都将返回-1。为了避免这种结果，建议使用 JDK 8，即[0]。此方法获取一个`long`类型的参数，并尝试将其转换为`int`。如果获取的值溢出了`int`，则该方法抛出`ArithmeticException`：
...
到目前为止的结论是，两个符号相同的整数（都是正的或负的）的除法底可以通过[0]算子得到。
...
从 jdk8 开始，所有这些病例都通过`Math.floorDiv()`方法被覆盖和暴露。此方法以表示被除数和除数的两个整数作为参数，返回小于或等于代数商的最大值（最接近正无穷大）`int`值：
...
在`Math.floorDiv()`之后，JDK 8 附带了`Math.floorMod()`，它返回给定参数的楼层模数。这是作为`x - (floorDiv(x, y) * y)`的结果计算的，因此对于符号相同的参数，它将返回与`%`运算符相同的结果；对于符号不相同的参数，它将返回不同的结果。
...
有一个整数值，比如 10，使得我们很容易得到下一个整数值，比如 10+1（在正无穷方向）或者 10-1（在负无穷方向）。尝试为`float`或`double`实现相同的目标并不像为整数那么容易。

从 JDK6 开始，`Math`类通过`nextAfter()`方法得到了丰富。此方法接受两个参数，即初始数字（`float`或`double`）和方向（`Float`/`Double.NEGATIVE`/`POSITIVE_INFINITY`），并返回下一个浮点值。在这里，返回 0.1 附近的下一个负无穷方向的浮点值是这种方法的一个特色：
...
从 jdk8 开始，`Math`类被丰富了两个方法，它们充当了`nextAfter()`的快捷方式，而且速度更快。这些方法是`nextDown()`和`nextUp()`：
...
因此，负无穷大方向的`nextAfter()`可通过`Math.nextDown()`和`nextAfter()`获得，而正无穷大方向的`nextAfter()`可通过`Math.nextUp()`获得。

# 37.将两个大 int/long 值相乘，操作溢出
...
这是一种非常简单的方法，适用于大多数涉及到[0]、[1]、[2]和[3]的计算。
...
因此，溢出域并依赖于[0]运算符的计算将最终导致错误的结果。

与其在进一步的计算中使用这些结果，不如在发生溢出操作时及时得到通知。JDK 8 附带了`Math.multiplyExact()`方法。此方法尝试将两个整数相乘。如果结果溢出，`int`只抛出`ArithmeticException`：
...
jdk9 附带了返回值为[1]的[0]。这种方法对于求两个整数的精确数学积非常有用，如下所示：
...
作为记录，jdk9 还附带了一个名为`Math.muliptlyHigh(long x, long y)`的方法，返回一个`long`。此方法返回的`long`值表示两个 64 位因子的 128 位乘积的最高有效 64 位：
...
对于处理大数，还应关注`BigInteger`（不可变的任意精度整数）和`BigDecimal`（不可变的任意精度有符号十进制数）类。

# 38 融合乘法加法

数学计算*（a*b）+c*在矩阵乘法中被大量利用，常用于**高性能计算**（**HPC**）、人工智能应用、机器学习、深度学习、神经网络等。

实现此计算的最简单方法直接依赖于[0]和[1]运算符，如下所示：
...
不过，多亏了英特尔 AVX 执行 SIMD 操作的指令和添加了`Math.fma()`方法的 JDK 9，这种计算才得以提高。依靠`Math.fma()`，使用“四舍五入到最接近的偶数四舍五入”模式只进行一次四舍五入：
...
从 JDK12 开始，添加了一个用于紧凑数字格式的新类。此类名为`java.text.CompactNumberFormat`。这个类的主要目标是扩展现有的 Java 数字格式化 API，支持区域设置和压缩。

数字可以格式化为短格式（例如，*1000*变成*1K*），也可以格式化为长格式（例如，*1000*变成*1000*）。这两种风格在`Style`枚举中分为`SHORT`和`LONG`。

除了`CompactNumberFormat`构造函数外，还可以通过`NumberFormat`类中添加的两个`static`方法创建`CompactNumberFormat`：

*   第一个是默认语言环境的压缩数字格式，其中包含[0]：

```
public static NumberFormat getCompactNumberInstance()
```

*   第二个是指定区域设置的压缩数字格式，其中包含[0]：
...
尝试将此信息压缩为一个名为`NumberFormatters`的实用程序类可以实现如下目的：
...
现在，我们将数字*1000*、*1000000*和*1000000000*格式化为`US`区域设置、`SHORT`样式和默认舍入模式：
...
最后，我们还可以使用`ITALIAN`区域设置和`LONG`样式：
...
从取整方式来看，分别取整为*1000*和*2000*。默认取整模式`HALF_EVEN`将*1200*取整为*1000*，将*1600*取整为*2000*。但是如果我们想让*1200*变成*2000*，让*1600*变成*1000*，那么我们需要明确设置取整模式如下：
...
解析是格式化的相反过程。我们有一个给定的字符串，并尝试将其解析为一个数字。这可以通过`NumberFormat.parse()`方法实现。默认情况下，解析不利用分组（例如，不分组时，*5,50k*解析为*5*；分组时，*5,50k*解析为*550000*）。
...
现在，我们用显式分组解析*5,50K*和*5,5 万*到*55 万*：
...
通过`setCurrency​()`、`setParseIntegerOnly()`、`setMaximumIntegerDigits()`、`setMinimumIntegerDigits()`、`setMinimumFractionDigits()`、`setMaximumFractionDigits()`方法可以获得更多的调谐。
