...
17.  **计算子串出现的次数字符串**：编写一个程序，计算给定字符串在另一个给定字符串中出现的次数。
18.  **检查两个字符串是否是 anagram**：编写一个检查两个字符串是否是 anagram 的程序。假设一个字符串的一个 anagram 是这个字符串的一个排列，忽略了大小写和空格。
...
26.  **两个大`int`/`long`数求和，运算溢出**：编写一个程序，对两个大`int`/`long`数求和，运算溢出时抛出算术异常。
...
31.  **`double`/`float`是一个有限浮点值**：编写一个程序来确定给定的`double`/`float`值是否是一个有限浮点值。
...
35.  **计算除法的下限和模**：编写程序，计算给定除法（*x*）和除法（*y*）的下限除法和下限模。
...
*   16 位*低代理*：1024 个值（U+DC00 到 U+DFFF）
...
要更详细地理解这段代码，请考虑[T0]Unicode 字符是什么？*计数重复字符*部分的小节
...
对于第三方库支持，请考虑 apachecommons Lang，`StringUtils.reverse()`。
...
将这个解决方案表示为字符串[T0]，可以这样做（注意排列是如何完成的）：
...
有 24 个排列。通过计算*n*阶乘（*n！*）。对于*n=4*（字符串长度），*4！=1 x 2 x 3 x 4=24*。当以递归方式表示时，这是*n！=nx（n-1）！*。
...
对于第三方库支持，请考虑 apachecommons Lang，`StringUtils.remove()`。
...
utility 方法返回`Pair<Character, Integer>`，其中包含出现次数最多的字符和出现次数（注意，忽略了空格）。如果你不喜欢这个额外的课程，也就是说，`Pair`，那就依赖`Map.Entry<K, V>`：
...
我们在这里给出的解决方案返回`void`，这意味着它们对给定的数组进行排序。为了返回一个新的排序数组而不改变给定的数组，我们可以使用 Java 8 函数样式，如下代码片段所示：
...
对于第三方库支持，请考虑 apachecommons Lang，`StringUtils.containsIgnoreCase()`。
...
在写这本书的时候，JDK12 有一个关于添加多行字符串的建议，称为*JEP326：原始字符串文字*。但这是在最后一刻掉的
...
文本块对于编写多行 SQL 语句、使用 polyglot 语言等非常有用。更多详情见[https://openjdk.java.net/jeps/355](https://openjdk.java.net/jeps/355)。
...
从 JDK 8 开始，解决方案可以依赖于`String.join()`，如下所示：
...
对于第三方库支持，请考虑 apachecommons Lang，`StringUtils.repeat()`和 Guava，`Strings.repeat()`。
...
这个问题的其他解决方案使用众所周知的算法，例如**二进制搜索**或**Trie**。在本书附带的源代码中，还有一个基于二进制搜索的解决方案。
...
在 JDK 8 中，我们可以通过`map()`来实现，如下两个简单的例子所示：
...
`Math`类为每个基元数字类型（`int`、`long`、`float`和`double`提供了`min()`和`max()`方法。
...
在功能风格的上下文中，潜在的解决方案将依赖于`BinaryOperator`功能接口。此接口有两种方式，`minBy()`和`maxBy()`：
...
此时，`z`将等于-2，这不是预期的结果，即 4294967294。仅将`z`类型从`int`更改为`long`将无济于事。但是，将`x`和`y`的类型从`int`改为`long`以及*将*有助于：
...
从 jdk8 开始，`+`操作符被一个原语数字类型的包装器以一种更具表现力的方式包装。因此，`Integer`、`Long`、`Float`和`Double`类具有`sum()`方法：
...
在函数式上下文中，潜在的解决方案将依赖于`BinaryOperator`函数接口，如下所示（只需定义相同类型的两个操作数的操作）：
...
在 Java 中，表示正数的字符串可以通过[T2]和[T3]JDK 8 方法解析为无符号的[T0]和[T1]类型。例如，我们将以下整数视为字符串：
...
第一个参数是数字，第二个参数是基数。基数应在[2，36]或[T0]范围内。
...
在 jdk8 中，这两个整数可以通过`Integer.compareUnsigned()`方法作为无符号值进行比较（这相当于无符号值的`Integer.compare()`）。该方法主要忽略了符号位 T2 的概念 T3，最左边的 T4 被认为是最重要的位 T5。在无符号值保护伞下，如果比较的数字相等，则此方法返回 0；如果第一个无符号值小于第二个无符号值，则此方法返回小于 0 的值；如果第一个无符号值大于第二个无符号值，则此方法返回大于 0 的值。
...
让我们考虑一下[T0]和[T1]有符号数，然后应用除法和模。这里没有什么新鲜事：
...
基本逻辑运算的真值表（**和**、**或**、**异或**如下：

![](img/482c6d19-72f9-41a3-9590-52cde694d0a0.png)

在 Java 中，逻辑**和**运算符表示为`&&`，逻辑**或**运算符表示为`||`，逻辑**异或**运算符表示为`^`。从 JDK 8 开始，这些运算符被应用于两个布尔值，并被包装在三个`static`方法中—`Boolean.logicalAnd()`、`Boolean.logicalOr()`和`Boolean.logicalXor()`：
...
此类还包含用于将`BigInteger`转换为`byte`、`long`或`double`等基元类型的方法（源于`java.lang.Number`）。然而，这些方法会产生意想不到的结果和混乱。例如，假设有`BigInteger`包裹`Long.MAX_VALUE`：
...
到目前为止，由于`Long.MAX_VALUE`是 9223372036854775807，`nrLong`原语变量正好有这个值，所以一切都按预期工作。
...
此时，`nrInt`原语变量的值将为-1（相同的结果将产生`shortValue()`和`byteValue()`。根据文档，如果`BigInteger`的值太大，无法容纳指定的基元类型，则只返回低位*n*位（*n*取决于指定的基元类型）。但是如果代码没有意识到这个语句，那么它将在进一步的计算中把值推为-1，这将导致混淆。

但是，从 jdk8 开始，添加了一组新的方法。这些方法专门用于识别从`BigInteger`转换为指定的原语类型过程中丢失的信息。如果检测到丢失的信息，则抛出`ArithmeticException`。这样，代码表示转换遇到了一些问题，并防止了这种不愉快的情况。
...
因此，溢出域并依赖于[T0]运算符的计算将最终导致误导性结果。
...
JDK9 带有返回值为 T1 的`Math.multiplyFull(int x, int y)`。此方法对于获得两个整数的精确数学积`long`非常有用，如下所示：
...
数学计算*（a*b）+c*在矩阵乘法中被大量利用，在**高性能计算**（**高性能计算**）、人工智能应用、机器学习、深度学习、神经网络等领域有着广泛的应用。
...
除了`CompactNumberFormat`构造函数外，还可以通过两个`static`方法创建`CompactNumberFormat`，这两个方法被添加到`NumberFormat`类中：

*   第一种是默认语言环境的压缩数字格式，带有`NumberFormat.Style.SHORT`：
...
尝试将这些信息压缩成一个名为`NumberFormatters`的实用程序类可以实现如下：
...
让我们将*5K*和*5000*解析为*5000*，而不显式分组：
