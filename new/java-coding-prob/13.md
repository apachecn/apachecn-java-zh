...
你还记得`HttpUrlConnection`吗？好吧，JDK 11 附带了 HTTP 客户机 API，它是对[1]的重新发明。HTTP 客户端 API 易于使用，支持 HTTP/2（默认）和 HTTP/1.1。为了向后兼容，当服务器不支持 HTTP/2 时，HTTP 客户端 API 将自动从 HTTP/2 降级到 HTTP 1.1。此外，HTTP 客户机 API 支持同步和异步编程模型，并依赖流来传输数据（反应流）。它还支持 WebSocket 协议，该协议用于实时 web 应用程序，以较低的消息开销提供客户机-服务器通信。
...
252.  **设置代理**：编写一个程序，使用 HTTP 客户端 API 通过代理建立连接。
253.  **设置/获取报头**：编写一个程序，在请求中添加额外的报头，获取响应的报头。

254.  **指定 HTTP 方法**：编写指定请求的 HTTP 方法的程序（如`GET`、`POST`、`PUT`、`DELETE`）。
255.  **设置请求主体**：编写一个程序，使用 HTTP 客户端 API 为请求添加主体。
256.  **设置连接认证**：编写一个程序，使用 HTTP 客户端 API 通过用户名和密码设置连接认证。
257.  **设置超时**：编写一个程序，使用 HTTP 客户端 API 设置等待响应的时间量（超时）。
258.  **设置重定向策略**：编写一个程序，根据需要使用 HTTP 客户端 API 自动重定向。
259.  **发送同步和异步请求**：编写一个以同步和异步方式发送相同请求的程序。
...
264.  **压缩**：编写一个处理压缩响应的程序（例如`.gzip`。
265.  **表单数据处理**：编写使用 HTTP 客户端 API 提交数据表单的程序（`application/x-www-form-urlencoded`。
266.  **下载资源**：编写一个使用 HTTP 客户端 API 下载资源的程序。
267.  **分块上传**：编写一个使用 HTTP 客户端 API 上传资源的程序。
268.  **HTTP/2 服务器推送**：编写一个程序，通过 HTTP 客户端 API 演示 HTTP/2 服务器推送功能。
...
作为全局的一部分，**HTTP/2**有两部分：

*   **帧层**：这是**HTTP/2**复用核心能力
*   **数据层**：包含数据（我们通常称之为 HTTP）

下图描述了**HTTP/1.1**（顶部）和**HTTP/2**（底部）中的通信：

![](img/0ecf8f36-941b-4f96-8d50-01b5ea7f9b31.png)

**HTTP/2**被服务器和浏览器广泛采用，与**HTTP/1.1**相比有以下改进：

*   **二进制协议**：**HTTP/2***分帧层*是一个二进制分帧协议，不易被人类读取，但更易于机器操作。
*   **复用**：请求和响应交织在一起。在同一连接上同时运行多个请求。
*   **服务器推送**：服务器可以决定向客户端发送额外的资源。
*   **与服务器**的单一连接：**HTTP/2**对每个源站（域）使用单一通信线路（TCP 连接）。
*   **头压缩**：**HTTP/2**依赖 HPACK 压缩来减少头。这对冗余字节有很大影响。
*   **加密**：通过有线传输的大部分数据都是加密的。

# 251 触发异步 GET 请求

触发异步`GET`请求分为三步，如下所示：
...
2.  建立`HttpRequest`对象（`java.net.http.HttpRequest`并指定请求（默认为`GET`请求）：
...
为了设置 URI，我们可以调用`HttpRequest.newBuilder(URI)`构造函数，也可以调用`Builder`实例上的`uri(URI)`方法（就像我们以前做的那样）。
...
默认情况下，此请求使用 HTTP/2 进行。但是，我们也可以通过`HttpRequest.Builder.version()`显式设置版本。此方法获取一个`HttpClient.Version`类型的参数，该参数是一个`enum`数据类型，它公开了两个常量：`HTTP_2`和`HTTP_1_1`。以下是显式降级到 HTTP/1.1 的示例：
...
当我们必须处理大量的查询参数时，这种解决方案不是很方便。但是，我们可以尝试编写一个 helper 方法，将[0]方法隐藏在查询参数集合的循环中，也可以依赖 URI 生成器。
...
在非 Spring 应用程序中，我们可以依赖 URI 构建器，例如`urlbuilder`库（[https://github.com/mikaelhg/urlbuilder](https://github.com/mikaelhg/urlbuilder) ). 这本书附带的代码包含了一个使用这个的例子。

# 252 设置代理

要设置代理，我们依赖于`Builder`方法的`HttpClient.proxy()`方法。`proxy()`方法获取一个`ProxySelector`类型的参数，它可以是系统范围的代理选择器（通过`getDefault()`或通过其地址指向的代理选择器（通过`InetSocketAddress`）。

假设在[0]地址有一个代理。我们可以按如下方式设置此代理：
...
`header()`和`setHeader()`的区别在于前者添加指定的头，后者设置指定的头。换句话说，`header()`将给定的值添加到该名称/键的值列表中，而`setHeader()`将覆盖该名称/键以前设置的任何值。

*   `headers​(String... headers)`：添加以逗号分隔的表头，代码如下：
...
可以使用`HttpRequest.headers()`方法获取请求头。`HttpResponse`中也有类似的方法来获取响应的头。两个方法都返回一个`HttpHeaders`对象。
...
可以使用`HttpHeaders.allValues()`获取表头的所有值，如下所示：
...
如果表头返回值为`Long`，则依赖`HttpHeaders.firstValueAsLong()`。此方法获取表示标头名称的参数并返回`Optional<Long>`。如果指定头的值不能解析为`Long`，则抛出`NumberFormatException`。

# 254 指定 HTTP 方法

我们可以使用`HttpRequest.Builder`中的以下方法来指示请求使用的 HTTP 方法：
...
当不需要请求主体时，我们可以依赖`BodyPublishers.noBody()`。以下解决方案使用`noBody()`方法触发 HTTP`HEAD`请求：
...
请求主体的设置可以通过`HttpRequest.Builder.POST()`和`HttpRequest.Builder.PUT()`来完成，也可以通过`method()`来完成（例如`method("PATCH", HttpRequest.BodyPublisher)`。`POST()` 和`PUT()`接受`HttpRequest.BodyPublisher`类型的参数。API 在`HttpRequest.BodyPublishers`类中附带了此接口（`BodyPublisher`的几个实现，如下所示：
...
从字符串创建主体可以使用`BodyPublishers.ofString()`完成，如下代码片段所示：
...
要指定一个`charset`调用，请使用`ofString(String s, Charset charset)`。

# 从 InputStream 创建主体

从`InputStream`创建主体可以使用`BodyPublishers.ofInputStream()`来完成，如下面的代码片段所示（这里，我们依赖`ByteArrayInputStream`，当然，任何其他`InputStream`都是合适的）：
...
我们也可以使用`ofByteArray(byte[] buf, int offset, int length)`只发送字节数组的一部分。此外，我们可以使用`ofByteArrays(Iterable<byte[]> iter)`提供来自字节数组`Iterable`的数据。
...
通常，对服务器的身份验证是使用用户名和密码完成的。在代码形式中，这可以通过同时使用`Authenticator`类（协商 HTTP 身份验证的凭据）和`PasswordAuthentication`类（用户名和密码的持有者）来完成，如下所示：
...
对于`Bearer`身份验证（HTTP 承载令牌），我们执行以下操作：
...
不同的请求可以使用不同的凭据。此外，`Authenticator`提供了一套方法（例如，`getRequestingSite()`），如果我们希望找出应该提供哪些值，这些方法非常有用。在生产环境中，应用程序不应该像在这些示例中那样以明文形式提供凭据。
...
当我们尝试访问移动到其他 URI 的资源时，服务器将返回一个范围为[0]的 HTTP 状态码，以及有关新 URI 的信息。当浏览器收到重定向响应（`301`、`302`、`303`、`307`和`308`）时，能够自动向新位置发送另一个请求。

如果我们通过`followRedirects()`显式设置重定向策略，HTTP 客户端 API 可以自动重定向到此新 URI，如下所示：
...
若要从不重定向，只需将`HttpClient.Redirect.NEVER`常量赋给`followRedirects()`（这是默认值）。

要始终重定向，除了从 HTTPS URL 到 HTTP URL，只需将[0]常量指定给[1]。
...
向服务器发送请求可以通过`HttpClient`中的以下两种方式完成：

*   `send()`：此方法同步发送一个请求（这将阻塞，直到响应可用或发生超时）
...
为了异步发送请求，HTTP 客户端 API 依赖于`CompletableFeature`，如[第 11 章](11.html)、*并发-深潜*和`sendAsync()`方法所述，如下所示：
...
我们知道，`CompletableFuture`附带了`allOf()`方法（详见[第 11 章](11.html)、*并发-深潜*），可以并行执行任务，等待所有任务完成，返回`CompletableFuture<Void>`。
...
要收集响应的主体（例如，在`List<String>`中），请考虑本书附带的代码中提供的`WaitAllResponsesFetchBodiesInList`类。

使用自定义`Executor`对象可以完成如下操作：
...
因此，HTTP 客户端 API 允许我们使用`HttpClient.Builder.cookieHandler()`方法设置 cookie 处理程序。此方法获取一个[1]类型的参数。

以下解决方案设置了不接受 Cookie 的[0]：
...
以下解决方案接受所有 cookie 并在控制台中显示它们（如果有任何凭据被报告为无效，则考虑从[0]获取新令牌）https://gorest.co.in/rest-console.html ):
...
我们将在下面几节中研究如何处理不同类型的响应体

# 将响应主体作为字符串处理

将主体响应作为字符串处理可以使用`BodyHandlers.ofString()`完成，如下代码片段所示：
...
将主体响应作为文件处理可以使用`BodyHandlers.ofFile()`完成，如下代码片段所示：
...
要指定打开的选项，请调用`ofFile(Path file, OpenOption... openOptions)`。
...
使用`BodyHandlers.ofInputStream()`可以将主体响应处理为`InputStream`，如下代码片段所示：
...
将主体响应作为字符串流处理可以使用`BodyHandlers.ofLines()`完成，如下代码片段所示：
...
然而，我们习惯于将 JSON 数据表示为 Java 对象（pojo），并在需要时依赖于 JSON 和 Java 之间的转换。我们可以为我们的问题编写一个解决方案，而不涉及 HTTP 客户机 API。但是，我们也可以使用一个自定义的`HttpResponse.BodyHandler`实现编写一个解决方案，该实现依赖于 JSON 解析器将响应转换为 Java 对象。例如，我们可以依赖 JSON-B（在[第 6 章](06.html)中介绍，*Java I/O 路径、文件、缓冲区、扫描和格式化*中）。
...
请注意，我们忽略任何通过`HttpResponse.BodyHandlers.discarding()`的响应体。

# 264 压缩

在服务器上启用`.gzip`压缩是一种常见的做法，旨在显著提高站点的加载时间。但是 JDK11 的 HTTP 客户端 API 没有利用`.gzip`压缩。换句话说，HTTP 客户机 API 不需要压缩响应，也不知道如何处理这些响应。

要请求压缩响应，我们必须发送带有`.gzip`值的`Accept-Encoding`头。此标头不是由 HTTP 客户端 API 添加的，因此我们将按如下方式添加它：
...
这只是工作的一半。到目前为止，如果服务器上启用了`gzip`编码，那么我们将收到一个压缩响应。为了检测响应是否被压缩，我们必须检查`Encoding`头，如下所示：
...
`gZipToString()`方法是一个 helper 方法，它接受`InputStream`并将其视为`GZIPInputStream`。换句话说，此方法从给定的输入流中读取字节并使用它们创建字符串：
...
如果响应没有压缩，那么`isToString()`就是我们需要的 helper 方法：
...
JDK 11 的 HTTP 客户端 API 没有内置支持用`x-www-form-urlencoded`触发`POST`请求。这个问题的解决方案是依赖于一个定制的`BodyPublisher`类。

如果我们考虑到以下几点，那么编写一个定制的`BodyPublisher`类非常简单：
...
因为数据是用键值对表示的，所以存储在`Map`中非常方便。此外，我们只是循环这个`Map`并应用前面的信息，如下所示：
...
依赖此解决方案，可以触发`POST`（`x-www-form-urlencoded`）请求，如下所示：
...
在这种情况下，响应只是发送数据的回音。根据服务器的响应，应用程序需要对其进行处理，如*处理响应主体类型*部分所示。

# 266 下载资源

正如我们在*设置请求主体*和*处理响应主体类型*部分看到的，HTTP 客户端 API 可以发送和接收文本和二进制数据（例如图像、视频等）。
...
更多可以测试的文件可以在这里找到：[http://demo.borland.com/testsite/download\u testpage.php](http://demo.borland.com/testsite/download_testpage.php) .

# 267 使用多部分上载

正如我们在*设置请求主体*部分看到的，我们可以通过`BodyPublishers.ofFile()`和`POST`请求向服务器发送一个文件（文本或二进制）。

但是发送一个经典的上传请求可能涉及一个多部分表单`POST`，其中`Content-Type`为`multipart/form-data`。

在这种情况下，请求主体由边界分隔的部分组成，如下图所示（`--779d334bbfa...`是边界）：

![](img/9309ddab-3151-46f3-88d8-46a357b1e531.png)

然而，JDK11 的 HTTP 客户端 API 并没有为构建这种请求体提供内置支持。不过，按照前面的截图，我们可以定义一个自定义的`BodyPublisher`，如下所示：
...
现在，我们可以创建一个 multipart 请求，如下所示（我们尝试将一个名为[0]的文本文件上传到一个只发送原始表单数据的服务器）：
...
*   acceptor 函数，必须成功调用该函数才能接受推送承诺（acceptor）

通过调用给定的接受者函数来接受推送承诺。接受函数必须传递一个非空的`BodyHandler`，用于处理承诺的响应主体。acceptor 函数将返回一个完成承诺响应的`CompletableFuture`实例。

基于这些信息，我们来看一下`PushPromiseHandler`的一个实现：
...
现在，让我们触发一个请求并将此`PushPromiseHandler`传递给`sendAsync()`：
...
如果我们想返回一个 push-promise 处理程序，该处理程序将 push-promises 及其响应累积到给定的映射中，那么我们可以使用`PushPromiseHandler.of()`方法，如下所示：
...
[0]参数用于标记消息的结束。如果此调用未完成，则消息将通过`false`。
...
最后，我们需要编写处理传入消息的[0]。这是一个包含一组具有默认实现的方法的接口。下面的代码只是覆盖了`onOpen()`、`onText()`和`onClose()`。粘合 WebSocket 侦听器和前面的代码将导致以下应用程序：
