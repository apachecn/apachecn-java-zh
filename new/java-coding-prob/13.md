...
为了异步发送请求，HTTP 客户端 API 依赖于`CompletableFeature`，如[第 11 章](11.html)、“并发-深入了解”和`sendAsync()`方法所述，如下所示：
...
据我们所知，`CompletableFuture`附带了`allOf()`方法（详见[第 11 章](11.html)、“并发-深入了解”），可以并行执行任务，等待所有任务完成，返回`CompletableFuture<Void>`。
