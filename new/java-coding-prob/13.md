...
为了设置 URI，我们可以调用`HttpRequest.newBuilder(URI)`构造函数，或者在`Builder`实例上调用`uri(URI)`方法（就像我们以前做的那样）。
...
在非 Spring 应用程序中，我们可以依赖 URI 生成器，比如`urlbuilder`库（[https://github.com/mikaelhg/urlbuilder](https://github.com/mikaelhg/urlbuilder)）。这本书附带的代码包含了一个使用这个的例子。
...
当不需要请求主体时，我们可以依赖于`BodyPublishers.noBody()`。以下解决方案使用`noBody()`方法触发 HTTP`HEAD`请求：
...
请求主体的设置可以通过`HttpRequest.Builder.POST()`和`HttpRequest.Builder.PUT()`来完成，也可以通过`method()`来完成（例如`method("PATCH", HttpRequest.BodyPublisher)`。`POST()`和`PUT()`采用`HttpRequest.BodyPublisher`类型的参数。API 在`HttpRequest.BodyPublishers`类中附带了此接口（`BodyPublisher`的几个实现，如下所示：
...
通常，对服务器的身份验证是使用用户名和密码完成的。在代码形式下，可以使用`Authenticator`类（此协商 HTTP 认证凭证）和`PasswordAuthentication`类（用户名和密码的持有者）一起完成，如下：
...
在`Bearer`身份验证（HTTP 承载令牌）的情况下，我们执行以下操作：
...
要始终重定向，除了从 HTTPS URL 到 HTTP URL，只需将[T0]常量指定给[T1]。
...
据我们所知，`CompletableFuture`附带了`allOf()`方法（详见[第 11 章](11.html)、*并发-深潜*），可以并行执行任务，等待所有任务完成，返回`CompletableFuture<Void>`。
...
然而，我们习惯于将 JSON 数据表示为 Java 对象（pojo），并在需要时依赖于 JSON 和 Java 之间的转换。我们可以为我们的问题编写一个解决方案，而不涉及 HTTP 客户机 API。但是，我们也可以使用`HttpResponse.BodyHandler`的自定义实现编写一个解决方案，该实现依赖于 JSON 解析器将响应转换为 Java 对象。例如，我们可以依赖 JSON-B（在[第 6 章](06.html)中介绍，*Java I/O 路径、文件、缓冲区、扫描和格式化*中）。
...
在这种情况下，响应只是发送数据的回音。根据服务器的响应，应用程序需要处理它，如*处理响应主体类型*部分所示。

# 266 下载资源

正如我们在*设置请求主体*和*处理响应主体类型*部分中看到的，HTTP 客户端 API 可以发送和接收文本和二进制数据（例如，图像、视频等）。
...
正如我们在*设置请求主体*部分所看到的，我们可以通过`BodyPublishers.ofFile()`和`POST`请求向服务器发送一个文件（文本或二进制文件）。
