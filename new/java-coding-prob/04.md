...
本章包括 21 个问题，涉及 jep286 或 Java**局部变量类型推理**（**LVTI**），也称为`var`类型。这些问题都经过了精心设计，以揭示最佳实践和使用`var`过程中的常见错误。到本章结束时，您将学习到将`var`推向生产所需的一切知识。

# 问题

使用以下问题来测试您的类型推断编程能力。我强烈建议您在使用解决方案和下载示例程序之前，先尝试一下每个问题：

78.  **简单的**`var`**示例**：编写一个程序，就代码的可读性而言，举例说明类型推断（`var`的正确用法。
79.  **将`var`与基元类型**一起使用：编写一个程序，举例说明将`var`与 Java 基元类型（`int`、`long`、`float`、`double`一起使用。
80.  **使用`var`和隐式类型转换来维持代码的可维护性**：编写一个程序，举例说明`var`和*隐式类型转换*如何维持代码的可维护性。
81.  **显式降级或更好地避免**`var`：编写一个程序，举例说明`var`和显式降级的组合，并解释为什么要避免`var`。
82.  **如果被调用的名称不包含足够的人类类型信息，请避免使用`var`**：请举例说明应避免使用`var`，因为它与被调用的*名称*的组合会导致人类信息的丢失。
83.  **结合 LVTI 和编程接口技术**：通过*编程接口技术*编写一个举例说明`var`用法的程序。

84.  **结合 LVTI 和 diamond 算子**：编写一个程序，举例说明`var`和*diamond 算子的用法。*
85.  **将数组赋给**`var`：编写将数组赋给`var`的程序。
86.  **在复合声明中使用 LVTI**：解释并举例说明 LVTI 在复合声明中的用法。
87.  **LVTI 和变量范围**：解释并举例说明为什么 LVTI 应该尽可能减少变量的范围。
88.  **LVTI 和三元运算符**：编写几个代码片段，举例说明 LVTI 和*三元运算符*组合的优点。
89.  **LVTI 和**`for`**循环**：写几个例子来举例说明 LVTI 在`for`循环中的用法。
90.  **LVTI 和流**：写几个代码片段，举例说明 LVTI 和 Java 流的用法。
91.  **使用 LVTI 分解嵌套的/大的表达式链**：编写一个程序，举例说明如何使用 LVTI 分解嵌套的/大的表达式链。
92.  **LVTI 和方法返回和参数类型**：编写几段代码，举例说明 LVTI 和 Java 方法在返回和参数类型方面的用法。
93.  **LVTI 和匿名类**：编写几个代码片段，举例说明 LVTI 在匿名类中的用法。
94.  **LVTI 可以是`final`和*有效的 final***：写几个代码片段，举例说明 LVTI 如何用于`final`和*有效的 final*变量。
...
98.  **LVTI、通配符、协方差和逆变式**：编写几个代码片段，举例说明如何将 LVTI 与通配符、协方差和逆变式结合使用。

# 解决

以下各节介绍上述问题的解决方案。记住，通常没有一个正确的方法来解决一个特定的问题。另外，请记住，这里显示的解释仅包括解决问题所需的最有趣和最重要的细节。您可以下载示例解决方案以查看更多详细信息，并从[中试用程序 https://github.com/PacktPublishing/Java-Coding-Problems](https://github.com/PacktPublishing/Java-Coding-Problems) .

# 78 简单 var 示例

从版本 10 开始，Java 附带了 JEP286 或 JavaLVTI，也称为[0]类型。
...
此功能可确保编译时安全。这意味着我们不能编译一个试图实现错误赋值的应用程序。如果编译器已推断出`var`的具体/实际类型，则只能赋值该类型的值。

LVTI 有很多好处；例如，它减少了代码的冗长，减少了冗余和*样板文件*代码。此外，LVTI 可以减少编写代码所花的时间，特别是在涉及大量声明的情况下，如下所示：
...
一个有争议的优点是代码可读性。一些声音支持使用[0]会降低代码可读性，而另一些声音则支持相反的观点。根据用例的不同，它可能需要在可读性上进行权衡，但事实是，通常情况下，我们非常关注字段（实例变量）的有意义的名称，而忽略了局部变量的名称。例如，让我们考虑以下方法：
...
让我们再看两个例子来执行上述语句。使用集合（例如，[0]）的方法如下：
...
将 LVTI 与基元类型（`int`、`long`、`float`和`double`一起使用的问题是，预期类型和推断类型可能不同。显然，这会导致代码中的混乱和意外行为。

这种情况下的犯罪方是`var`类型使用的*隐式类型铸造*。

例如，让我们考虑以下两个依赖显式基元类型的声明：
...
很好！到目前为止没有问题！现在，让我们看看另一组基于显式基元类型的声明：
...
让我们按照第一个示例中的逻辑，用 LVTI 替换显式原语类型：
...
变量名表明`10.5`是`float`，但实际上推断为`double`。因此，即使是带小数的数字（尤其是带小数的数字），也建议使用*文字*：
...
在上一节中，*将 var 与基元类型*结合使用，我们看到将`var`与*隐式类型转换*结合使用会产生实际问题。但在某些情况下，这种组合可能是有利的，并维持代码的可维护性。
...
另一种方式接收价格为`int`并进行支付。如果支付成功，则返回`true`：
...
但是过了一段时间，`ShoppingAddicted`API 的拥有者意识到，他们将实际价格转换为`int`是赔钱的（例如，实际价格是 399.99，但在`int`形式中，它是 399.0，这意味着损失 99 美分）。因此，他们决定放弃这种做法，将实际价格作为`float`返回：
...
因为返回的价格是`float`，所以也更新了`debitCard()`：
...
但是，一旦我们升级到新版本的`ShoppingAddicted`API，代码将失败，并可能出现*从*`float`到`int`的有损转换异常。这是正常的，因为我们的代码需要[3]。由于我们的代码不能很好地容忍这些修改，因此需要相应地修改代码。

然而，如果我们已经预料到了这种情况，并且使用了`var`而不是`int`，那么由于*隐式类型转换*，代码将不会出现问题：
...
在*将 var 与基元类型一起使用*一节中，我们讨论了将*文本*与基元类型（`int`、`long`、`float`、`double`一起使用，以避免*隐式类型转换带来的问题。* 但并不是所有 Java 原语类型都能利用*文本*。在这种情况下，最好的方法是避免使用`var`。但让我们看看为什么！
...
不幸的是，这两个基元类型没有可用的*文本*。帮助编译器推断正确类型的唯一方法是依赖显式向下转换：
...
虽然此代码编译成功并按预期工作，但我们不能说使用[0]比使用显式类型带来了任何价值。因此，在这种情况下，最好避免`var`和显式的悲观情绪。

# 82 如果被调用的名称没有包含足够的类型信息，请避免使用 var

好吧，`var`并不是一颗灵丹妙药，这个问题将再次凸显这一点。以下代码片段可以使用显式类型或`var`编写，而不会丢失信息：
...
因此，将前面的代码片段迁移到`var`将产生以下代码（通过从右侧目视检查被调用的*名称*，选择了变量名称）：
...
因此，前面的代码在选择变量的名称和可读性时不会引起任何问题。被称为*的名称*包含了足够的信息让人类对`var`感到舒服。
...
对于人类来说，如果不检查这个*名称*、`fetchBinContent()`的返回类型，就很难推断出被调用的*名称*返回的类型。根据经验，在这种情况下，解决方案应该避免`var`并依赖显式类型，因为右侧没有足够的信息让我们为变量选择合适的名称并获得可读性很高的代码：
...
因此，如果`var`与被调用的*名称*组合使用会导致清晰度下降，那么最好避免使用`var`。忽略此语句可能会导致混淆，并会增加理解和/或扩展代码所需的时间。

考虑另一个基于`java.nio.channels.Selector`类的示例。此类公开了一个名为`open()`的`static`方法，该方法返回一个新打开的`Selector`。但是如果我们在一个用`var`声明的变量中捕获这个返回值，我们很可能会认为这个方法可能返回一个`boolean`，表示打开当前选择器的成功。不考虑可能的清晰度损失而使用`var`恰恰会产生这类问题。像这样的一些问题和代码将成为一个真正的痛苦。

# 83LVTI 与编程相结合的接口技术

Java 最佳实践鼓励我们将代码绑定到抽象。换句话说，我们需要依赖*编程到接口*技术。
...
通过遵循第一个示例，代码实例化了`ArrayList`类（或`HashSet`、`HashMap`等），但声明了一个`List`类型的变量（或`Set`、`Map`等）。由于`List`、`Set`、`Map`等多个都是接口（或契约），很容易用`List`（`Set`、`Map`的其他实现来代替实例化，而不需要对代码进行后续修改。

不幸的是，LVTI 不能利用*编程接口*技术。换句话说，当我们使用`var`时，推断类型是具体的实现，而不是契约。例如，将`List<String>`替换为`var`将导致推断类型`ArrayList<String>`：
...
*   LVTI 在局部级别（局部变量）起作用，其中对接口技术的*编程使用的少于方法参数/返回类型或字段类型。*
*   由于局部变量的作用域很小，因此切换到另一个实现所引起的修改也应该很小。切换实现对检测和修复代码的影响应该很小。
*   LVTI 将右侧的代码视为一个用于推断实际类型的*初始值设定项*。如果这个*初始值设定项*将来要修改，那么推断的类型可能会不同，这将导致使用这个变量的代码出现问题。

# 84 结合 LVTI 和菱形算子

根据经验，如果右侧不存在推断预期类型所需的信息，则 LVTI 与[0]diamond[1]运算符结合可能会导致意外的推断类型。

在 JDK 7 之前，即项目 Coin，`List<String>`将声明如下：

```
List<String> players = new ArrayList<String>();
```

基本上，前面的示例显式指定泛型类的实例化参数类型。从 JDK 7 开始，Project Coin 引入了*diamond*操作符，可以推断泛型类实例化参数类型，如下所示：
...
但是现在推断出的类型是什么呢？我们遇到了一个问题，因为推断的类型将是`ArrayList<Object>`，而不是`ArrayList<String>`。解释很明显：推断预期类型（`String`所需的信息不存在（注意，右侧没有提到`String`类型）。这指示 LVTI 推断出最广泛适用的类型，在本例中是`Object`。
...
根据经验，将数组分配给`var`不需要括号`[]`。通过相应的显式类型定义一个`int`的数组可以做如下操作：
...
现在，尝试直觉地使用`var`而不是`int`，可能会导致以下尝试：
...
因此，前面的代码声明了一个具有四个名称的堆栈，包含 50 行不使用此堆栈的代码，并通过`forEach()`方法完成此堆栈的循环。此方法继承自`java.util.Vector`，将堆栈作为任意向量（`John`、`Martin`、`Anghel`、`Christian`）循环。这是我们想要的遍历顺序。

但后来，我们决定从堆栈切换到`ArrayDeque`（原因无关紧要）。这次，`forEach()`方法将是`ArrayDeque`类提供的方法。此方法的行为不同于`Vector.forEach()`，即循环将遍历**后进先出**（**后进先出**）遍历（`Christian`、`Anghel`、`Martin`、`John`）之后的条目：
...
这不是我们的本意！我们切换到`ArrayDeque`是为了其他目的，不是为了影响循环顺序。但是很难看出代码中有 bug，因为包含[1]部分的代码部分不在我们完成修改的代码附近（在这行代码下面 50 行）。我们有责任提出一个解决方案，最大限度地提高快速修复这个 bug 的机会，避免一堆上下滚动来了解正在发生的事情。解决方案包括遵循我们之前调用的干净代码规则，并使用小范围的`stack`变量编写此代码：
...
只要写得正确，*三元*运算符允许我们在右侧使用不同类型的操作数。例如，以下代码将不会编译：
...
因此，为了在右侧有一个具有不同类型操作数的[0]三元[1]运算符，开发人员必须匹配支持两个条件分支的正确类型。或者，开发人员可以依赖 LVTI，如下所示（当然，这也适用于相同类型的操作数）：
...
不要从这些示例中得出结论认为`var`类型是在运行时推断出来的！不是的！

# 89LVTI 和 for 循环

使用显式类型声明简单的[0]循环是一项简单的任务，如下所示：
...
当循环数组、集合等的类型更改时，使用`var`可能会有所帮助。例如，通过使用`var`，可以在不指定显式类型的情况下循环以下`array`的两个版本：
...
使用 LVTI 代替[0]非常简单。只需将`Stream<Integer>`替换为`var`，如下所示：
...
大型/嵌套表达式通常是一些代码片段，它们看起来非常令人印象深刻，令人生畏。它们通常被视为*聪明的*或*聪明的*代码。关于这是好是坏是有争议的，但最有可能的是，这种平衡倾向于有利于那些声称应该避免这种代码的人。例如，检查以下表达式：
...
令人惊叹的！现在，编译器的任务是推断这些局部变量的类型。我们只选择打断表达式的点，并用[0]来划分它们。
...
*   [0]类型的变量可以作为方法参数传递，也可以存储返回方法。以下代码编译成功并正常工作：
...
LVTI 可以用于匿名类。下面是一个匿名类的示例，该类对[0]变量使用显式类型：
...
作为快速提醒，*从 JavaSE8 开始，本地类可以访问封闭块的局部变量和参数，这些变量和参数是 final 或实际上是 final。一个变量或参数，其值在初始化后从未改变，实际上是最终的。*

下面的代码片段表示一个*有效的最终*变量的用例（尝试重新分配`ratio`变量将导致错误，这意味着这个变量实际上是*最终的*和两个`final`变量（试图重新分配`limit`和`bmi`变量会导致一个错误，这意味着这些变量是`final`：
...
现在，让我们用[0]替换显式类型。编译器将为这些变量（`ratio`、`limit`和`bmi`推断正确的类型并保持它们的状态-`ratio`将是*有效的最终*，而`limit`和`bmi`是`final`。尝试重新分配其中任何一个将导致特定错误：
...
使用 LVTI 和 lambdas 的问题是无法推断具体类型。不允许使用 lambda 和方法参考*初始值设定项*。此语句是`var`限制的一部分；因此，lambda 表达式和方法引用需要显式的目标类型。
...
但是在 lambdas 的上下文中，Java11 允许我们在 lambda 参数中使用`var`。例如，以下代码在 Java11 中工作（更多详细信息可在*JEP323 中找到：Lambda 参数的局部变量语法*在[中）https://openjdk.java.net/jeps/323](https://openjdk.java.net/jeps/323) ):
...
另一方面，`var`类型非常适合*使用资源*进行尝试，如下例所示：
...
为了理解 LVTI 如何与泛型类型相结合，让我们从一个示例开始。下面的方法是泛型类型`T`的经典用例：
...
用 LVTI 替换通配符、协方差和逆变是一项微妙的工作，应该在充分意识到后果的情况下完成。

# LVTI 和通配符

首先，我们来讨论 LVTI 和通配符（`?`。通常的做法是将通配符与[1]相关联，并编写如下内容：
...
但是请注意，用 LVTI 替换通配符应该小心，并且您应该意识到其后果（或副作用）。让我们看一个例子，用[0]替换通配符是一个错误的选择。考虑以下代码：
...
由于类型不兼容，此代码无法编译。一种非常糟糕的方法是用[0]替换通配符来修复此代码，如下所示：
...
通过使用`var`，错误会消失，但这不是我们在编写前面的代码（存在类型不兼容错误的代码）时想到的。所以，根据经验，不要因为一些恼人的错误会神奇地消失，就用`var`代替`*Foo*<?>`！试着思考一下预期的任务是什么，并相应地采取行动。例如，可能在前面的代码片段中，我们试图定义`ArrayList<String>`，但错误地以`Collection<?>`结束。

# LVTI 和协变/逆变

用 LVTI 替换协方差（[0]）或反变量（[1]）是一种危险的方法，应该避免。
...
在协变中，我们有一个上界由`Number`类表示，而在逆变中，我们有一个下界由`FilterReader`类表示。有了这些边界（或约束），以下代码将触发特定的编译时错误：
...
这是本章的最后一个问题。请看*JEP 323:Lambda 参数的局部变量语法*（[https://openjdk.java.net/jeps/323](https://openjdk.java.net/jeps/323) ）和*JEP 301：增强的**枚举*[http://openjdk.java.net/jeps/301](http://openjdk.java.net/jeps/301) ）了解更多信息。只要您熟悉本章介绍的问题，采用这些特性应该是相当顺利的。
