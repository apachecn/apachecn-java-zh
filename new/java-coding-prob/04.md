...
79.  **将`var`与基元类型**结合使用：编写一个程序，举例说明将`var`与 Java 基元类型（`int`、`long`、`float`、`double`结合使用。
...
82.  **如果被调用的名称没有包含足够的人类类型信息，请避免使用`var`**：请举例说明应避免使用`var`，因为它与被调用的*名称*的组合会导致人类信息的丢失。
83.  **结合 LVTI 和编程到接口技术**：编写一个程序，通过*编程到接口*技术来举例说明`var`的用法。

84.  **结合 LVTI 和菱形算子**：编写一个程序，举例说明`var`和*菱形算子*的用法。
85.  **将数组分配给**`var`：编写一个将数组分配给`var`的程序。
...
90.  **LVTI 和 streams**：编写几个代码片段，举例说明 LVTI 和 Java 流的用法。
...
以下各节介绍上述问题的解决方案。记住，通常没有一个正确的方法来解决一个特定的问题。另外，请记住，这里显示的解释仅包括解决问题所需的最有趣和最重要的细节。您可以从[下载示例解决方案以查看更多详细信息并尝试程序 https://github.com/PacktPublishing/Java-Coding-Problems](https://github.com/PacktPublishing/Java-Coding-Problems)。
...
变量名表明`10.5`是`float`，但实际上是推断为`double`。因此，即使是带小数的数字（尤其是带小数的数字），也建议使用*文字*：
...
在*将 var 与基元类型结合使用*一节中，我们讨论了将*文本*与基元类型结合使用（`int`、`long`、`float`和`double`来避免*隐式类型转换带来的问题。*但并非所有 Java 原语类型都可以利用*文本*。在这种情况下，最好的方法是避免使用`var`。但让我们看看为什么！
...
不幸的是，这两种基本类型没有可用的*文本*。帮助编译器推断正确类型的唯一方法是依赖显式向下转换：
...
虽然这段代码编译成功并按预期工作，但我们不能说使用`var`比使用显式类型带来了任何价值。因此，在这种情况下，最好避免`var`和显式的向下投射。
...
对于人类来说，如果不检查*名*、`fetchBinContent()`的返回类型，就很难推断出*名*返回的类型。根据经验，在这种情况下，解决方案应该避免`var`并依赖显式类型，因为右侧没有足够的信息让我们为变量选择合适的名称并获得可读性很高的代码：
...
Java 最佳实践鼓励我们将代码绑定到抽象。换句话说，我们需要依赖于*编程到*接口的技术。
...
不幸的是，LVTI 不能利用*编程接口*技术。换句话说，当我们使用`var`时，推断的类型是具体的实现，而不是合同。例如，将`List<String>`替换为`var`将导致推断类型`ArrayList<String>`：
...
*   LVTI 在局部级别（局部变量）起作用，其中对接口技术的*编程的使用少于方法参数/返回类型或字段类型。*
...
基本上，前面的示例显式指定泛型类的实例化参数类型。从 JDK 7 开始，Project Coin 引入了*菱形*操作符，可以推断泛型类实例化参数类型，如下所示：
...
但后来，我们决定从堆栈切换到`ArrayDeque`（原因无关紧要）。这次，`forEach()`方法将是由`ArrayDeque`类提供的方法。此方法的行为不同于`Vector.forEach()`，即循环将遍历**后进先出**（**后进先出**）遍历（`Christian`、`Anghel`、`Martin`、`John`之后的条目：
...
大型/嵌套表达式通常是一些代码片段，它们看起来非常令人印象深刻，令人生畏。它们通常被视为*智能*或*智能*代码的片段。关于这是好是坏是有争议的，但最有可能的是，这种平衡倾向于有利于那些声称应该避免这种代码的人。例如，检查以下表达式：
...
现在，让我们用`var`替换显式类型。编译器将推断出这些变量（`ratio`、`limit`和`bmi`的正确类型并保持它们的状态-`ratio`将是*有效的最终*，而`limit`和`bmi`是`final`。尝试重新分配其中任何一个将导致特定错误：
...
但是在 lambdas 的上下文中，java11 允许我们在 lambda 参数中使用`var`。例如，下面的代码在 Java11 中工作（更多详细信息可以在*JEP323 中找到：Lambda 参数的局部变量语法*在[中）https://openjdk.java.net/jeps/323](https://openjdk.java.net/jeps/323)：
...
另一方面，`var`类型非常适合*try with resource*，如下例所示：
...
但是请注意，用 LVTI 替换通配符应该小心，并且您应该意识到其后果（或副作用）。让我们看一个例子，用[T0]替换通配符是一个错误的选择。考虑以下代码：
...
由于类型不兼容，此代码无法编译。一种非常糟糕的方法是用[T0]替换通配符来修复此代码，如下所示：
...
这是本章的最后一个问题。请看*jep323：Lambda 参数的局部变量语法*（[https://openjdk.java.net/jeps/323](https://openjdk.java.net/jeps/323)、*JEP 301：增强**枚举*（[http://openjdk.java.net/jeps/301](http://openjdk.java.net/jeps/301)了解更多信息。只要您熟悉本章介绍的问题，采用这些特性应该是相当顺利的。
