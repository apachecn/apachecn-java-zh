...
168.  **实现环绕执行模式**：基于 lambdas 编写实现环绕执行模式的程序。
169.  **实现工厂模式**：基于 lambdas 编写一个实现工厂模式的程序。
170.  **实现策略模式**：基于 lambdas 编写一个实现策略模式的程序。
171.  **实现模板方法模式**：基于 lambdas 编写一个实现模板方法模式的程序。
172.  **实现观察者模式**：基于 lambdas 编写一个实现观察者模式的程序。
173.  **实现贷款模式**：基于 lambdas 编写实现贷款模式的程序。
174.  **实现装饰器模式**：基于 lambdas 编写一个实现装饰器模式的程序。
175.  **实现级联生成器模式**：基于 lambdas 编写一个实现级联生成器模式的程序。
176.  **实现命令模式**：基于 lambdas 编写一个实现命令模式的程序。
...
从概念上讲，函数接口只有一个抽象方法。此外，我们定义的`Predicate`接口已经作为`java.util.function.Predicate`接口存在于 Java8 中。`java.util.function`包包含 40 多个这样的接口。因此，在定义一个新的包之前，最好检查这个包的内容。大多数情况下，六个标准的内置函数式接口就可以完成这项工作。具体如下：
...
这种方法的缺点在于*样板文件*代码和冗长。但是，我们可以通过 lambdas 解决这个缺点。我们可以将模板方法的可变步骤表示为 lambdas 表达式。根据具体情况，我们必须选择合适的函数式接口。在我们的情况下，我们可以依赖于`Consumer`，如下所示：
...
我们已经在[第 2 章](02.html)、“对象、不变性和`switch`表达式”中讨论过这个模式，“通过构建器模式编写一个不可变类”部分。处理这个问题是明智的，就像快速提醒构建器模式一样。
...
显然，我们这里有很多*样板*代码。查看命令类。我们真的需要所有这些课程吗？好吧，如果我们意识到`Command`接口实际上是一个函数式接口，那么我们可以删除它的实现并通过 lambdas 提供行为（命令类只是行为块，因此它们可以通过 lambdas 表示），如下所示：
