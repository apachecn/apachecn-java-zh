...
因此，每种类型的 pizza 都需要一个新类来重写`addTopIngredients()`方法。最后，我们可以这样做比萨饼：
...
我们已经在[第 2 章](02.html)、“对象、不变性和`switch`表达式”中讨论过这个模式，在*通过构建器模式*部分编写一个不可变类。处理这个问题是明智的，就像快速提醒构建器模式一样。
