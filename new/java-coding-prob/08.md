...
在这个问题上，我们将讨论如何实施贷款模式。假设我们有一个包含三个数字的文件（比如说，`double`），每个数字都是一个公式的系数。例如，数字`x`、`y`、`z`是以下两个公式的系数：`x + y - z`和`x - y * sqrt(z)`。同样的，我们也可以写出其他的公式。
...
这个函数式接口成为一个`static`方法的参数，该方法旨在实现贷款模式。让我们把这个方法放在一个名为`Formula`的类中：
