...
***   **兰达斯简而言之**：**解释什么是 lambda 表达式。*****   **实现环绕执行模式**：**基于 lambdas 编写表示环绕执行模式实现的程序。******

 ******169.  **实现工厂模式**：**编写一个表示基于 lambdas 的工厂模式实现的程序。**
***   **实现策略模式**：**编写一个基于 lambdas 的表示策略模式实现的程序。*****   **实现模板方法模式**：**编写一个基于 lambdas 的表示模板方法模式实现的程序。*****   **实现观察者模式**：**编写一个表示基于 lambdas 的观察者模式实现的程序。*****   **实现贷款模式**：**基于 lambdas 编写表示贷款模式实现的程序。*****   **实现 Decorator 模式**：**编写一个表示基于 lambdas 的 Decorator 模式实现的程序。*****   **实现级联生成器模式**：**编写一个基于 lambdas 的表示级联生成器模式实现的程序。*****   **实现命令模式**：**编写一个表示基于 lambdas 的命令模式实现的程序。**********************  ******# 解决

以下各节介绍上述问题的解决方案。记住，通常没有一个正确的方法来解决一个特定的问题。另外，请记住，这里显示的解释仅包括解决这些问题所需的最有趣和最重要的细节。您可以从[下载示例解决方案以查看更多详细信息并尝试程序 https://github.com/PacktPublishing/Java-Coding-Problems](https://github.com/PacktPublishing/Java-Coding-Problems)。
...
有一天，马克让我们提供一个功能，可以按瓜的类型过滤瓜。因此，我们创建了一个名为`Filters`的实用程序类，并实现了一个`static`方法，该方法将瓜列表和要过滤的类型作为参数。
...
从概念上讲，函数接口只有一个抽象方法。此外，我们定义的`Predicate`接口已经作为`java.util.function.Predicate`接口存在于 Java8 中。`java.util.function`包包含 40 多个这样的接口。因此，在定义一个新的包之前，最好检查这个包的内容。大多数情况下，六个标准的内置功能接口就可以完成这项工作。具体如下：
...
*   在箭头的右侧，我们有 lambda 主体，在本例中，它检查找到文件的文件夹是否可以读取，以及文件名是否以[T0]后缀结尾。
...
executearound 模式试图消除围绕特定任务的*样板*代码。例如，为了打开和关闭文件，特定于文件的任务需要被代码包围。

主要地，executearound 模式在暗示在资源的开-关生命周期内发生的任务的场景中很有用。例如，假设我们有一个`Scanner`，我们的第一个任务是从文件中读取一个`double`值：
...
为了避免这个*b**oilerplate*代码，Execute Around 模式依赖于行为参数化（在*编写功能接口*一节中进一步详细说明）。实现这一点所需的步骤如下：

1.  第一步是定义一个与`Scanner -> double`签名匹配的功能接口，该接口可能抛出一个`IOException`：
...
传递给`read()`方法的 lambda 在这个方法的主体中执行。当我们传递 lambda 时，我们提供了一个称为直接内联的`abstract`方法的实现。主要是作为函数接口`ScannerDoubleFunction`的一个实例，因此我们可以调用`readDouble()`方法来获得期望的结果。
...
executearound 模式对于消除特定于打开和关闭资源（I/O 操作）的*样板*代码非常有用。
...
这里，`Gac`、`Hemi`、`Cantaloupe`实现相同的`Fruit`接口，并有空构造函数。如果该方法生活在名为`MelonFactory`的实用类中，则可以调用如下：

```
Gac gac = (Gac) MelonFactory.newInstance(Gac.class);
```

但是，Java8 函数样式允许我们使用*方法引用*技术引用构造函数。这意味着我们可以定义一个`Supplier<Fruit>`来引用`Gac`空构造函数，如下所示：
...
然而，很明显，构造函数并不总是空的。例如，下面的`Melon`类公开了一个具有三个参数的构造函数：
...
如果我们通过 Template-Method 模式将其放入代码中，那么我们会得到如下结果（方法 T0 表示 Template 方法，并以明确定义的顺序包含固定和可变的步骤）：
...
因此，每种类型的 pizza 都需要一个新类来重写[T0]方法。最后，我们可以这样做比萨饼：
...
这种方法的缺点在于*样板文件*代码和冗长。但是，我们可以通过 lambdas 解决这个缺点。我们可以将模板方法的可变步骤表示为 lambdas 表达式。根据具体情况，我们必须选择合适的功能接口。在我们的情况下，我们可以依赖于`Consumer`，如下所示：
...
例如，消防站总部可以是*主体*，地方消防站可以是*观察员*。火灾发生后，消防局总部通知所有当地消防局，并向他们发送火灾发生的地址。每个*观察者*分析接收到的地址，并根据不同的标准决定是否灭火。
...
各地方消防站（*观察员*）实现此接口，并在`fire()`实现中决定是否灭火。在这里，我们有三个本地站（`Brookhaven`、`Vinings`和`Decatur`）：
...
一半的工作完成了！现在，我们需要注册这些*观察者*，由*受试者*通知。也就是说，每个地方消防站都需要注册为消防站总部的*观察员*（*主体*）。为此，我们声明了另一个接口，它定义了*主题*合同，用于注册和通知其*观察者*：
...
现在，让我们把我们的三个本地站（*观察员*）登记到消防站总部（*主题*）：
...
在这个问题上，我们将讨论如何实施贷款模式。假设我们有一个包含三个数字的文件（比如说，doubles），每个数字都是一个公式的系数。例如，数字*x*、*y*、*z*是以下两个公式的系数：*x+y-z*和*x-y*sqrt（z）。*同样的，我们也可以写出其他的公式。
...
这个函数接口成为一个`static`方法的参数，该方法旨在实现 Loan 模式。让我们把这个方法放在一个名为`Formula`的类中：
...
这里应该强调三点。首先，当我们创建一个新的`Formula`实例时，我们实际上在我们的文件中打开了一个新的`Scanner`（检查这个类的`private`构造函数）：
...
为了简洁起见，我们跳过了[T0]修饰符。然而，凭直觉很容易看出这个装饰器与`Nuts`基本相同。
...
此外，不需要将装饰器作为单独的类来编写。我们将依靠 lambdas 将装饰人员传递给`CakeDecorator`：
...
*   在构造函数中，它调用`reduceDecorations()`方法。此方法将通过`Stream.reduce()`和`Function.andThen()`方法链接传递的`Function`数组。结果是由给定的`Function`数组组成的单个`Function`。
...
我们已经在[第 2 章](02.html)、*对象、不变性和开关表达式*中讨论过这个模式，在*通过构建器模式*部分编写一个不可变类。处理这个问题是明智的，就像快速提醒构建器模式一样。
...
显然，`Consumer<Delivery>`参数有助于使用 lambdas。
...
*   `Command`接口负责执行某个动作（在这种情况下，可能的动作是 move、copy 和 delete）。该接口的具体实现为`CopyCommand`、`MoveCommand`、`DeleteCommand`。
...
因此，`HardDisk`实现了在`IODevice`接口中给出的动作。作为*接收者*，`HardDisk`负责在调用某个命令的`execute()`方法时运行实际动作。`IODevice`的源代码如下：
...
此外，`Sequence`类充当*调用方*类。*调用程序*知道如何执行给定的命令，但对命令的实现没有任何线索（它只知道命令的接口）。在这里，我们将命令记录在一个`List`中，并在调用`runSequence()`方法时批量执行这些命令：
...
显然，我们这里有很多*样板*代码。查看命令类。我们真的需要所有这些课程吗？好吧，如果我们意识到`Command`接口实际上是一个功能接口，那么我们可以删除它的实现并通过 lambdas 提供行为（命令类只是行为块，因此它们可以通过 lambdas 表示），如下所示：
