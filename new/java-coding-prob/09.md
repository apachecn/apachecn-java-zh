...
185.  **流的总和、最大、最小**：通过`Stream`和`Stream.reduce()`的原语特化编写计算给定流的总和、最大、最小的程序。
...
在[第 8 章](08.html)、“函数式编程——基础与设计模式”中，在“编写函数式接口”部分，我们定义了一个基于函数式接口`Predicate`的`filter()`方法。Java Stream API 已经有了这样的方法，函数式接口称为`java.util.function.Predicate`。
...
用户问题：*这次我只对半甜瓜感兴趣。有两个容器：一个用于装重量小于（或等于）2000 克的半甜瓜，另一个用于装重量大于 2000 克的半甜瓜*。
...
这三个步骤通过默认的`ForkJoinPool`方法在幕后进行，正如我们在[第 10 章](10.html)、“并发-线程池、可调用对象和同步器”和[第 11 章](11.html)、“并发-深入”中所讨论的。
