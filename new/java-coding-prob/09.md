...
本章包括 22 个涉及 Java 函数式编程的问题。这里，我们将重点讨论在流中遇到的涉及经典操作的几个问题（例如，`filter`和`map`），并讨论无限流、空安全流和缺省方法。这个问题的综合列表将涵盖分组、分区和收集器，包括 jdk12`teeing()`收集器和编写自定义收集器。此外，还将讨论`takeWhile()`、`dropWhile()`、组合函数、谓词和比较器、lambdas 测试和调试以及其他一些很酷的话题。
...
185.  **流中的和、最大、最小**：通过`Stream`和`Stream.reduce()`的原语专门化编写计算给定流的和、最大、最小的程序。
186.  **收集流的结果**：编写一些代码片段，用于收集列表、映射和集合中的流的结果。
187.  **连接流的结果**：写几个代码片段，将流的结果连接到`String`中。
188.  **摘要采集器**：写几个代码片段来展示摘要采集器的用法。
189.  **分组**：编写用于处理`groupingBy()`采集器的代码片段。
190.  **分区**：编写几个代码片段，用于使用`partitioningBy()`采集器。
191.  **过滤、展平和映射收集器**：编写几段代码，举例说明过滤、展平和映射收集器的用法。
...
我们还假设我们有一个方法，该方法接受[T0]类型的 lambda，如下所示：
...
让我们考虑到 lambda 生成函数接口实例；然后，我们可以测试该实例的行为，如下所示：
...
*   检查堆栈跟踪
*   登录中
*   依赖 IDE 支持（例如，NetBeans、Eclipse 和 IntelliJ IDEA 支持调试 lambdas*开箱即用*或为其提供插件）
...
而且，如果我们在列表中添加一个[T0]值，那么输出将再次变得混乱。
...
在[第 8 章](08.html)、*函数式编程——基础与设计模式*中，在*编写函数接口*部分，我们定义了一个基于函数接口`Predicate`的`filter()`方法。Java Stream API 已经有了这样的方法，函数接口称为`java.util.function.Predicate`。
...
当`filter()`操作需要复杂/复合或长期条件时，建议采用辅助`static`方法提取，并依赖*方法参考*。因此，避免这样的事情：
...
当然，我们可以将`Stream.iterate()`和`limit()`的味道结合起来形成更复杂的场景。例如，下面的流将创建新元素，直到*具有下一个*谓词`i -> i <= 10`。因为我们使用的是随机值，`hasNext`谓词返回`false`的时刻是不确定的：
...
映射一个流的元素是一个中间操作，用于将这些元素*转换成一个新的版本，方法是将给定的函数应用于每个元素，并将结果累加到一个新的`Stream`（例如，将`Stream<String>`转换成`Stream<Integer>`，或将`Stream<String>`转换成另一个`Stream<String>`等）。*
...
除`map()`之外，`Stream`类还为`mapToInt()`、`mapToLong()`和`mapToDouble()`等原语提供口味。这些方法返回`Stream`（`IntStream`）的`int`原初特化、`Stream`（`LongStream`）的`long`原初特化和`Stream`（`StreamDouble`）的`double`原初特化。
...
你觉得诱惑怎么样？好吧，`peek()`代表*看，但不要碰*，但它可以用来改变状态，如下所示：
...
与`map()`不同，该方法通过展平所有分离的流来返回流。因此，所有数组都将在同一个流中结束：
...
除`flatMap()`之外，`Stream`类还为`flatMapToInt()`、`flatMapToLong()`和`flatMapToDouble()`等原语提供口味。这些方法返回`Stream`（`IntStream`）的`int`原特化、`Stream`（`LongStream`）的`long`原特化和`Stream`（`StreamDouble`的`double`原特化。
...
然而，如果流没有*遭遇**顺序*，则可以返回任何元素。根据文档，*流可能有也可能没有定义的相遇顺序。这取决于源和中间操作*。同样的规则也适用于并行性。
...
这三种操作依赖于*短路*技术。换句话说，在我们处理整个流之前，这些方法可能会返回。例如，如果`allMatch()`匹配`false`（将给定的`Predicate`评估为`false`，则没有理由继续。最终结果为`false`。
...
`max()`和`min()`操作返回一个`OptionalInt`（例如`OptionalLong`）。如果无法计算最大值或最小值（例如，在空流的情况下），则我们选择返回`-1`。既然我们是在处理权值，以及正数的性质，返回[T5]是有意义的。但不要把这当成一个规则。根据情况，应该返回另一个值，或者使用`orElseGet()`/`orElseThrow()`更好。
...
`sum()`、`max()`、`min()`被称为*还原*的特例。我们所说的*还原*，是指基于两个主要陈述的抽象：
...
然而，要注意那些可能导致不想要的结果的案例。例如，如果我们要计算给定数字的调和平均数，那么就没有一个开箱即用的*特例*减少*，因此我们只能依赖`reduce()`，如下所示：*
...
*   过滤一个不同的`Melon`并使用随机密钥通过`toMap()`将结果收集到`Map<Integer, Integer>`中（如果生成两个相同的密钥，则容易产生`java.lang.IllegalStateException`重复密钥）：
...
在上一个问题中，我们讨论了内置于`Collectors`中的`Stream`API。在这个类别中，我们还有`Collectors.joining()`。这些收集器的目标是将流中的元素串联成一个按*遭遇顺序*的`String`。或者，这些收集器可以使用分隔符、前缀和后缀，因此最全面的`joining()`风格是`String joining​(CharSequence delimiter, CharSequence prefix, CharSequence suffix)`。
...
Java`Stream`API 将 count、sum、min、average 和 max 操作分组在术语*摘要*下。用于执行*摘要*操作的方法可以在`Collectors`类中找到。

我们将在下面的部分中查看所有这些操作。

# 求和

假设我们要把所有的西瓜重量加起来。我们通过`Stream`的原始专门化在流部分的*Sum、max 和 min 中实现了这一点。现在，让我们通过`summingInt​(ToIntFunction<? super T> mapper)`收集器来完成：*
...
`reducing()`的论点很直截了当。我们有用于减少的`identity`值（以及没有输入元素时返回的值）、应用于每个输入值的映射函数和用于减少映射值的函数。
...
当没有合适的内置解决方案时，依赖`reducing()`。把`reducing()`想象成*的概括*。
...
在*流*的求和、max、min 部分，我们已经通过`min()`和`max()`方法计算了最小值和最大值。这次，让我们通过`Collectors.maxBy()`和`Collectors.minBy()`收集器来计算最重和最轻的`Melon`。这些收集器以一个`Comparator`作为参数来比较流中的元素，并返回一个`Optional`（如果流为空，则该`Optional`将为空）：
...
换句话说，`groupingBy()`方法能够对具有特定区别特征的元素进行分组。在 streams 和函数式编程（java8）之前，这样的任务是通过一堆繁琐、冗长且容易出错的*意大利面*代码应用于集合的。从 Java8 开始，我们有*分组收集器*。
...
好吧，没有重复的了，但是结果不是有序的。这张地图最好按键排序，所以默认的`HashMap`不是很有用。如果我们可以指定一个`TreeMap`而不是默认的`HashMap`，那么问题就解决了。我们可以通过另一种口味的`groupingBy()`，也就是`groupingBy​(Function<? super T,​? extends K> classifier, Supplier<M> mapFactory, Collector<? super T,​A,​D> downstream)`。
...
嗯，转化一个流的元素通常是`map()`的工作。但是在`groupingBy()`中，这是`Collectors.mapping()`的工作（更多细节可以在本章的*过滤、展平和映射收集器*部分找到）：
...
好的，到目前为止，很好！现在，让我们关注一个事实，`groupingBy()`的三种风格中有两种接受收集器作为参数（例如，`toSet()`。这可以是任何收集器。例如，我们可能需要按类型对西瓜进行分组并计数。为此，`Collectors.counting()`很有帮助（更多细节可以在*摘要集*部分找到）：
...
前面我们提到过三种口味的`groupingBy()`中有两种以另一个收藏家为论据。此外，我们说，这可以是任何收藏家。任何一个收藏家，我们也指`groupingBy()`。
...
*   `A`表示收集过程中使用的对象类型，称为*累加器*，用于将流元素累加到*可变结果容器中。*
...
# 特征–设置<characteristics>特征（）；</characteristics>
...
这种口味的`collect()`非常适合，只要我们处理`IDENTITY_FINISH`采集操作，我们可以提供供应商、累加器和合路器。
...
在上一章的*实现工厂模式*一节中提供了更多关于方法引用构造函数的细节和示例。
...
影响线程数是一个重要的决定。根据环境确定最佳线程数不是一件容易的任务，在大多数情况下，默认设置（*线程数=处理器数量*最合适。
...
显然，编写一个自定义的`Spliterator`不是一项日常任务，但是假设我们正在进行一个项目，由于某种原因，需要我们处理包含表意字符的字符串（**CJKV**（简写为**中文**、**日文**、**韩文**、**越南语）**）和非表意字符。我们要并行处理这些字符串。这就要求我们只能在代表表意字符的位置将它们拆分为字符。
...
创建元素的`Stream`的问题可能是`null`，也可能不是`null`，可以使用`Optional.ofNullable()`来解决，或者更好地通过 JDK 9 来解决`Stream.ofNullable()`：
...
如果这个方法存在于名为`AsStreams`的实用程序类中，那么我们可以执行几个调用，如下所示：
...
如果我们通过我们的列表，我们会得到一个没有[T1]值的[T0]流：
...
我们的任务完成了！本章介绍无限流、空安全流和默认方法。一系列问题涵盖了分组、分区和收集器，包括 jdk12[T0]收集器和编写自定义收集器。此外，`takeWhile()`、`dropWhile()`、组合函数、谓词和比较器、lambdas 的测试和调试，以及其他一些很酷的话题。
