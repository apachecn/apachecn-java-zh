...
我们可以将装配线视为我们工厂的助手，因此它可以实现为助手或实用程序类（当然，它也可以很容易地切换到非[T0]实现，因此如果对您的情况更有意义，请随意切换）：
...
请注意，对[T0]语句的第一次调用将阻塞直到所有的[T1]实例都完成。这将导致以下输出：
...
*   只得到它将提供的对象的人
*   获得超时的线程（在另一个线程进入交换之前，如果经过指定的等待时间，将抛出一个[T0]）。
...
一个`Phaser`可以由零个参与方、一个通过空构造函数的显式参与方数或一个采用整数参数`Phaser​(int parties)`的构造函数创建。`Phaser`还可以通过`Phaser​(Phaser parent)`或`Phaser​(Phaser parent, int parties)`指定父级。通常由一方启动`Phaser`，称为控制器或控制方。通常，这个聚会在[T6]寿命期内寿命最长。
