...
203.  **具有固定线程数的线程池**：编写一个程序，模拟一条装配线，使用多个工人检查和打包灯泡。

204.  **缓存的****和计划的线程池**：编写一个程序，模拟装配线，根据需要使用工人检查和打包灯泡（例如，调整打包机的数量（增加或减少）以吸收检查器产生的传入流量）。
205.  **偷工线程池**：编写依赖偷工线程池的程序。更准确地说，编写一个程序，模拟一条装配线来检查和打包灯泡，如下所示：检查在白天进行，打包在晚上进行。检查过程导致每天有 1500 万只灯泡排队。
206.  **`Callable`、`Future`**：用`Callable`、`Future`编写模拟灯泡检查打包流水线的程序。
207.  **调用多个`Callable`任务**：编写一个模拟装配线的程序，对灯泡进行检查和打包：检查在白天进行，打包在晚上进行。检查过程导致每天有 100 个灯泡排队。包装过程应一次包装并归还所有灯泡。换句话说，我们应该提交所有的`Callable`任务，等待它们全部完成。
208.  **闩锁**：编写一个依赖`CountDownLatch`的程序来模拟启动服务器的过程。服务器在其内部服务启动后被视为已启动。服务可以同时启动并且相互独立。
209.  **屏障**：编写一个依赖`CyclicBarrier`的程序来模拟服务器的启动过程。服务器在其内部服务启动后被视为已启动。服务可以同时启动（这很费时），但它们是相互依赖的—因此，一旦准备好启动，就必须一次启动所有服务。
210.  **交换器**：编写一个程序，模拟使用`Exchanger`，一条由两名工人组成的灯泡检查和包装流水线。一个工人（检查人员）正在检查灯泡，并把它们放进篮子里。当篮子装满时，工人将篮子交给另一个工人（包装工），他们从另一个工人那里得到一个空篮子。这个过程不断重复，直到装配线停止。
...
如果 Java 线程已创建但未启动，则该线程处于**新**状态（线程构造函数创建处于**新**状态的线程）。在调用[0]方法之前，这是它的状态。本书附带的代码包含几个代码片段，这些代码片段通过不同的构造技术（包括 lambdas）揭示了这种状态。为简洁起见，以下只是其中一种结构：
...
从**NEW**到**RUNNABLE**的转换是通过调用`start()`方法实现的。在此状态下，线程可以正在运行或准备运行。当它准备好运行时，线程正在等待 JVM 线程调度程序为它分配运行所需的资源和时间。一旦处理器可用，线程调度程序就会运行线程。
...
当线程试图执行 I/O 任务或同步块时，它可能会进入**阻塞**状态。例如，如果一个线程`t1`试图进入另一个线程`t2`正在访问的同步代码块，那么`t1`将保持在**阻塞**状态，直到它可以获得锁为止。
...
2.  通过`start()`方法启动`t1`：
    ...
    2.  因为`syncMethod()`有一个无限的循环，所以它将永远把`t1`留在里面。
3.  两秒后（任意时间），通过`start()`方法启动`t2`：
    1.  `t2`将执行`run()`代码，由于无法获取`syncMethod()`的锁，因此最终处于**阻塞**状态。
...
4.  在`t2`的`run()`方法中`t2`打印`t1`的状态，应该是**等待**（打印`t1`状态时`t2`在运行，所以`t1`在等待）。
...
4.  当`t1`运行时，主线程打印`t1`状态，状态应该是**TIMED\u WAITING**，因为`t1`在`sleep()`中，两秒后过期。
...
成功完成任务或异常中断的线程处于**终止**状态。模拟起来非常简单，如下面的代码片段（应用程序的主线程打印线程的状态，`t`——当发生这种情况时，线程，`t`已经完成了它的工作）：
...
在 Java 中，标记为`synchronized`的代码块一次只能由一个线程执行。由于 Java 是一个多线程环境（它支持并发），因此它需要一个同步机制来避免并发环境特有的问题（例如死锁和内存一致性）。

线程可以在对象级或类级实现锁。

# 在对象级别锁定

对象级的锁定可以通过用`synchronized`标记非`static`代码块或非`static`方法（该方法的对象的锁定对象）来实现。在下面的示例中，一次只允许一个线程在类的给定实例上执行`synchronized`方法/块：
...
为了保护`static`数据，可以通过标记`static`方法/块或在`.class`引用上用`synchronized`获取锁来实现类级锁定。在以下示例中，一次只允许一个运行时可用实例的一个线程执行[4]块：
...
*   两个线程不能同时执行同一类的两个不同的`synchronized static`方法（或同一`synchronized static`方法）（请检查`P200_ObjectVsClassLevelLocking`应用程序的`TwoCll`类）。这不起作用，因为第一个线程获得了类级锁。以下组合将输出，`staticMethod1(): Thread-0`，因此，只有一个线程执行一个`static synchronized`方法：
...
*   从需要相同锁的同一类的另一个`synchronized`方法调用`synchronized`方法是安全的，这是因为`synchronized`是*重入者*（只要是相同的锁，第一个方法获得的锁也会用于第二个方法）。检查`P200_ObjectVsClassLevelLocking`应用程序的`TwoSyncs`类。

根据经验，`synchronized`关键字只能用于`static`/非`static`方法（不是构造函数）/代码块。避免同步非`final`字段和`String`文字（通过`new`创建的`String`实例可以）。
...
*   `workQueue`：在`Runnable`的实例（只有`execute()`方法提交的`Runnable`个任务）执行之前，用来存放实例的队列
*   `threadFactory`：执行器新建线程时使用此工厂
...
`main()`方法触发`Runnable`的 50 个实例。每个`Runnable`睡眠两秒钟并打印一条信息。工作队列限制为 5 个`Runnable`实例—核心线程限制为 10 个，最大线程数限制为 20 个，空闲超时限制为 1 秒。可能的输出如下所示：
...
`ScheduledExecutorService`是一个`ExecutorService`，它可以安排任务在给定的延迟后执行，或者定期执行。这里有`schedule()`、`scheduleAtFixedRate()`、`scheduleWithFixedDelay​()`等方法。`schedule()`用于一次性任务，`scheduleAtFixedRate()`和`scheduleWithFixedDelay()`用于周期性任务。

# 通过执行器的线程池

更进一步，我们将介绍 helper 类`Executors`。此类使用以下方法公开几种类型的线程池：

*   `newSingleThreadExecutor()`：这是一个线程池，只管理一个无限队列的线程，一次只执行一个任务：
...
*   `newWorkStealingThreadPool()`：这是一个基于工作窃取算法的线程池（作为 fork/join 框架上的一层）：
...
为了演示具有单个线程的线程池是如何工作的，我们假设要编写一个程序，模拟装配线（或传送带），用两个工人来检查和打包灯泡。

通过*检查*，我们了解到工人测试灯泡是否亮起。通过*包装*，我们了解到工人将经过验证的灯泡拿到盒子里。这种工艺在几乎所有工厂都很常见。
...
这种问题非常适合下图所示的生产者-消费者设计模式：

![](img/817c7786-9093-401c-9afd-bf9b39aabd25.png)

最常见的是，在这种模式中，生产者和消费者通过队列（生产者排队数据，消费者排队数据）进行通信。此队列称为*数据缓冲区*。当然，根据流程设计，其他数据结构也可以起到数据缓冲的作用。
...
当装配线开始时，生产商将逐一检查来料灯泡，而消费者将对其进行包装（每箱一个灯泡）。此流程重复进行，直到装配线停止。

下图是**生产者**和**消费者**之间的流程的图示：

![](img/3cf100e5-7ad8-4548-9a33-1e55a146b655.png)

我们可以将装配线视为我们工厂的助手，因此它可以实现为助手或实用程序类（当然，它也可以很容易地切换到非[0]实现，因此如果对您的情况更有意义，请随意进行切换）：
...
当然，有很多方法可以实现这个场景，但是我们对使用 Java`ExecutorService`，更确切地说是`Executors.newSingleThreadExecutor()`感兴趣。通过此方法创建了一个`Executor`，它使用一个在无限队列之外运行的工作线程。

我们只有两个 worker，所以我们可以使用两个实例`Executor`（一个`Executor`将为生产者通电，另一个将为消费者通电）。因此，生产者将是一个线程，消费者将是另一个线程：
...
这个场景适用于`TransferQueue`或`SynchronousQueue`，它执行的过程与前面的场景非常相似。让我们使用`TransferQueue`。这是一个`BlockingQueue`，生产者可以等待消费者接收元素。`BlockingQueue` 实现是线程安全的：
...
生产者和消费者之间的工作流程是**先进先出**类型（**先进先出**：检查的第一个灯泡是包装的第一个灯泡），因此`LinkedTransferQueue`是一个不错的选择。
...
因此，生产者通过`tryTransfer()`方法将检查过的灯泡传输给消费者。如果可以在超时时间过去之前将元素传输到使用者，则此方法将执行此操作。

避免使用`transfer()`方法，这可能会无限期地阻塞螺纹。
...
消费者可以通过`queue.take()`方式从生产商处取灯泡。此方法检索并删除此队列的头，如有必要，请等待，直到灯泡可用。或者它可以调用`poll()`方法，在该方法中检索并移除队列的头，或者如果该队列为空，则返回`null`。但这两个都不适合我们。如果生产商不在，消费者可能仍然停留在`take()`方法中。另一方面，如果队列是空的（生产者正在检查当前灯泡），则会很快一次又一次地调用[4]方法，从而导致伪重复。解决方法是`poll​(long timeout, TimeUnit unit)`。此方法检索并删除此队列的头，并在指定的等待时间内（如果需要）等待灯泡变为可用。仅当等待时间过后队列为空时，它才会返回[6]。

为了模拟耗电元件包装灯泡所需的时间，相应的线程将在 0 到 7 之间随机休眠几秒（7 秒是包装灯泡所需的最长时间）。

启动生产者和消费者是一个非常简单的任务，通过一个名为`startAssemblyLine()`的方法完成，如下所示：
...
为了形成这个场景，我们可以依赖`ConcurrentLinkedQueue`（或`LinkedBlockingQueue`）。这是一个基于链接节点的无限线程安全队列：
...
此问题重复了使用单个线程[1]节的[0]线程池中的场景。这一次，装配线使用了三个生产商和两个消费者，如下图所示：

![](img/99335ce7-243d-4e71-ac87-c22ada2bd28d.png)

我们可以依赖`Executors.newFixedThreadPool​(int nThreads)`来模拟固定数量的生产者和消费者。我们为每个生产者（分别为消费者）分配一个线程，因此代码非常简单：
...
此问题重复了使用单个线程[1]节的[0]线程池中的场景。这一次，我们假设生产者（也可以使用多个生产者）在不超过 1 秒的时间内检查一个灯泡。此外，一个耗电元件（包装器）最多需要 10 秒来包装一个灯泡。生产器和耗电元件的时间可以如下所示：
...
因为只有一个生产商，所以我们可以依赖`Executors.newSingleThreadExecutor()`：
...
除了`extraProdTime`变量外，`Producer`与前面的问题基本相同：
...
当生产商高速运转时，我们将需要更多的消费者（包装商）。但是有多少？使用固定数量的消费者（[0]）至少会带来两个缺点：
...
不要混淆`nrOfConsumers`和`threadGroup.activeCount()`。`nrOfConsumers`变量存储当前打包灯泡的使用者（线程）的数量，而`threadGroup.activeCount()`表示所有活动的使用者（线程），包括那些现在不工作（空闲）并且正等待从缓存中重用或调度的使用者（线程）。
...
出于测试目的，我们的主管`newSingleThreadScheduledExecutor()`将是一个单线程执行器，可以将给定的命令安排在指定的延迟后运行。它还可以定期执行命令：
...
因此，每个线程都会将任务从中心队列中出列，并将它们放入自己的队列中。每个线程都有自己的本地任务队列。此外，当一个线程想要处理一个任务时，它只是将一个任务从它自己的本地队列中取出。只要它的本地队列不是空的，线程就将继续处理来自它的任务，而不会影响其他线程（与其他线程没有冲突）。当其本地队列为空时（如上图中的**线程 2**的情况），它尝试从属于其他线程的本地队列中窃取（通过工作窃取算法）任务（例如，**线程 2**从**线程 3**中窃取任务）。如果找不到任何可窃取的内容，它将访问共享的中心入站队列。
...
我们刚才描述的是 JDK7 中引入的 fork/join 框架，并在[3]fork/join 框架[4]一节中举例说明。从 jdk8 开始，`Executors`类通过使用可用处理器数量作为其目标并行级别的工作线程池进行了丰富。可通过`Executors.newWorkStealingPool()`和`Executors.newWorkStealingPool​(int parallelism)`获取。
...
因此，在内部，这个线程池通过以下构造函数实例化了`ForkJoinPool`：
...
我们将并行级别设置为`availableProcessors()`，返回新线程的默认线程工厂为`Thread.UncaughtExceptionHandler`，传递为`null`，将`asyncMode`设置为`true`。将`asyncMode`设置为`true`意味着它授权本地**先进先出**（**FIFO**调度模式，用于分叉且从未联接的任务。在依赖工作线程仅处理事件样式异步任务的程序中，此模式可能比默认模式（基于本地堆栈）更合适。

然而，不要忘记，只有当工作线程在自己的本地队列中调度新任务时，本地任务队列和工作窃取算法才被授权。否则，`ForkJoinPool`只是一个有额外开销的`ThreadPoolExecutor`。

当我们直接使用`ForkJoinPool`时，我们可以使用`ForkJoinTask`指示任务在执行期间显式地安排新任务（通常通过`RecursiveTask`或`RecursiveAction`）。

但由于`newWorkStealingPool()`是`ForkJoinPool`的更高抽象级别，我们无法指示任务在执行期间显式地调度新任务。因此，`newWorkStealingPool()`将根据我们通过的任务在内部决定如何工作。我们可以尝试比较一下`newWorkStealingPool()`、`newCachedThreadPool()`和`newFixedThreadPool()`，看看它们在两种场景下的表现：
...
由于生产者（checker）和消费者（packer）不同时工作，我们可以通过一个微不足道的`for`循环（我们对装配线的这一部分不太感兴趣）轻松地用 15000000 个灯泡填满一个队列。这在以下代码段中显示：
...
[0]包装了一个简单的[1]操作，因此它应该运行得非常快，如下面的代码段所示：
...
每个`Consumer`使用以下代码循环 1000000 个灯泡：
...
此问题重复了使用单个线程[1]节的[0]线程池中的场景。我们需要一个单一的生产商和消费者遵循以下场景：
...
为了形成这个场景，生产者应该能够返回一个结果并抛出一个异常。因为我们的制作人是一个`Runnable`，所以这两个都不能做。但是 Java 定义了一个名为`Callable`的接口。这是一个函数接口，有一个名为`call()`的方法。与`Runnable`的`run()`方法相比，`call()`方法可以返回结果，甚至抛出异常`V call() throws Exception`。
...
executor 服务可以通过`submit()`方法向`Callable`提交任务，但不知道提交任务的结果何时可用。因此，`Callable`立即返回一个名为`Future`的特殊类型。异步计算的结果用一个`Future`来表示—通过`Future`，我们可以在任务的结果可用时获取它。从概念上讲，我们可以将[6]看作 JavaScript 承诺，或者是将在稍后时间点完成的计算的结果。现在，我们创建一个`Producer`并提交给一个`Callable`：
...
可以使用阻塞方法`Future.get()`检索`Future`的结果。此方法将阻塞，直到结果可用或指定的超时已过（如果在超时之前结果不可用，则抛出一个[2]：
...
一旦结果出来，我们就可以把它传递给`Consumer`，再提交一个任务给`Producer`。只要消费者和生产者都在运行，这个循环就会重复。其代码如下：
...
`Consumer`仍然是`Runnable`，因此无法返回结果或抛出异常：
...
一个`Future`可以取消。这是使用`cancel​(boolean mayInterruptIfRunning)`方法完成的。如果我们将其作为`true`传递，则执行该任务的线程被中断，否则，该线程可以完成该任务。如果任务取消成功，则返回`true`，否则返回`false`（通常是因为任务已经正常完成）。下面是一个简单的示例，用于在运行任务所需时间超过 1 秒时取消该任务：
...
由于生产者（checker）不与消费者（packer）同时工作，因此我们可以通过在队列中添加 100 个已检查灯泡的`for`来模拟他们的工作：
...
现在，消费者必须将每个灯泡打包并退回。这意味着`Consumer`是一个`Callable`：
...
但是请记住，我们应该提交所有的`Callable`任务并等待它们全部完成。这可以通过`ExecutorService.invokeAll()`方法实现。此方法接受任务集合`(Collection<? extends Callable<T>>)`，并返回`Future`（`List<Future<T>>`的实例列表作为参数。对`Future.get()`的任何调用都将被阻止，直到`Future`的所有实例都完成。
...
请注意，对[0]语句的第一次调用将阻塞，直到[1]的所有实例都完成。这将导致以下输出：
...
有时，我们需要提交几个任务，然后等待其中任何一个任务完成。可通过`ExecutorService.invokeAny()`实现。与`invokeAll()`完全一样，此方法获取一个任务集合（`Collection<? extends Callable<T>>`作为参数）。但它返回最快任务的结果（不是一个`Future`），并取消所有其他尚未完成的任务，例如：

```
String bulb = consumerService.invokeAny(tasks);
```

如果您不想等待所有的`Future`完成，请按以下步骤进行：
...
*闩锁*是一个 Java 同步器，它允许一个或多个线程等待其他线程中的一组事件完成。它从给定的计数器开始（通常表示应该等待的事件数），完成的每个事件负责递减计数器。当计数器达到零时，所有等待的线程都可以通过。这是闩锁的终端状态。闩锁不能重置或重用，因此等待的事件只能发生一次。下图分四个步骤显示了具有三个线程的闩锁的工作原理：

![](img/cb9e4c29-f8bd-4812-a42d-6008818744aa.png)

在 API 术语中，闩锁是使用`java.util.concurrent.CountDownLatch`实现的。
...
所有调用`await()`方法的线程都将被阻塞，直到计数器达到零。因此，希望在闩锁达到终端状态之前被阻塞的线程将调用`await()`。每个完成的事件都可以调用`countDown()`方法。此方法用一个值递减计数器。直到计数器变为零，调用[3]的线程仍然被阻塞。

闩锁可用于各种各样的问题。现在，让我们集中讨论应该模拟启动服务器过程的问题。服务器在其内部服务启动后被视为已启动。服务可以同时启动并且相互独立。启动服务器需要一段时间，需要我们启动该服务器的所有底层服务。因此，完成并验证服务器启动的线程应该等到其他线程中的所有服务器服务（事件）都已启动。如果假设我们有三个服务，我们可以编写一个`ServerService`类，如下所示：
...
每个启动成功（或失败）的服务都将通过`countDown()`减少闩锁。一旦计数器达到零，服务器就被认为已启动。我们称之为：
...
*屏障*是允许一组线程（称为*方*）到达公共屏障点的 Java 同步器。基本上，一组线程等待彼此在屏障处相遇。就像一群朋友决定了一个会面点，当他们都得到这个点时，他们一起走得更远。他们不会离开集合点，直到他们都到了或者直到他们觉得他们已经等了太久。
...
一个`CyclicBarrier`可以通过两个构造器构造：

*   其中一个允许我们指定参与方的数量（这是一个整数）
*   另一个允许我们添加一个动作，该动作应该在各方到达关卡后发生（这是一个`Runnable`）

此操作在参与方中的所有线程到达时发生，但在释放任何线程之前发生。

当线程准备好在屏障处等待时，它只调用`await()`方法。此方法可以无限期地等待或直到指定的超时（如果指定的超时已过或线程被中断，则此线程用一个`TimeoutException`释放；屏障被认为*已破*，屏障处所有等待的线程用一个`BrokenBarrierException`释放）。我们可以通过`getParties()`方法找出需要多少方来跨越这个障碍，通过`getNumberWaiting()`方法找出目前有多少方在障碍处等待。
...
`ServerService`负责准备每一项服务的启动，并通过`await()`将其阻塞在屏障上：
...
一个`CyclicBarrier`是循环的，因为它可以重置和重用。为此，请在释放所有等待屏障的线程后调用`reset()`方法，否则会抛出`BrokenBarrierException`。

处于*破*状态的屏障将导致`isBroken()`flag 方法返回`true`。

# 210 交换器

*交换器*是一个 Java 同步器，允许两个线程在一个交换点或同步点交换对象。

这种同步器主要起屏障作用。两条线在一道屏障处互相等待。他们交换一个物体，当他们到达时继续他们通常的任务。
...
可以通过空构造函数创建一个`Exchanger`，并公开两个`exchange()`方法：

*   只得到它将提供的对象的人
*   获得超时的线程（在另一个线程进入交换之前，如果经过指定的等待时间，将抛出一个[0]）。

还记得我们的灯泡生产线吗？好吧，假设生产者（检查者）将检查过的灯泡添加到一个篮子中（例如，[0]）。当篮子装满时，生产者与消费者（包装商）交换一个空篮子（例如，另一个`List<String>`）。只要装配线还在运行，这个过程就会重复。
...
另一方面，消费者在交换点等待从生产商那里收到装满灯泡的篮子，然后给出一个空的篮子作为交换。此外，当生产者再次装满篮子时，消费者从收到的篮子中包装灯泡。完成后，他们将再次前往兑换点等待另一个满满的篮子。所以，`Consumer`可以写成：
...
*信号量*是一个 Java 同步器，它允许我们控制在任何时候可以访问资源的线程数。从概念上讲，这个同步器管理一堆*许可证*（例如，类似于令牌）。需要访问资源的线程必须从同步器获得许可。在线程使用资源完成其工作之后，它必须通过将许可返回给信号量来释放它，以便另一个线程可以获取它。线程可以立即获取许可证（如果许可证是空闲的），可以等待一定的时间，或者可以等待直到许可证变为空闲。此外，一个线程一次可以获取和释放多个许可证，一个线程即使没有获取许可证也可以释放许可证。这将向信号量添加一个许可证；因此信号量可以从一个许可证数开始，然后从另一个许可证数结束。

在 API 术语中，这个同步器用`java.util.concurrent.Semaphore`表示。

创建一个`Semaphore`就像调用它的两个构造函数之一一样简单：

*   `public Semaphore​(int permits)`
*   `public Semaphore​(int permits, boolean fair)`

公平的[0]保证 FIFO 在争议中授予许可。

可通过`acquire()`方式获取许可证。该过程可以用以下项目符号表示：
...
*   要从该信号机获取许可，可以通过`acquireUninterruptibly()`和`acquireUninterruptibly(int permits)`获得阻塞，直到有许可为止
*   要发布许可证，请使用`release()`
...
*相位器*是一种灵活的 Java 同步器，结合了`CyclicBarrier`和`CountDownLatch`在以下上下文中的功能：
...
*   相位器的每个相位可以通过从 0 开始的相关编号/索引来识别。第一阶段是 0，下一阶段是 1，下一阶段是 2，依此类推直到`Integer.MAX_VALUE`。
*   一个移相器的任何一个阶段都可以有三种类型的参与方：*注册的*、*到达的*（这些是在当前阶段/关卡等待的注册参与方）和*未到达的*（这些是正在前往当前阶段的注册参与方）。
...
可以使用零参与方、通过空构造函数显式创建的参与方数或采用整数参数的构造函数`Phaser​(int parties)`来创建`Phaser`。`Phaser`也可以通过`Phaser​(Phaser parent)`或`Phaser​(Phaser parent, int parties)`指定父级。通常由一方启动`Phaser`，称为控制器或控制方。通常情况下，这个聚会在`Phaser`的寿命中寿命最长。
...
为了到达当前阶段（barrier）并等待其他方到达，我们需要调用`arriveAndAwaitAdvance()`方法。此方法将一直阻止，直到所有注册方到达当前阶段。最后一个注册方到达当前阶段后，各方将进入本`Phaser`的下一阶段。
...
我们使用匿名类创建这个`Phaser`对象并重写它的`onAdvance()`方法来定义一个有两个主要目的的操作：
...
管理服务器服务的线程需要启动这些服务并从[0]注销自己。因此，每个服务在一个单独的线程中启动，该线程将在其作业结束时通过`arriveAndDeregister()`取消注册。为此，我们可以使用以下`Runnable`：
...
最后，在这五个服务启动之后，控制线程执行最后一个检查，该检查在下面的方法中作为虚拟的`Thread.sleep()`实现。注意，在这个操作结束时，启动服务器的控制线程从[1]注销了自己。当发生这种情况时，意味着不再有注册方，并且由于从`onAdvance()`方法返回`true`而终止`Phaser`：
...
我们可以随时通过`getRegisteredParties()`查询注册方的数量，通过`getArrivedParties()`查询到达方的数量，通过`getUnarrivedParties()`查询未到达方的数量。您可能还需要检查`arrive()`、`awaitAdvance​(int phase)`和`awaitAdvanceInterruptibly​(int phase)`方法。

# 摘要

本章概述了 Java 并发的主要坐标，应该为下一章做好准备。我们讨论了线程生命周期、对象级和类级锁定、线程池以及[0]和[1]等几个基本问题
